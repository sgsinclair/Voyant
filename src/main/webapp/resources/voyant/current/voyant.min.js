...
e>c&&(c=e)}this.setSmallestWordSize(b);this.setLargestWordSize(c);this.setRelativeSizes();this.setAdjustedSizes();this.getVisLayout().words(a).start()}else Ext.defer(this.buidlFromTerms,50,this)},draw:function(a,b){var c=this;d3.scale.category20b();this.getLayout().getRenderTarget();var d=this.getVisLayout().size()[0],e=this.getVisLayout().size()[1],f=b?Math.min(d/Math.abs(b[1].x-d/2),d/Math.abs(b[0].x-d/2),e/Math.abs(b[1].y-e/2),e/Math.abs(b[0].y-e/2))/2:1,g=this.getVis().selectAll("text").data(a,
function(a){return a.text});g.transition().duration(1E3).attr("transform",function(a){return"translate("+[a.x,a.y]+")rotate("+a.rotate+")"}).style("font-size",function(a){return a.fontSize+"px"});g.enter().append("text").attr("text-anchor","middle").attr("data-freq",function(a){return a.rawFreq}).attr("transform",function(a){return"translate("+[a.x,a.y]+")rotate("+a.rotate+")"}).style("font-size","1px").transition().duration(1E3).style("font-size",function(a){return a.fontSize+"px"});g.style("font-family",
function(a){return a.font}).style("fill",function(a){console.warn(c.getApplication().getColorForTerm(a.text,!0));return c.getApplication().getColorForTerm(a.text,!0)}).text(function(a){return a.text}).on("click",function(a){c.dispatchEvent("termsClicked",c,[a.text])});g.exit().remove();this.getVis().transition().duration(1E3).attr("transform","translate("+d/2+","+e/2+")scale("+f+")")},map:function(a,b,c,d,e){return d+(a-b)/(c-b)*(e-d)},calculateSizeAdjustment:function(){var a=this.getTerms();if(void 0!==
