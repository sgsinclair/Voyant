...
name:"categoryName",allowBlank:!1,validator:function(a){return void 0===this.app.getCategoryTerms(a)?!0:this.localize("exists")}.bind(this),enableKeyEvents:!0,listeners:{keypress:function(a,b){b.getKey()===Ext.event.Event.ENTER&&a.up("form").queryById("addCategoryButton").click()},scope:this}}],buttons:[{text:this.localize("cancel"),handler:function(a){a.up("window").close()}},{itemId:"addCategoryButton",text:this.localize("add"),handler:function(a){var b=a.up("form");b.isValid()&&(b=b.getValues().categoryName,
this.addCategory(b),a.up("window").close())},scope:this}]},listeners:{show:function(a){a=a.down("form").getForm();a.reset();a.clearInvalid()}}}));this.callParent(arguments)},addCategory:function(a){this.app.addCategory(a);this.queryById("features").getStore().add({category:a});var b=[],c=this.app.getCategoryTerms(a);if(void 0!==c)for(var d=0;d<c.length;d++)b.push({term:c[d]});var e=this.queryById("categories").add({xtype:"grid",category:a,title:a,frame:!0,width:150,margin:"10 0 10 10",layout:"fit",
tools:[{type:"close",tooltip:this.localize("removeCategory"),callback:function(b){Ext.Msg.confirm(this.localize("removeCategory"),this.localize("confirmRemove"),function(b){"yes"===b&&this.removeCategory(a)},this)},scope:this}],store:Ext.create("Ext.data.JsonStore",{data:b,fields:["term"]}),viewConfig:{plugins:{ptype:"gridviewdragdrop",ddGroup:"terms",dragZone:{getDragText:function(){var a="";this.dragData.records.forEach(function(b){a+=b.get("term")+", "});return a.substr(0,a.length-2)}}}},selModel:{mode:"MULTI"},
columns:[{dataIndex:"term",flex:1,sortable:!0}],listeners:{beforedrop:function(a,b){a=this.up("categoriesbuilder").app;for(var c=b.records.length-1;0<=c;c--){var d=b.records[c].get("term");void 0!==a.getCategoryForTerm(d)&&b.records.splice(c,1)}},drop:function(b,c){c.view.getSelectionModel().deselectAll();this.getSelectionModel().deselectAll();b=this.up("categoriesbuilder").app;for(var d=[],e=0;e<c.records.length;e++){var f=c.records[e].get("term");void 0===b.getCategoryForTerm(f)&&d.push(f)}b.addTerms(a,
d);c=c.view.up("grid");c.category&&b.removeTerms(c.category,d)}}}),f=new Ext.Editor({updateEl:!0,alignment:"l-l",autoSize:{width:"boundEl"},field:{xtype:"textfield",allowBlank:!1,validator:function(a){return void 0===this.app.getCategoryTerms(a)||a===e.getTitle()?!0:this.localize("exists")}.bind(this)},listeners:{complete:function(a,b,c){this.app.renameCategory(c,b);this.buildFeatures()},scope:this}});e.header.getTitle().textEl.on("dblclick",function(a,b){f.startEdit(b)})},removeCategory:function(a){var b=
this.queryById("categories"),c=b.queryBy(function(b){return b.category&&b.category==a?!0:!1});b.remove(c[0]);b=this.queryById("features").getStore();b.removeAt(b.findExact("category",a));this.app.removeCategory(a)},addFeature:function(a){this.app.addFeature(a);this.buildFeatures()},buildFeatures:function(){this.queryById("features").getStore().removeAll();var a=["category"],b=[{sortable:!1,text:this.localize("category"),dataIndex:"category",flex:1}],c=[],d;for(d in this.app.getCategories())c.push({category:d});
var e=this.app.getFeatures(),f=Ext.ClassManager.getClass(this).features,g;for(g in e){a.push(g);e=f[g];var k=Ext.applyIf({feature:g,listeners:{change:function(a,b){if(a.rendered){var c=a.up("gridview").indexOf(a.el.up("table")),d=a.up("grid").getStore().getAt(c);d?d.set(a.feature,b):window.console&&console.warn("no record for",c,a)}},scope:this}},e);e.listeners&&Ext.applyIf(k.listeners,e.listeners);b.push({sortable:!1,hideable:!1,text:this.localize(g),dataIndex:g,flex:.5,xtype:"widgetcolumn",widget:k});
for(d in this.app.getCategories())for(e=this.app.getCategoryFeature(d,g),k=0;k<c.length;k++)if(c[k].category==d){c[k][g]=e;break}}a=Ext.create("Ext.data.JsonStore",{fields:a,data:c});this.queryById("features").reconfigure(a,b)},processFeatures:function(){var a=this.queryById("features").getStore(),b=Object.keys(this.app.getFeatures());a.each(function(a){var c=a.get("category");b.forEach(function(b){var d=a.get(b);void 0!==d&&this.app.setCategoryFeature(c,b,d)},this)},this)},buildCategories:function(){this.queryById("categories").removeAll();
...
this;this.mixins["Voyant.util.Api"].constructor.apply(this,arguments);this.mixins["Voyant.util.Colors"].constructor.apply(this,arguments);this.mixins["Voyant.util.CategoriesManager"].constructor.apply(this,arguments);this.addFeature("color");this.addFeature("font",'"Palatino Linotype", "Book Antiqua", Palatino, serif');this.callParent(arguments);var b=this.getColor;this.getColor=function(a,c){return b.apply(this,[this.getApiParam("palette"),a,c])};var c=this.getColorForTerm;this.getColorForTerm=function(a,
b){return c.apply(this,[this.getApiParam("palette"),a,b])}},getBaseUrl:function(){var a=this.callParent();return 0==a.indexOf("//")?location.protocol+a:a},getBaseUrlFull:function(){return window.location.origin+this.getBaseUrl()},getRelativeUrl:function(){for(var a="",b=0,c=window.location.pathname.substring(this.getBaseUrl().length).split("/").length-1;b<c;b++)a+="../";return a},getTools:function(){return[{type:"maximize"},{type:"help"}]},launch:function(){Ext.tip.QuickTipManager.init();Ext.apply(Ext.tip.QuickTipManager.getQuickTip(),
...
scope:this})},listeners:{loadedCorpus:function(a,b){this.setCorpus(b);this.getApiParam("categories")&&this.loadCategoryData(this.getApiParam("categories"));this.on("unhandledEvent",function(a,d,e){a=this.getBaseUrl()+"?corpus\x3d"+b.getAliasOrId();var c=this.getModifiedApiParams()||{};delete c.view;if("termsClicked"==d)if(Ext.isArray(e)&&"term"in e[0]&&"docIndex"in e[0]){var g={},k={};e.forEach(function(a){g[a.term]=!0;k[a.docIndex]=!0});c.query=Object.keys(g);c.docIndex=Object.keys(k)}else c.query=
e;else if("documentsClicked"==d){var h=[];e.forEach&&e.forEach(function(a){h.push(a.getIndex())});c.docIndex=h}else if("corpusTermsClicked"==d)e.map&&(c.query=e.map(function(a){return a.getTerm()}));else if("documentTermsClicked"==d)e.map&&(c.query=e.map(function(a){return a.getTerm()}),c.docIndex=e.map(function(a){return a.getDocIndex()}));else{console&&console.warn("Unhandled event: "+d,e);return}c.toolFlow&&(d=Ext.Array.from(c.toolFlow.split(",")),c.view=d.shift(),0<d.length?c.toolFlow=d.join(","):
delete c.toolFlow);a+="\x26"+Ext.Object.toQueryString(c);this.openUrl(a)})}}});Ext.define("Voyant.panel.DocumentClusters",{extend:"Ext.panel.Panel",alias:"widget.documentclusters",title:"Document Clusters"});
