...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];l=0;f=function(){var a,f,h,k,n=this.wordsToArrange-1,t=g.canvas.width,q=g.canvas.height,s=.5*t,r=.5*q;do{h=p[l];if(void 0!==h){for(var v=Math.random()*Math.PI,w=.25*Math.random()*h.size,A=.5*(Math.random()-.5),y=.5*h.width,z=.5*h.height;;){a=Math.floor((s+Math.cos(v)*w*this.ratio-y)/this.COARSENESS)*this.COARSENESS;f=Math.floor((r+Math.sin(v)*w-z)/this.COARSENESS)*this.COARSENESS;
k=a+y>=t||f+z>=q?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;v+=A;w+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<l;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}l++;if(l>=p.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(n--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;m=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
b){f.push({x:b,y:a})},this);e.push(f);d.push({name:a.get("term"),mark:b(c),active:!0})},this);c.loadData(d);var f=this.getVisLayout()(e),g;g=this.getMode()===this.MODE_DOCUMENT?this.getApiParam("bins"):this.getCorpus().getDocumentsCount();g--;var c=this.body.down("svg").getWidth()-this.graphMargin.left-this.graphMargin.right,h=d3.scale.linear().domain([0,g]).range([0,c]),k=d3.max(f,function(a){return d3.max(a,function(a){return a.y0+a.y})});a=this.body.down("svg").getHeight()-this.graphMargin.top-
this.graphMargin.bottom;var l=d3.scale.linear().domain([0,k]).range([a,0]),m=d3.svg.area().x(function(a){return h(a.x)}).y0(function(a){return l(a.y0)}).y1(function(a){return l(a.y0+a.y)}),n=d3.svg.axis().scale(h).orient("bottom");this.getMode()===this.MODE_CORPUS&&n.tickFormat("");k=d3.svg.axis().scale(l).orient("left");f=this.getVis().selectAll("path").data(f);f.attr("d",function(a){return m(a)});f.enter().append("path").attr("d",function(a){return m(a)}).style("fill",function(a,c){return b(c)});
f.exit().remove();this.getVis().selectAll("g.axis").remove();this.getVis().append("g").attr("class","axis x").attr("transform","translate(0,"+a+")").call(n);if(this.getMode()===this.MODE_CORPUS){var q=c/g,r=0;this.getCorpus().getDocuments().each(function(a){this.getVis().select("g.x").append("text").attr("text-anchor","end").attr("transform","translate("+r+", 10) rotate(-45)").text(a.getTinyTitle());r+=q},this);g=this.localize("documents")}else g=this.localize("documentSegments");this.getVis().select("g.x").append("text").attr("text-anchor",
"middle").attr("transform","translate("+c/2+", "+(this.graphMargin.bottom-20)+")").text(g);this.getVis().append("g").attr("class","axis y").attr("transform","translate(0,0)").call(k);c="raw"===this.getApiParam("withDistributions")?this.localize("rawFrequencies"):this.localize("relativeFrequencies");this.getVis().select("g.y").append("text").attr("text-anchor","middle").attr("transform","translate(-"+(this.graphMargin.left-20)+", "+a/2+") rotate(-90)").text(c)},getCurrentTerms:function(){var a=[];
this.down("[xtype\x3dlegend]").getStore().each(function(b){b.get("active")&&a.push(b.get("name"))},this);return a},initGraph:function(){if(void 0===this.getVisLayout()){var a=this.getLayout().getRenderTarget(),b=this.graphMargin.left+this.graphMargin.right,c=this.graphMargin.top+this.graphMargin.bottom,d=a.getWidth()-b,e=a.getHeight()-c;this.setVisLayout(d3.layout.stack().offset("silhouette"));this.setVis(d3.select(a.dom).append("svg").attr("id","streamGraph").attr("width",d+b).attr("height",e+c).append("g").attr("transform",
"translate("+this.graphMargin.left+","+this.graphMargin.top+")"))}},updateMode:function(a){this.queryById("segmentsSlider").setHidden(a==this.MODE_CORPUS)}});Ext.define("Voyant.VoyantApp",{extend:"Ext.app.Application",mixins:["Voyant.util.Deferrable","Voyant.util.Localization","Voyant.util.Api"],requires:["Voyant.util.ResponseError"],name:"VoyantApp",statics:{i18n:{error:{en:"Error"},serverResponseError:{en:"The server error reponse:"}}},config:{baseUrl:void 0,tromboneUrl:void 0},constructor:function(a){this.setBaseUrl(this.config.baseUrl);this.setTromboneUrl(this.config.baseUrl+"trombone");Voyant.application=this;this.mixins["Voyant.util.Api"].constructor.apply(this,
