...
this.lastClickedBubbles[b],l=0,n;for(n in k)if(-1!==this.termsFilter.indexOf(n)){var q=k[n];if(q){l++;this.SEPARATE_LINES_FOR_TERMS&&c();var r=0,p=q.color.join(",");this.ctx.strokeStyle="rgba("+p+", 1)";this.ctx.fillStyle="rgba("+p+", 0.35)";this.ctx.lineWidth=.25;for(var h=h+q.rawFreq,r=r+q.rawFreq,s=m&&this.lastClickedBubbles[b][n],C=0;C<q.pos.length;C++){var F=q.pos[C];if(0<F.radius){var D=!1;s&&this.lastClickedBubbles[b][n]==F.id&&(this.ctx.strokeStyle="rgba(0, 0, 0, 0.75)",this.ctx.fillStyle=
"rgba("+p+", 0.5)",this.ctx.lineWidth=1,D=!0);this.ctx.beginPath();this.ctx.arc(F.x+f,this.yIndex,F.radius,0,e,!0);this.ctx.closePath();this.ctx.fill();this.ctx.stroke();D&&(this.ctx.strokeStyle="rgba("+p+", 1)",this.ctx.fillStyle="rgba("+p+", 0.35)",this.ctx.lineWidth=.25)}}a.freqCounts[n]=r;this.SEPARATE_LINES_FOR_TERMS&&(this.ctx.fillStyle="rgba(0, 0, 0, 1.0)",this.ctx.font="10px Verdana",this.ctx.fillText(r,f+d+5,this.yIndex-4),this.yIndex+=this.maxRadius)}}this.SEPARATE_LINES_FOR_TERMS&&0==l&&
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];m=0;f=function(){var a,f,h,k,s=this.wordsToArrange-1,n=g.canvas.width,q=g.canvas.height,C=.5*n,r=.5*q;do{h=p[m];if(void 0!==h){for(var H=Math.random()*Math.PI,w=.25*Math.random()*h.size,u=.5*(Math.random()-.5),v=.5*h.width,M=.5*h.height;;){a=Math.floor((C+Math.cos(H)*w*this.ratio-v)/this.COARSENESS)*this.COARSENESS;f=Math.floor((r+Math.sin(H)*w-M)/this.COARSENESS)*this.COARSENESS;
k=a+v>=n||f+M>=q?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;H+=u;w+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<m;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}m++;if(m>=p.length){clearTimeout(l);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(s--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;l=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
l=a;return b};b.showTokenExtra=function(a){if(!arguments.length)return n;n=a;return b};b.scaleLabels=function(a){if(!arguments.length)return q;q=a;return b};b.succeeded=function(){return H};b.sortFun=function(a){if(!arguments.length)return r;r=a;return b};b.nodeText=function(a){if(!arguments.length)return p;p=a;return b};b.tokenExtraText=function(a){if(!arguments.length)return s;s=a;return b};b.rectColor=function(a){if(!arguments.length)return C;C=a;return b};b.rectBorderColor=function(a){if(!arguments.length)return F;
F=a;return b};b.continuationColor=function(a){if(!arguments.length)return D;D=a;return b};b.basicStyles=function(a){if(!arguments.length)return G;Object.keys(G).forEach(function(b){b in a&&Object.keys(G[b]).forEach(function(c){c in a[b]&&(G[b][c]=a[b][c])})});return b};return b};doubletree.Tree=function(a,b,c,d,k,m,l,n,q,r,p,s,C,F,D){function G(a){a.children&&(a._children=a.children,a._children.forEach(G),a.children=null)}function H(a){a.parent&&a.parent.children.forEach(function(b){b!=a&&G(b)})}
...
f=f.append("text").attr("class",function(a){return 0==a.depth?"rtNdText":""}).attr("x",function(a){return a.children||a._children?0:k?10:-10}).attr("text-anchor",function(a){return a.parent?a.children||a._children?k?"end":"start":k?"start":"end":"middle"}).style("font-size",function(a){return n(a.info.count)+"pt"});f.append("tspan").attr("dy",".35em").attr("class","tokenText").text(function(a){return r(a.info,1>a.depth)}).style("fill-opacity",1E-6);f.append("tspan").attr("dx",".35em").attr("class",
...
function(a){var b;a:{a=a.info.ids;var c=v.continuationIDs||{};for(b in a)if(b in c){b=!0;break a}b=!1}return b}).style("fill",function(a){return d3.select(this).classed("continuation")?F(a.info):"#444"})};this.search=function(b){a.selectAll("g.node text").classed("foundText",function(a){return b.test(r(a.info))})};this.clearSearch=function(){a.selectAll("g.node text").classed("foundText",!1)};this.showTokenExtras=function(a){if(0==arguments.length)return A;A=a;this.drawRects();return this};this.setRectColor=
...
(function(){doubletree.Trie=function(a,b,c,d,e){function f(a,b,c){this.id=b;this.count=c;this.info={count:c,ids:{}};if(null==a)this.item=k;else{this.item=a;this.info.ids={};this.info.ids[b]=!0;a=a.split(n);for(var d in a)this.info[l[d]]=[a[d]]}this.nodes={};this.addNgram=function(a,b,c){c||(c=1);var d,e;if(0<a.length){d=a.shift();var g=d.split(n);e=r&&this.item==k?"":g.filter(function(a,b){return-1<q.indexOf(l[b])}).map(function(a){return m?a.toLocaleLowerCase():a}).join(n)}else e=d=h;var s;if(e in
this.nodes&&this.nodes[e]instanceof f){s=this.nodes[e];s.info.count+=c;s.info.ids[b]=!0;for(var p in g)e=g[p],-1==s.info[l[p]].indexOf(e)&&s.info[l[p]].push(e)}else s=new f(d,b,c),this.nodes[e]=s;d!=h&&s.addNgram(a,b,c)};this.getUniqRoot=function(){if(this.item==k){var a=Object.keys(this.nodes);if(1==a.length)return this.nodes[a[0]]}return this};this.toTree=function(a){function b(a,c,d){var e={children:[]};e.name=d.item;e.info={};for(var f in d.info)if("Object"===typeof d.info[f]){e.info[f]={};for(var h in d.info[f])e.info[f][h]=
...
e.maxLen=Math.max(a,e.name.length),e.minCount=d3.min(e.children.map(function(a){return a.minCount})));return e}a||(a=[]);var c=JSON.parse(JSON.stringify(this));return b(a,0,c)}}function g(a,b){for(var c in b)a[c]=b[c]}var h=" ",k="_root_",m=!a&&!0;b||(b=["item"]);var l=b;n||(n="\t");var n=c;q||(q=[l[0]]);var q=d,r=e;void 0==r&&(r=!0);var p=new f(k,-1,0);this.addNgram=function(a,b,c){p.addNgram(a,b,c)};this.getUniqRoot=function(){var a=new doubletree.Trie(!m,l,n,q);a.trie(p.getUniqRoot());return a};
...
0,2*Math.PI),b.fill(),b.stroke())}if(e){var n=this;setTimeout(function(){n.draw()},100)}else if(this.documents){c=Math.max(c/this.documents.length,50);m=0;for(l=this.documents.length;m<l;m++)for(var q=0;q<l;q++)if(m<q){var r=this.documents[m].x-this.documents[q].x,p=this.documents[m].y-this.documents[q].y;Math.sqrt(r*r+p*p)<c&&(r*=.1,p*=.1,this.documents[m].targetX+=r,this.documents[q].targetX-=r,this.documents[m].targetY+=p,this.documents[q].targetY-=p,e=!0)}e&&(n=this,setTimeout(function(){n.draw()},
...
c.attr("d",function(a){return l(a.values)}).style("fill",function(a,c){return b(c)});c.enter().append("path").attr("d",function(a){return l(a.values)}).style("fill",function(a,c){return b(c)}).append("title").text(function(a){return a.name});c.exit().remove();this.getVis().selectAll("g.axis").remove();this.getVis().append("g").attr("class","axis x").attr("transform","translate(0,"+a+")").call(k);if(this.getMode()===this.MODE_CORPUS){var r=g/f,p=0;this.getCorpus().getDocuments().each(function(a){this.getVis().select("g.x").append("text").attr("text-anchor",
"end").attr("transform","translate("+p+", 10) rotate(-45)").text(a.getTinyTitle());p+=r},this);c=this.localize("documents")}else c=this.localize("documentSegments");this.getVis().select("g.x").append("text").attr("text-anchor","middle").attr("transform","translate("+g/2+", "+(this.graphMargin.bottom-20)+")").text(c);this.getVis().append("g").attr("class","axis y").attr("transform","translate(0,0)").call(n);c="raw"===this.getApiParam("withDistributions")?this.localize("rawFrequencies"):this.localize("relativeFrequencies");
