...
b,f){a.dom.firstChild&&a.dom.firstChild.nodeValue.match(d)&&(a.addCls("keyword"),c.push(a.dom))})},fetchPrevious:function(){var a=this.innerContainer.first(),b=a.first(".word");if(null!=b&&!1===b.hasCls("loading"))for(;b;){if(b.hasCls("word")){var a=Voyant.data.model.Token.getInfoFromElement(b),c=parseInt(a.docIndex),a=parseInt(a.position),d=this.getDocumentsStore().getAt(c),e=this.getApiParam("limit"),g=!1;if(0===c&&0===a)break;0<c&&0===a?(g=!0,c--,d=this.getDocumentsStore().getAt(c),c=d.get("tokensCount-lexical"),
a=c-e,0>a&&(a=0,this.setApiParam("limit",c))):(e--,a-=e);0>a&&(a=0);b.insertSibling("\x3cdiv class\x3d'loading'\x3e"+this.localize("loading")+"\x3c/div\x3e","before",!1).mask();g||b.destroy();b=d.getId();this.setApiParams({skipToDocId:b,start:a});this.setScrollDownwards(!1);this.load();this.setApiParam("limit",this.INITIAL_LIMIT);break}b.destroy();b=a.first()}},fetchNext:function(){var a=this.innerContainer.first(),b=a.last();if(!1===b.hasCls("loading"))for(var c=$(a.dom).contents().filter(function(){return 3===
this.nodeType}).last();b;){if(b.hasCls("word")){var a=Voyant.data.model.Token.getInfoFromElement(b),d=parseInt(a.docIndex),e=parseInt(a.position),g=this.getDocumentsStore().getAt(a.docIndex),f=g.getId(),g=g.get("tokensCount-lexical");if(e+this.getApiParam("limit")>=g&&d==this.getCorpus().getDocumentsCount()-1)if(d=g-e,1>=d)break;else this.setApiParam("limit",d);b.el.dom.nextSibling===c[0]&&c.remove();b.insertSibling("\x3cdiv class\x3d'loading'\x3e"+this.localize("loading")+"\x3c/div\x3e","after",
!1).mask();b.destroy();this.setApiParams({skipToDocId:f,start:a.position});this.setScrollDownwards(!0);this.load();this.setApiParam("limit",this.INITIAL_LIMIT);break}b.destroy();b=a.last()}},load:function(a,b){a&&(this.innerContainer.first().destroy(),this.innerContainer.setHtml('\x3cdiv class\x3d"readerContainer"\x3e\x3cdiv class\x3d"loading"\x3e'+this.localize("loading")+"\x3c/div\x3e\x3c/div\x3e"),this.innerContainer.first().first().mask());this.getTokensStore().load(Ext.apply(b||{},{params:Ext.apply(this.getApiParams(),
{stripTags:"blocksOnly"})}))},updateText:function(a){var b=this.innerContainer.down(".loading");b&&b.destroy();b=this.getScrollDownwards()?"beforeEnd":"afterBegin";this.innerContainer.first().insertHtml(b,a)},updateChart:function(){}});Ext.define("Voyant.panel.ScatterPlot",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],requires:["Ext.chart.CartesianChart"],alias:"widget.scatterplot",config:{corpus:void 0,options:{xtype:"stoplistoption"}},statics:{i18n:{title:{en:"ScatterPlot"},analysis:{en:"Analysis"},ca:{en:"Correspondence Analysis"},pca:{en:"Principal Components Analysis"},rawFreq:{en:"Raw Frequency"},relFreq:{en:"Relative Frequency"},terms:{en:"Terms"},term:{en:"Term"},numTerms:{en:"Terms Count"},addTerm:{en:"Add Term"},
