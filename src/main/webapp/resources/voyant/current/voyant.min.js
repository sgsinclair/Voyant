...
radius:5,lineWidth:2,strokeStyle:"#fff"}}]}];this.callParent()}});Ext.define("Voyant.panel.RezoViz",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.rezoviz",statics:{i18n:{title:{en:"RezoViz"},categories:{en:"Categories"},people:{en:"People"},locations:{en:"Locations"},organizations:{en:"Organizations"},reload:{en:"Reload"},repulsion:{en:"Repulsion"},stiffness:{en:"Stiffness"},friction:{en:"Friction"},noEntities:{en:"No entities to graph."},loadingEntities:{en:"Loading entities\u2026"}},api:{query:void 0,limit:25,stopList:"auto",type:["organization",
...
type:this.getApiParam("type"),limit:this.getApiParam("limit"),minEdgeCount:this.getApiParam("minEdgeCount"),corpus:a},success:function(a){b.unmask();a=Ext.decode(a.responseText);0==a.entityCollocationsGraph.edges.length?this.showError({msg:this.localize("noEntities")}):(this.processEntities(a.entityCollocationsGraph),this.initGraph())},scope:this})},processEntities:function(a){var b=a.nodes;a=a.edges;for(var c=d3.extent(b,function(a){return a.rawFreq}),d=c[0],c=c[1],e=d3.scale.linear().domain([d,
c]).range([10,24]),d=[],c=0;c<b.length;c++){var f=b[c];f.id=c;d.push({id:c,label:f.term,value:b[c].rawFreq,font:{size:e(f.rawFreq),color:this.categorizedNodeOptions[f.type].font.color},type:f.type,rawFreq:f.rawFreq,title:f.term+(f.rawFreq?" ("+f.rawFreq+")":"")})}this.getNodesStore().loadData(b);b=[];for(c=0;c<a.length;c++)e=a[c].nodes,b.push({from:e[0],to:e[1],title:a[c].count,value:200*a[c].count});this.setNodesDataSet(new vis.DataSet(d));this.setEdgesDataSet(new vis.DataSet(b))},initGraph:function(){var a=
this.getLayout().getRenderTarget();a.update("");a.setWidth(a.getWidth());a.setHeight(a.getHeight());var b={interaction:{hover:!0,hoverConnectedEdges:!0,multiselect:!1},physics:{solver:"barnesHut"},nodes:this.nodeOptions,edges:this.edgeOptions},c=new vis.Network(a.dom,{nodes:this.getNodesDataSet(),edges:this.getEdgesDataSet()},b);c.on("selectNode",function(a){this.doNodeSelect(a.nodes[0])}.bind(this));c.on("deselectNode",function(a){this.removeHighlight();c.unselectAll();a=a.nodes[0];void 0!==a&&setTimeout(this.doNodeSelect.bind(this),
