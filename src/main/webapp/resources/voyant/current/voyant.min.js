...
start:0>d?0:d});this.load(!0)},e)}b.removeAll();var e=a.getDocuments();if(50>e.getTotalCount()){for(var f=a.getWordTokensCount(),g=[],h=1E6,k=-1,l=0;l<e.getCount();l++){var m=e.getAt(l),n=m.get("index"),m=m.get("tokensCount-lexical");m<h&&(h=m);m>k&&(k=m);g.push({index:n,count:m,fraction:m/f})}for(l=0;l<g.length;l++)m=g[l],m.relativeHeight=m.count==k?1:0+(m.count-h)/(k-h)*1,d(m,this)}},highlightKeywords:function(a,b){Ext.isArray(a)||(a=[a]);this.innerContainer.first().select("span[class*\x3dkeyword]").removeCls("keyword");
