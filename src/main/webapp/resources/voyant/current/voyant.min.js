...
api:{panels:void 0},glyph:"xf17a@FontAwesome"},layout:"border",header:!1,height:"100%",width:"100%",listeners:{afterrender:function(){var a=Ext.urlDecode(window.location.search.substring(1)),a=decodeURI(a.layout).replace(/r1/g,"region").replace(/i1/g,"items").replace(/s1/g,"split").replace(/c1/g,"collapsible").replace(/c2/g,"collapsed").replace(/w1/g,"width").replace(/h1/g,"height").replace(/p1/g,"%").replace(/"x1":"/g,'"xtype":"').replace(/c3/g,"center").replace(/n1/g,"north").replace(/e1/g,"east").replace(/s2/g,
"south").replace(/w2/g,"west").replace(/"xtype":"(\w+)"/g,function(a,b){Ext.ClassManager.getByAlias("widget."+b.toLowerCase())||(b="Links"==b?"CollocatesGraph":"CorpusGrid"==b?"Documents":"CorpusSummary"==b?"Summary":"CorpusTypeFrequenciesGrid"==b?"CorpusTerms":"DocumentInputAdd"==b?"CorpusTerms":"DocumentTypeCollocateFrequenciesGrid"==b?"CorpusTerms":"DocumentTypeFrequenciesGrid"==b?"DocumentTerms":"DocumentTypeKwicsGrid"==b?"Contexts":"TypeFrequenciesChart"==b?"Trends":"VisualCollocator"==b?"CollocatesGraph":
"NoTool");return'"xtype":"'+b.toLowerCase()+'"'}),b;try{b=Ext.decode(a)}catch(c){b={region:"center",html:"\x3cdiv\x3eError constructing layout:"+c+"\x3c/div\x3e"}}null==b&&(b={region:"center",html:"\x3cdiv\x3eError: no layout information found.\x3c/div\x3e"});this.addLayouts(b);this.add(b)}},addLayouts:function(a){for(var b=Ext.getBody().getSize(),c=0;c<a.length;c++){var d=a[c];Ext.isString(d.width)?d.width=Math.round(b.width*parseInt(d.width)/100):Ext.isString(d.height)&&(d.height=Math.round(b.height*
parseInt(d.height)/100));d.items&&1<d.items.length?(d.layout="border",this.addLayouts(d.items)):d.layout="fit"}}});Ext.define("Voyant.VoyantApp",{extend:"Ext.app.Application",mixins:["Voyant.util.Deferrable","Voyant.util.Localization","Voyant.util.Api"],requires:["Voyant.util.ResponseError"],name:"VoyantApp",statics:{i18n:{error:{en:"Error"},serverResponseError:{en:"The server error reponse:"}}},config:{baseUrl:void 0,tromboneUrl:void 0},constructor:function(a){this.setBaseUrl(this.config.baseUrl);this.setTromboneUrl(this.config.baseUrl+"trombone");Voyant.application=this;this.mixins["Voyant.util.Api"].constructor.apply(this,
