/* This file created by JSCacher. Last modified: Sat May 13 18:13:54 EDT 2017 */
...
this.loadData(void 0,c)}a=this;window.addEventListener("beforeunload",function(b){if(a.getIsEdited()){var c=a.localize("editsAndLeaving");return(b||window.event).returnValue=c}})},setIsEdited:function(a){this.getSaveItTool()&&this.getSaveItTool().setDisabled(0==a);this.callParent(arguments)},listeners:{boxready:function(){this.init()},notebookWrapperMoveUp:function(a){a=this.items.findIndex("id",a.id);0==a?Ext.Msg.show({title:this.localize("error"),msg:this.localize("cannotMoveHigher"),buttons:Ext.MessageBox.OK,
icon:Ext.MessageBox.WARNING}):(this.move(a,a-1),this.redoOrder())},notebookWrapperMoveDown:function(a){a=this.items.findIndex("id",a.id);a==this.items.getCount()-1?Ext.Msg.show({title:this.localize("error"),msg:this.localize("cannotMoveLower"),buttons:Ext.MessageBox.OK,icon:Ext.MessageBox.WARNING}):(this.move(a,a+1),this.redoOrder())},notebookWrapperRemove:function(a){this.remove(a);this.redoOrder()},notebookWrapperRun:function(a){},notebookWrapperAdd:function(a,b){var c=this.items.findIndex("id",
a.id),d=a.getXType(a);("notebooktexteditorwrapper"==d&&!b.hasModifier()||"notebookcodeeditorwrapper"==d&&b.hasModifier()?this.addCode("",c+1):this.addText("",c+1)).getTargetEl().scrollIntoView(this.getTargetEl(),null,!0,!0);this.redoOrder()}},redoOrder:function(){this.query("notebookwrappercounter").forEach(function(a,b){a.setOrder(b)})},loadFromUrl:function(a,b){var c=this;if(0==a.indexOf("https://gist.github.com/")||0==a.indexOf("https://gist.githubusercontent.com/"))a="gist"+a.substring(a.indexOf(".com")+
4,a.length-("/"==a.charAt(a.length-1)?1:0));if(0==a.indexOf("gist/")){var d=a.substring(5).split("/");if(2>d.length)return this.showError(this.localize("invalidGistUrl"));d[2]&&"raw"==d[2]&&d.splice(2,1);var e="gist/"+d.slice(0,3).join("/");window.history.pushState({url:a},"Spiral Notebook: "+e,this.getBaseUrl()+"spiral/"+e);a="https://gist.githubusercontent.com/"+d.slice(0,2).join("/")+"/raw/"+(d[2]?d[2]:"")}else 0==a.indexOf("http")&&window.history.pushState({url:a},"Spiral Notebook: "+a,this.getBaseUrl()+
"spiral/?input\x3d"+a);c.mask(this.localize("fetchingNotebook"));Ext.Ajax.request({url:this.getTromboneUrl(),params:{tool:"notebook.NotebookManager",notebook:a},scope:this}).then(function(d,e){c.unmask();var h=Ext.decode(d.responseText),k=c.getBlocksFromString(h.notebook.jsonData);k.metadata=k.metadata||{};k.metadata.previousNotebook=a;Ext.applyIf(k.metadata,{originalUrl:a});if(k&&k.metadata&&k.metadata.url&&(new URL(k.metadata.url)).hostname!=window.location.hostname)return Ext.Msg.confirm(c.localize("differentUrlTitle"),
(new Ext.XTemplate(c.localize("differentUrl"))).apply([k.metadata.url,window.location.href]),function(a){c.loadData(k,"yes"==a)},c);c.loadData(k,b)},function(b,d){c.showResponseError("Unable to load specified notebook: "+a,b);c.unmask();c.loadData()})},loadData:function(a,b){a||(a={blocks:[{type:"text",input:"\x3ch1 style\x3d'text-align: center;'\x3eSpiral Notebook Template (title)\x3c/h1\x3e\x3cp\x3eThis is a Spiral Notebook, a dynamic document that combines writing, code and data in service of reading, analyzing and interpreting digital texts.\x3c/p\x3e\x3cp\x3eSpiral Notebooks are composed of text blocks (like this one) and code blocks (like the one below). You can \x3cspan class\x3d'marker'\x3eclick on the blocks to edit\x3c/span\x3e them and add new blocks by clicking add icon that appears in the left column when hovering over a block.\x3c/p\x3e"},
...
input:this.wrap(d)};a.push(b)},this);var e=this.getMetadata();Ext.applyIf(e,{created:(new Date).getTime(),modified:(new Date).getTime(),version:this.getVersion(),url:window.location.href});return{metadata:e,blocks:a}},exportAll:function(){var a=this.getExportAllJson();1==a.blocks.length&&"code"==a.blocks[0].type&&(a.blocks=blocks[0].input);Ext.Msg.prompt("Export Notebook","Currently only copying and pasting the notebook is available. You can select all the contents of the box below and copy to the clipboard.",
