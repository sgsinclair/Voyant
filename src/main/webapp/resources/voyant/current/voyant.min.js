...
2;this.maxFreq={term:null,value:0};this.maxRadius=0;this.selectedDocs=new Ext.util.MixedCollection;this.cache=new Ext.util.MixedCollection;this.currentTerms={};this.termsFilter=[];this.initialized=!1}
...
a/30;this.MAX_LINE_WIDTH=a-this.maxRadius/2},calculateGraphHeights:function(){var a=.5*this.maxRadius;if(this.SEPARATE_LINES_FOR_TERMS){var b=this.termsFilter;this.cache.each(function(c,e,f){e=this.maxRadius*b.length;for(f=0;f<b.length;f++)c.terms[b[f]]||(e-=this.maxRadius);0==e&&(e=this.maxRadius);c.height=e+a},this)}else{var c=Math.max(this.maxRadius,this.MIN_GRAPH_SEPARATION);this.cache.each(function(b,e,f){b.height=c+a},this)}},findLongestDocument:function(a){a=a.getTotalWordTokens();a>this.maxDocLength&&
(this.maxDocLength=a)},getTitleWidth:function(a){var b=a.title;this.DRAW_SHORT_TITLES&&(b=this.cache.indexOf(a)+1+")");this.ctx.textBaseline="top";this.ctx.font="bold 12px Verdana";return this.ctx.measureText(b).width},setTitleWidthsAndMaxTitleWidth:function(){this.cache.each(function(a,b){var c=this.getTitleWidth(a);a.titleWidth=c;c>this.MAX_LABEL_WIDTH&&(this.MAX_LABEL_WIDTH=c)},this)},setLineLengths:function(){this.cache.each(function(a,b){var c;c=this.UNIFORM_LINE_LENGTH?this.MAX_LINE_WIDTH:Math.log(a.getTotalWordTokens())/
Math.log(this.maxDocLength)*this.MAX_LINE_WIDTH;a.lineLength=c},this)},drawGraph:function(a){null!=this.intervalId&&clearInterval(this.intervalId);this.mouseOver?this.intervalId=setInterval(this.doDraw.bind(this,[a]),this.INTERVAL):setTimeout(this.doDraw.bind(this,[a]),5)},doDraw:function(a){this.clearCanvas();this.yIndex=this.maxRadius;!0===a&&(this.yIndex+=30);this.cache.each(this.drawDocument,this);!0===a?this.drawLegend():this.drawToolTip();this.doDrag()},drawDocument:function(a,b,c){var d=a.lineLength,
e=this.MAX_LABEL_WIDTH-a.titleWidth,f=5;this.ctx.textBaseline="top";this.ctx.font="bold 12px Verdana";if(null!=this.dragInfo&&this.dragInfo.oldIndex==b)this.ctx.fillStyle="rgba(128, 128, 128, 0.5)",f+=e,this.ctx.fillText(a.title,f,this.yIndex),this.SEPARATE_LINES_FOR_TERMS&&(this.yIndex+=a.height);else{c=function(){f=g.MAX_LABEL_WIDTH+g.maxRadius;g.ctx.strokeStyle="rgba(128, 128, 128, 1.0)";g.ctx.fillStyle="rgba(128, 128, 128, 1.0)";g.ctx.lineWidth=.25;g.ctx.beginPath();g.ctx.moveTo(f,g.yIndex-6);
g.ctx.lineTo(f,g.yIndex+6);g.ctx.stroke();g.ctx.beginPath();g.ctx.moveTo(f,g.yIndex);g.ctx.lineTo(f+d,g.yIndex);g.ctx.stroke();g.ctx.beginPath();g.ctx.moveTo(f+d,g.yIndex-6);g.ctx.lineTo(f+d,g.yIndex+6);g.ctx.stroke()};this.DRAW_TITLES&&(f+=e,this.ctx.fillStyle="rgba(128, 128, 128, 1.0)",e=a.title,this.DRAW_SHORT_TITLES&&(e=b+1+")"),this.ctx.fillText(e,f,this.yIndex));this.yIndex+=4;var g=this;this.SEPARATE_LINES_FOR_TERMS?null!=this.termsFilter&&0!==this.termsFilter.length||c():c();var e=2*Math.PI,
h=0;a.freqCounts={};var k=a.terms,m=null!=this.lastClickedBubbles[b],l=0,n;for(n in k)if(-1!==this.termsFilter.indexOf(n)){var q=k[n];if(q){l++;this.SEPARATE_LINES_FOR_TERMS&&c();var r=0,p=q.color.join(",");this.ctx.strokeStyle="rgba("+p+", 1)";this.ctx.fillStyle="rgba("+p+", 0.35)";this.ctx.lineWidth=.25;for(var h=h+q.rawFreq,r=r+q.rawFreq,s=m&&this.lastClickedBubbles[b][n],t=0;t<q.pos.length;t++){var u=q.pos[t];if(0<u.radius){var x=!1;s&&this.lastClickedBubbles[b][n]==u.id&&(this.ctx.strokeStyle=
"rgba(0, 0, 0, 0.75)",this.ctx.fillStyle="rgba("+p+", 0.5)",this.ctx.lineWidth=1,x=!0);this.ctx.beginPath();this.ctx.arc(u.x+f,this.yIndex,u.radius,0,e,!0);this.ctx.closePath();this.ctx.fill();this.ctx.stroke();x&&(this.ctx.strokeStyle="rgba("+p+", 1)",this.ctx.fillStyle="rgba("+p+", 0.35)",this.ctx.lineWidth=.25)}}a.freqCounts[n]=r;this.SEPARATE_LINES_FOR_TERMS&&(this.ctx.fillStyle="rgba(0, 0, 0, 1.0)",this.ctx.font="10px Verdana",this.ctx.fillText(r,f+d+5,this.yIndex-4),this.yIndex+=this.maxRadius)}}this.SEPARATE_LINES_FOR_TERMS&&
0==l&&(c(),this.ctx.fillStyle="rgba(0, 0, 0, 1.0)",this.ctx.font="10px Verdana",this.ctx.fillText(0,f+d+5,this.yIndex-4),this.yIndex+=this.maxRadius);f+=d;this.SEPARATE_LINES_FOR_TERMS||(this.ctx.fillStyle="rgba(0, 0, 0, 1.0)",this.ctx.font="10px Verdana",this.ctx.fillText(h,f+5,this.yIndex-4))}this.yIndex=this.SEPARATE_LINES_FOR_TERMS?this.yIndex+.5*this.maxRadius:this.yIndex+a.height;this.yIndex-=4},drawLegend:function(){var a=this.MAX_LABEL_WIDTH+this.maxRadius;this.ctx.textBaseline="top";this.ctx.font=
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];m=0;f=function(){var a,f,h,k,n=this.wordsToArrange-1,t=g.canvas.width,s=g.canvas.height,q=.5*t,r=.5*s;do{h=p[m];if(void 0!==h){for(var v=Math.random()*Math.PI,w=.25*Math.random()*h.size,A=.5*(Math.random()-.5),y=.5*h.width,z=.5*h.height;;){a=Math.floor((q+Math.cos(v)*w*this.ratio-y)/this.COARSENESS)*this.COARSENESS;f=Math.floor((r+Math.sin(v)*w-z)/this.COARSENESS)*this.COARSENESS;
k=a+y>=t||f+z>=s?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;v+=A;w+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<m;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}m++;if(m>=p.length){clearTimeout(l);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(n--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;l=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
"color"],listeners:{load:function(a,b,c,d){a=this.down("#termsView");for(c=0;c<b.length;c++)d=b[c],console.log("select",d.get("term")),a.select(d,!0)},scope:this}}),constructor:function(){Ext.apply(this,{title:this.localize("title")});this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments);this.on("loadedCorpus",function(a,b){this.setCorpus(b);this.getDocStore().getProxy().setExtraParam("corpus",b.getId());this.isVisible()&&this.getDocStore().load();this.getDocTermStore().getProxy().setExtraParam("corpus",
...
k.id;void 0===c[b]&&(c[b]=!0,d.push(k))}a={id:a.getContextTerm(),label:a.getContextTerm(),title:a.getContextTerm()+" ("+a.getContextTermRawFreq()+")",type:"context",value:a.getContextTermRawFreq(),start:0};var m=a.id;void 0===c[m]&&(c[m]=!0,d.push(a));var l=!1;f.forEach(function(a){if(a.from==b&&a.to==m||a.from==m&&a.to==b)l=!0});l||e.push({from:b,to:m})}});this.getNodeDataSet().add(d);f.add(e);this.forceUpdate();this.getNetwork().fit()}},initGraph:function(){var a=this.getLayout().getRenderTarget();
