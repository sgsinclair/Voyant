...
a.on("dragEnd",function(a){var b=a.nodes[0];null!=b&&(this.isOffCanvas(a.pointer.DOM)?(this.unmask(),this.mask("cleaning"),this.getNodeDataSet().remove(b),this.unmask()):this.getNodeDataSet().update({id:b,fixed:!0}))}.bind(this));a.on("click",function(a){this.getContextMenu().hide()}.bind(this));a.on("doubleClick",function(a){a=a.nodes[0];null!=a&&(a=this.getNodeDataSet().get(a),this.itemdblclick(a))}.bind(this));a.on("oncontext",function(a){a.event.preventDefault();var b=this.getNetwork().getNodeAt(a.pointer.DOM);
if(null!=b){this.getNetwork().selectNodes([b]);var b=this.getNodeDataSet().get(b),e=this.getContextMenu();e.queryById("label").setText(b.label);e.queryById("fixed").setChecked(b.fixed);e.showAt(a.event.pageX,a.event.pageY)}}.bind(this))}},start:function(){var a=this,b=this.getForce(),c=b.drag();c.on("dragstart",this.dragstart);c.on("drag",function(b){a.drag.call(a,b)});c.on("dragend",function(b){a.isOffCanvas(b)&&(b.fixed=!1,d3.select(this).classed("fixed",!1));a.dragend.call(a,b)});link=this.getLink().data(b.links(),
function(a){return a.source.term+a.source.type+"-"+a.target.term+a.source.type});link.enter().insert("line",".node").attr("class","link");link.exit().remove();this.setLink(link);node=this.getNode().data(b.nodes(),function(a){return a.term+a.type});var d=b.nodes().filter(function(a){return"keyword"==a.type}).map(function(a){return a.value}),e=b.nodes().filter(function(a){return"context"==a.type}).map(function(a){return a.value}),f=d3.min(d),d=d3.max(d),g=d3.min(e),e=d3.max(e),h=d3.scale.linear().domain([f<
g?f:g,d>e?d:e]).range([8,20]),k=this.getCorpusColours();node.enter().append("text").attr("class",function(a){return"node "+a.type}).attr("text-anchor","middle").style("fill",function(a){return k("keyword"==a.type?1:2)}).attr("dx",12).attr("dy",".35em").text(function(a){return a.term}).style("font-size",function(a){return h(a.value)+"pt"}).on("dblclick",function(){a.dragstart.apply(this,arguments);a.itemdblclick.apply(a,arguments)}).on("click",this.itemclick).on("mouseover",function(b){this.textContent=
b.term+" ("+b.value+")";b.wasfixed=b.fixed;a.itemclick.apply(this,arguments)}).on("mouseout",function(a,b,c,d){this.textContent=a.term;d3.select(this).classed("fixed",a.fixed=a.wasfixed)}).call(c);node.exit().remove();this.setNode(node);b.start()},isOffCanvas:function(a){return 0>a.x||0>a.y||a.x>this.getGraphWidth()||a.y>this.getGraphHeight()},itemdblclick:function(a){this.getCorpus().getCorpusCollocates({autoLoad:!1}).load({params:Ext.apply(this.getApiParams(),{query:a.label,start:a.start,limit:10}),
callback:function(b,c,d){if(d){var e=this.getNodeDataSet(),f=this.getEdgeDataSet();this.getApiParam("limit");a.start+=10;var g=[a.label,a.type].join(";");b.forEach(function(a){if(a.getContextTerm()!=a.getTerm()){var b={label:a.getContextTerm(),type:"context",value:a.getContextTermRawFreq(),start:0};a=[b.label,b.type].join(";");b.id=a;var c=e.get(a);null==c&&e.add(b);b=[g,a].join("--");c=f.get(b);null==c&&f.add({id:b,from:g,to:a})}})}},scope:this})}});Ext.define("Voyant.panel.Contexts",{extend:"Ext.grid.Panel",mixins:["Voyant.panel.Panel"],requires:["Voyant.data.store.Contexts"],alias:"widget.contexts",statics:{i18n:{title:{en:"Contexts"},emptyText:{en:"No matching results."},document:{en:"Document"},documentTip:{en:"The document of the occurrence."},helpTip:{en:"The Keywords in Context tool shows each occurrence of a keyword with a bit of surounding text (the context). It can be useful for studying more closely how terms are used in different contexts. Features include:\x3c/p\x3e\x3cul\x3e\x3cli\x3ereordering document, by keyword or by left or right context\x3c/li\x3e\x3cli\x3ea search box for queries (hover over the magnifying icon for help with the syntax)\x3c/li\x3e\x3c/ul\x3e"},
...
id:{en:"ID"},documentTitle:{en:"Title"},documentAuthor:{en:"Author"},tokensCountLexical:{en:"Words"},typesCountLexical:{en:"Types"},typeTokenRatioLexical:{en:"Ratio"},language:{en:"Language"},matchingDocuments:{en:"Matching documents: {count}"},error:{en:"Error"},remove:{en:"Remove"},reorder:{en:"Reorder"},keep:{en:"Keep"},modify:{en:"Modify"},newCorpus:{en:"New Corpus"},modifyTip:{en:"Click this button to create a new corpus by selecting a subset of documents or by re-ordering documents."},allSelectedError:{en:"You have selected all documents, you must select a subset of documents to remove or keep."},
removeSelectedDocuments:{en:"Create a \x3ci\x3enew\x3c/i\x3e corpus that removes (does NOT include) the {0:plural('selected document')}?"},removeFilteredDocuments:{en:"Create a \x3ci\x3enew\x3c/i\x3e corpus that removes (does NOT include) the {0:plural('filtered document')}?"},keepSelectedDocuments:{en:"Create a \x3ci\x3enew\x3c/i\x3e corpus that only keeps the {0:plural('selected document')}?"},keepFilteredDocuments:{en:"Create a \x3ci\x3enew\x3c/i\x3e corpus that only keeps the {0:plural('filtered document')}?"},
selectOrFilterError:{en:"You need to first select documents by clicking on one or more rows or by performing a search query."},onlyOneError:{en:"Your corpus has only one document, you can't remove or keep documents to create a new corpus."},reorderFilteredError:{en:"You cannot reorder a filtered (after search query) corpus. Please create a new corpus first (with the \x3ci\x3eRemove\x3c/i\x3e or \x3ci\x3eKeep\x3c/i\x3e button) and then reorder the new corpus."},reorderOriginalError:{en:"Please reorder the corpus first (drag and drop the rows in the table)."},
reorderSelectedDocuments:{en:"Create a \x3ci\x3enew\x3c/i\x3e corpus based on the order shown?"}},api:{query:void 0,docIndex:void 0,docId:void 0},glyph:"xf0ce@FontAwesome"},MODE_EDITING:"editing",MODE_NORMAL:"normal",config:{mode:this.MODE_NORMAL},constructor:function(a){var b=Ext.create("Voyant.data.store.Documents",{selModel:{pruneRemoved:!1}}),c=[{xtype:"querysearchfield"},{xtype:"totalpropertystatus"}];a&&a.mode==this.MODE_EDITING||c.push({text:this.localize("modify"),tooltip:this.localize("modifyTip"),
glyph:"xf044@FontAwesome",scope:this,handler:function(a){Ext.create("Ext.window.Window",{title:this.localize("title"),modal:!0,width:"80%",minWidth:300,minHeight:200,height:"80%",layout:"fit",items:{xtype:"documents",mode:this.MODE_EDITING,corpus:this.getStore().getCorpus(),header:!1,viewConfig:{plugins:{ptype:"gridviewdragdrop"},listeners:{beforedrop:function(a,b,c,d,k){return this.getStore().getCount()<this.getStore().getCorpus().getDocumentsCount()?(a=this.up("panel"),Ext.Msg.show({title:a.localize("error"),
message:a.localize("reorderFilteredError"),buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR}),!1):!0}}}},buttons:[{text:this.localize("remove"),glyph:"xf00d@FontAwesome",hidden:1==this.getStore().getCorpus().getDocumentsCount(),handler:this.keepRemoveReorderHandler,itemId:"remove",scope:this},{text:this.localize("keep"),glyph:"xf00c@FontAwesome",hidden:1==this.getStore().getCorpus().getDocumentsCount(),handler:this.keepRemoveReorderHandler,itemId:"keep",scope:this},{text:this.localize("reorder"),glyph:"xf00c@FontAwesome",
hidden:1==this.getStore().getCorpus().getDocumentsCount(),handler:this.keepRemoveReorderHandler,itemId:"reorder",scope:this},{text:"Cancel",handler:function(a){a.up("window").close()}}]}).show()}});Ext.apply(this,{title:this.localize("title"),emptyText:this.localize("emptyText"),columns:[{xtype:"rownumberer",renderer:function(a,b,c){return c.getIndex()+1},sortable:!1},{text:this.localize("documentTitle"),dataIndex:"title",sortable:!0,renderer:function(a,b,c){return c.getTitle()},flex:3},{text:this.localize("documentAuthor"),
dataIndex:"author",sortable:!0,hidden:!0,renderer:function(a,b,c){return c.getAuthor()},flex:2},{text:this.localize("tokensCountLexical"),dataIndex:"tokensCount-lexical",renderer:Ext.util.Format.numberRenderer("0,000"),sortable:!0,width:"autoSize"},{text:this.localize("typesCountLexical"),dataIndex:"typesCount-lexical",renderer:Ext.util.Format.numberRenderer("0,000"),width:"autoSize"},{text:this.localize("typeTokenRatioLexical"),dataIndex:"typeTokenRatio-lexical",renderer:function(a){return Ext.util.Format.percent(a)},
width:"autoSize"},{text:this.localize("language"),dataIndex:"language",hidden:!0,renderer:function(a,b,c,g,h,k,m){return m.ownerCt.getLanguage(a)},width:"autoSize"}],store:b,selModel:Ext.create("Ext.selection.RowModel",{mode:"MULTI",listeners:{selectionchange:{fn:function(a,b){this.getApplication().dispatchEvent("documentsClicked",this,b,this.getStore().getCorpus())},scope:this}}}),dockedItems:[{dock:"bottom",xtype:"toolbar",items:c}]});this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,
arguments);this.on("loadedCorpus",function(a,b){this.store.setCorpus(b);this.store.load({params:this.getApiParams()})});this.on("query",function(a,b){this.setApiParam("query",b);this.store.load({params:this.getApiParams()})});a.embedded&&("Voyant.data.model.Corpus"==Ext.getClass(a.embedded).getName()?a.corpus=a.embedded:"Voyant.data.store.Documents"==Ext.getClass(a.embedded).getName()&&(this.store.setRecords(a.embedded.getData()),a.corpus=a.embedded.getCorpus()));a.corpus&&this.fireEvent("loadedCorpus",
