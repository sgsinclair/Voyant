...
"-"+a.target}).join("; ")}});Ext.define("Voyant.data.util.Geonames",{mixins:["Voyant.util.Localization"],statics:{i18n:{failedToFetchGeonames:"Failed to load location information."}},config:{data:{},queries:void 0,corpus:void 0,isIncrementalLoadingOccurrences:!1},constructor:function(a,b){a=a||{};this.callParent([a]);this.setCorpus(a.corpus)},load:function(a,b){b=b||Voyant.application.getDeferred(this);var c=this,d={corpus:this.getCorpus().getAliasOrId(),queries:this.getQueries(),tool:"corpus.Geonames",limit:200};Ext.apply(d,
a||{});a.noOverwrite||c.setData({});Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:d,scope:this}).then(function(d){d=Ext.JSON.decode(d.responseText);d.progress&&d.progress.progress&&new Voyant.widget.ProgressMonitor({progress:d.progress.progress,success:function(){debugger;c.call(c,a,b)},failure:function(a){Voyant.application.showResponseError(c.localize("failedToFetchGeonames"),a)},scope:c});d&&d.geonames&&(a.noOverwrite||c.setData(d.geonames),b.resolve(d))},function(a){Voyant.application.showResponseError(c.localize("failedToFetchGeonames"),
a)});return b.promise},getCitiesCount:function(){return Object.keys(this.getData().cities.cities).length},getTotalCitiesCount:function(){return this.getData().cities.total},hasMoreCities:function(){return this.getCitiesCount()<this.getTotalCitiesCount()},eachCity:function(a,b,c){var d=this.getData().cities.cities,e=[],f;for(f in d)e.push(Ext.apply(d[f],{id:f}));e.sort(function(a,b){return a.rawFreq==b.rawFreq?a.label-b.label:b.rawFreq-a.rawFreq});c&&e.length>c&&(e=e.slice(0,c));e.forEach(function(c){a.call(b,
c)})},getConnectionsCount:function(){return Object.keys(this.getData().connectionCounts.connectionCounts).length},getTotalConnectionsCount:function(){return this.getData().connectionCounts.total},eachConnection:function(a,b,c){var d=this.getData().connectionCounts.connectionCounts,e=this.getData().cities.cities;orderedConnections=[];for(var f in d){var g=f.split("-");orderedConnections.push({source:Ext.apply({id:g[0]},e[g[0]]),target:Ext.apply({id:g[1]},e[g[1]]),rawFreq:d[f]})}orderedConnections.sort(function(a,
b){return b.rawFreq-a.rawFreq});c&&orderedConnections.length>c&&(orderedConnections=orderedConnections.slice(0,c));orderedConnections.forEach(function(c){a.call(b,c)})},getConnectionOccurrence:function(a){if(this.getData().cities){var b=this.getData().connections,c=this.getData().cities.cities;if(!this.getIsIncrementalLoadingOccurrences()&&b.connections.length<b.total&&a+100>b.connections.length){this.setIsIncrementalLoadingOccurrences(!0);var d=this;this.load({start:b.connections.length,noOverwrite:!0,
suppressCities:!0,suppressConnections:!0}).then(function(a){d.setIsIncrementalLoadingOccurrences(!1);b.connections=b.connections.concat(a.geonames.connections.connections)})}if(b&&b.connections[a]){var e=b.connections[a];e.index=a;Ext.apply(e.source,c[e.source.id]);Ext.apply(e.target,c[e.target.id]);return e}return null}},getAllConnectionOccurrences:function(a,b){for(var c=[],d=0;d<this.getTotalConnectionsCount();d++){var e=this.getConnectionOccurrence(d);e&&e.target.id==b&&e.source.id==a&&c.push(e)}return c}});Ext.define("Voyant.data.model.Corpus",{alternateClassName:["Corpus"],mixins:["Voyant.notebook.util.Embed","Voyant.notebook.util.Show","Voyant.util.Transferable","Voyant.util.Localization","Voyant.util.Assignable"],transferable:"loadCorpusTerms loadTokens getPlainText getText getWords getString getLemmasArray".split(" "),embeddable:"Voyant.panel.Summary Voyant.panel.Cirrus Voyant.panel.Documents Voyant.panel.CorpusTerms Voyant.panel.Reader Voyant.panel.Trends Voyant.panel.TermsRadio Voyant.panel.DocumentTerms Voyant.panel.TermsBerry Voyant.panel.CollocatesGraph Voyant.panel.Contexts Voyant.panel.WordTree Voyant.panel.Veliza Voyant.panel.ScatterPlot Voyant.panel.Topics".split(" "),
...
millisPerAnimation:2E3,annotationsId:void 0},glyph:"xf124@FontAwesome"},config:{map:void 0,overlay:void 0,contentEl:void 0,isOpenLayersLoaded:!1,isArcLoaded:!1,animationDelay:25,isProj4Loaded:!1,projection:void 0,filterWidgets:new Ext.util.MixedCollection,drawInteraction:void 0,drawMode:!1,currentAnnotation:void 0,baseLayers:{},annotations:void 0,annotationsLoadedIfAvailable:!1},html:'\x3cdiv class\x3d"map"\x3e\x3c/div\x3e\x3cdiv class\x3d"ticker"\x3e\x3c/div\x3e\x3cdiv class\x3d"ol-popup"\x3e\x3ca href\x3d"#" class\x3d"ol-popup-closer"\x3e\x3c/a\x3e\x3cdiv class\x3d"popup-content"\x3e\x3c/div\x3e\x3c/div\x3e',
...
this.getIsArcLoaded()&&this.getIsProj4Loaded()&&this.getAnnotationsLoadedIfAvailable()){var a=this.body.down(".map").setId(Ext.id()),b=this.body.down(".ol-popup"),c=new ol.Overlay({element:b.dom,autoPan:!0,autoPanAnimation:{duration:250}});this.setOverlay(c);var d=this,e=b.down(".ol-popup-closer").dom;e.onclick=function(){c.setPosition(void 0);e.blur();return!1};var f=this.getBaseLayers();f.wms4326=new ol.layer.Tile({preload:Infinity,source:new ol.source.TileWMS({url:"https://ahocevar.com/geoserver/wms",
...
b.get("selected")?(d.dispatchEvent("termsClicked",d,[b.get("forms").join("|")]),c=!0):"connection"===b.get("type")&&b.get("selected")&&!c?(d.getFilterWidgets().each(function(c){c=c.getGeonames();if(null!=c){var e="\x3cul\x3e",f=-1;c.getAllConnectionOccurrences(b.get("source"),b.get("target")).forEach(function(a){a.docIndex!=f&&(f=a.docIndex,e+=d.getCorpus().getDocument(f).getFullLabel()+":\x3cbr\x3e");e+="\x3cli\x3e"+a.source.left+'\x3ca href\x3d"http://www.geonames.org/'+a.source.id+'" target\x3d"_blank" docIndex\x3d'+
a.docIndex+" offset\x3d"+a.source.position+" location\x3d"+a.source.form+' class\x3d"termLocationLink"\x3e'+a.source.form+"\x3c/a\x3e "+a.source.right+" [...] "+a.target.left+'\x3ca href\x3d"http://www.geonames.org/'+a.target.id+'" target\x3d"_blank" docIndex\x3d'+a.docIndex+" offset\x3d"+a.target.position+" location\x3d"+a.target.form+' class\x3d"termLocationLink"\x3e'+a.target.form+"\x3c/a\x3e "+a.target.right+"\x3c/li\x3e"});c=b.get("text");e+="\x3c/ul\x3e";d.getContentEl().setHtml("\x3ch3\x3e"+
c+"\x3c/h3\x3e"+e);d.getOverlay().setPosition(a.coordinate);Ext.select(".termLocationLink").elements.forEach(function(a){a.onmouseover=function(){d.showTermInCorpus(a.getAttribute("docIndex"),a.getAttribute("offset"),a.getAttribute("location"))}})}}),c=!0):"annotation"!==b.get("type")||c||(d.setCurrentAnnotation(b),d.getContentEl().setHtml('\x3ctextarea class\x3d"annotation-text" rows\x3d"5" cols\x3d"60"\x3e'+(b.get("text")?b.get("text"):"")+'\x3c/textarea\x3e\x3cbutton class\x3d"saveAnnotation"\x3eSave\x3c/button\x3e\x3cbutton class\x3d"deleteAnnotation"\x3eDelete\x3c/button\x3e'),
...
a.get("type"))return d.travelStyleFunction(a,g.getView().getResolution())},updateWhileAnimating:!0,updateWhileInteracting:!0});g.on("pointermove",function(a){if(!d.getDrawMode()){h.getSource().clear();var b=g.getFeaturesAtPixel(a.pixel);if(b){for(var c=0;b[c].get("selected")&&(c++,c!==b.length););if(c<b.length){var e=b[c],b={font:"12px Calibri,sans-serif",textAlign:"center",offsetY:-15,fill:new ol.style.Fill({color:[0,0,0,1]}),stroke:new ol.style.Stroke({color:[255,255,255,.5],width:4})};b.text=e.get("text");
b=new ol.style.Style({text:new ol.style.Text(b),zIndex:1});"annotation"!==e.get("type")&&(c=new ol.Feature({text:e.get("text"),geometry:e.getGeometry(),forms:e.get("forms"),selected:!0,coordinates:e.get("coordinates"),width:e.get("width"),visible:e.get("visible"),color:e.get("color"),type:e.get("type"),confidence:e.get("confidence"),source:e.get("source"),target:e.get("target")}),h.getSource().addFeature(c));e.getGeometry();a=new ol.Feature({geometry:new ol.geom.Point(a.coordinate),selected:!0});
a.setStyle(b);h.getSource().addFeature(a);"city"===e.get("type")&&d.getMap().getLayers().forEach(function(a){var b=a.get("id");b&&-1!==b.indexOf("connections")&&(connections=a.getSource().getFeatures(),connections.forEach(function(a){a.get("target")!==e.get("cityId")&&a.get("source")!==e.get("cityId")||h.getSource().addFeature(new ol.Feature({geometry:a.getGeometry(),selected:!0,coordinates:a.get("coordinates"),width:a.get("width"),visible:a.get("visible"),color:a.get("color"),type:a.get("type")}))}))})}}}});
a=new ol.source.Vector({wrapX:!1});b=new ol.layer.Vector({source:a,id:"annotations"});if(f=this.getAnnotations())f=(new ol.format.GeoJSON).readFeatures(f),a.addFeatures(f);g.addLayer(b);this.setDrawInteraction(new ol.interaction.Draw({source:a,type:"Polygon",freehand:!0,alias:"draw"}));this.getDrawInteraction().on("drawend",function(a){a.feature.set("type","annotation");d.setCurrentAnnotation(a.feature);d.getContentEl().setHtml('\x3ctextarea class\x3d"annotation-text" rows\x3d"5" cols\x3d"60"\x3e\x3c/textarea\x3e\x3cbutton class\x3d"saveAnnotation"\x3eSave\x3c/button\x3e\x3cbutton class\x3d"deleteAnnotation"\x3eCancel\x3c/button\x3e');
d.getContentEl().down(".deleteAnnotation").dom.onclick=function(){d.deleteAnnotation()};d.getContentEl().down(".saveAnnotation").dom.onclick=function(){d.saveAnnotation()};d.getOverlay().setPosition(a.feature.getGeometry().getLastCoordinate());d.getContentEl().down(".annotation-text").dom.focus()});this.setMap(g);this.getTargetEl().down(".ol-zoom");Ext.create("Ext.Button",{glyph:"xf075@FontAwesome",cls:"annotate",renderTo:this.getTargetEl().down(".ticker").parent(),tooltip:this.localize("annotateTip"),
enableToggle:!0,toggleHandler:function(a,b){b?this.getMap().addInteraction(this.getDrawInteraction()):this.getMap().removeInteraction(this.getDrawInteraction())},scope:this});this.getDockedComponent("bottomToolbar").getComponent("addFilter").click()}else Ext.defer(this.tryInit,500,this)},deleteAnnotation:function(){this.body.down(".ol-popup").down(".ol-popup-closer").dom.click();this.getMap().getLayer("annotations").getSource().removeFeature(this.getCurrentAnnotation());var a=new ol.format.GeoJSON,
b=this.getMap().getLayer("annotations").getSource().getFeatures(),c=[],d=this.getProjection();b.forEach(function(a){transformedFeature=a.clone();transformedFeature.getGeometry().transform(d?d:"EPSG:3857","EPSG:3857");c.push(transformedFeature)});a=a.writeFeatures(c);this.storeAnnotations(a)},storeAnnotations:function(a){this.mask("storingAnnotations");var b=this;Ext.Ajax.request({url:this.getTromboneUrl(),params:{tool:"resource.StoredResource",storeResource:JSON.stringify(a)},scope:this}).then(function(a){b.unmask();
a=Ext.JSON.decode(a.responseText);b.setApiParam("annotationsId",a.storedResource.id);b.toastInfo(b.localize("annotationsUpdated"))},function(a){b.unmask();b.showResponseError(b.localize("annotationsUpdateFailed"),a)})},saveAnnotation:function(){this.getCurrentAnnotation().set("text",this.getContentEl().down(".annotation-text").dom.value);this.getOverlay().setPosition(void 0);var a=new ol.format.GeoJSON,b=this.getMap().getLayer("annotations").getSource().getFeatures(),c=[],d=this.getProjection();b.forEach(function(a){transformedFeature=
a.clone();transformedFeature.getGeometry().transform(d?d:"EPSG:3857","EPSG:3857");c.push(transformedFeature)});a=a.writeFeatures(c);this.storeAnnotations(a)},travelStyleFunction:function(a,b){var c=new ol.style.Stroke({color:a.get("color"),width:a.get("width")}),d=[new ol.style.Style({stroke:c})],e=a.getGeometry(),f=e.getLastCoordinate(),e=e.getCoordinateAt(.9),e=Math.atan2(f[1]-e[1],f[0]-e[0]),g=new ol.geom.LineString([f,[f[0]-10*b,f[1]+10*b]]);g.rotate(e,f);var h=new ol.geom.LineString([f,[f[0]-
10*b,f[1]-10*b]]);h.rotate(e,f);d.push(new ol.style.Style({geometry:g,stroke:c}));d.push(new ol.style.Style({geometry:h,stroke:c}));return d},cityStyleFunction:function(a,b){if(a.get("visible")){var c=2*Math.PI*a.get("width"),d=a.get("confidence")?c*a.get("confidence"):c;return new ol.style.Style({image:new ol.style.Circle({radius:a.get("width"),fill:new ol.style.Fill({color:a.get("color")}),stroke:new ol.style.Stroke({lineDash:[d,c-d],color:"rgb(255, 255, 255)",width:2})})})}return!1},animationStyleFunction:function(a){return[new ol.style.Style({stroke:new ol.style.Stroke({color:a.get("color"),
width:5})}),new ol.style.Style({geometry:new ol.geom.Circle(a.getGeometry().getFirstCoordinate()),stroke:new ol.style.Stroke({color:"white",width:10})}),new ol.style.Style({geometry:new ol.geom.Circle(a.getGeometry().getLastCoordinate()),stroke:new ol.style.Stroke({color:"white",width:10})})]},showTermInCorpus:function(a,b,c){a=Ext.create("Voyant.data.model.Context",{docIndex:a,position:b,term:c});this.getApplication().dispatchEvent("termLocationClicked",this,[a])},filterUpdate:function(a){var b=
this;a.clearAnimation();var c=this.getMap(),d=a.getColor(),e=parseInt(this.getApiParam("citiesMaxCount")),f=0;a.getGeonames().eachCity(function(a){a.rawFreq>f&&(f=a.rawFreq)},this,e);var g=c.getLayer(a.getId()+"-cities").getSource(),h=Ext.Array.from(this.getApiParam("hide"));g.clear();var k=parseInt(this.getApiParam("minPopulation")),l=parseInt(this.getApiParam("citiesMinFreq")),m={};a.getGeonames().eachCity(function(a){if((!k||a.population>=k)&&(!l||a.rawFreq>=l)){m[a.id]=!0;var c=[parseFloat(a.longitude),
parseFloat(a.latitude)];a=new ol.Feature({geometry:(new ol.geom.Point(c)).transform(ol.proj.get("EPSG:4326"),b.getProjection()?b.getProjection():ol.proj.get("EPSG:3857")),text:a.label+" ("+a.rawFreq+")",description:a.label,color:d,selected:!1,width:3+20*Math.sqrt(a.rawFreq/f),visible:!0,coordinates:c,forms:a.forms,cityId:a.id,type:"city",confidence:a.confidence?100*a.confidence:void 0});g.addFeature(a)}},this,e);0<g.getFeatures().length&&c.getView().fit(g.getExtent());c.getLayer(a.getId()+"-cities").setVisible(0==
Ext.Array.contains(h,"cities"));g=c.getLayer(a.getId()+"-connections").getSource();g.clear();0==Object.keys(m).length&&this.toastInfo("No cities available for current criteria.");f=0;a.getGeonames().eachConnection(function(a){a.rawFreq>f&&(f=a.rawFreq)},this);var n=0,q=parseInt(this.getApiParam("connectionsMaxCount")),r=parseInt(this.getApiParam("connectionsMinFreq"));a.getGeonames().eachConnection(function(a){if((!r||a.rawFreq>=r)&&(!q||n<q)&&a.source.id in m&&a.target.id in m&&(!k||a.source.population>=
k&&a.target.population>=k)){var c=(new arc.GreatCircle({x:a.source.longitude,y:a.source.latitude},{x:a.target.longitude,y:a.target.latitude})).Arc(100,{offset:100}),e=a.source.label+" -\x3e "+a.target.label+" ("+a.rawFreq+")";c.geometries.forEach(function(c){c=new ol.geom.LineString(c.coords);c.transform(ol.proj.get("EPSG:4326"),b.getProjection()?b.getProjection():ol.proj.get("EPSG:3857"));c=new ol.Feature({geometry:c,text:e,color:d,width:3+10*Math.sqrt(a.rawFreq/f),source:a.source.id,target:a.target.id,
type:"connection"});g.addFeature(c)},this);n++}},this);c.getLayer(a.getId()+"-connections").setVisible(0==Ext.Array.contains(h,"connections"));a.animate()}});
...
"-connections").getSource().getFeatures(),d=!1,f=0,g=e.length;f<g;f++)if(b.source.id==e[f].get("source")&&b.target.id==e[f].get("target")){d=e[f];break}if(d){this.getKeepAnimationInFrame()&&(g=a.getMap(),e=a.getMap().getView().calculateExtent(g.getSize()),ol.extent.containsExtent(e,d.getGeometry().getExtent())||g.getView().setZoom(g.getView().getZoom()-1));var d=(new arc.GreatCircle({x:b.source.longitude,y:b.source.latitude},{x:b.target.longitude,y:b.target.latitude})).Arc(100,{offset:100}),h=b.source.label+
" -\x3e "+b.target.label;d.geometries.forEach(function(b){var d=new ol.geom.LineString([]);d.transform(ol.proj.get("EPSG:4326"),a.getProjection()?a.getProjection():ol.proj.get("EPSG:3857"));b=new ol.Feature({geometry:d,allcoords:b.coords,text:h,color:this.getColor(),width:5,start:(new Date).getTime()});c.getSource().addFeature(b)},this);a.body.down(".ticker");a.body.down(".ticker").setHtml(b.source.left+" \x3cspan class\x3d'keyword'\x3e"+b.source.middle+"\x3c/span\x3e "+b.source.right+" \u2026 "+
b.target.left+" \x3cspan class\x3d'keyword'\x3e"+b.target.middle+"\x3c/span\x3e "+b.target.right);return this.setTimeout(setTimeout(this.animate.bind(this),1))}if(this.getGeonames())return b=this.getGeonames().getConnectionOccurrence(b.index+1),this.setCurrentConnectionOccurrence(b),c.getSource().clear(),this.setTimeout(setTimeout(this.animate.bind(this),1))}}else if(0<d.length){g=d[0].getGeometry().getCoordinates();e=d[0].get("allcoords");if(g.length<e.length)return g=((new Date).getTime()-d[0].get("start"))*
