...
$.ajax({type:"POST",url:Voyant.application.getTromboneUrl(),data:b,dataType:"json"}).done(function(a){d.set(a.corpus.metadata);a=Ext.create("Voyant.data.store.Documents",{corpus:d});d.setDocumentsStore(a);if(!("docsLimit"in b)||!1!==b.docsLimit&&0<b.docsLimit){debugger;a.load({params:{limit:"docsLimit"in b?b.docsLimit:d.getDocumentsCount()},callback:function(a,b,e){d.setDocumentsStore(this);c.resolve(d)},scope:a})}else c.resolve(d)}).fail(function(a){Voyant.application.showResponseError(d.localize("failedCreateCorpus"),
