...
(d=90,c.ty=0):(d=-90,c.ty=c.height,c.tx=c.width));c.size=Math.max(c.height,c.width);c.rotation=Math.PI/180*d;f.context.fillStyle=f.canvas.style.backgroundColor;f.context.fillRect(0,0,f.canvas.width,f.canvas.height);c.draw(f.context);d=f.context.getImageData(c.x,c.y,c.width,c.height);for(var e=[],h=0;h<c.width;h++){var m=Math.floor(h/g.COARSENESS)*g.COARSENESS;null==e[m]&&(e[m]={});for(var k=0;k<c.height;k++){var l=Math.floor(k/g.COARSENESS)*g.COARSENESS,n;n=4*(h+k*d.width);n=[d.data[n],d.data[n+1],
d.data[n+2],d.data[n+3]];"rgb("+n[0]+", "+n[1]+", "+n[2]+")"!=f.canvas.style.backgroundColor&&(e[m][l]=!0);e[m][l]&&(k=l+g.COARSENESS)}}c.mask=e}}function e(){f.clear();for(var a=p.length;a--;){var b=p[a];b.alpha=1;b.live&&b.draw(f.context)}a=$(f.canvas);if(null!=n){a.css("cursor","pointer");f.context.save();f.context.textBaseline="alphabetic";f.context.font="12px Arial";var b=f.context.measureText(n.text).width,c=f.context.measureText(n.value).width,b=(b>c?b:c)+20,c=q+15,d=r+25,e=a.width(),a=a.height();
...
0;var k=this.MIXED=1;this.getWordOrientation=function(){return k};this.setWordOrientation=function(a){k=a};this.UPDATE_RATE=25;this.COARSENESS=5;this.grid=[];var m=0,l;this.doingArrange=!1;this.wordsToArrange=5;var n=null,q=0,r=0,p=[];this.getWords=function(){return p};this.setWords=function(a){p=a};this.sizeAdjustment=100;this.minFontSize=12;this.largestWordSize=0;this.smallestWordSize=1E4;var s={};this.addWords=function(a){for(var b=!1,e=0;e<a.length;e++){var h=a[e],m;m=void 0==typeof h.color||
null==h.color||""==h.color?f.colors[Math.floor(Math.random()*f.colors.length)]:h.color;var k;k=void 0==typeof h.size||null==h.size||""==h.size?Math.floor(40*Math.random()):parseFloat(h.size);var l=h.word,n=h.label,q=h.value,h=!1;null==s[l]&&(s[l]=!0,k>g.largestWordSize&&(g.largestWordSize=k,h=!0),k<g.smallestWordSize&&(g.smallestWordSize=.8*k,h=!0),l=new Word(l,k,m,n,q),p.push(l));b=h||b}c();this.setRelativeSizes();this.calculateSizeAdjustment();b?this.resizeWords():d()};this.resetWordCoordinates=
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},g;this.grid=[];m=0;g=function(){var a,g,h,k,t=this.wordsToArrange-1,n=f.canvas.width,s=f.canvas.height,q=.5*n,r=.5*s;do{h=p[m];if(void 0!==h){for(var v=Math.random()*Math.PI,w=.25*Math.random()*h.size,A=.5*(Math.random()-.5),y=.5*h.width,z=.5*h.height;;){a=Math.floor((q+Math.cos(v)*w*this.ratio-y)/this.COARSENESS)*this.COARSENESS;g=Math.floor((r+Math.sin(v)*w-z)/this.COARSENESS)*this.COARSENESS;
k=a+y>=n||g+z>=s?!0:d(a,g,h.height,h.width,h.mask);if(!k)break;v+=A;w+=.05}c(a,g,h);if(f.useFadeEffect)for(a=h.alpha=0;a<m;a++)g=p[a],1>g.alpha&&b(g);else h.alpha=1,h.draw(f.context)}m++;if(m>=p.length){clearTimeout(l);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(t--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;l=setInterval(g,50)}else alert("Error: There are no words to arrange.")};
...
this.innerContainer=a.getLayout().getRenderTarget();a.body.on("scroll",function(a,b){var e=this.innerContainer.first(),g=this.getLastScrollTop()<b.scrollTop;if(!g&&1>b.scrollTop){if(e=e.first(".word"),null!==e){var f=Voyant.data.model.Token.getInfoFromElement(e),h=parseInt(f.docIndex),e=parseInt(f.position);if(0<h){var k=this.getDocumentsStore().getAt(h),m=this.getApiParam("limit");0===e?(h--,k=this.getDocumentsStore().getAt(h),h=k.get("tokensCount-lexical"),e=h-m,0>e&&(e=0,this.setApiParam("limit",
h))):e-=m;0>e&&(e=0);var l=k.getId();this.setApiParams({skipToDocId:l,start:e});this.setScrollDownwards(g);this.load();this.setApiParam("limit",this.INITIAL_LIMIT)}}}else if(g&&b.scrollTop+b.offsetHeight>b.scrollHeight/2){var m=e.last(),n=$(e.dom).contents().filter(function(){return 3===this.nodeType}).last();if(!1===m.hasCls("loading"))for(;m;){if(m.hasCls("word")){f=Voyant.data.model.Token.getInfoFromElement(m);h=parseInt(f.docIndex);e=parseInt(f.position);k=this.getDocumentsStore().getAt(f.docIndex);
l=k.getId();if(e+this.getApiParam("limit")>=k.get("tokensCount-lexical")&&h==this.getCorpus().getDocumentsCount()-1)break;m.el.dom.nextSibling===n[0]&&n.remove();m.insertSibling("\x3cdiv class\x3d'loading'\x3e"+this.localize("loading")+"\x3c/div\x3e","after",!1).mask();m.destroy();this.setApiParams({skipToDocId:l,start:f.position});this.setScrollDownwards(g);this.load();break}m.destroy();m=e.last()}}this.setLastScrollTop(b.scrollTop)},this);a.body.on("click",function(a,b){b=Ext.get(b);if(b.hasCls("word")){var e=
Voyant.data.model.Token.getInfoFromElement(b),e=[{term:b.getHtml(),docIndex:e.docIndex}];this.getApplication().dispatchEvent("termsClicked",this,e)}},this);this.getCorpus()&&this.load()},this);Ext.apply(this,{dockedItems:[{dock:"bottom",xtype:"toolbar",items:[{xtype:"querysearchfield"}]}],listeners:{loadedCorpus:function(a,c){this.setCorpus(c);this.getTokensStore().setCorpus(c);this.getDocumentTermsStore().getProxy().setExtraParam("corpus",c.getId());var d=c.getDocuments();this.setDocumentsStore(d);
d=this.down('panel[region\x3d"south"]');this.generateChart(c,d);this.rendered&&this.load()},termsClicked:function(a,c){var d=[];c.forEach(function(a){a.term&&d.push(a.term)});this.loadQueryTerms(d)},corpusTermsClicked:function(a,c){var d=[];c.forEach(function(a){a.getTerm()&&d.push(a.getTerm())});this.loadQueryTerms(d)},documentTermsClicked:function(a,c){var d=[];c.forEach(function(a){a.getTerm()&&d.push(a.getTerm())});this.loadQueryTerms(d)},documentsClicked:function(a,c,d){c&&(this.setApiParams({skipToDocId:c[0].getId(),
start:0}),this.load(!0))},termLocationClicked:function(a,c){if(void 0!==c[0]){var d=c[0],e=d.get("docIndex"),g=d.get("position"),f=g-this.getApiParam("limit")/2,h=this.getCorpus().getDocument(e);this.setApiParams({skipToDocId:h.getId(),start:0>f?0:f});this.load(!0,{callback:function(){var a=this.body.dom.querySelector("#_"+e+"_"+g);a&&a.scrollIntoView();this.highlightKeywords(d.get("term"),!1)},scope:this})}},scope:this}});this.callParent(arguments)},loadQueryTerms:function(a){a&&0<a.length&&this.getDocumentTermsStore().load({params:{query:a,
docIndex:void 0,docId:void 0,page:void 0,start:void 0,limit:void 0}})},generateChart:function(a,b){function c(a,b){var c=[[116,116,181],[139,163,83],[189,157,60],[171,75,75],[174,61,155]],c=c[a%c.length];return"rgba("+c[0]+","+c[1]+","+c[2]+","+b+")"}function d(d,e){var f=d.index,g=d.fraction,h=d.relativeHeight,k=c(f,.3),f=c(f,1);b.add({xtype:"cartesian",flex:g,height:"100%",insetPadding:0,background:{type:"linear",degrees:90,stops:[{offset:0,color:k},{offset:h,color:k},{offset:h,color:"white"},{offset:1,
color:"white"}]},axes:[{type:"numeric",position:"left",fields:"distribution",hidden:!0},{type:"category",position:"bottom",fields:"bin",hidden:!0}],series:[{type:"line",xField:"bin",yField:"distribution",style:{lineWidth:2,strokeStyle:f},tooltip:{corpus:a,trackMouse:!0,style:"background: #fff",showDelay:0,dismissDelay:500,hideDelay:5,renderer:function(b,c){this.setHtml(a.getDocument(b.get("docIndex")).getTitle()+"\x3cbr\x3e"+b.get("term")+": "+b.get("distribution"))}}}],store:Ext.create("Ext.data.ArrayStore",
