...
updateWhileAnimating:!0,updateWhileInteracting:!0});d.on("pointermove",function(a){e.getSource().clear();var b=a.coordinate;if(a=d.getFeaturesAtPixel(a.pixel)){for(var c=0;a[c].get("selected")&&(c++,c!==a.length););if(c<a.length){c=a[c];a={font:"12px Calibri,sans-serif",textAlign:"center",offsetY:-15,fill:new ol.style.Fill({color:[0,0,0,1]}),stroke:new ol.style.Stroke({color:[255,255,255,.5],width:4})};a.text=c.get("text");a=new ol.style.Style({text:new ol.style.Text(a),zIndex:1});var k=new ol.Feature({text:c.get("text"),
geometry:c.getGeometry(),forms:c.get("forms"),selected:!0,coordinates:c.get("coordinates"),width:c.get("width"),visible:c.get("visible")});e.getSource().addFeature(k);b=c.getGeometry().getClosestPoint(b);b=new ol.Feature({geometry:new ol.geom.Point(b),selected:!0});b.setStyle(a);e.getSource().addFeature(b)}}});this.setMap(d);this.getDockedComponent("bottomToolbar").getComponent("addFilter").click()}else Ext.defer(this.tryInit,500,this)},travelStyleFunction:function(a,b){var c=new ol.style.Stroke({color:a.get("color"),
width:a.get("width")}),d=[new ol.style.Style({stroke:c})],e=a.getGeometry(),f=e.getLastCoordinate(),e=e.getCoordinateAt(.9),e=Math.atan2(f[1]-e[1],f[0]-e[0]),g=new ol.geom.LineString([f,[f[0]-10*b,f[1]+10*b]]);g.rotate(e,f);var h=new ol.geom.LineString([f,[f[0]-10*b,f[1]-10*b]]);h.rotate(e,f);d.push(new ol.style.Style({geometry:g,stroke:c}));d.push(new ol.style.Style({geometry:h,stroke:c}));return d},cityStyleFunction:function(a,b){return a.get("visible")?new ol.style.Style({stroke:new ol.style.Stroke({color:a.get("selected")?
"rgb(0,0,0)":a.get("color"),width:a.get("width")})}):!1},animationStyleFunction:function(a){return[new ol.style.Style({stroke:new ol.style.Stroke({color:a.get("color"),width:5})}),new ol.style.Style({geometry:new ol.geom.Circle(a.getGeometry().getFirstCoordinate()),stroke:new ol.style.Stroke({color:"white",width:10})}),new ol.style.Style({geometry:new ol.geom.Circle(a.getGeometry().getLastCoordinate()),stroke:new ol.style.Stroke({color:"white",width:10})})]},showTermInCorpus:function(a,b,c){a=Ext.create("Voyant.data.model.Context",
{docIndex:a,position:b,term:c});this.getApplication().dispatchEvent("termLocationClicked",this,[a])},filterUpdate:function(a){a.clearAnimation();var b=this.getMap(),c=a.getColor(),d=parseInt(this.getApiParam("citiesMaxCount")),e=0;a.getGeonames().eachCity(function(a){a.rawFreq>e&&(e=a.rawFreq)},this,d);var f=b.getLayer(a.getId()+"-cities").getSource(),g=Ext.Array.from(this.getApiParam("hide"));f.clear();var h=parseInt(this.getApiParam("minPopulation")),k=parseInt(this.getApiParam("citiesMinFreq")),
l={};a.getGeonames().eachCity(function(a){if((!h||a.population>=h)&&(!k||a.rawFreq>=k)){l[a.id]=!0;var b=[parseFloat(a.longitude),parseFloat(a.latitude)];a=new ol.Feature({geometry:(new ol.geom.Circle(b)).transform(ol.proj.get("EPSG:4326"),ol.proj.get("EPSG:3857")),text:a.label+" ("+a.rawFreq+")",description:a.label,color:c,selected:!1,width:3+20*Math.sqrt(a.rawFreq/e),visible:!0,coordinates:b,forms:a.forms});f.addFeature(a)}},this,d);0<f.getFeatures().length&&b.getView().fit(f.getExtent());b.getLayer(a.getId()+
"-cities").setVisible(0==Ext.Array.contains(g,"cities"));f=b.getLayer(a.getId()+"-connections").getSource();f.clear();0==Object.keys(l).length&&this.toastInfo("No cities available for current criteria.");e=0;a.getGeonames().eachConnection(function(a){a.rawFreq>e&&(e=a.rawFreq)},this);var m=0,n=parseInt(this.getApiParam("connectionsMaxCount")),q=parseInt(this.getApiParam("connectionsMinFreq"));a.getGeonames().eachConnection(function(a){if((!q||a.rawFreq>=q)&&(!n||m<n)&&a.source.id in l&&a.target.id in
l&&(!h||a.source.population>=h&&a.target.population>=h)){var b=(new arc.GreatCircle({x:a.source.longitude,y:a.source.latitude},{x:a.target.longitude,y:a.target.latitude})).Arc(100,{offset:100}),d=a.source.label+" -\x3e "+a.target.label+" ("+a.rawFreq+")";b.geometries.forEach(function(b){b=new ol.geom.LineString(b.coords);b.transform(ol.proj.get("EPSG:4326"),ol.proj.get("EPSG:3857"));b=new ol.Feature({geometry:b,text:d,color:c,width:3+10*Math.sqrt(a.rawFreq/e),source:a.source.id,target:a.target.id});
f.addFeature(b)},this);m++}},this);b.getLayer(a.getId()+"-connections").setVisible(0==Ext.Array.contains(g,"connections"));a.animate()}});
...
geonames:void 0,color:"#f00",timeout:void 0,currentConnectionOccurrence:void 0,animationLayer:void 0,millisPerAnimation:2E3},constructor:function(a){a=a||{};var b=new Voyant.data.util.Geonames({corpus:a.corpus});this.setGeonames(b);var c=this;Ext.applyIf(a,{tooltip:this.localize("filterTip"),style:"background-color: white; color: "+a.color,glyph:"xf0b0@FontAwesome",menu:{defaults:{xtype:"querysearchfield",labelWidth:60,labelAlign:"right",width:250,maxWidth:250,listeners:{change:function(a,b){c.loadGeonames()},
load:function(a,b,f,g){0==b.length&&""==g.getParams().query&&(Ext.Msg.show({buttonText:{ok:c.localize("close")},icon:Ext.MessageBox.INFO,message:c.localize("noDataForField"),buttons:Ext.Msg.OK}),this.disable())}}},items:[{fieldLabel:this.localize("authorLabel"),tokenType:"author",itemId:"author"},{fieldLabel:this.localize("titleLabel"),itemId:"title",tokenType:"title"},{fieldLabel:this.localize("keywordLabel"),hidden:!0},{xtype:"multislider",fieldLabel:this.localize("pubDateLabel"),itemId:"pubDate",
hidden:!0},{xtype:"button",text:"remove",scope:this,handler:function(a){this.fireEvent("removeFilterWidget",this);a.ownerCt.ownerCmp.destroy()}}]}});this.callParent([a]);this.on("afterrender",function(a){a.getTargetEl().down(".x-btn-glyph").setStyle("color",this.getColor());var b=a.up("panel");b.mask(a.localize("loading"));a.loadGeonames().then(function(c){b.unmask();if(1==b.getFilterWidgets().getCount()){var g=c.getCitiesCount(),h=c.getTotalCitiesCount(),k=c.getConnectionsCount();c=c.getTotalConnectionsCount();
message=a.localize(g==h&&k==c?"loadedAll":"loadedSome")+"\x3cbr\x3e\x3cbr\x3e"+a.localize("disclaimer");b.toastInfo({closable:!0,maxWidth:"90%",html:(new Ext.Template(a.localize("disclaimer"))).apply({currentCitiesCount:g,totalCitiesCount:h,currentConnectionsCount:k,totalConnectionsCount:c,url:b.getBaseUrl()+"docs/#!/guide/dreamscape"})})}})},this)},loadGeonames:function(a){var b=this;a=a||{};var c=[];this.query("querysearchfield").forEach(function(a){var b=a.getItemId();a.getValue().forEach(function(a){c.push(b+
":"+a)})});0<c.length&&!("query"in a)&&(a.query=c);var d=this.up("panel");d&&d.getApiParams&&Ext.applyIf(a,d.getApiParams(["minPopulation","connectionsMaxCount","connectionsMinFreq"]));this.mask("\u2026");var e=this.getGeonames();return e.load(a).then(function(){b.unmask();b.fireEvent("filterUpdate",b,e);return e})},clearAnimation:function(){this.getTimeout()&&clearTimeout(this.getTimeout())},animate:function(){this.clearAnimation();var a=this.getCurrentConnectionOccurrence();if(a){var b=this.getAnimationLayer();
b||(b=this.up("panel").getMap().getLayer(this.getId()+"-animation"),this.setAnimationLayer(b));if(b){var c=b.getSource().getFeatures();if(0==c.length){if(c=this.up("panel")){for(var d=c.getMap().getLayer(this.getId()+"-connections").getSource().getFeatures(),e=!1,f=0,g=d.length;f<g;f++)if(a.source.id==d[f].get("source")&&a.target.id==d[f].get("target")){e=!0;break}if(e){var g=(new arc.GreatCircle({x:a.source.longitude,y:a.source.latitude},{x:a.target.longitude,y:a.target.latitude})).Arc(100,{offset:100}),
h=a.source.label+" -\x3e "+a.target.label+" ("+a.rawFreq+")";g.geometries.forEach(function(a){var c=new ol.geom.LineString([]);c.transform(ol.proj.get("EPSG:4326"),ol.proj.get("EPSG:3857"));a=new ol.Feature({geometry:c,allcoords:a.coords,text:h,color:this.getColor(),width:5,start:(new Date).getTime()});b.getSource().addFeature(a)},this);c.body.down(".ticker");c.body.down(".ticker").setHtml(a.source.left+" \x3cspan class\x3d'keyword'\x3e"+a.source.middle+"\x3c/span\x3e "+a.source.right+" \u2026 "+
a.target.left+" \x3cspan class\x3d'keyword'\x3e"+a.target.middle+"\x3c/span\x3e "+a.target.right);return this.setTimeout(setTimeout(this.animate.bind(this),1))}if(this.getGeonames())return a=this.getGeonames().getConnectionOccurrence(a.index+1),this.setCurrentConnectionOccurrence(a),b.getSource().clear(),this.setTimeout(setTimeout(this.animate.bind(this),1))}}else if(0<c.length){g=c[0].getGeometry().getCoordinates();d=c[0].get("allcoords");if(g.length<d.length)return g=((new Date).getTime()-c[0].get("start"))*
d.length/this.getMillisPerAnimation(),line=new ol.geom.LineString(d.slice(0,g)),line.transform(ol.proj.get("EPSG:4326"),ol.proj.get("EPSG:3857")),c[0].set("geometry",line),this.setTimeout(setTimeout(this.animate.bind(this),25));a=this.getGeonames().getConnectionOccurrence(a.index+1);this.setCurrentConnectionOccurrence(a);b.getSource().clear();return this.setTimeout(setTimeout(this.animate.bind(this),1))}}}else if(this.getGeonames()&&(a=this.getGeonames().getConnectionOccurrence(0)))return this.setCurrentConnectionOccurrence(a),
...
items:["reader","summary","veliza"]}]},launch:function(){this.callParent(arguments);if(this.hasQueryToLoad()){var a=Ext.Object.fromQueryString(document.location.search);!a.corpus&&this.getCorpusId&&this.getCorpusId()&&(a.corpus=this.getCorpusId());this.loadCorpusFromParams(a);if(a.palette)if(-1<a.palette.indexOf(",")){var b=Ext.decode(a.palette);this.addColorPalette(a.palette,b)}else this.loadColorPaletteForCorpus(a.corpus,a.palette)}else(a=this.getViewport())&&(a=a.down("corpuscreator"))&&a.toastInfo({html:this.localize("didYouKnowText"),
title:this.localize("didYouKnow"),anchor:a.down("textareafield"),align:"tr",listeners:{beforeclose:{fn:function(){this.getHeader().getTools().forEach(function(a){"gear"!=a.type&&"help"!=a.type||a.getEl().frame("#ff0000",1,{duration:3E3})})},scope:a}}})},loadCorpusFromParams:function(a){var b=this,c=b.getViewport();c.mask(this.localize("fetchingCorpus"));a.archive&&(Ext.isString(a.archive)&&(a.archive=[a.archive]),a.archive=a.archive.map(function(a){return a.replace("/blogs.sub.uni-hamburg.de/hup/lhn/",
"/wikis.sub.uni-hamburg.de/lhn/index.php/").replace("/hup.sub.uni-hamburg.de/","/wikis.sub.uni-hamburg.de/")}));this.validateCorpusLoadParams(a);(new Voyant.data.model.Corpus(a)).then(function(a){c.unmask();b.setCorpus(a);b.validateCorpusAccess()&&b.dispatchEvent("loadedCorpus",this,a)}).otherwise(function(){c.unmask()})},validateCorpusLoadParams:function(a){},validateCorpusAccess:function(){var a=this,b=a.getViewport(),c=this.getCorpus();if(c&&c.requiresPassword()&&!a.getViewport().query("panel").every(function(a){return!a.isConsumptive})){var d=
c.getNoPasswordAccess(),e=Ext.create("Ext.window.Window",{title:a.localize("passwordRequiredTitle"),layout:"fit",items:{padding:10,flex:1,width:300,layout:{type:"vbox",align:"stretch"},items:[{html:"\x3cp\x3e"+a.localize("passwordRequiredMessage")+"\x3c/p\x3e"+("NONCONSUMPTIVE"==d?"\x3cp\x3e"+a.localize("nonConsumptiveMessage")+"\x3c/p\x3e":"")+"\x3c/p\x3e"},{xtype:"textfield",fieldLabel:a.localize("password")}],bbar:{layout:{pack:"center"},items:[{text:a.localize("passwordValidateButton"),ui:"default",
handler:function(){var d=e.query("textfield")[0].getValue().trim();0==d.length?a.showError({message:a.localize("noPasswordGiven")}):(e.mask(),Ext.Ajax.request({url:a.getTromboneUrl(),params:{corpus:c.getId(),passwordForSession:d},method:"POST",success:function(d,f){e.unmask();var k=d.responseText;"ADMIN"==k||"ACCESS"==k?(e.close(),b.unmask(),a.setCorpusAccess(k),a.dispatchEvent("loadedCorpus",this,c)):a.showError({message:a.localize("badPassword")})},failure:function(b,c){e.unmask();a.showError({message:a.localize("passwordValidationError")})}}))}},
{text:a.localize("nonConsumptiveButton"),hidden:"NONE"==c.getNoPasswordAccess(),handler:function(){e.mask();Ext.Ajax.request({url:a.getTromboneUrl(),params:{corpus:c.getId(),passwordForSessionRemove:!0},method:"POST",callback:function(d,g){e.unmask();e.close();b.unmask();a.dispatchEvent("loadedCorpus",a,c)}})}}]}}}).show();return!1}return!0},hasQueryToLoad:function(a){a||(a=Ext.Object.fromQueryString(document.location.search));return a.corpus||a.input||this.getCorpusId&&this.getCorpusId()},loadColorPaletteForCorpus:function(a,
b){Ext.Ajax.request({url:this.getTromboneUrl(),params:{tool:"resource.StoredResource",retrieveResourceId:b,corpus:a},success:function(a,d){var e=Ext.util.JSON.decode(a.responseText).storedResource.resource,e=Ext.decode(e);this.addColorPalette(b,e)},failure:function(a){this.setApiParam("palette",void 0)},scope:this})},listeners:{loadedCorpus:function(a,b){this.setCorpus(b);this.on("unhandledEvent",function(a,d,e){a=this.getBaseUrl()+"?corpus\x3d"+b.getAliasOrId();var f=this.getModifiedApiParams()||
{};delete f.view;if("termsClicked"==d)f.query=e;else if("documentsClicked"==d){var g=[];e.forEach&&e.forEach(function(a){g.push(a.getIndex())});f.docIndex=g}else if("corpusTermsClicked"==d)e.map&&(f.query=e.map(function(a){return a.getTerm()}));else if("documentTermsClicked"==d)e.map&&(f.query=e.map(function(a){return a.getTerm()}),f.docIndex=e.map(function(a){return a.getDocIndex()}));else{console&&console.warn("Unhandled event: "+d,e);return}f.toolFlow&&(d=Ext.Array.from(f.toolFlow.split(",")),
f.view=d.shift(),0<d.length?f.toolFlow=d.join(","):delete f.toolFlow);a+="\x26"+Ext.Object.toQueryString(f);this.openUrl(a)})}}});Ext.define("Voyant.panel.DocumentClusters",{extend:"Ext.panel.Panel",alias:"widget.documentclusters",title:"Document Clusters"});Ext.define("Voyant.panel.RezoViz",{extend:"Ext.panel.Panel",alias:"widget.rezoviz",title:"RezoViz"});
