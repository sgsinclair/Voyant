...
this.lastClickedBubbles[b],l=0,n;for(n in k)if(-1!==this.termsFilter.indexOf(n)){var q=k[n];if(q){l++;this.SEPARATE_LINES_FOR_TERMS&&c();var u=0,p=q.color.join(",");this.ctx.strokeStyle="rgba("+p+", 1)";this.ctx.fillStyle="rgba("+p+", 0.35)";this.ctx.lineWidth=.25;for(var h=h+q.rawFreq,u=u+q.rawFreq,r=m&&this.lastClickedBubbles[b][n],C=0;C<q.pos.length;C++){var s=q.pos[C];if(0<s.radius){var H=!1;r&&this.lastClickedBubbles[b][n]==s.id&&(this.ctx.strokeStyle="rgba(0, 0, 0, 0.75)",this.ctx.fillStyle=
"rgba("+p+", 0.5)",this.ctx.lineWidth=1,H=!0);this.ctx.beginPath();this.ctx.arc(s.x+f,this.yIndex,s.radius,0,e,!0);this.ctx.closePath();this.ctx.fill();this.ctx.stroke();H&&(this.ctx.strokeStyle="rgba("+p+", 1)",this.ctx.fillStyle="rgba("+p+", 0.35)",this.ctx.lineWidth=.25)}}a.freqCounts[n]=u;this.SEPARATE_LINES_FOR_TERMS&&(this.ctx.fillStyle="rgba(0, 0, 0, 1.0)",this.ctx.font="10px Verdana",this.ctx.fillText(u,f+d+5,this.yIndex-4),this.yIndex+=this.maxRadius)}}this.SEPARATE_LINES_FOR_TERMS&&0==l&&
...
0;var k=this.MIXED=1;this.getWordOrientation=function(){return k};this.setWordOrientation=function(a){k=a};this.UPDATE_RATE=25;this.COARSENESS=5;this.grid=[];var m=0,l;this.doingArrange=!1;this.wordsToArrange=5;var n=null,q=0,u=0,p=[];this.getWords=function(){return p};this.setWords=function(a){p=a};this.sizeAdjustment=100;this.minFontSize=12;this.largestWordSize=0;this.smallestWordSize=1E4;var r={};this.addWords=function(a){for(var b=!1,e=0;e<a.length;e++){var h=a[e],k;k=void 0==typeof h.color||
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];m=0;f=function(){var a,f,h,k,r=this.wordsToArrange-1,n=g.canvas.width,C=g.canvas.height,q=.5*n,u=.5*C;do{h=p[m];if(void 0!==h){for(var D=Math.random()*Math.PI,v=.25*Math.random()*h.size,w=.5*(Math.random()-.5),F=.5*h.width,N=.5*h.height;;){a=Math.floor((q+Math.cos(D)*v*this.ratio-F)/this.COARSENESS)*this.COARSENESS;f=Math.floor((u+Math.sin(D)*v-N)/this.COARSENESS)*this.COARSENESS;
k=a+F>=n||f+N>=C?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;D+=w;v+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<m;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}m++;if(m>=p.length){clearTimeout(l);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(r--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;l=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
a});d.info.continuations=d.children.length;f=d3.max(d.children.map(function(a){return a.maxChildren}));d.maxChildren=Math.max(d.children.length,f)}}function b(a,b){if(!a||!b)return!1;for(var c in a)if(!(c in b))return!1;return!0}function c(a,b){for(var c in b)a[c]=b[c]}function d(){}doubletree.DoubleTree=function(){function b(a){a.each(function(a,b){f.push(this[b])})}var f=[],g=600,h=400,k=!1,m={left:[],right:[]},l={alt:d,shift:d,click:d},n=!0,q=!0,u=doubletree.sortByStrFld("token"),p=doubletree.tokenText,
r=function(a){return doubletree.fieldText(a,"POS")},C=function(a){return"rgba(255,255,255,1)"},s=function(a){return"rgba(255,255,255,1)"},H=function(a){return"red"},L={node:{fill:"white",stroke:"steelblue","stroke-width":"1.5px"},branch:{stroke:"#aaa","stroke-width":"1.5px"}},D=!1,G=d3.dispatch("idsUpdated");G.on("idsUpdated",function(){this==E?(A.setIds(E.continuationIDs),A.updateContinuations()):this==A&&(E.setIds(A.continuationIDs),E.updateContinuations())});var z,I,E,A,J,M;b.init=function(a){d3.select(d3.selectAll(a)).call(this);
return b};b.redraw=function(){void 0!==z&&void 0!==I&&b.setupFromTries(z,I);return b};b.setupFromTries=function(d,k){z=d.getUniqRoot();I=k.getUniqRoot();var B=z.toTree(m.left),v=I.toTree(m.right),w=!0;0<Object.keys(v.pruned).length&&(a(v,v.pruned,w),a(B,v.pruned,w),w=!1);0<Object.keys(B.pruned).length&&(a(B,B.pruned,w),a(v,B.pruned,w));var w={},F;for(F in v.info)"continuations"!=F&&"ids"!=F&&"count"!=F&&(w[F]=v.info[F]);w["right continuations"]=v.info.continuations;w["left continuations"]=B.info.continuations;
w.ids={};c(w.ids,v.info.ids);c(w.ids,B.info.ids);w.count=Object.keys(w.ids).length;J=Object.keys(w.ids);if(v.info.origIDs||B.info.origIDs)w.origIDs={},c(w.origIDs,v.info.origIDs),c(w.origIDs,B.info.origIDs),w.origCount=Object.keys(w.origIDs).length;v.info=w;B.info=w;F=Math.max(B.maxChildren,v.maxChildren);if(isNaN(F)||0==F)return D=!1,b;q?M=d3.scale.log().range([8,14]):(M=function(){return 14},M.domain=function(){});F=Math.min(B.minCount,v.minCount);M.domain([F,B.info.count]);var N=g-20-20,O=h-20-
...
g,h,B,!0,u,G,M,n,p,r,C,s,H,L);A=new doubletree.Tree(e.select("g"),g,h,v,!1,u,G,M,n,p,r,C,s,H,L)});E.handleAltPress=l.alt;A.handleAltPress=l.alt;E.handleShiftPress=l.shift;A.handleShiftPress=l.shift;E.handleClick=l.click;A.handleClick=l.click;D=!0;return b};b.setupFromArrays=function(a,c,d,f,g,h,m,l){void 0==g&&z&&(g=z.caseSensitive());void 0==h&&z&&(h=z.fieldNames());void 0==m&&z&&(m=z.fieldDelim());void 0==l&&z&&(l=z.distinguishingFieldsArray());z=new doubletree.Trie(g,h,m,l);I=new doubletree.Trie(g,
h,m,l);g=c.length;for(h=0;h<g;h++){m=f?f[h]:h;l=c[h];var r=a[h].slice(),n=d[h].slice();r.push(l);r.reverse();n.unshift(l);k?(I.addNgram(r,m),z.addNgram(n,m)):(z.addNgram(r,m),I.addNgram(n,m))}b.setupFromTries(z,I);return b};b.filteredIDs=function(){return J};b.search=function(a){E.search(a);A.search(a);a=d3.select(f[0][0]);var b=a.selectAll("text.foundText");if(b.empty())return 0;b=b[0].length;null!=a.selectAll("text.rtNdText.foundText")[0][0]&&b--;return b};b.clearSearch=function(){E.clearSearch();
A.clearSearch();return b};b.updateTokenExtras=function(){E.showTokenExtras(n);A.showTokenExtras(n);var a=d3.select(f[0][0]).select('.tokenExtra[display\x3d"inline"]');a.empty()||"0px"==a.style("height")&&b.redraw();return b};b.visWidth=function(a){if(!arguments.length)return g;g=a;return b};b.visHeight=function(a){if(!arguments.length)return h;h=a;return b};b.prefixesOnRight=function(a){if(!arguments.length)return k;k=a;return b};b.filters=function(a){if(!arguments.length)return m;m=a;return b};b.handlers=
function(a){if(!arguments.length)return l;l=a;return b};b.showTokenExtra=function(a){if(!arguments.length)return n;n=a;return b};b.scaleLabels=function(a){if(!arguments.length)return q;q=a;return b};b.succeeded=function(){return D};b.sortFun=function(a){if(!arguments.length)return u;u=a;return b};b.nodeText=function(a){if(!arguments.length)return p;p=a;return b};b.tokenExtraText=function(a){if(!arguments.length)return r;r=a;return b};b.rectColor=function(a){if(!arguments.length)return C;C=a;return b};
b.rectBorderColor=function(a){if(!arguments.length)return s;s=a;return b};b.continuationColor=function(a){if(!arguments.length)return H;H=a;return b};b.basicStyles=function(a){if(!arguments.length)return L;Object.keys(L).forEach(function(b){b in a&&Object.keys(L[b]).forEach(function(c){c in a[b]&&(L[b][c]=a[b][c])})});return b};return b};doubletree.Tree=function(a,b,c,d,k,m,l,n,q,u,p,r,C,s,H){function L(a){a.children&&(a._children=a.children,a._children.forEach(L),a.children=null)}function D(a){a.parent&&
a.parent.children.forEach(function(b){b!=a&&L(b)})}function G(a,b){d3.event.altKey?F.handleAltPress(a,b):d3.event.shiftKey?F.handleShiftPress(a,b):(F.handleClick(a,b),a.parent&&(F.continuationIDs!=a.info.ids&&(F.setIds(a.info.ids),F.clickedNode=a.id,l.idsUpdated.apply(F)),D(a),z(a,!0)))}function z(a,b){a.children?(a.children&&1==a.children.length&&z(a.children[0],!0),a._children=a.children,a.children=null):(a.children=a._children,a._children=null,a.children&&1==a.children.length&&z(a.children[0],
!1));b&&F.update(a)}function I(a){return k?J/2-a:J/2+a}var E=q,A=r,J=b-20-20,M=c-20-20,K=0,x,B;m||(m=doubletree.sortByStrFld("token"));var v=d3.layout.tree().nodeSize([40,40]).sort(m),w=d3.svg.diagonal().projection(function(a){return[I(a.y),a.x-B]});a=a.append("g").attr("transform","translate(20,20)");this.readJSONTree=function(a){x=a;x.x0=M/2;x.y0=J/2;x.children.forEach(L);this.update(x)};var F=this;this.update=function(b){b||(b=x);var c=v.nodes(x).reverse();B=x.x-M/2;c.forEach(function(a){var b=
a.textSize;if(void 0==b){for(var b=0,c=a.parent;null!=c;)b+=c.textSize||Ext.draw.TextMeasurer.measureText(c.name,"arial").width,c=c.parent;a.textSize=b}a.y=50*a.depth+b});var d=a.selectAll("g.node_"+k).data(c,function(a){return a.id||(a.id=++K)}),f=d.enter().append("g").attr("class","node node_"+k).attr("cursor","pointer").attr("transform",function(a){return"translate("+I(b.y0)+","+(b.x0-B)+")"}).on("click",G);f.append("title").text(function(a){return doubletree.infoToText(a.info)});f.append("circle").attr("r",
1E-6).style("fill",function(a){return a._children?"#fff":H.node.fill}).style("stroke",function(a){return H.node.stroke});f=f.append("text").attr("class",function(a){return 0==a.depth?"rtNdText":""}).attr("x",function(a){return a.children||a._children?0:k?10:-10}).attr("text-anchor",function(a){return a.parent?a.children||a._children?k?"end":"start":k?"start":"end":"middle"}).style("font-size",function(a){return n(a.info.count)+"pt"});f.append("tspan").attr("dy",".35em").attr("class","tokenText").text(function(a){return u(a.info,
...
a};this.updateContinuations=function(){a.selectAll("g.node_"+k+" text").classed("continuation",function(a){var b;a:{a=a.info.ids;var c=F.continuationIDs||{};for(b in a)if(b in c){b=!0;break a}b=!1}return b}).style("fill",function(a){return d3.select(this).classed("continuation")?s(a.info):"#444"})};this.search=function(b){a.selectAll("g.node text").classed("foundText",function(a){return b.test(u(a.info))})};this.clearSearch=function(){a.selectAll("g.node text").classed("foundText",!1)};this.showTokenExtras=
function(a){if(0==arguments.length)return E;E=a;this.drawRects();return this};this.setRectColor=function(a){if(0==arguments.length)return A;A=a;this.drawRects();return this};this.handleAltPress=function(){};this.handleShifttPress=function(){};this.handleClick=function(){};this.readJSONTree(d);return this};doubletree.sortByStrFld=function(a){return function(b,c){var d=void 0==b.info[a],k=void 0==c.info[a];if(d&&k)return 0;if(d)return-1;if(k)return 1;d=b.info[a].join(" ").toLowerCase();k=c.info[a].join(" ").toLowerCase();
...
(function(){doubletree.Trie=function(a,b,c,d,e){function f(a,b,c){this.id=b;this.count=c;this.info={count:c,ids:{}};if(null==a)this.item=k;else{this.item=a;this.info.ids={};this.info.ids[b]=!0;a=a.split(n);for(var d in a)this.info[l[d]]=[a[d]]}this.nodes={};this.addNgram=function(a,b,c){c||(c=1);var d,e;if(0<a.length){d=a.shift();var g=d.split(n);e=u&&this.item==k?"":g.filter(function(a,b){return-1<q.indexOf(l[b])}).map(function(a){return m?a.toLocaleLowerCase():a}).join(n)}else e=d=h;var r;if(e in
this.nodes&&this.nodes[e]instanceof f){r=this.nodes[e];r.info.count+=c;r.info.ids[b]=!0;for(var s in g)e=g[s],-1==r.info[l[s]].indexOf(e)&&r.info[l[s]].push(e)}else r=new f(d,b,c),this.nodes[e]=r;d!=h&&r.addNgram(a,b,c)};this.getUniqRoot=function(){if(this.item==k){var a=Object.keys(this.nodes);if(1==a.length)return this.nodes[a[0]]}return this};this.toTree=function(a){function b(a,c,d){var e={children:[]};e.name=d.item;e.info={};for(var f in d.info)if("Object"===typeof d.info[f]){e.info[f]={};for(var h in d.info[f])e.info[f][h]=
...
e.maxLen=Math.max(a,e.name.length),e.minCount=d3.min(e.children.map(function(a){return a.minCount})));return e}a||(a=[]);var c=JSON.parse(JSON.stringify(this));return b(a,0,c)}}function g(a,b){for(var c in b)a[c]=b[c]}var h=" ",k="_root_",m=!a&&!0;b||(b=["item"]);var l=b;n||(n="\t");var n=c;q||(q=[l[0]]);var q=d,u=e;void 0==u&&(u=!0);var p=new f(k,-1,0);this.addNgram=function(a,b,c){p.addNgram(a,b,c)};this.getUniqRoot=function(){var a=new doubletree.Trie(!m,l,n,q);a.trie(p.getUniqRoot());return a};
...
a+="\x3ci\x3e"+e.getTitle()+"\x3c/i\x3e";for(facet in b)if(0!=b[facet].length){var f="";if("facet.title"!=facet){var g=facet.replace(/^.+?\./,""),m=e.get(g);if(m){var r=Ext.isArray(m);r?f+="\x3cli\x3e"+g+"\x3cul\x3e":m=[m];m.forEach(function(a){var c=!1;b[facet].forEach(function(b){b.label==a?c=!0:-1==b.facet.indexOf("facet")&&b.label.split(/\W+/).forEach(function(b){0<b.trim().length&&-1<a.toLowerCase().indexOf(b.toLowerCase())&&(c=!0)})});f+="\x3cli\x3e"+(r?"":g.replace("extra.","")+": ")+(c?'\x3cspan class\x3d"keyword"\x3e'+
...
scope:this})},this)},loadFromCorpusTermRecords:function(a){if(Ext.isArray(a)&&0<a.length){var b=[];a.forEach(function(a){b.push(a.getTerm())});this.loadFromQuery(b)}},loadFromCorpusCollocateRecords:function(a,b){if(Ext.isArray(a)){var c=this.getApiParam("limit"),d=this.getNodeDataSet(),e=this.getEdgeDataSet(),f={};d.forEach(function(a){f[a.id]=!0});var g=[],h=[];a.forEach(function(a,m){var l=a.getTerm(),n=a.getContextTerm(),q=a.getKeywordRawFreq(),u=a.getContextTermRawFreq(),p=q,r=u;this.getGraphMode()===
this.CENTRALIZED_MODE&&(p=0,r=Math.log(u));0==m&&(void 0===b&&(b=l),void 0!==f[b]?d.update({id:b,value:p,title:l+" ("+q+")",type:"keyword",font:{color:this.keywordColor}}):(f[b]=!0,g.push({id:l,label:l,title:l+" ("+q+")",type:"keyword",value:p,start:c,font:{color:this.keywordColor}})));if(l!=n){void 0===f[n]&&(f[n]=!0,g.push({id:n,label:n,title:n+" ("+u+")",type:"context",value:r,start:0,font:{color:this.contextColor}}));var C=null;e.forEach(function(a){if(a.from==b&&a.to==n||a.from==n&&a.to==b)C=
a});l=a.getContextTermRawFreq();null===C?h.push({from:b,to:n,value:l}):C.value<l&&e.update({id:C.id,value:l})}},this);d.add(g);e.add(h);this.forceUpdate();this.getNetwork().fit()}},doCentralize:function(a){a=this.getNodeDataSet().get(a);a.fixed=!0;a.shape="circle";a.scaling={min:75,max:75,label:{min:75,max:75}};this.getNodeDataSet().clear();this.getEdgeDataSet().clear();this.getNodeDataSet().add(a);this.setGraphMode(this.CENTRALIZED_MODE);var b=this.getApiParam("limit");this.setApiParam("limit",150);
...
!0);Ext.tip.QuickTipManager.register({target:b.getEl(),text:a.localize("UploadLocal")})},beforedestroy:function(a){Ext.tip.QuickTipManager.unregister(a.getEl())},change:function(b,c){if(c){var d=b.up("form").getForm();if(d.isValid()){for(var e=b.fileInputEl.dom.files,f=/\.(png|gif|jpe?g|xls|mp[234a]|mpeg|exe|wmv|avi|ppt|mpg|tif|wav|mov|psd|wma|ai|bmp|pps|aif|pub|dwg|indd|swf|asf|mbd|dmg|flv)$/i,g=/\.(txt|pdf|html?|xml|docx?|rtf|pages|odt|zip|jar|tar|gz|ar|cpio|bzip2|bz2|gzip|xlsx?)$/i,h=[],k=[],m=
0,l=e.length;m<l;m++){var n=e[m].name;f.test(n)?h.push(n.split("/").pop()):g.test(n)||k.push(n.split("/").pop())}0<h.length||0<k.length?Ext.Msg.show({title:a.localize("fileTypesWarning"),icon:Ext.MessageBox.ERROR,message:a.localize("fileTypesMessage")+"\x3cul\x3e"+(0<h.length?"\x3cli\x3e"+a.localize("badFiles")+h.slice(0,5).join(", ")+(5<h.length?"\u2026":"")+"\x3c/li\x3e":"")+(0<k.length?"\x3cli\x3e"+a.localize("unknownFiles")+k.slice(0,5).join(", ")+(5<k.length?"\u2026":"")+"\x3c/li\x3e":"")+"\x3c/ul\x3e"+
...
b.fillStyle="rgba(0,0,0,.8)",b.arc(a.x,a.y,h,0,2*Math.PI),b.fill(),b.stroke();var g=Math.abs(a.x-a.targetX),l=Math.abs(a.y-a.targetY);if(0!=g||0!=l)1>g?a.x=a.targetX:(g/=2,a.x=a.x>a.targetX?a.x-g:a.x+g),1>l?a.y=a.targetY:(l/=2,a.y=a.y>a.targetY?a.y-l:a.y+l),e=!0},this);var m=0,k=parseInt(this.textFont)+4;b.textAlign="center";b.textBaseline="middle";for(var l in this.magnets)d||l in g||this.magnets[l].isHovering?(m=this.magnets[l].width+4,b.fillStyle=l in g||this.magnets[l].isHovering||!0===f?"white":
"rgba(255,255,255,.05)",b.fillRect(this.magnets[l].x-m/2,this.magnets[l].y-k/2,m,k),b.strokeStyle=l in g||this.magnets[l].isHovering||!0===f?"rgb("+this.magnets[l].colour.join(",")+")":"rgba(0,0,0,.05)",b.strokeRect(this.magnets[l].x-m/2,this.magnets[l].y-k/2,m,k),b.textAlign="center",b.fillStyle=l in g||this.magnets[l].isHovering||!0===f?"rgba(0,0,0,.8)":"rgba(0,0,0,.05)",b.fillText(l,this.magnets[l].x,this.magnets[l].y)):(b.beginPath(),b.fillStyle="rgb("+this.magnets[l].colour.join(",")+")",b.strokeStyle=
"rgb("+this.magnets[l].colour.join(",")+")",b.arc(this.magnets[l].x,this.magnets[l].y,12,0,2*Math.PI),b.fill(),b.stroke())}if(e){var n=this;setTimeout(function(){n.draw()},100)}else if(this.documents){c=Math.max(c/this.documents.length,50);m=0;for(l=this.documents.length;m<l;m++)for(var q=0;q<l;q++)if(m<q){var u=this.documents[m].x-this.documents[q].x,p=this.documents[m].y-this.documents[q].y;Math.sqrt(u*u+p*p)<c&&(u*=.1,p*=.1,this.documents[m].targetX+=u,this.documents[q].targetX-=u,this.documents[m].targetY+=
...
constructor:function(){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(){var a=Ext.create("Voyant.data.store.Tokens"),b=this;a.on("beforeload",function(a){return b.hasCorpusAccess(a.getCorpus())});a.on("load",function(a,b,e){if(e){var f="",g=this.localize("documentFrequency"),h=!1,k=-1,m=!1;b.forEach(function(a){0==a.getPosition()&&(f+="\x3ch3\x3e"+this.getDocumentsStore().getById(a.getDocId()).getFullLabel()+"\x3c/h3\x3e");a.getDocIndex()!=
k&&(h=this.getDocumentsStore().getById(a.getDocId()).isPlainText(),k=a.getDocIndex());if(a.isWord())m=!1,f+="\x3cspan class\x3d'word' id\x3d'"+a.getId()+"' data-qtip\x3d'"+g+" "+a.getDocumentRawFreq()+"'\x3e"+a.getTerm()+"\x3c/span\x3e";else{a=a.getTermWithLineSpacing(h);var b=0==a.indexOf("\x3cbr /\x3e");if(!m||!b&&0!=a.trim().length)f+=a,m=b}},this);this.updateText(f,!0);this.down("querysearchfield").getValue()}},this);this.setTokensStore(a);this.on("query",function(a,b){this.loadQueryTerms(b)},
this);this.setDocumentTermsStore(Ext.create("Ext.data.Store",{model:"Voyant.data.model.DocumentTerm",autoLoad:!1,remoteSort:!1,proxy:{type:"ajax",url:Voyant.application.getTromboneUrl(),extraParams:{tool:"corpus.DocumentTerms",withDistributions:!0,withPositions:!0,bins:25},reader:{type:"json",rootProperty:"documentTerms.terms",totalProperty:"documentTerms.total"},simpleSortMode:!0},listeners:{load:function(a,b,e,f){a.sort("docIndex","ASC");var g={},h=0,k;a.each(function(a){var b=[],c=a.get("distributions"),
d=a.get("docId"),e=a.get("docIndex");k=a.get("term");for(a=0;a<c.length;a++){var f=a,r=c[a];r>h&&(h=r);b.push([d,e,f,r,k])}g[e]=b},this);this.highlightKeywords(k);a=this.query("cartesian");for(b=0;b<a.length;b++)e=a[b],f=g[b],void 0!==f?(e.getAxes()[0].setMaximum(h),e.getStore().loadData(f)):e.getStore().removeAll()},scope:this}}));this.on("afterrender",function(){var a=this.down('panel[region\x3d"center"]');this.setInnerContainer(a.getLayout().getRenderTarget());a.body.on("scroll",function(a,b){this.getInnerContainer().first();
var c=this.getLastScrollTop()<b.scrollTop;!c&&1>b.scrollTop?this.fetchPrevious(!0):c&&b.scrollHeight-b.scrollTop<1.5*b.offsetHeight?this.fetchNext(!1):250<new Date-this.getLastLocationUpdate()&&this.updateLocationMarker(b);this.setLastScrollTop(b.scrollTop)},this);a.body.on("click",function(a,b){b=Ext.get(b);if(b.hasCls("word")){var c=Voyant.data.model.Token.getInfoFromElement(b),g=b.getHtml(),c=[{term:g,docIndex:c.docIndex}];this.loadQueryTerms([g]);this.getApplication().dispatchEvent("termsClicked",
this,c)}},this);this.getCorpus()&&(this.load(),(a=this.getApiParam("query"))&&this.loadQueryTerms(Ext.isString(a)?[a]:a))},this);Ext.apply(this,{title:this.localize("title"),cls:"voyant-reader",layout:"fit",items:{layout:"border",items:[{bodyPadding:10,region:"center",border:!1,autoScroll:!0,html:'\x3cdiv class\x3d"readerContainer"\x3e\x3c/div\x3e'},{region:"south",height:40,split:{size:2},splitterResize:!0,border:!1,layout:{type:"hbox"}}]},dockedItems:[{dock:"bottom",xtype:"toolbar",overflowHandler:"scroller",
items:[{glyph:"xf060@FontAwesome",handler:function(){this.fetchPrevious(!0)},scope:this},{glyph:"xf061@FontAwesome",handler:function(){this.fetchNext(!0)},scope:this},{xtype:"tbseparator"},{xtype:"querysearchfield"}]}],listeners:{loadedCorpus:function(a,b){this.getTokensStore().setCorpus(b);this.getDocumentTermsStore().getProxy().setExtraParam("corpus",b.getId());var e=b.getDocuments();this.setDocumentsStore(e);e=this.down('panel[region\x3d"south"]');this.setLocationMarker(Ext.DomHelper.append(e.el,
{tag:"div",style:"background-color: #157fcc; height: 100%; width: 2px; position: absolute; top: 0; left: 0;"}));this.generateChart(b,e);this.rendered&&(this.load(),0==this.hasCorpusAccess(b)&&this.mask(this.localize("limitedAccess"),"mask-no-spinner"),(e=this.getApiParam("query"))&&this.loadQueryTerms(Ext.isString(e)?[e]:e))},termsClicked:function(a,b){var e=[];b.forEach(function(a){Ext.isString(a)?e.push(a):a.term?e.push(a.term):a.getTerm&&e.push(a.getTerm())});0<e.length&&this.loadQueryTerms(e)},
corpusTermsClicked:function(a,b){var e=[];b.forEach(function(a){a.getTerm()&&e.push(a.getTerm())});this.loadQueryTerms(e)},documentTermsClicked:function(a,b){var e=[];b.forEach(function(a){a.getTerm()&&e.push(a.getTerm())});this.loadQueryTerms(e)},documentsClicked:function(a,b,e){0<b.length&&(this.setApiParams({skipToDocId:b[0].getId(),start:0}),this.load(!0))},termLocationClicked:function(a,b){if(void 0!==b[0]){var e=b[0],f=e.get("docIndex"),g=e.get("position"),h=g-this.getApiParam("limit")/2,k=
this.getCorpus().getDocument(f);this.setApiParams({skipToDocId:k.getId(),start:0>h?0:h});this.load(!0,{callback:function(){var a=this.body.dom.querySelector("#_"+f+"_"+g);a&&a.scrollIntoView();this.highlightKeywords(e.get("term"),!1)},scope:this})}},documentIndexTermsClicked:function(a,b){if(void 0!==b[0]){var e=Ext.create("Voyant.data.model.Token",b[0]);this.fireEvent("termLocationClicked",this,[e])}},scope:this}});this.callParent(arguments)},loadQueryTerms:function(a){a&&0<a.length&&this.getDocumentTermsStore().load({params:{query:a}})},
...
c.insertSibling("\x3cdiv class\x3d'loading'\x3e"+this.localize("loading")+"\x3c/div\x3e","after",!1).mask();c.destroy();this.setApiParams({skipToDocId:h,start:b.position});this.setInsertWhere("beforeEnd");this.setScrollIntoView(a);this.load();this.setApiParam("limit",this.INITIAL_LIMIT);break}c.destroy();c=b.last()}},load:function(a,b){a&&(this.getInnerContainer().first().destroy(),this.getInnerContainer().setHtml('\x3cdiv class\x3d"readerContainer"\x3e\x3cdiv class\x3d"loading"\x3e'+this.localize("loading")+
"\x3c/div\x3e\x3c/div\x3e"),this.getInnerContainer().first().first().mask());this.getTokensStore().load(Ext.apply(b||{},{params:Ext.apply(this.getApiParams(),{stripTags:"blocksOnly"})}))},updateText:function(a){var b=this.getInnerContainer().down(".loading");b&&b.destroy();var c=this.getInnerContainer().first().insertHtml(this.getInsertWhere(),a,!0);c&&this.getScrollIntoView()&&(c.dom.scrollIntoView(),Ext.Function.defer(function(){var a=Ext.get(c.id);a&&a.frame("red")},100));a=this.down('panel[region\x3d"center"]').body.dom;
...
a.y})});a=this.body.down("svg").dom.clientHeight-this.graphMargin.top-this.graphMargin.bottom;var m=d3.scale.linear().domain([0,k]).range([a,0]),l=d3.svg.area().x(function(a){return h(a.x)}).y0(function(a){return m(a.y0)}).y1(function(a){return m(a.y0+a.y)}),k=d3.svg.axis().scale(h).orient("bottom");if(this.getMode()===this.MODE_CORPUS){for(var n=[],q=0;q<=f;q++)n.push(q);k.tickValues(n);k.tickFormat("")}n=d3.svg.axis().scale(m).orient("left");c=this.getVis().selectAll("path").data(c,function(a){return a.name});
c.attr("d",function(a){return l(a.values)}).style("fill",function(a,c){return b.getColorForTerm(a.name,!0)});c.enter().append("path").attr("d",function(a){return l(a.values)}).style("fill",function(a,c){return b.getColorForTerm(a.name,!0)}).append("title").text(function(a){return a.name});c.exit().remove();this.getVis().selectAll("g.axis").remove();this.getVis().append("g").attr("class","axis x").attr("transform","translate(0,"+a+")").call(k);if(this.getMode()===this.MODE_CORPUS){var u=g/f,p=0;this.getCorpus().getDocuments().each(function(a){this.getVis().select("g.x").append("text").attr("text-anchor",
"end").attr("transform","translate("+p+", 10) rotate(-45)").text(a.getTinyTitle());p+=u},this);c=this.localize("documents")}else c=this.localize("documentSegments");this.getVis().select("g.x").append("text").attr("text-anchor","middle").attr("transform","translate("+g/2+", "+(this.graphMargin.bottom-20)+")").text(c);this.getVis().append("g").attr("class","axis y").attr("transform","translate(0,0)").call(n);c="raw"===this.getApiParam("withDistributions")?this.localize("rawFrequencies"):this.localize("relativeFrequencies");
...
type:a.getTerm(),val:Ext.util.Format.number(a.get("rawFreq"),"0,000"),docId:a.get("docId")})});var l;f.forEach(function(a){if(m[a]){var c=this.getCorpus().getDocument(a);l=m[a].length;Ext.dom.Helper.append(b,{tag:"li","voyant:index":String(a),html:'\x3ca href\x3d"#" onclick\x3d"return false" class\x3d"document-id document-id-distinctive" voyant:val\x3d"'+c.get("id")+'"\x3e'+c.getShortTitle()+"\x3c/a\x3e"+this.localize("colon")+" "+(new Ext.XTemplate(this.localize("documentType"))).apply({types:m[a]})+
...
f=this.sourceTerm.get("y"),g=this.targetTerm.get("x"),h=this.targetTerm.get("y");this.previousTerm&&this.previousTerm.get("x");this.previousTerm&&this.previousTerm.get("y");var k=Math.max(100,.5*Math.abs(e-g)),m=Math.max(100,.5*Math.abs(f-h));this.previousBeziers.unshift([e,f,e>g?e-k:e+k,h>f?f+m:f-m,g,h]);10<this.previousBeziers.length&&this.previousBeziers.pop()}for(e=0;e<this.previousBeziers.length;e++)b.strokeStyle="rgba(0,0,255,"+(1-.1*e)+")",this.drawBezierSplit.apply(this,Ext.Array.merge([b],
...
Ext.override(Ext.chart.axis.sprite.Axis,{renderLabels:function(a,b,c,d){var e=this,f=e.attr,g=.5*f.lineWidth,h=f.position,k=f.matrix,m=f.textPadding,l=k.getXX(),n=k.getDX(),q=k.getYY(),u=k.getDY(),p=0,k=c.majorTicks,r=Math.max(f.majorTickSize,f.minorTickSize)+f.lineWidth,C=Ext.draw.Draw.isBBoxIntersect,s=e.getLabel(),H,L,D=null,G=0,z=0,I=c.segmenter,E=e.getRenderer(),A=e.getAxis(),J=(H=A.getTitle())&&""!==H.attr.text&&H.getBBox(),M,K=null,x,B,v;if(k&&s&&!s.attr.hidden){H=s.attr.font;b.font!==H&&(b.font=
H);s.setAttributes({translationX:0,translationY:0},!0);s.applyTransformations();M=s.attr.inverseMatrix.elements.slice(0);switch(h){case "left":b=J?J.x+J.width:0;switch(s.attr.textAlign){case "start":d=a.roundPixel(b+n)-g;break;case "end":d=a.roundPixel(d[2]-r+n)-g;break;default:d=a.roundPixel(b+(d[2]-b-r)/2+n)-g}s.setAttributes({translationX:d},!0);break;case "right":b=J?d[2]-J.x:0;switch(s.attr.textAlign){case "start":d=a.roundPixel(r+n)+g;break;case "end":d=a.roundPixel(d[2]-b+n)+g;break;default:d=
a.roundPixel(r+(d[2]-r-b)/2+n)+g}s.setAttributes({translationX:d},!0);break;case "top":b=J?J.y+J.height:0;s.setAttributes({translationY:a.roundPixel(b+(d[3]-b-r)/2)-g},!0);break;case "bottom":b=J?d[3]-J.y:0;s.setAttributes({translationY:a.roundPixel(r+(d[3]-r-b)/2)+g},!0);break;case "radial":s.setAttributes({translationX:f.centerX},!0);break;case "angular":s.setAttributes({translationY:f.centerY},!0);break;case "gauge":s.setAttributes({translationY:f.centerY},!0)}if("left"===h||"right"===h)e.iterate(k,
function(b,d,f){void 0!==d&&(v=E?Ext.callback(E,null,[A,d,c,D],0,A):I.renderer(d,c,D),D=d,s.setAttributes({text:String(v),translationY:a.roundPixel(b*q+u)},!0),s.applyTransformations(),p=Math.max(p,s.getBBox().width+r),p<=e.thickness&&(B=Ext.draw.Matrix.fly(s.attr.matrix.elements.slice(0)),x=B.prepend.apply(B,M).transformBBox(s.getBBox(!0)),!K||C(x,K,m)))&&(a.renderSprite(s),K=x,G+=x.height,z++)});else if("top"===h||"bottom"===h)e.iterate(k,function(b,d,f){void 0!==d&&(v=E?Ext.callback(E,null,[A,
d,c,D],0,A):I.renderer(d,c,D),D=d,s.setAttributes({text:String(v),translationX:a.roundPixel(b*l+n)},!0),s.applyTransformations(),p=Math.max(p,s.getBBox().height+r),p<=e.thickness&&(B=Ext.draw.Matrix.fly(s.attr.matrix.elements.slice(0)),x=B.prepend.apply(B,M).transformBBox(s.getBBox(!0)),K&&C(x,K,m),a.renderSprite(s),K=x,G+=x.width,z++))});else if("radial"===h)e.iterate(k,function(b,d,e){void 0!==d&&(v=E?Ext.callback(E,null,[A,d,c,D],0,A):I.renderer(d,c,D),D=d,"undefined"!==typeof v&&(s.setAttributes({text:String(v),
translationX:f.centerX-a.roundPixel(b)/f.max*f.length*Math.cos(f.baseRotation+Math.PI/2),translationY:f.centerY-a.roundPixel(b)/f.max*f.length*Math.sin(f.baseRotation+Math.PI/2)},!0),s.applyTransformations(),x=s.attr.matrix.transformBBox(s.getBBox(!0)),!K||C(x,K)))&&(a.renderSprite(s),K=x,G+=x.width,z++)});else if("angular"===h)L=f.majorTickSize+.5*f.lineWidth+(parseInt(s.attr.fontSize,10)||10)/2,e.iterate(k,function(b,d,e){void 0!==d&&(v=E?Ext.callback(E,null,[A,d,c,D],0,A):I.renderer(d,c,D),D=d,
p=Math.max(p,Math.max(f.majorTickSize,f.minorTickSize)+("butt"!==f.lineCap?.5*f.lineWidth:0)),"undefined"!==typeof v&&(b=b/(f.max+1)*Math.PI*2+f.baseRotation,s.setAttributes({text:String(v),translationX:f.centerX+(f.length+L)*Math.cos(b),translationY:f.centerY+(f.length+L)*Math.sin(b)},!0),s.applyTransformations(),x=s.attr.matrix.transformBBox(s.getBBox(!0)),!K||C(x,K)))&&(a.renderSprite(s),K=x,G+=x.width,z++)});else if("gauge"===h){var w=e.getGaugeAngles();e.iterate(k,function(b,d,e){void 0!==d&&
(v=E?Ext.callback(E,null,[A,d,c,D],0,A):I.renderer(d,c,D),D=d,"undefined"!==typeof v&&(b=(b-f.min)/(f.max-f.min+1)*f.totalAngle-f.totalAngle+w.start,s.setAttributes({text:String(v),translationX:f.centerX+(f.length+10)*Math.cos(b),translationY:f.centerY+(f.length+10)*Math.sin(b)},!0),s.applyTransformations(),x=s.attr.matrix.transformBBox(s.getBBox(!0)),!K||C(x,K)))&&(a.renderSprite(s),K=x,G+=x.width,z++)})}f.enlargeEstStepSizeByText&&z&&(G/=z,G+=r,G*=2,f.estStepSize<G&&(f.estStepSize=G));if(1<Math.abs(e.thickness-
...
e=d.getSize(!1),d=d.dom.rows,f=0;f<d.length;f++)for(var g=d[f].cells,h=0;h<g.length;h++){var k=Ext.get(g[h]),m=k.down(".x-panel"),l=m.id;if(void 0!==a&&void 0!==b)k=m.getSize(!1),k.width*=a,k.height*=b;else{var m=k.getSize(!1),n=Ext.getCmp(l),q=n.initialConfig.cellWidth,n=n.initialConfig.cellHeight;void 0!==q&&(m.width=q/100*e.width,k.setWidth(m.width));void 0!==n&&(m.height=n/100*e.height,k.setHeight(m.height));k=m}c[l]=k}for(var u in c)k=c[u],Ext.getCmp(u).setSize(k);this.updateLayout()}});Ext.define("Voyant.panel.Veliza",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],xtype:"veliza",autoScroll:!0,statics:{i18n:{title:"Veliza",typeAndEnter:"Type text and hit enter.",send:"send",fromCorpus:"from text"},api:{},glyph:"xf0e6@FontAwesome"},config:{previous:[]},initComponent:function(a){var b=this;Ext.apply(this,{title:this.localize("title"),glyph:"xf0e6@FontAwesome",html:"\x3cform class\x3d'chat'\x3e\x3c/form\x3e",dockedItems:[{dock:"bottom",xtype:"toolbar",overflowHandler:"scroller",
