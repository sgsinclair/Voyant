/* This file created by JSCacher. Last modified: Mon Jan 14 12:49:39 EST 2019 */
...
this.lastClickedBubbles[b],m=0,n;for(n in k)if(-1!==this.termsFilter.indexOf(n)){var p=k[n];if(p){m++;this.SEPARATE_LINES_FOR_TERMS&&c();var r=0,q=p.color.join(",");this.ctx.strokeStyle="rgba("+q+", 1)";this.ctx.fillStyle="rgba("+q+", 0.35)";this.ctx.lineWidth=.25;for(var h=h+p.rawFreq,r=r+p.rawFreq,s=l&&this.lastClickedBubbles[b][n],u=0;u<p.pos.length;u++){var A=p.pos[u];if(0<A.radius){var D=!1;s&&this.lastClickedBubbles[b][n]==A.id&&(this.ctx.strokeStyle="rgba(0, 0, 0, 0.75)",this.ctx.fillStyle=
...
d.data[n+2],d.data[n+3]];"rgb("+n[0]+", "+n[1]+", "+n[2]+")"!=g.canvas.style.backgroundColor&&(e[k][m]=!0);e[k][m]&&(l=m+f.COARSENESS)}}c.mask=e}}function e(){g.clear();for(var a=q.length;a--;){var b=q[a];b.alpha=1;b.live&&b.draw(g.context)}a=$(g.canvas);if(null!=n){a.css("cursor","pointer");g.context.save();g.context.textBaseline="alphabetic";g.context.font="12px Arial";var b=g.context.measureText(n.text).width,c=g.context.measureText(n.value).width,b=(b>c?b:c)+20,c=p+15,d=r+25,e=a.width(),a=a.height();
...
0;var k=this.MIXED=1;this.getWordOrientation=function(){return k};this.setWordOrientation=function(a){k=a};this.UPDATE_RATE=25;this.COARSENESS=5;this.grid=[];var l=0,m;this.doingArrange=!1;this.wordsToArrange=5;var n=null,p=0,r=0,q=[];this.getWords=function(){return q};this.setWords=function(a){q=a};this.sizeAdjustment=100;this.minFontSize=12;this.largestWordSize=0;this.smallestWordSize=1E4;var s={};this.addWords=function(a){for(var b=!1,e=0;e<a.length;e++){var h=a[e],k;k=void 0==typeof h.color||
null==h.color||""==h.color?g.colors[Math.floor(Math.random()*g.colors.length)]:h.color;var l;l=void 0==typeof h.size||null==h.size||""==h.size?Math.floor(40*Math.random()):parseFloat(h.size);var m=h.word,n=h.label,p=h.value,h=!1;null==s[m]&&(s[m]=!0,l>f.largestWordSize&&(f.largestWordSize=l,h=!0),l<f.smallestWordSize&&(f.smallestWordSize=.8*l,h=!0),m=new Word(m,l,k,n,p),q.push(m));b=h||b}c();this.setRelativeSizes();this.calculateSizeAdjustment();b?this.resizeWords():d()};this.resetWordCoordinates=
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];l=0;f=function(){var a,f,h,k,n=this.wordsToArrange-1,s=g.canvas.width,p=g.canvas.height,r=.5*s,u=.5*p;do{h=q[l];if(void 0!==h){for(var v=Math.random()*Math.PI,z=.25*Math.random()*h.size,w=.5*(Math.random()-.5),F=.5*h.width,I=.5*h.height;;){a=Math.floor((r+Math.cos(v)*z*this.ratio-F)/this.COARSENESS)*this.COARSENESS;f=Math.floor((u+Math.sin(v)*z-I)/this.COARSENESS)*this.COARSENESS;
k=a+F>=s||f+I>=p?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;v+=w;z+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<l;a++)f=q[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}l++;if(l>=q.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(n--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;m=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
w.ids={};c(w.ids,z.info.ids);c(w.ids,v.info.ids);w.count=Object.keys(w.ids).length;K=Object.keys(w.ids);if(z.info.origIDs||v.info.origIDs)w.origIDs={},c(w.origIDs,z.info.origIDs),c(w.origIDs,v.info.origIDs),w.origCount=Object.keys(w.origIDs).length;z.info=w;v.info=w;F=Math.max(v.maxChildren,z.maxChildren);if(isNaN(F)||0==F)return I=!1,b;p?E=d3.scaleLog().range([8,14]):(E=function(){return 14},E.domain=function(){});F=Math.min(v.minCount,z.minCount);E.domain([F,v.info.count]);var L=g-20-20,M=h-20-
...
B=new doubletree.Tree(c.select("g"),g,h,v,!0,r,J,E,n,q,s,u,A,D,H);C=new doubletree.Tree(c.select("g"),g,h,z,!1,r,J,E,n,q,s,u,A,D,H)});B.handleAltPress=m.alt;C.handleAltPress=m.alt;B.handleShiftPress=m.shift;C.handleShiftPress=m.shift;B.handleClick=m.click;C.handleClick=m.click;I=!0;return b};b.setupFromArrays=function(a,c,d,f,g,h,l,m){void 0==g&&x&&(g=x.caseSensitive());void 0==h&&x&&(h=x.fieldNames());void 0==l&&x&&(l=x.fieldDelim());void 0==m&&x&&(m=x.distinguishingFieldsArray());x=new doubletree.Trie(g,
...
!1));b&&v.update(a)}function G(a){return k?-a:a}var B=p,C=s;b=b-20-20;c=c-20-20;var K=0,E;l||(l=doubletree.sortByStrFld("token"));var K=0,O=d3.tree().size([c,b]).nodeSize([40,40]),N=d3.linkHorizontal().x(function(a){return G(a.y)}).y(function(a){return a.x});a=a.append("g").attr("transform","translate(20,20)");this.readJSONTree=function(a){E=d3.hierarchy(a);E.x0=0;E.y0=0;E.children.forEach(H);this.update(E)};this.update=function(b){var c=O(E).descendants();c.forEach(function(a){var b=a.textSize;if(void 0==
...
e.maxLen=Math.max(a,e.name.length),e.minCount=d3.min(e.children.map(function(a){return a.minCount})));return e}a||(a=[]);var c=JSON.parse(JSON.stringify(this));return b(a,0,c)}}function g(a,b){for(var c in b)a[c]=b[c]}var h=" ",k="_root_",l=!a&&!0;b||(b=["item"]);var m=b;n||(n="\t");var n=c;p||(p=[m[0]]);var p=d,r=e;void 0==r&&(r=!0);var q=new f(k,-1,0);this.addNgram=function(a,b,c){q.addNgram(a,b,c)};this.getUniqRoot=function(){var a=new doubletree.Trie(!l,m,n,p);a.trie(q.getUniqRoot());return a};
this.toTree=function(a,b){return q.toTree(a,b)};this.serialize=function(){return JSON.stringify(this)};this.deserialize=function(a){a=JSON.parse(a);h=a.endNG();k=a.rootName();l=a.caseSensitive();m=a.fieldNames();n=a.fieldDelim();p=a.distinguishingFieldsArray();q=a.trie()};this.endNG=function(){return h};this.rootName=function(){return k};this.trie=function(a){0<arguments.length&&(q=a);return q};this.caseSensitive=function(){return!l};this.fieldNames=function(){return m};this.fieldDelim=function(){return n};
...
2,f=d.getHeight()/2,g={};this.getNodeData().forEach(function(a){g[a.id]=!0},this);var h=[],k=[];a.forEach(function(a,d){var n=a.getTerm(),p=a.getContextTerm(),r=a.getKeywordRawFreq(),q=a.getContextTermRawFreq(),s=r,u=q;this.getNetworkMode()===this.CENTRALIZED_MODE&&(s=0,u=Math.log(q));var A=void 0,A=void 0;0==d&&(void 0===b&&(b=this.idGet(n)),void 0!==g[b]?this.updateDataForNode(b,{title:n+" ("+r+")",type:"keyword",value:s}):(g[b]=!0,A={id:b,term:n,title:n+" ("+r+")",type:"keyword",value:s,start:c,
fixed:!1,x:e,y:f},h.push(A)));if(n!=p){n=this.idGet(p);void 0===g[n]&&(g[n]=!0,A={id:n,term:p,title:p+" ("+q+")",type:"context",value:u,start:0,fixed:!1,x:e,y:f},h.push(A));p=null;q=this.getLinkData();for(u=0;u<q.length;u++)if(r=q[u],r.source.id==b&&r.target.id==n||r.source.id==n&&r.target.id==b){p=r;break}q=a.getContextTermRawFreq();null===p&&k.push({source:b,target:n,value:q,id:b+"-"+n})}},this);this.setNodeData(this.getNodeData().concat(h));this.setLinkData(this.getLinkData().concat(k));this.refresh()}},
...
f);for(var g=void 0,k=currentItem-1;-1<k;k--)8<=c?(e-=c,c-=.5):(void 0==g&&(g=e/k),e-=g),.1<f&&(f-=.05),d3.select(b[k]).attr("y",function(a){return e}).attr("font-size",c).attr("fill-opacity",f);c=14;e=Math.max(a[1],c);f=1;g=void 0;k=currentItem+1;for(a=terms.length;k<a;k++)8<=c?(e+=c,c-=.5):(void 0==g&&(g=(d-e)/(a-k)),e+=g),.1<f&&(f-=.05),d3.select(b[k]).attr("y",function(a){return e}).attr("font-size",c).attr("fill-opacity",f)})}}},{itemId:"threads",layout:"fit",flex:1,listeners:{filterchange:function(a){var b=
this.getTargetEl(),c=b.getWidth(),d=b.getHeight();b.setHtml(" ");terms=a.getRange().map(function(a){return{term:a.getTerm(),vals:a.getDistributions()}});var k=d3.select(b.dom).append("svg").attr("width",b.getWidth()).attr("height",b.getHeight());if(0==terms.length)return this.up("panel").toastInfo("No hits");var l=c/terms[0].vals.length;a=this.up("loom").getApiParam("scaling");var m=("sqrt"==a?d3.scaleSqrt():"log"==a?d3.scaleLog():d3.scaleLinear()).domain([Math.max(1E-6,Ext.Array.min(terms.map(function(a){return Ext.Array.min(a.vals)}))),
Math.max(1E-6,Ext.Array.max(terms.map(function(a){return Ext.Array.max(a.vals)})))]).range([1E-6,d-2]),n=d3.line().curve(d3.curveCardinal).x(function(a,b){return l/2+l*b}).y(function(a){return d-1-m(a)}),p=k.append("g").attr("opacity",0),r=p.append("rect").attr("fill","white").attr("text-anchor","middle").attr("alignment-baseline","middle").attr("stroke","rgba(0,0,0,.05)").attr("class","rect").attr("x",100).attr("y",100).attr("width",70).attr("height",16).attr("opacity",1),q=p.append("text").attr("text-anchor",
"middle").attr("alignment-baseline","middle").attr("x",100).attr("y",100).text("testing");terms.forEach(function(a){var b=Voyant.application.getColorForTerm(a.term,!0);k.append("path").datum(a.vals).attr("fill","none").attr("stroke",b).attr("opacity",.5).attr("stroke-linejoin","round").attr("stroke-linecap","round").attr("stroke-width",1).attr("class","line").attr("d",n);k.append("path").datum(a.vals).attr("fill","none").attr("stroke",b).attr("opacity",0).attr("stroke-width",3).attr("class","line").attr("term",
...
e=d.getSize(!1),d=d.dom.rows,f=0;f<d.length;f++)for(var g=d[f].cells,h=0;h<g.length;h++){var k=Ext.get(g[h]),l=k.down(".x-panel"),m=l.id;if(void 0!==a&&void 0!==b)k=l.getSize(!1),k.width*=a,k.height*=b;else{var l=k.getSize(!1),n=Ext.getCmp(m),p=n.initialConfig.cellWidth,n=n.initialConfig.cellHeight;void 0!==p&&(l.width=p/100*e.width,k.setWidth(l.width));void 0!==n&&(l.height=n/100*e.height,k.setHeight(l.height));k=l}c[m]=k}for(var r in c)k=c[r],Ext.getCmp(r).setSize(k);this.updateLayout()}});Ext.define("Voyant.panel.Veliza",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],xtype:"veliza",autoScroll:!0,statics:{i18n:{},api:{script:"",message:void 0},glyph:"xf0e6@FontAwesome"},config:{previous:[]},constructor:function(){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments);var a=this;Ext.apply(this,{title:this.localize("title"),glyph:"xf0e6@FontAwesome",layout:{type:"border",align:"stretch"},items:[{itemId:"chat",html:"\x3cform class\x3d'chat'\x3e\x3c/form\x3e",
