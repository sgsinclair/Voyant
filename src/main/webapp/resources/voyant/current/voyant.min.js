...
this)}},_run:function(){this.results.show();this.results.update(this.EMPTY_RESULTS_TEXT);this.results.mask("working\u2026");var a=this.editor.getValue();Voyant.notebook.util.Show.TARGET=this.results.getEl();Voyant.notebook.Notebook.currentBlock=this;Voyant.notebook.Notebook.currentNotebook.setCurrentBlock(this);try{var b=eval.call(window,a)}catch(d){return this.results.unmask(),Voyant.notebook.util.Show.showError(d),this.getTargetEl().fireEvent("resize"),d}this.setIsRun(!0);if(void 0!==b)if(b.then&&
b.catch&&b.finally){var c=this;b.then(function(a){c.results.unmask();void 0!==a&&c._showResult(a)}).catch(function(a){c.results.unmask();Voyant.notebook.util.Show.showError(a)}).finally(function(){Ext.defer(function(){this.getTargetEl().fireEvent("resize")},50,c)})}else this.results.unmask(),this._showResult(b),this.getTargetEl().fireEvent("resize");else this.results.unmask(),this.getTargetEl().fireEvent("resize");return b},_showResult:function(a){this.results.getEl().dom.innerHTML===this.EMPTY_RESULTS_TEXT&&
this.results.update(a)},clearResults:function(){this.results.show();var a=this.results.el.down(".x-panel");if(a){var b=Ext.getCmp(a.id);b?b.destroy():a.destroy()}else this.results.update(" ");this.getTargetEl().fireEvent("resize")},tryToUnmask:function(){Spyral&&Spyral.promises&&Ext.defer(this.tryToUnmask,20,this);if(0==Voyant.application.getDeferredCount()){for(var a in window)"object"==typeof window[a]&&window[a]&&"opener"!=a&&window[a].isFulfilled&&window[a].isFulfilled()&&(window[a]=window[a].valueOf());
this.results.unmask();0==this.results.getTargetEl().getHtml().trim().length&&this.results.hide();this.getTargetEl().fireEvent("resize")}else Ext.defer(this.tryToUnmask,20,this)},getContent:function(){var a={input:this.editor.getValue(),mode:this.editor.getMode().split("/").pop()};if("javascript"==a.mode){var b=this.results.getTargetEl(),c=b.dom.cloneNode(!0);a.output=c.innerHTML;c.style.height||(a.output="\x3cdiv style\x3d'height: "+b.getHeight()+"px'\x3e"+a.output+"\x3c/div\x3e")}return a}});Ext.define("Voyant.notebook.editor.TextEditor",{extend:"Ext.Component",mixins:["Voyant.util.Localization"],alias:"widget.notebooktexteditor",cls:"notebook-text-editor",config:{ckeditorConfig:{toolbar:[{name:"basicstyles",items:["Bold","Italic","-","RemoveFormat"]},{name:"paragraph",items:"NumberedList BulletedList - Justify Outdent Indent Blockquote JustifyLeft JustifyCenter JustifyRight".split(" ")},{name:"colors",items:["TextColor","BGColor"]},{name:"styles",items:["Styles","Format"]},{name:"links",
...
"\x3cp class\x3d'error'\x3e\n"+a.getError()+" \u2026 \x3ca href\x3d'#' onclick\x3d\"window.open('').document.write(unescape('\x3cpre\x3e"+escape(a.getDetails())+"\x3c/pre\x3e')); return false;\"\x3emore\x3c/a\x3e\x3c/p\x3e"};else if(Ext.isString(a))a={message:a};else if(Ext.isObject(a)&&(a.responseText||a.statusText))return this.showResponseError(a.statusText,a);Ext.applyIf(a,{title:this.localize("error"),buttons:Ext.Msg.OK,icon:Ext.MessageBox.ERROR,autoScroll:!0});Ext.Msg.show(a)},getToolConfigFromToolXtype:function(a){cls=
...
[228,200,124],[197,179,159]]},rgbToHex:function(a){return"#"+(16777216+(a[0]<<16)+(a[1]<<8)+a[2]).toString(16).slice(1)},hexToRgb:function(a){a=a.replace(/^#?([a-f\d])([a-f\d])([a-f\d])$/i,function(a,c,d,e){return c+c+d+d+e+e});return(a=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(a))?[parseInt(a[1],16),parseInt(a[2],16),parseInt(a[3],16)]:null},addColorPalette:function(a,b){this.palettes[a]=b},getColorPalette:function(a,b){a=a||this.getApiParam("palette")||"default";a=this.palettes[a];void 0===
a&&(a=[]);if(b){b=[];for(var c=0;c<a.length;c++)b.push(this.rgbToHex(a[c]));return b}return a},getColor:function(a,b){var c=this.getApiParam("palette")||"default";c=this.palettes[c];a>=c.length&&(a%=c.length);return b?this.rgbToHex(c[a]):c[a]},colorTermAssociations:new Ext.util.MixedCollection,getColorForTerm:function(a,b){-1!=a.indexOf(":")&&(a=a.split(":")[1]);var c=this.colorTermAssociations.get(a);if(null==c){c=this.getApiParam("palette")||"default";c=this.palettes[c];var d=this.colorTermAssociations.getCount()%
c.length;c=c[d];this.colorTermAssociations.add(a,c)}b&&(c=this.rgbToHex(c));return c},getRgb:function(a,b){a=this.getColor(a);return"rgba("+a[0]+","+a[1]+","+a[2]+","+b+")"},getRgbaForTerm:function(a,b){a=this.getColorForTerm(a);return"rgba("+a[0]+","+a[1]+","+a[2]+","+b+")"},openUrl:function(a){window.open(a)||Ext.Msg.show({title:"Popup Blocked",buttonText:{ok:"Close"},icon:Ext.MessageBox.INFO,message:"A popup window was blocked. \x3ca href\x3d'"+a+"' target\x3d'_blank' class\x3d'link'\x3eClick here\x3c/a\x3e to open the new window.",
buttons:Ext.Msg.OK})}});Ext.define("Voyant.VoyantCorpusApp",{extend:"Voyant.VoyantApp",name:"VoyantCorpusApp",requires:"Voyant.panel.CorpusSet Voyant.data.model.Corpus Voyant.panel.VoyantHeader Voyant.panel.VoyantFooter Voyant.panel.CorpusCreator Voyant.panel.Cirrus Voyant.panel.Summary Voyant.panel.DreamScape Voyant.panel.CorpusTerms Voyant.panel.Reader Voyant.panel.Documents Voyant.panel.Trends Voyant.panel.Contexts Voyant.panel.DocumentTerms Voyant.panel.CorpusCollocates Voyant.panel.CollocatesGraph Voyant.panel.Phrases Voyant.panel.ScatterPlot Voyant.panel.TopicContexts Voyant.panel.TermsRadio Voyant.panel.TermsBerry".split(" "),
