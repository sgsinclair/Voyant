...
useWorker:!0},enableSnippets:!1,enableBasicAutocompletion:!1})})},removed:function(a,b){a.getEditor()&&a.getEditor().destroy()}},switchModes:function(a){this.setMode("ace/mode/"+a);this.getEditor().getSession().setMode(this.getMode());this.getEditor().setOptions({maxLines:-1<this.getMode().indexOf("javascript")?Infinity:10})},getValue:function(){return this.getEditor().getValue()}});Ext.define("Voyant.notebook.editor.CodeEditorWrapper",{extend:"Voyant.notebook.editor.EditorWrapper",requires:["Voyant.notebook.editor.CodeEditor","Voyant.notebook.editor.button.Run","Voyant.notebook.editor.button.RunAll"],alias:"widget.notebookcodeeditorwrapper",cls:"notebook-code-wrapper",statics:{i18n:{runUntil:"Run up to here",runUntilTip:"Run previous code blocks and this one.",runFrom:"Run from here onwards",runFromTip:"Run this and following code blocks."}},config:{isRun:!1,autoExecute:!1},
layout:{type:"vbox",align:"stretch"},height:130,border:!1,EMPTY_RESULTS_TEXT:" ",constructor:function(a){this.results=Ext.create("Ext.Component",{align:"stretch",cls:"notebook-code-results",html:Ext.Array.from(a.output).join("")});this.editor=Ext.create("Voyant.notebook.editor.CodeEditor",{content:Ext.Array.from(a.input).join("\n"),docs:a.docs,mode:"ace/mode/"+(a.mode?a.mode:"javascript")});Ext.apply(this,{dockedItems:[{xtype:"toolbar",dock:"left",defaults:{textAlign:"left"},items:[{xtype:"notebookwrapperadd"},
{xtype:"notebookwrapperrun",listeners:{click:{fn:this.run,scope:this}}},{xtype:"button",glyph:"f0d0@FontAwesome",tooltip:this.localize("toggleAutoExecute"),itemId:"toggleAutoExecute",enableToggle:!0,pressed:a.autoExecute,listeners:{click:function(a){this.setAutoExecute(a.pressed)},scope:this}},{xtype:"button",glyph:"xf1c9@FontAwesome",tooltip:this.localize("codeModeTip"),listeners:{click:function(){var a=this,c=this.editor.getMode().split("/").pop();(new Ext.Window({title:this.localize("codeModeTitle"),
layout:"fit",width:200,items:[{xtype:"form",layout:{type:"vbox",align:"stretch"},bodyPadding:10,items:[{xtype:"fieldset",title:this.localize("modeCode"),items:{xtype:"radiofield",boxLabel:this.localize("modeJavascript"),name:"codeMode",inputValue:"javascript",flex:1,checked:"javascript"==c}},{xtype:"fieldset",title:this.localize("modeData"),items:[{items:{xtype:"radiofield",boxLabel:this.localize("modeJson"),name:"codeMode",inputValue:"json",flex:1,checked:"json"==c}},{items:{xtype:"radiofield",boxLabel:this.localize("modeText"),
name:"codeMode",inputValue:"text",flex:1,checked:"text"==c}},{items:{xtype:"radiofield",boxLabel:this.localize("modeHtml"),name:"codeMode",inputValue:"html",flex:1,checked:"html"==c}},{items:{xtype:"radiofield",boxLabel:this.localize("modeXml"),name:"codeMode",inputValue:"xml",flex:1,checked:"xml"==c}}]}]}],buttons:[{text:this.localize("ok"),handler:function(){var b=this.up("window");values=b.down("form").getForm().getValues();a.switchModes(values.codeMode);b.close()}},{text:this.localize("cancel"),
...
this.getTargetEl().fireEvent("resize");else this.results.unmask(),this.getTargetEl().fireEvent("resize");return b},_showResult:function(a){this.results.getEl().dom.innerHTML===this.EMPTY_RESULTS_TEXT&&this.results.update(a)},clearResults:function(){this.results.show();var a=this.results.el.down(".x-panel");if(a){var b=Ext.getCmp(a.id);b?b.destroy():a.destroy()}else this.results.update(" ");this.getTargetEl().fireEvent("resize")},tryToUnmask:function(){Spyral&&Spyral.promises&&Ext.defer(this.tryToUnmask,
20,this);if(0==Voyant.application.getDeferredCount()){for(var a in window)"object"==typeof window[a]&&window[a]&&"opener"!=a&&window[a].isFulfilled&&window[a].isFulfilled()&&(window[a]=window[a].valueOf());this.results.unmask();0==this.results.getTargetEl().getHtml().trim().length&&this.results.hide();this.getTargetEl().fireEvent("resize")}else Ext.defer(this.tryToUnmask,20,this)},getContent:function(){var a={input:this.editor.getValue(),mode:this.editor.getMode().split("/").pop()};if("javascript"==
