...
mode:"corpus"})},loadFromCorpusCollocateRecords:function(a,b){if(Ext.isArray(a)){var c=this.getApiParam("limit"),d=this.getNodeDataSet(),e=this.getEdgeDataSet(),f={};d.forEach(function(a){f[a.id]=!0});var g=[],h=[];a.forEach(function(a,l){var m=a.getTerm(),n=a.getContextTerm(),q=a.getKeywordRawFreq(),s=a.getContextTermRawFreq();0==l&&(void 0===b&&(b=m),void 0!==f[b]?d.update({id:b,value:q,title:m+" ("+q+")",type:"keyword",font:{color:this.keywordColor}}):(f[b]=!0,g.push({id:m,label:m,title:m+" ("+
q+")",type:"keyword",value:q,start:c,font:{color:this.keywordColor}})));if(m!=n){void 0===f[n]&&(f[n]=!0,g.push({id:n,label:n,title:n+" ("+s+")",type:"context",value:s,start:0,font:{color:this.contextColor}}));var p=null;e.forEach(function(a){if(a.from==b&&a.to==n||a.from==n&&a.to==b)p=a});m=a.getContextTermRawFreq();null===p?h.push({from:b,to:n,value:m}):p.value<m&&e.update({id:p.id,value:m})}},this);d.add(g);e.add(h);this.forceUpdate();this.getNetwork().fit()}},initGraph:function(){var a=this.getLayout().getRenderTarget();
a.setWidth(a.getWidth());a.setHeight(a.getHeight());this.setGraphHeight(a.getHeight());this.setGraphWidth(a.getWidth());if(void 0===this.getNetwork()){var b={autoResize:!0,interaction:{hover:!0,hoverConnectedEdges:!0,multiselect:!1},physics:{barnesHut:{gravitationalConstant:-1500,centralGravity:6,damping:.5,avoidOverlap:.5}},nodes:this.nodeOptions,edges:this.edgeOptions},a=new vis.Network(a.dom,{nodes:this.getNodeDataSet(),edges:this.getEdgeDataSet()},b);this.setNetwork(a);this.getNodeDataSet().on("remove",
function(a,b,e){var f=b.items[0];a=this.getEdgeDataSet().get({filter:function(a){return a.from==f||a.to==f}});this.getEdgeDataSet().remove(a);var g=[];this.getNodeDataSet().forEach(function(a){0==this.getEdgeDataSet().get({filter:function(b){return b.from==a.id||b.to==a.id}}).length&&g.push(a.id)}.bind(this));this.getNodeDataSet().remove(g)}.bind(this));a.on("dragStart",function(a){a=a.nodes[0];null!=a&&this.getNodeDataSet().update({id:a,fixed:!1})}.bind(this));a.on("dragging",function(a){null!=a.nodes[0]&&
(this.isMasked()?this.isOffCanvas(a.pointer.DOM)||this.unmask():this.isOffCanvas(a.pointer.DOM)&&this.mask(this.localize("releaseToRemove")))}.bind(this));a.on("dragEnd",function(a){var b=a.nodes[0];null!=b&&(this.isOffCanvas(a.pointer.DOM)?(this.unmask(),this.mask("cleaning"),this.getNodeDataSet().remove(b),this.forceUpdate(),this.unmask()):this.getNodeDataSet().update({id:b,fixed:!0}))}.bind(this));a.on("click",function(a){this.getContextMenu().hide()}.bind(this));a.on("doubleClick",function(a){a=
a.nodes[0];null!=a&&(a=this.getNodeDataSet().get(a),this.itemdblclick(a))}.bind(this));a.on("oncontext",function(a){a.event.preventDefault();var b=this.getNetwork().getNodeAt(a.pointer.DOM);if(null!=b){this.getNetwork().selectNodes([b]);var b=this.getNodeDataSet().get(b),e=this.getContextMenu();e.queryById("label").setText(b.label);e.queryById("fixed").setChecked(b.fixed);e.showAt(a.event.pageX,a.event.pageY)}}.bind(this))}},forceUpdate:function(){var a=this.getNodeDataSet().map(function(a){return{id:a.id}});
...
c+d-1);if(e&&Ext.isArray(e)){var f=[];e.forEach(function(a){f.push(a.getIndex())});0<f.length&&this.getCorpus().getDocumentTerms().load({addRecords:!0,params:{docIndex:f,perDocLimit:parseInt(this.getApiParam("limit")),limit:d*parseInt(this.getApiParam("limit")),stopList:this.getApiParam("stopList"),sort:"TFIDF",dir:"DESC"},scope:this,callback:function(e,h,k){var l={};if(k&&e&&Ext.isArray(e)&&(e.forEach(function(a,b,c){b=a.getDocIndex();b in l||(l[b]=[]);l[b].push({id:a.getId(),type:a.getTerm(),val:Ext.util.Format.number(a.get("rawFreq"),
"0,000"),docId:a.get("docId")})}),f.forEach(function(a){var c=this.getCorpus().getDocument(a);len=l[a].length;Ext.dom.Helper.append(b,{tag:"li","voyant:index":String(a),html:'\x3ca href\x3d"#" onclick\x3d"return false" class\x3d"document-id document-id-distinctive" voyant:val\x3d"'+c.get("id")+'"\x3e'+c.getShortTitle()+"\x3c/a\x3e"+this.localize("colon")+(new Ext.XTemplate(this.localize("documentType"))).apply({types:l[a]})+"."})},this),a.updateLayout(),len=d,remaining=this.getCorpus().getDocuments().getTotalCount()-
c-f.length,0<remaining)){e=new Ext.Template(this.localize("moreDistinctiveWords"));var m=Ext.dom.Helper.append(b,{tag:"li",cls:"more",html:e.apply([len>remaining?remaining:len,remaining])},!0);m.on("click",function(){m.remove();this.showMoreDistinctiveWords()},this)}}})}}});Ext.define("Voyant.panel.TopicContexts",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.topiccontexts",statics:{i18n:{title:{en:"Topic Contexts"},helpTip:{en:"Topic Contexts"},reset:{en:"reset"}},api:{},glyph:"xf06e@FontAwesome"},constructor:function(a){Ext.apply(this,{title:this.localize("title"),cls:"twic_body"});this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},listeners:{loadedCorpus:function(a,b){this.loadFromCorpus(b)}},loadFromCorpus:function(a){a=
