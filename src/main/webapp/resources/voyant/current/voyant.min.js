...
{},f=0,h;a.each(function(a){var b=[],c=a.get("distributions"),d=a.get("docId"),e=a.get("docIndex");h=a.get("term");for(a=0;a<c.length;a++){var r=a,p=c[a];p>f&&(f=p);b.push([d,e,r,p,h])}g[e]=b},this);this.highlightKeywords(h);this.down("querysearchfield").setValue(h);a=this.query("cartesian");for(c=0;c<a.length;c++)d=a[c],e=g[c],void 0!==e?(d.getAxes()[0].setMaximum(f),d.getStore().loadData(e)):d.getStore().removeAll()},scope:this}}));this.on("afterrender",function(){var a=this.down('panel[region\x3d"center"]');
this.innerContainer=a.getLayout().getRenderTarget();a.body.on("scroll",function(a,b){var e=this.innerContainer.first(),g=this.getLastScrollTop()<b.scrollTop;if(!g&&1>b.scrollTop){var f=e.first(".word");if(null!==f){var e=Voyant.data.model.Token.getInfoFromElement(f),h=parseInt(e.docIndex),k=parseInt(e.position),m=this.getDocumentsStore().getAt(h),l=this.getApiParam("limit");if(0<h&&0===k){h--;var m=this.getDocumentsStore().getAt(h),n=m.get("tokensCount-lexical"),k=n-l;0>k&&(k=0,this.setApiParam("limit",
n))}else k-=l;0>k&&(k=0);var q=m.getId();this.setApiParams({skipToDocId:q,start:k});this.setScrollDownwards(g);this.load();this.setApiParam("limit",this.INITIAL_LIMIT)}}else if(g&&b.scrollTop+b.offsetHeight>b.scrollHeight/2){var f=e.last(),r=$(e.dom).contents().filter(function(){return 3===this.nodeType}).last();if(!1===f.hasCls("loading"))for(;f;){if(f.hasCls("word")){e=Voyant.data.model.Token.getInfoFromElement(f);h=parseInt(e.docIndex);k=parseInt(e.position);m=this.getDocumentsStore().getAt(e.docIndex);
q=m.getId();n=m.get("tokensCount-lexical");if(k+this.getApiParam("limit")>=n&&h==this.getCorpus().getDocumentsCount()-1)if(l=n-k,1>=l)break;else this.setApiParam("limit",l);f.el.dom.nextSibling===r[0]&&r.remove();f.insertSibling("\x3cdiv class\x3d'loading'\x3e"+this.localize("loading")+"\x3c/div\x3e","after",!1).mask();f.destroy();this.setApiParams({skipToDocId:q,start:e.position});this.setScrollDownwards(g);this.load();this.setApiParam("limit",this.INITIAL_LIMIT);break}f.destroy();f=e.last()}}this.setLastScrollTop(b.scrollTop)},
this);a.body.on("click",function(a,b){b=Ext.get(b);if(b.hasCls("word")){var e=Voyant.data.model.Token.getInfoFromElement(b),e=[{term:b.getHtml(),docIndex:e.docIndex}];this.getApplication().dispatchEvent("termsClicked",this,e)}},this);this.getCorpus()&&this.load()},this);Ext.apply(this,{dockedItems:[{dock:"bottom",xtype:"toolbar",items:[{xtype:"querysearchfield"}]}],listeners:{loadedCorpus:function(a,c){this.setCorpus(c);this.getTokensStore().setCorpus(c);this.getDocumentTermsStore().getProxy().setExtraParam("corpus",
...
{fields:"term x y z rawFreq relativeFreq cluster isDoc docIndex".split(" "),data:r});f=Ext.create("Ext.data.JsonStore",{fields:"term x y z rawFreq relativeFreq cluster isDoc docIndex".split(" "),data:p});a={itemId:"chart",xtype:"cartesian",interactions:["crosszoom","panzoom","itemhighlight"],plugins:{ptype:"chartitemevents"},axes:[{type:"numeric",position:"bottom",fields:["x"]},{type:"numeric",position:"left",fields:["y"]}],sprites:[{type:"text",text:a,hidden:0<this.labelsMode,x:70,y:70}],innerPadding:{top:25,
