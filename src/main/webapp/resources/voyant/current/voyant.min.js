...
setCorpus:function(a){a&&this.getProxy().setExtraParam("corpus",Ext.isString(a)?a:a.getId());this.callParent(arguments)}});Ext.define("Voyant.data.store.Contexts",{extend:"Ext.data.BufferedStore",model:"Voyant.data.model.Context",config:{corpus:void 0},constructor:function(a){a=a||{};Ext.apply(a,{pagePurgeCount:0,pageSize:100,leadingBufferZone:100,remoteSort:!0,proxy:{type:"ajax",url:Voyant.application.getTromboneUrl(),extraParams:{tool:"corpus.DocumentContexts",corpus:a&&a.corpus?Ext.isString(a.corpus)?a.corpus:a.corpus.getId():void 0,stripTags:a.stripTags},reader:{type:"json",rootProperty:"documentContexts.contexts",
...
termTip:{en:"The keyword for the context."},left:{en:"Left"},leftTip:{en:"Context to the left of the keyword."},right:{en:"Right"},rightTip:{en:"Context to the right of the keyword."},context:{en:"context"},expand:{en:"expand"},corpus:{en:"corpus"},corpusTip:{en:"Reset to corpus mode (contexts from all documents)."}},api:{query:void 0,docId:void 0,docIndex:void 0,stopList:"auto",context:5,expand:50},glyph:"xf0ce@FontAwesome"},constructor:function(){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,
arguments)},initComponent:function(){var a=this;Ext.apply(a,{title:this.localize("title"),emptyText:this.localize("emptyText"),store:Ext.create("Voyant.data.store.Contexts",{stripTags:"all",remoteSort:!1,sortOnLoad:!0,sorters:{property:"position",direction:"ASC"}}),selModel:Ext.create("Ext.selection.RowModel",{listeners:{selectionchange:{fn:function(a,c){this.getApplication().dispatchEvent("termLocationClicked",this,c)},scope:this}}}),plugins:[{ptype:"rowexpander",rowBodyTpl:new Ext.XTemplate("")}],
dockedItems:[{dock:"bottom",xtype:"toolbar",items:[{xtype:"querysearchfield"},{xtype:"totalpropertystatus"},this.localize("context"),{xtype:"slider",minValue:5,value:5,maxValue:50,increment:5,width:50,listeners:{render:function(b){b.setValue(a.getApiParam("context"))},changecomplete:function(b,c){a.setApiParam("context",b.getValue());a.getStore().loadPage(1,{params:a.getApiParams()})}}},this.localize("expand"),{xtype:"slider",minValue:5,value:5,maxValue:500,increment:10,width:50,listeners:{render:function(b){b.setValue(a.getApiParam("expand"))},
changecomplete:function(b,c){a.setApiParam("expand",c);var d=a.getView(),e=a.plugins[0].recordsExpanded,g=d.getStore();for(id in e)if(e[id]){var f=g.getByInternalId(id),h=d.getRow(f);d.fireEvent("expandbody",h,f,h.parentNode.childNodes[1],{force:!0})}}}},{text:this.localize("corpus"),tooltip:this.localize("corpusTip"),itemId:"corpus",handler:function(a){a.hide();this.setApiParams({docIndex:void 0,docId:void 0});this.getStore().load({params:this.getApiParams()})},hidden:!0,scope:this}]}],columns:[{text:this.localize("document"),
toolTip:this.localize("documentTip"),width:"autoSize",dataIndex:"docIndex",sortable:!1,renderer:function(a,c,d,e,g,f){return f.getCorpus().getDocument(a).getTinyLabel()}},{text:this.localize("left"),tooltip:this.localize("leftTip"),align:"right",dataIndex:"left",sortable:!0,flex:1},{text:this.localize("term"),tooltip:this.localize("termTip"),dataIndex:"term",sortable:!0,width:"autoSize"},{text:this.localize("right"),tooltip:this.localize("rightTip"),dataIndex:"right",sortable:!0,flex:1}],listeners:{documentSegmentTermClicked:{fn:function(a,
...
b&&this.fireEvent("loadedCorpus",this,a.embedded.getCorpus());"Voyant.data.model.Corpus"==b&&this.fireEvent("loadedCorpus",this,a.embedded)}else a.corpus&&this.fireEvent("loadedCorpus",this,a.corpus)},initComponent:function(){var a=Ext.create("Voyant.data.store.CorpusTerms");a.getProxy().setExtraParam("withDistributions","relative");Ext.apply(this,{title:this.localize("title"),emptyText:this.localize("emptyText"),store:a,selModel:Ext.create("Ext.selection.CheckboxModel",{pruneRemoved:!1,listeners:{selectionchange:{fn:function(a,
c){c&&0<c.length&&this.getApplication().dispatchEvent("corpusTermsClicked",this,c)},scope:this}},mode:"SIMPLE"}),dockedItems:[{dock:"bottom",xtype:"toolbar",items:[{xtype:"querysearchfield"},{xtype:"totalpropertystatus"}]}],columns:[{xtype:"rownumberer",width:"autoSize",sortable:!1},{text:this.localize("term"),tooltip:this.localize("termTip"),dataIndex:"term",flex:1,sortable:!0},{text:this.localize("rawFreq"),tooltip:this.localize("rawFreqTip"),dataIndex:"rawFreq",width:"autoSize",sortable:!0},{text:this.localize("relativeFreq"),
...
this.setApiParams({docId:e,query:void 0});this.isVisible()&&this.getStore().loadPage(1,{params:this.getApiParams()})});this.on("activate",function(){this.getStore().getCorpus()&&this.getStore().loadPage(1,{params:this.getApiParams()})},this)},initComponent:function(){var a=Ext.create("Voyant.data.store.DocumentTerms");Ext.apply(this,{title:this.localize("title"),emptyText:this.localize("emptyText"),store:a,selModel:Ext.create("Ext.selection.CheckboxModel",{listeners:{selectionchange:{fn:function(a,
...
for(a=0;a<c.length;a++){var s=a,p=c[a];p>f&&(f=p);b.push([d,e,s,p,h])}g[e]=b},this);this.highlightKeywords(h);this.down("querysearchfield").setValue(h);a=this.query("cartesian");for(c=0;c<a.length;c++)d=a[c],e=g[c],void 0!==e?(d.getAxes()[0].setMaximum(f),d.getStore().loadData(e)):d.getStore().removeAll()},scope:this}}));this.on("afterrender",function(){var a=this.down('panel[region\x3d"center"]');this.innerContainer=a.getLayout().getRenderTarget();a.body.on("scroll",function(a,b){var e=this.items.getAt(0).body.dom;
if(e.scrollTop+e.offsetHeight>e.scrollHeight/2){var g=this.innerContainer.first(),e=g.last();if(0==e.hasCls("loading"))for(;e;){if(e.hasCls("word")){e.insertSibling("\x3cdiv class\x3d'loading'\x3e"+this.localize("loading")+"\x3c/div\x3e","after",!1).mask();g=Voyant.data.model.Token.getInfoFromElement(e);e.destroy();e=this.getDocumentsStore().getAt(g.docIndex).getId();this.setApiParams({skipToDocId:e,start:g.position});this.load();break}e.destroy();e=g.last()}}},this);a.body.on("click",function(a,
b){b=Ext.get(b);if(b.hasCls("word")){var e=Voyant.data.model.Token.getInfoFromElement(b),e=[{term:b.getHtml(),docIndex:e.docIndex}];this.getApplication().dispatchEvent("termsClicked",this,e)}},this);this.getCorpus()&&this.load()},this);Ext.apply(this,{dockedItems:[{dock:"bottom",xtype:"toolbar",items:[{xtype:"querysearchfield"}]}],listeners:{loadedCorpus:function(a,c){this.setCorpus(c);this.getTokensStore().setCorpus(c);this.getDocumentTermsStore().getProxy().setExtraParam("corpus",c.getId());var d=
c.getDocuments();this.setDocumentsStore(d);d=this.down('panel[region\x3d"south"]');this.generateChart(c,d);this.rendered&&this.load()},termsClicked:function(a,c){var d=[];c.forEach(function(a){a.term&&d.push(a.term)});this.loadQueryTerms(d)},corpusTermsClicked:function(a,c){var d=[];c.forEach(function(a){a.getTerm()&&d.push(a.getTerm())});this.loadQueryTerms(d)},documentTermsClicked:function(a,c){var d=[];c.forEach(function(a){a.getTerm()&&d.push(a.getTerm())});this.loadQueryTerms(d)},documentsClicked:function(a,
c,d){c&&(this.setApiParams({skipToDocId:c[0].getId(),start:0}),this.load(!0))},termLocationClicked:function(a,c){if(void 0!==c[0]){var d=c[0],e=d.get("docIndex"),g=d.get("position"),f=g-this.getApiParam("limit")/2,h=this.getCorpus().getDocument(e);this.setApiParams({skipToDocId:h.getId(),start:0>f?0:f});this.load(!0,{callback:function(){var a=this.body.dom.querySelector("#_"+e+"_"+g);a&&a.scrollIntoView();this.highlightKeywords(d.get("term"),!1)},scope:this})}},scope:this}});this.callParent(arguments)},
loadQueryTerms:function(a){a&&0<a.length&&this.getDocumentTermsStore().load({params:{query:a,docIndex:void 0,docId:void 0,page:void 0,start:void 0,limit:void 0}})},generateChart:function(a,b){function c(a,b){var c=[[116,116,181],[139,163,83],[189,157,60],[171,75,75],[174,61,155]],c=c[a%c.length];return"rgba("+c[0]+","+c[1]+","+c[2]+","+b+")"}function d(d,e){var f=d.index,g=d.fraction,h=d.relativeHeight,k=c(f,.3),f=c(f,1);b.add({xtype:"cartesian",flex:g,height:"100%",insetPadding:0,background:{type:"linear",
degrees:90,stops:[{offset:0,color:k},{offset:h,color:k},{offset:h,color:"white"},{offset:1,color:"white"}]},axes:[{type:"numeric",position:"left",fields:"distribution",hidden:!0},{type:"category",position:"bottom",fields:"bin",hidden:!0}],series:[{type:"line",xField:"bin",yField:"distribution",style:{lineWidth:2,strokeStyle:f},tooltip:{corpus:a,trackMouse:!0,style:"background: #fff",showDelay:0,dismissDelay:500,hideDelay:5,renderer:function(b,c){this.setHtml(a.getDocument(b.get("docIndex")).getTitle()+
"\x3cbr\x3e"+b.get("term")+": "+b.get("distribution"))}}}],store:Ext.create("Ext.data.ArrayStore",{fields:["docId","docIndex","bin","distribution","term"],data:[]})}).body.on("click",function(a,b){var c=Ext.get(b);a.getX();c.getBox();var c=c.parent(".x-panel"),d=c.parent(),c=Ext.toArray(d.dom.children).indexOf(c.dom),c=this.getDocumentsStore().getAt(c);this.getApplication().dispatchEvent("documentsClicked",this,[c])},e)}b.removeAll();var e=a.getDocuments();if(50>e.getTotalCount()){for(var g=a.getWordTokensCount(),
f=[],h=1E6,k=-1,m=0;m<e.getCount();m++){var l=e.getAt(m),n=l.get("index"),l=l.get("tokensCount-lexical");l<h&&(h=l);l>k&&(k=l);f.push({index:n,count:l,fraction:l/g})}for(m=0;m<f.length;m++)l=f[m],l.relativeHeight=l.count==k?1:0+(l.count-h)/(k-h)*1,d(l,this)}},highlightKeywords:function(a,b){Ext.isArray(a)||(a=[a]);this.innerContainer.first().select("span[class*\x3dkeyword]").removeCls("keyword");var c=[],d=new RegExp("^"+a[0]+"$","i");this.innerContainer.first().select("span.word").each(function(a,
b,f){a.dom.firstChild&&a.dom.firstChild.nodeValue.match(d)&&(a.addCls("keyword"),c.push(a.dom))})},load:function(a,b){a&&(this.innerContainer.first().destroy(),this.innerContainer.setHtml('\x3cdiv class\x3d"readerContainer"\x3e\x3cdiv class\x3d"loading"\x3e'+this.localize("loading")+"\x3c/div\x3e\x3c/div\x3e"),this.innerContainer.first().first().mask());this.getTokensStore().load(Ext.apply(b||{},{params:Ext.apply(this.getApiParams(),{stripTags:"blocksOnly"})}))},updateText:function(a){var b=this.innerContainer.first().last();
b&&b.isMasked()&&b.destroy();this.innerContainer.first().insertHtml("beforeEnd",a)},updateChart:function(){}});Ext.define("Voyant.panel.ScatterPlot",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],requires:["Ext.chart.CartesianChart"],alias:"widget.scatterplot",config:{corpus:void 0,options:{xtype:"stoplistoption"}},statics:{i18n:{title:{en:"ScatterPlot"},analysis:{en:"Analysis"},ca:{en:"Correspondence Analysis"},pca:{en:"Principal Components Analysis"},rawFreq:{en:"Raw Frequency"},relFreq:{en:"Relative Frequency"},terms:{en:"Terms"},term:{en:"Term"},numTerms:{en:"Terms Count"},addTerm:{en:"Add Term"},
