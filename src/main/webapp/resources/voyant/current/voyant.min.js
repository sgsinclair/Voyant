...
this.getCorpus().getDocumentsCount()]));this.exportBtn.setDisabled(0==a)}});Ext.define("Voyant.panel.Dummy",{extend:"Ext.Panel",xtype:"dummy",autoScroll:!0,initComponent:function(){Ext.apply(this,{layout:{type:"table",columns:2,tableAttrs:{style:{width:"100%",height:"100%"}},tdAttrs:{style:{padding:"0px",verticalAlign:"top",width:"10",height:"10"}}},defaults:{width:10,height:10,border:!0},items:[{xtype:"summary",colspan:2},{xtype:"documents"},{xtype:"cirrus"}]});this.on("boxready",function(){for(var a=this.getTargetEl().dom.querySelector(".x-table-layout").querySelectorAll("tr"),
b=this.getTargetEl().getWidth(),c=this.getTargetEl().getHeight(),d=0;d<a.length;d++){Ext.get(a[d]).setHeight(c/a.length);var e=a[d].querySelectorAll("td");for(j=0;j<e.length;j++){var f=Ext.getCmp(e[j].children[0].id);f.setWidth(b/e.length);f.setHeight(c/a.length);f.updateLayout()}}});this.callParent()}});Ext.define("Voyant.panel.RezoViz",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.rezoviz",statics:{i18n:{},api:{query:void 0,limit:25,stopList:"auto",type:["organization","location","person"],minEdgeCount:2},glyph:"xf1cb@FontAwesome"},config:{corpus:void 0,network:void 0,nodesStore:void 0,nodesDataSet:void 0,edgesDataSet:void 0,isNetworkBounded:!0},categorizedNodeOptions:{location:{font:{color:"green"}},person:{font:{color:"maroon"}},organization:{font:{color:"purple"}}},nodeOptions:{shape:"box",
