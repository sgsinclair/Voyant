...
this.getCorpus().getDocumentsCount()]));this.exportBtn.setDisabled(0==a)}});Ext.define("Voyant.panel.Dummy",{extend:"Ext.Panel",xtype:"dummy",autoScroll:!0,initComponent:function(){Ext.apply(this,{layout:{type:"table",columns:2,tableAttrs:{style:{width:"100%",height:"100%"}},tdAttrs:{style:{padding:"0px",verticalAlign:"top",width:"100%",height:"100%"}}},defaults:{width:500,height:500},items:[{xtype:"summary"},{xtype:"documents"},{xtype:"cirrus"},{xtype:"reader"}]});this.on("boxready",function(){rows=this.getTargetEl().dom.querySelector(".x-table-layout").querySelectorAll("tr");
rowHeight=this.getTargetEl().getHeight()/rows.length;for(var a=0;a<rows.length;a++){Ext.get(rows[a]).setHeight(rowHeight);var b=rows[a].querySelectorAll("td"),c=rowHeight;width=rows[a].clientWidth;cellWidth=width/b.length;for(j=0;j<b.length;j++){var d=Ext.getCmp(b[j].children[0].id);d.setWidth(cellWidth*parseInt(b[0].getAttribute("colspan")));d.setHeight(c);d.updateLayout()}}});this.callParent()}});Ext.define("Voyant.panel.RezoViz",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.rezoviz",statics:{i18n:{},api:{query:void 0,limit:25,stopList:"auto",type:["organization","location","person"],minEdgeCount:2},glyph:"xf1cb@FontAwesome"},config:{corpus:void 0,network:void 0,nodesStore:void 0,nodesDataSet:void 0,edgesDataSet:void 0,isNetworkBounded:!0},categorizedNodeOptions:{location:{font:{color:"green"}},person:{font:{color:"maroon"}},organization:{font:{color:"purple"}}},nodeOptions:{shape:"box",
