...
codeModeTitle:"Code Mode",codeModeTip:"Select from multiple code formats for this cell.",configureTip:"Configuration Options",autoExecuteOnLoad:"auto-run this cell on page load",ok:"OK",cancel:"Cancel"}},config:{isRun:!1,autoExecute:!1},layout:{type:"vbox",align:"stretch"},height:130,border:!1,EMPTY_RESULTS_TEXT:" ",constructor:function(a){this.results=Ext.create("Ext.Component",{align:"stretch",cls:"notebook-code-results",html:Ext.Array.from(a.output).join("")});this.editor=Ext.create("Voyant.notebook.editor.CodeEditor",
...
notebookWrapperRemove:this.notebookWrapperRemove,notebookWrapperAdd:this.notebookWrapperAdd,notebookLoaded:this.autoExecuteCells,scope:this}});this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments);this.voyantStorageDialogs=new Voyant.notebook.StorageDialogs({listeners:{fileLoaded:function(a){},fileSaved:function(a,c){this.unmask();null!==c&&((a=this.getNotebookId())&&c==a||this.setNotebookId(c),this.toastInfo({html:this.localize("saved"),anchor:"tr"}),this.setIsEdited(!1))},
saveCancelled:function(){},close:function(){this.unmask()},scope:this}});this.githubDialogs=new Voyant.notebook.github.GitHubDialogs({listeners:{fileLoaded:function(a,c){a=c.owner;var b=c.repo,e=c.path;c=c.file;this.githubDialogs.close();this.clear();this.loadFromString(c);c=encodeURIComponent(a+"/"+b+"/"+e);-1===location.search.indexOf(c)&&(a=this.getBaseUrl()+"spyral/?githubId\x3d"+c,window.history.pushState({url:a},"Spyral Notebook: "+c,a))},fileSaved:function(a,c){a=c.owner;var b=c.repo;c=c.path;
this.githubDialogs.close();this.unmask();this.toastInfo({html:this.localize("saved"),anchor:"tr"});this.setIsEdited(!1);c=encodeURIComponent(a+"/"+b+"/"+c);-1===location.search.indexOf(c)&&(a=this.getBaseUrl()+"spyral/?githubId\x3d"+c,window.history.pushState({url:a},"Spyral Notebook: "+c,a))},saveCancelled:function(a){this.unmask()},scope:this}})},init:function(){window.Corpus=Spyral.Corpus;window.Table=Spyral.Table;window.loadCorpus=function(){return Spyral.Corpus.load.apply(Spyral.Corpus.load,
...
loadFromId:function(a){this.mask(this.localize("loading"));var b=this;Spyral.Load.trombone({tool:"notebook.NotebookManager",action:"load",id:a,noCache:1}).then(function(a){b.unmask();b.loadFromString(a.notebook.data);a.notebook.id&&a.notebook.id!=b.getNotebookId()&&b.setNotebookId(a.notebook.id);b.setIsEdited(!1)}).catch(function(a){b.unmask()})},loadFromHtmlString:function(a){var b=(new DOMParser).parseFromString(a,"text/html");this.setMetadata(new Spyral.Metadata(b));b.querySelectorAll("section.notebook-editor-wrapper").forEach(function(b){var c=
b.classList;if(c.contains("notebooktexteditorwrapper"))c=b.querySelector(".notebook-text-editor").innerHTML,this.addText(c,void 0,b.id);else if(c.contains("notebookcodeeditorwrapper")){var e=b.querySelector(".notebook-code-editor-raw");c=/\beditor-mode-(\w+)\b/.exec(e.className)[1];var f=a.indexOf("\x3csection id\x3d'y5ebznmjvht8r7kj' class\x3d'notebook-editor-wrapper notebookcodeeditorwrapper'\x3e");f=a.indexOf("\x3cpre class\x3d'notebook-code-editor-raw editor-mode-",f);f+=62;var g=a.indexOf("\x3c/pre\x3e\n\x3cdiv class\x3d'notebook-code-results'\x3e",
f);this.showError(this.localize("Can't parse the input correctly."));input=a.substring(f,g);e=null!==/\bautoexec\b/.exec(e.className);f=b.querySelector(".notebook-code-results").innerHTML;this.addCode({input:input,output:f,mode:c,autoExecute:e},void 0,b.id)}},this);this.fireEvent("notebookLoaded")},runUntil:function(a){var b=[];Ext.Array.each(this.query("notebookcodeeditorwrapper"),function(c){b.push(c);c.clearResults();if(a&&a==c)return!1},this);this._run(b)},runFrom:function(a){var b=[],c=!1;Ext.Array.each(this.query("notebookcodeeditorwrapper"),
function(d){a&&a==d&&(c=!0);c&&(b.push(d),d.clearResults())},this);this._run(b)},runAll:function(){var a=[];Ext.Array.each(this.query("notebookcodeeditorwrapper"),function(b){a.push(b);b.clearResults()},this);this._run(a)},_run:function(a){if(0<a.length){var b=a.shift().run(!0);if(void 0!==b&&b.then&&b.catch&&b.finally){var c=this;b.then(function(){Ext.defer(c._run,100,c,[a])})}else Ext.defer(this._run,100,this,[a])}},autoExecuteCells:function(){var a=[];Ext.Array.each(this.query("notebookcodeeditorwrapper"),
function(b){b.getAutoExecute()&&a.push(b)});this._run(a)},loadFromUrl:function(a,b){var c=this;Spyral.Load.text(a).then(function(a){c.loadFromString(a)})},addText:function(a,b,c){return this._add(a,b,"notebooktexteditorwrapper",c)},addCode:function(a,b,c){return this._add(a,b,"notebookcodeeditorwrapper",c,{docs:this.spyralTernDocs})},_add:function(a,b,c,d,e){Ext.isString(a)&&(a={input:a});var f=this.getComponent("cells");b="undefined"===typeof b?f.items.length:b;d="undefined"===typeof d?Spyral.Util.id():
d;return f.insert(b,Ext.apply(a,{xtype:c,order:b,cellId:d},e))},updateMetadata:function(){this.getMetadata();this.getComponent("spyralHeader").update(this.getInnerHeaderHtml());this.getComponent("spyralFooter").update(this.getInnerFooterHtml())},notebookWrapperMoveUp:function(a){var b=this.getComponent("cells");a=b.items.findIndex("id",a.id);0==a?Ext.Msg.show({title:this.localize("error"),msg:this.localize("cannotMoveHigher"),buttons:Ext.MessageBox.OK,icon:Ext.MessageBox.WARNING}):(b.move(a,a-1),
this.redoOrder())},notebookWrapperMoveDown:function(a){var b=this.getComponent("cells");a=b.items.findIndex("id",a.id);a==b.items.getCount()-1?Ext.Msg.show({title:this.localize("error"),msg:this.localize("cannotMoveLower"),buttons:Ext.MessageBox.OK,icon:Ext.MessageBox.WARNING}):(b.move(a,a+1),this.redoOrder())},notebookWrapperRemove:function(a){var b=this.getComponent("cells");b.remove(a);0==b.items.length&&this.addText("(Click to edit).");this.redoOrder()},notebookWrapperAdd:function(a,b){var c=
this.getComponent("cells").items.findIndex("id",a.id);a=a.getXType(a);("notebooktexteditorwrapper"==a&&!b.hasModifier()||"notebookcodeeditorwrapper"==a&&b.hasModifier()?this.addCode("",c+1):this.addText("",c+1)).getTargetEl().scrollIntoView(this.getTargetEl(),null,!0,!0);this.redoOrder()},redoOrder:function(){this.query("notebookwrappercounter").forEach(function(a,b){a.setOrder(b)});this.setIsEdited(!0)},setIsEdited:function(a){this.getHeader()&&(this.getHeader().down("#saveItTool").setDisabled(0==
a),a||(this.query("notebookcodeeditor").forEach(function(a){a.setIsChangeRegistered(!1)}),this.query("notebooktexteditor").forEach(function(a){a.setIsEditRegistered(!1)})));this.callParent(arguments)},setNotebookId:function(a){if(a&&-1===location.pathname.indexOf("/spyral/"+a)){var b=this.getBaseUrl()+"spyral/"+a+"/";window.history.pushState({url:b},"Spyral Notebook: "+a,b)}this.callParent(arguments)},generateExportHtml:function(){var a="\x3c!DOCTYPE HTML\x3e\n\x3chtml\x3e\n\x3chead\x3e\n\t\x3cmeta charset\x3d'UTF-8'\x3e\n"+
