...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];l=0;f=function(){var a,f,h,k,r=this.wordsToArrange-1,t=g.canvas.width,n=g.canvas.height,q=.5*t,s=.5*n;do{h=p[l];if(void 0!==h){for(var v=Math.random()*Math.PI,w=.25*Math.random()*h.size,A=.5*(Math.random()-.5),y=.5*h.width,z=.5*h.height;;){a=Math.floor((q+Math.cos(v)*w*this.ratio-y)/this.COARSENESS)*this.COARSENESS;f=Math.floor((s+Math.sin(v)*w-z)/this.COARSENESS)*this.COARSENESS;
k=a+y>=t||f+z>=n?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;v+=A;w+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<l;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}l++;if(l>=p.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(r--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;m=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
buttons:Ext.Msg.OK})},getTromboneUrl:function(){return this.getApplication().getTromboneUrl()},dispatchEvent:function(){var a=this.getApplication();a.dispatchEvent.apply(a,arguments)},showError:function(a){this.getApplication().showError.apply(this,arguments)},toastError:function(a){Ext.isString(a)&&(a={html:a});Ext.applyIf(a,{glyph:"xf071@FontAwesome",title:this.localize("error")});this.toast(a)},toastInfo:function(a){Ext.isString(a)&&(a={html:a});Ext.applyIf(a,{glyph:"xf05a@FontAwesome",title:this.localize("info")});
...
documentFrequency:{en:"document frequency:"},limitedAccess:{en:"This is a limited access corpus and this tool's functionality is restricted."}},api:{start:0,limit:1E3,skipToDocId:void 0},glyph:"xf0f6@FontAwesome"},config:{corpus:void 0,tokensStore:void 0,documentsStore:void 0,documentTermsStore:void 0,exportVisualization:!1,lastScrollTop:0,scrollIntoView:!1,insertWhere:"beforeEnd",locationMarker:void 0,lastLocationUpdate:new Date},INITIAL_LIMIT:1E3,innerContainer:null,cls:"voyant-reader",layout:"fit",
items:{layout:"border",items:[{bodyPadding:10,region:"center",border:!1,autoScroll:!0,html:'\x3cdiv class\x3d"readerContainer"\x3e\x3c/div\x3e'},{region:"south",height:40,split:{size:2},splitterResize:!0,border:!1,layout:{type:"hbox"}}]},constructor:function(){Ext.apply(this,{title:this.localize("title")});this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(){var a=Ext.create("Voyant.data.store.Tokens");a.on("beforeload",function(a){return"NONCONSUMPTIVE"!=
a.getCorpus().getNoPasswordAccess()});a.on("load",function(a,c,d){if(d){var e="",f=this.localize("documentFrequency"),g=!1,h=-1,k=!1;c.forEach(function(a){0==a.getPosition()&&(e+="\x3ch3\x3e"+this.getDocumentsStore().getById(a.getDocId()).getFullLabel()+"\x3c/h3\x3e");a.getDocIndex()!=h&&(g=this.getDocumentsStore().getById(a.getDocId()).isPlainText(),h=a.getDocIndex());if(a.isWord())k=!1,e+="\x3cspan class\x3d'word' id\x3d'"+a.getId()+"' data-qtip\x3d'"+f+" "+a.getDocumentRawFreq()+"'\x3e"+a.getTerm()+
"\x3c/span\x3e";else{a=a.getTermWithLineSpacing(g);var b=0==a.indexOf("\x3cbr /\x3e");if(!k||!b&&0!=a.trim().length)e+=a,k=b}},this);this.updateText(e,!0);a=this.down("querysearchfield").getValue();""!=a&&this.highlightKeywords(a)}},this);this.setTokensStore(a);this.on("query",function(a,c){this.loadQueryTerms([c])},this);this.setDocumentTermsStore(Ext.create("Ext.data.Store",{model:"Voyant.data.model.DocumentTerm",autoLoad:!1,remoteSort:!1,proxy:{type:"ajax",url:Voyant.application.getTromboneUrl(),
...
this.generateChart(c,d);this.rendered&&(this.load(),"NONCONSUMPTIVE"==c.getNoPasswordAccess()&&this.mask(this.localize("limitedAccess"),"mask-no-spinner"))},termsClicked:function(a,c){var d=[];c.forEach(function(a){Ext.isString(a)?d.push(a):a.term?d.push(a.term):a.getTerm&&d.push(a.getTerm())});0<d.length&&this.loadQueryTerms(d)},corpusTermsClicked:function(a,c){var d=[];c.forEach(function(a){a.getTerm()&&d.push(a.getTerm())});this.loadQueryTerms(d)},documentTermsClicked:function(a,c){var d=[];c.forEach(function(a){a.getTerm()&&
d.push(a.getTerm())});this.loadQueryTerms(d)},documentsClicked:function(a,c,d){0<c.length&&(this.setApiParams({skipToDocId:c[0].getId(),start:0}),this.load(!0))},termLocationClicked:function(a,c){if(void 0!==c[0]){var d=c[0],e=d.get("docIndex"),f=d.get("position"),g=f-this.getApiParam("limit")/2,h=this.getCorpus().getDocument(e);this.setApiParams({skipToDocId:h.getId(),start:0>g?0:g});this.load(!0,{callback:function(){var a=this.body.dom.querySelector("#_"+e+"_"+f);a&&a.scrollIntoView();this.highlightKeywords(d.get("term"),
!1)},scope:this})}},documentIndexTermsClicked:function(a,c){if(void 0!==c[0]){var d=Ext.create("Voyant.data.model.Token",c[0]);this.fireEvent("termLocationClicked",this,[d])}},scope:this}});this.callParent(arguments)},loadQueryTerms:function(a){a&&0<a.length&&this.getDocumentTermsStore().load({params:{query:a,docIndex:void 0,docId:void 0,page:void 0,start:void 0,limit:void 0}})},updateLocationMarker:function(a){var b;b=0==a.scrollTop?0:a.scrollHeight-a.scrollTop==a.clientHeight?1:(a.scrollTop+.5*
a.clientHeight)/a.scrollHeight;a=$(a).find(".readerContainer").find(".word");var c=a.first()[0],d=a.last()[0];if(void 0!==c&&void 0!==d){a=this.getCorpus();var e=!1,c=Voyant.data.model.Token.getInfoFromElement(Ext.get(c)),d=Voyant.data.model.Token.getInfoFromElement(Ext.get(d));0!==c.position&&(e=!0);for(var f={},g=0,h=c.docIndex;h<=d.docIndex;){var k=a.getDocument(h).get("tokensCount-lexical");h===d.docIndex&&(k=d.position);h===c.docIndex&&(k-=c.position);g+=k;f[h]=k;h++}g=Math.round(g*b);h=b=0;
for(k=c.docIndex;k<=d.docIndex&&!(b=k,h+=f[k],h>=g);k++);d=f[b]-(h-g);e&&b===c.docIndex&&(d+=c.position);a=d/a.getDocument(b).get("tokensCount-lexical");e=this.query("cartesian")[b];a=e.getX()+e.getWidth()*a;Ext.get(this.getLocationMarker()).setX(a)}this.setLastLocationUpdate(new Date)},generateChart:function(a,b){function c(a,b){var c=[[116,116,181],[139,163,83],[189,157,60],[171,75,75],[174,61,155]],c=c[a%c.length];return"rgba("+c[0]+","+c[1]+","+c[2]+","+b+")"}function d(d,e){var f=d.index,g=d.fraction,
h=d.relativeHeight,k=c(f,.3),f=c(f,1);b.add({xtype:"cartesian",plugins:{ptype:"chartitemevents"},flex:g,height:"100%",insetPadding:0,background:{type:"linear",degrees:90,stops:[{offset:0,color:k},{offset:h,color:k},{offset:h,color:"white"},{offset:1,color:"white"}]},axes:[{type:"numeric",position:"left",fields:"distribution",hidden:!0},{type:"category",position:"bottom",fields:"bin",hidden:!0}],series:[{type:"line",xField:"bin",yField:"distribution",style:{lineWidth:2,strokeStyle:f},tooltip:{corpus:a,
trackMouse:!0,style:"background: #fff",showDelay:0,dismissDelay:500,hideDelay:5,renderer:function(b,c){this.setHtml(a.getDocument(b.get("docIndex")).getTitle()+"\x3cbr\x3e"+b.get("term")+": "+b.get("distribution"))}}}],store:Ext.create("Ext.data.ArrayStore",{fields:["docId","docIndex","bin","distribution","term"],data:[]}),listeners:{itemclick:function(a,b,c){a=b.record.data;a=this.getDocumentsStore().getAt(a.docIndex);this.getApplication().dispatchEvent("documentsClicked",this,[a])},scope:e}}).body.on("click",
function(a,b){var c=Ext.get(b),d=a.getX(),e=c.getBox(),d=(d-e.x)/e.width,c=c.parent(".x-panel"),e=c.parent(),c=Ext.toArray(e.dom.children).indexOf(c.dom),c=this.getDocumentsStore().getAt(c),e=c.get("tokensCount-lexical"),d=Math.floor(e*d)-this.getApiParam("limit")/2;this.setApiParams({skipToDocId:c.getId(),start:0>d?0:d});this.load(!0)},e)}b.removeAll();var e=a.getDocuments();if(50>e.getTotalCount()){for(var f=a.getWordTokensCount(),g=[],h=Number.MAX_VALUE,k=-1,l=0;l<e.getCount();l++){var m=e.getAt(l),
n=m.get("index"),m=m.get("tokensCount-lexical");m<h&&(h=m);m>k&&(k=m);g.push({index:n,count:m,fraction:m/f})}for(l=0;l<g.length;l++)m=g[l],m.relativeHeight=m.count==k?1:.25+(m.count-h)/(k-h)*.75,d(m,this)}},highlightKeywords:function(a,b){Ext.isArray(a)||(a=[a]);this.innerContainer.first().select("span[class*\x3dkeyword]").removeCls("keyword");var c=[],d=new RegExp("^"+a[0]+"$","i");this.innerContainer.first().select("span.word").each(function(a,b,g){a.dom.firstChild&&a.dom.firstChild.nodeValue.match(d)&&
(a.addCls("keyword"),c.push(a.dom))})},fetchPrevious:function(a){var b=this.innerContainer.first(),c=b.first(".word");if(null!=c&&!1===c.hasCls("loading"))for(;c;){if(c.hasCls("word")){var b=Voyant.data.model.Token.getInfoFromElement(c),d=b.docIndex,b=b.position,e=this.getDocumentsStore().getAt(d),f=this.getApiParam("limit"),g=!1;if(0===d&&0===b){c.dom.scrollIntoView();c.frame("red");break}0<d&&0===b?(g=!0,d--,e=this.getDocumentsStore().getAt(d),d=e.get("tokensCount-lexical"),b=d-f,0>b&&(b=0,this.setApiParam("limit",
d))):(f--,b-=f);0>b&&(b=0);c.insertSibling("\x3cdiv class\x3d'loading'\x3e"+this.localize("loading")+"\x3c/div\x3e","before",!1).mask();g||c.destroy();c=e.getId();this.setApiParams({skipToDocId:c,start:b});this.setInsertWhere("afterBegin");this.setScrollIntoView(a);this.load();this.setApiParam("limit",this.INITIAL_LIMIT);break}c.destroy();c=b.first()}},fetchNext:function(a){var b=this.innerContainer.first(),c=b.last();if(!1===c.hasCls("loading"))for(var d=$(b.dom).contents().filter(function(){return 3===
this.nodeType}).last();c;){if(c.hasCls("word")){var b=Voyant.data.model.Token.getInfoFromElement(c),e=b.docIndex,f=b.position,g=this.getDocumentsStore().getAt(b.docIndex),h=g.getId(),g=g.get("tokensCount-lexical");if(f+this.getApiParam("limit")>=g&&e==this.getCorpus().getDocumentsCount()-1)if(e=g-f,1>=e){c.dom.scrollIntoView();c.frame("red");break}else this.setApiParam("limit",e);c.el.dom.nextSibling===d[0]&&d.remove();c.insertSibling("\x3cdiv class\x3d'loading'\x3e"+this.localize("loading")+"\x3c/div\x3e",
"after",!1).mask();c.destroy();this.setApiParams({skipToDocId:h,start:b.position});this.setInsertWhere("beforeEnd");this.setScrollIntoView(a);this.load();this.setApiParam("limit",this.INITIAL_LIMIT);break}c.destroy();c=b.last()}},load:function(a,b){a&&(this.innerContainer.first().destroy(),this.innerContainer.setHtml('\x3cdiv class\x3d"readerContainer"\x3e\x3cdiv class\x3d"loading"\x3e'+this.localize("loading")+"\x3c/div\x3e\x3c/div\x3e"),this.innerContainer.first().first().mask());this.getTokensStore().load(Ext.apply(b||
...
glyph:"xf1ea@FontAwesome"},config:{corpus:void 0,options:{xtype:"stoplistoption"}},autoScroll:!0,cls:"corpus-summary",corpusTermsStore:null,docsStore:null,documentTermsStore:null,summaryListParent:null,constructor:function(a){Ext.apply(this,{title:this.localize("title")});this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments);this.on("loadedCorpus",function(a,c){this.setCorpus(c);this.corpusTermsStore=c.getCorpusTerms();this.docStore=Ext.create("Ext.data.Store",
{model:"Voyant.data.model.Document",autoLoad:!1,remoteSort:!1,corpus:this.getCorpus(),proxy:{type:"ajax",url:Voyant.application.getTromboneUrl(),extraParams:{tool:"corpus.DocumentsMetadata",corpus:this.getCorpus().getId()},reader:{type:"json",rootProperty:"documentsMetadata.documents",totalProperty:"documentsMetadata.total"},simpleSortMode:!0},listeners:{load:function(a,b,c,d){var k=this.getCorpus();b.forEach(function(a){a.set("corpus",k)})},scope:this}});this.documentTermsStore=Ext.create("Ext.data.Store",
...
"\x3cli\x3e"+c.getShow()+"\x3c/li\x3e");d=c.getDocumentsCount();1<d&&this.showLongestDocuments();1<d&&this.showMostDenseDocuments();this.showMostFrequentWords();1<d&&this.showDistinctiveWords();d=Ext.apply({},{limit:null},this.getApiParams());this.docStore.load({params:d});this.addLinksHandler()}else Ext.defer(this.fireEvent,100,this)});this.on("activate",function(){this.getCorpus()&&this.dispatchEvent("ensureCorpusView",this,this.getCorpus())},this);a.embedded&&"Voyant.data.model.Corpus"==Ext.getClass(a.embedded).getName()&&
