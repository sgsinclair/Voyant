...
this.lastClickedBubbles[b],m=0,n;for(n in k)if(-1!==this.termsFilter.indexOf(n)){var q=k[n];if(q){m++;this.SEPARATE_LINES_FOR_TERMS&&c();var s=0,p=q.color.join(",");this.ctx.strokeStyle="rgba("+p+", 1)";this.ctx.fillStyle="rgba("+p+", 0.35)";this.ctx.lineWidth=.25;for(var h=h+q.rawFreq,s=s+q.rawFreq,r=l&&this.lastClickedBubbles[b][n],A=0;A<q.pos.length;A++){var F=q.pos[A];if(0<F.radius){var D=!1;r&&this.lastClickedBubbles[b][n]==F.id&&(this.ctx.strokeStyle="rgba(0, 0, 0, 0.75)",this.ctx.fillStyle=
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];l=0;f=function(){var a,f,h,k,r=this.wordsToArrange-1,n=g.canvas.width,A=g.canvas.height,q=.5*n,s=.5*A;do{h=p[l];if(void 0!==h){for(var H=Math.random()*Math.PI,w=.25*Math.random()*h.size,u=.5*(Math.random()-.5),v=.5*h.width,M=.5*h.height;;){a=Math.floor((q+Math.cos(H)*w*this.ratio-v)/this.COARSENESS)*this.COARSENESS;f=Math.floor((s+Math.sin(H)*w-M)/this.COARSENESS)*this.COARSENESS;
k=a+v>=n||f+M>=A?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;H+=u;w+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<l;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}l++;if(l>=p.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(r--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;m=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
r=function(a){return doubletree.fieldText(a,"POS")},A=function(a){return"rgba(255,255,255,1)"},F=function(a){return"rgba(255,255,255,1)"},D=function(a){return"red"},G={node:{fill:"white",stroke:"steelblue","stroke-width":"1.5px"},branch:{stroke:"#aaa","stroke-width":"1.5px"}},H=!1,L=d3.dispatch("idsUpdated");L.on("idsUpdated",function(){this==B?(C.setIds(B.continuationIDs),C.updateContinuations()):this==C&&(B.setIds(C.continuationIDs),B.updateContinuations())});var z,E,B,C,K,I;b.init=function(a){d3.select(d3.selectAll(a)).call(this);
return b};b.redraw=function(){b.setupFromTries(z,E);return b};b.setupFromTries=function(d,k){z=d.getUniqRoot();E=k.getUniqRoot();var x=z.toTree(l.left),w=E.toTree(l.right),u=!0;0<Object.keys(w.pruned).length&&(a(w,w.pruned,u),a(x,w.pruned,u),u=!1);0<Object.keys(x.pruned).length&&(a(x,x.pruned,u),a(w,x.pruned,u));var u={},v;for(v in w.info)"continuations"!=v&&"ids"!=v&&"count"!=v&&(u[v]=w.info[v]);u["right continuations"]=w.info.continuations;u["left continuations"]=x.info.continuations;u.ids={};c(u.ids,
...
a=a||{};Ext.isNumber(a)&&(a={limit:a});Ext.applyIf(a,{limit:0});var c=this.getTokens();c.load({params:a,callback:function(a,e,f){f?b.resolve(c):b.reject(e)}});return b.promise},getCorpusTerms:function(a){return Ext.create("Voyant.data.store.CorpusTerms",Ext.apply(a||{},{corpus:this}))},getTokens:function(a){return Ext.create("Voyant.data.store.Tokens",Ext.apply(a||{},{corpus:this}))},each:function(a){debugger;this.getDocuments().each.call(this,arguments)},getCorpusCollocates:function(a){return Ext.create("Voyant.data.store.CorpusCollocates",
...
b.up("form").getForm();if(d.isValid()){for(var e=b.fileInputEl.dom.files,f=/\.(png|gif|jpe?g|xls|mp[234a]|mpeg|exe|wmv|avi|ppt|mpg|tif|wav|mov|psd|wma|ai|bmp|pps|aif|pub|dwg|indd|swf|asf|mbd|dmg|flv)$/i,g=/\.(txt|pdf|html?|xml|docx?|rtf|pages|odt|zip|jar|tar|gz|ar|cpio|bzip2|bz2|gzip|xlsx?)$/i,h=[],k=[],l=0,m=e.length;l<m;l++){var n=e[l].name;f.test(n)?h.push(n.split("/").pop()):g.test(n)||k.push(n.split("/").pop())}0<h.length||0<k.length?Ext.Msg.show({title:a.localize("fileTypesWarning"),icon:Ext.MessageBox.ERROR,
...
a.targetX),l=Math.abs(a.y-a.targetY);if(0!=g||0!=l)1>g?a.x=a.targetX:(g/=2,a.x=a.x>a.targetX?a.x-g:a.x+g),1>l?a.y=a.targetY:(l/=2,a.y=a.y>a.targetY?a.y-l:a.y+l),e=!0},this);c=0;k=parseInt(this.textFont)+4;b.textAlign="center";b.textBaseline="middle";for(var l in this.magnets)d||l in g||this.magnets[l].isHovering?(c=this.magnets[l].width+4,b.fillStyle=l in g||this.magnets[l].isHovering||!0===f?"white":"rgba(255,255,255,.05)",b.fillRect(this.magnets[l].x-c/2,this.magnets[l].y-k/2,c,k),b.strokeStyle=
l in g||this.magnets[l].isHovering||!0===f?"rgb("+this.magnets[l].colour.join(",")+")":"rgba(0,0,0,.05)",b.strokeRect(this.magnets[l].x-c/2,this.magnets[l].y-k/2,c,k),b.textAlign="center",b.fillStyle=l in g||this.magnets[l].isHovering||!0===f?"rgba(0,0,0,.8)":"rgba(0,0,0,.05)",b.fillText(l,this.magnets[l].x,this.magnets[l].y)):(b.beginPath(),b.fillStyle="rgb("+this.magnets[l].colour.join(",")+")",b.strokeStyle="rgb("+this.magnets[l].colour.join(",")+")",b.arc(this.magnets[l].x,this.magnets[l].y,12,
0,2*Math.PI),b.fill(),b.stroke())}if(e){var m=this;setTimeout(function(){m.draw()},100)}else if(this.documents){c=0;for(l=this.documents.length;c<l;c++)for(var n=0;n<l;n++)if(c<n){var q=this.documents[c].x-this.documents[n].x,s=this.documents[c].y-this.documents[n].y;15>Math.sqrt(q*q+s*s)&&(q*=.1,s*=.1,this.documents[c].targetX+=q,this.documents[n].targetX-=q,this.documents[c].targetX+=s,this.documents[n].targetX-=s,e=!0)}e&&(m=this,setTimeout(function(){m.draw()},100))}}});Ext.define("Voyant.panel.Reader",{extend:"Ext.panel.Panel",requires:["Voyant.data.store.Tokens"],mixins:["Voyant.panel.Panel"],alias:"widget.reader",isConsumptive:!0,statics:{i18n:{},api:{start:0,limit:1E3,skipToDocId:void 0,query:void 0},glyph:"xf0f6@FontAwesome"},config:{corpus:void 0,tokensStore:void 0,documentsStore:void 0,documentTermsStore:void 0,exportVisualization:!1,lastScrollTop:0,scrollIntoView:!1,insertWhere:"beforeEnd",locationMarker:void 0,lastLocationUpdate:new Date},INITIAL_LIMIT:1E3,
...
e=c.getDimensions(),g=0;g<e.length&&!(g>=d);g++)h=parseFloat(e[g].data),a+=this.localize("dimension")+" "+(g+1)+" ("+this.localize(k[g])+"): "+Math.round(100*h)/100+"%\n";var l=0,m=Number.MAX_VALUE,n=0,q=Number.MAX_VALUE;"docSim"!==this.getApiParam("analysis")&&this.termStore.removeAll();var s=[],p=[];c.getTokens().forEach(function(a){var b=a.get("rawFreq"),c=a.get("category");void 0===c&&(c="term");var e="term"===c;e&&(b>l&&(l=b),b<m&&(m=b),this.termStore.findExact("term",-1===a.get("term"))&&this.termStore.addSorted(a));
...
type:a.getTerm(),val:Ext.util.Format.number(a.get("rawFreq"),"0,000"),docId:a.get("docId")})});var m;f.forEach(function(a){var c=this.getCorpus().getDocument(a);m=l[a].length;Ext.dom.Helper.append(b,{tag:"li","voyant:index":String(a),html:'\x3ca href\x3d"#" onclick\x3d"return false" class\x3d"document-id document-id-distinctive" voyant:val\x3d"'+c.get("id")+'"\x3e'+c.getShortTitle()+"\x3c/a\x3e"+this.localize("colon")+(new Ext.XTemplate(this.localize("documentType"))).apply({types:l[a]})+"."})},this);
