...
(d=90,c.ty=0):(d=-90,c.ty=c.height,c.tx=c.width));c.size=Math.max(c.height,c.width);c.rotation=Math.PI/180*d;g.context.fillStyle=g.canvas.style.backgroundColor;g.context.fillRect(0,0,g.canvas.width,g.canvas.height);c.draw(g.context);d=g.context.getImageData(c.x,c.y,c.width,c.height);for(var e=[],h=0;h<c.width;h++){var k=Math.floor(h/f.COARSENESS)*f.COARSENESS;null==e[k]&&(e[k]={});for(var l=0;l<c.height;l++){var m=Math.floor(l/f.COARSENESS)*f.COARSENESS,s;s=4*(h+l*d.width);s=[d.data[s],d.data[s+1],
d.data[s+2],d.data[s+3]];"rgb("+s[0]+", "+s[1]+", "+s[2]+")"!=g.canvas.style.backgroundColor&&(e[k][m]=!0);e[k][m]&&(l=m+f.COARSENESS)}}c.mask=e}}function e(){g.clear();for(var a=p.length;a--;){var b=p[a];b.alpha=1;b.live&&b.draw(g.context)}a=$(g.canvas);if(null!=n){a.css("cursor","pointer");g.context.save();g.context.textBaseline="alphabetic";g.context.font="12px Arial";var b=g.context.measureText(n.text).width,c=g.context.measureText(n.value).width,b=(b>c?b:c)+20,c=q+15,d=r+25,e=a.width(),a=a.height();
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];l=0;f=function(){var a,f,h,k,s=this.wordsToArrange-1,t=g.canvas.width,n=g.canvas.height,q=.5*t,r=.5*n;do{h=p[l];if(void 0!==h){for(var v=Math.random()*Math.PI,w=.25*Math.random()*h.size,A=.5*(Math.random()-.5),y=.5*h.width,z=.5*h.height;;){a=Math.floor((q+Math.cos(v)*w*this.ratio-y)/this.COARSENESS)*this.COARSENESS;f=Math.floor((r+Math.sin(v)*w-z)/this.COARSENESS)*this.COARSENESS;
k=a+y>=t||f+z>=n?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;v+=A;w+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<l;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}l++;if(l>=p.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(s--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;m=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
this.query("facet").forEach(function(a){a.setCorpus(c)})});this.on("afterrender",function(a){var c=this.getApiParam("facet");Ext.isString(c)&&(c=c.split(","));var d=this.queryById("facets");c.forEach(function(c){var f=a.localize(c+"Title");f=="["+c+"Title]"&&(f=c.replace(/^facet\./,"").replace(/^extra./,""));var g=d.add({title:f,facet:c,bbar:[{xtype:"querysearchfield",tokenType:c.replace("facet.",""),inDocumentsCountOnly:!0}]});g.getSelectionModel().on("selectionchange",function(d,f){var g=[];f.forEach(function(a){g.push({facet:a.getFacet(),
label:a.getLabel()})});a.getFacets()[c]=g;a.updateResults()});g.on("query",function(c,d){a.getFacets()[g.facet]=[];a.updateResults()})});d.add({title:a.localize("lexicalTitle"),store:Ext.create("Voyant.data.store.CorpusTerms",{parentPanel:this}),facet:"lexical",columns:[{renderer:function(a,b,c){return"("+c.getRawFreq()+") "+c.getTerm()},flex:1}],bbar:[{xtype:"querysearchfield"}]}).getSelectionModel().on("selectionchange",function(c,d){var g=[];d.forEach(function(a){g.push({facet:"lexical",label:a.getTerm()})});
a.getFacets().lexical=g;a.updateResults()})})},updateResults:function(a){var b=this.getFacets();if(!a){a=[];for(facet in b)b[facet].forEach(function(b){a.push(b.facet+":"+b.label)});if(a)return this.updateResults(a)}var c=this.queryById("results").getTargetEl();c.update("");this.queryById("export").setDisabled(!0);if(0<a.length){this.mask(this.localize("loading"));var d=this.getCorpus().getDocumentQueryMatches();d.load({params:{query:a,includeDocIds:!0},callback:function(a,f,g){this.unmask();if(a&&
0<a.length){this.queryById("status").setHtml(a.length);var h="\x3cul\x3e",k=[];a.forEach(function(a){a.getDocIds().forEach(function(a){k.push(a);a=d.getCorpus().getDocument(a);var c;c="\x3cli\x3e"+("\x3ci\x3e"+a.getTitle()+"\x3c/i\x3e");for(facet in b)if(0!=b[facet].length){var e="";if("facet.title"!=facet){var f=facet.replace(/^.+?\./,""),g=a.get(f);if(g){var l=Ext.isArray(g);l?e+="\x3cli\x3e"+f+"\x3cul\x3e":g=[g];g.forEach(function(a){var c=!1;b[facet].forEach(function(b){b.label==a?c=!0:-1==b.facet.indexOf("facet")&&
b.label.split(/\W+/).forEach(function(b){0<b.trim().length&&-1<a.toLowerCase().indexOf(b.toLowerCase())&&(c=!0)})});e+="\x3cli\x3e"+(l?"":f+": ")+(c?'\x3cspan class\x3d"keyword"\x3e'+a+"\x3c/span\x3e":a)+"\x3c/li\x3e"});l&&(e+="\x3c/ul\x3e\x3c/li\x3e")}}e&&(c+="\x3cul\x3e"+e+"\x3c/ul\x3e")}h+=c+"\x3c/li\x3e"})});h+="\x3c/ul\x3e";c.update(h);this.queryById("status").update(this.localize("queryMatches",[k.length,this.getCorpus().getDocumentsCount()]));this.setMatchingDocIds(k);0<k.length&&this.queryById("export").setDisabled(!1)}},
scope:this})}}});Ext.define("Voyant.panel.Cirrus",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.cirrus",statics:{i18n:{title:{en:"Cirrus"},helpTip:{en:"\x3cp\x3eCirrus provides a wordcloud view of the most frequently occurring words in the corpus or document \u2013\u00a0this provides a convenient (though reductive) overview of the content. Features include\x3c/p\x3e\x3cul\x3e\x3cli\x3eterm frequency appears when hovering over words\x3c/li\x3e\x3cli\x3eclicking on terms may produce results in other tools if any are displayed\x3c/li\x3e\x3c/ul\x3e"},
...
parseFloat(e[g].data),a+=this.localize("dimension")+" "+(g+1)+" ("+this.localize(k[g])+"): "+Math.round(100*h)/100+"%\n";var l=0,m=Number.MAX_VALUE,n=0,q=Number.MAX_VALUE;"docSim"!==this.getApiParam("analysis")&&this.termStore.removeAll();var r=[],p=[];c.getTokens().forEach(function(a){var b=a.get("rawFreq"),c="term"===a.get("category");c&&(b>l&&(l=b),b<m&&(m=b),this.termStore.findExact("term",-1===a.get("term"))&&this.termStore.addSorted(a));if(3===d){var e=a.get("vector")[2];e<q&&(q=e);e>n&&(n=
e)}b={x:a.get("vector")[0],y:a.get("vector")[1],z:a.get("vector")[2],term:a.get("term"),rawFreq:b,relativeFreq:a.get("relativeFreq"),cluster:a.get("cluster"),category:a.get("category")};c?r.push(b):("bin"===a.get("category")?b.term=b.title="Bin "+a.get("docIndex"):(b.docIndex=a.get("docIndex"),a=this.getCorpus().getDocument(b.docIndex),null!==a&&(b.term=a.getTinyTitle(),b.title=a.getTitle())),p.push(b))},this);c=this.termStore.getCount();this.queryById("limit").setRawValue(c);this.setApiParam("limit",
c);c=Ext.create("Ext.data.JsonStore",{fields:"term x y z rawFreq relativeFreq cluster category docIndex".split(" "),data:r});g=Ext.create("Ext.data.JsonStore",{fields:"term x y z rawFreq relativeFreq cluster category docIndex".split(" "),data:p});a={itemId:"chart",xtype:"cartesian",interactions:["crosszoom","panzoom","itemhighlight"],plugins:{ptype:"chartitemevents"},axes:[{type:"numeric",position:"bottom",fields:["x"]},{type:"numeric",position:"left",fields:["y"]}],sprites:[{type:"text",text:a,hidden:0<
