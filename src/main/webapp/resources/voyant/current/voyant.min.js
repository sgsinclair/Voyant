...
b&&(e[key]=b[key]);e.corpus||("Voyant.data.model.Corpus"==Ext.getClassName(this)?e.corpus=this.getId():this.getCorpus&&(e.corpus=this.getCorpus().getId()));Ext.applyIf(b,{style:"width: "+(b.width||"90%")+(Ext.isNumber(b.width)?"px":"")+"; height: "+(b.height||"400px")+(Ext.isNumber(b.height)?"px":"")});delete b.width;delete b.height;Ext.applyIf(e,Voyant.application.getModifiedApiParams());e=Ext.encode(e);e=encodeURIComponent(e);var f=Ext.id(),g=Voyant.application.getBaseUrlFull()+"tool/"+d.substring(d.lastIndexOf(".")+
1)+"/?";show('\x3ciframe style\x3d"'+b.style+'" id\x3d"'+f+'" name\x3d"'+f+'"\x3e\x3c/iframe\x3e');var h=Voyant.application.getDeferred(this);Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:{tool:"resource.StoredResource",storeResource:e}}).then(function(a){a={minimal:!0,embeddedApiId:Ext.util.JSON.decode(a.responseText).storedResource.id};Ext.applyIf(a,Voyant.application.getModifiedApiParams());document.getElementById(f).setAttribute("src",g+Ext.Object.toQueryString(a));h.resolve()}).otherwise(function(a){showError(a);
h.reject()});c=!0;return!1}},this)&&Voyant.notebook.util.Embed.showWidgetNotRecognized.call(this);c||(Ext.create(a,b).embed(b),c=!0)}c||Voyant.notebook.util.Embed.showWidgetNotRecognized.call(this)}},showWidgetNotRecognized:function(){var a=Voyant.notebook.util.Embed.i18n.widgetNotRecognized;this.embeddable&&(a+=Voyant.notebook.util.Embed.i18n.tryWidget+"\x3cul\x3e"+this.embeddable.map(function(a){a=a.substring(a.lastIndexOf(".")+1).toLowerCase();return"\"\x3ca href\x3d'../../docs/#!/guide/"+a+"' target\x3d'voyantdocs'\x3e"+
...
Ext.isString(a)?a:a.getId());this.callParent(arguments)},getString:function(a){a=this.getCount();return"This store contains "+this.getCount()+" items"+(0<a?" with these fields: "+this.getAt(0).getFields().map(function(a){return a.getName()}).join(", "):"")+"."}});Ext.define("Voyant.data.store.CAAnalysis",{extend:"Ext.data.Store",model:"Voyant.data.model.StatisticalAnalysis",config:{corpus:void 0},constructor:function(a){a=a||{};Ext.apply(a,{proxy:{type:"ajax",url:Voyant.application.getTromboneUrl(),extraParams:{tool:"corpus.CA",corpus:a&&a.corpus?Ext.isString(a.corpus)?a.corpus:a.corpus.getId():void 0},reader:{type:"json",rootProperty:"correspondenceAnalysis",totalProperty:"correspondenceAnalysis.totalTerms"},simpleSortMode:!0}});this.callParent([a])},setCorpus:function(a){a&&
this.getProxy().setExtraParam("corpus",Ext.isString(a)?a:a.getId());this.callParent(arguments)}});Ext.define("Voyant.data.store.ContextsMixin",{mixins:["Voyant.data.store.VoyantStore"],model:"Voyant.data.model.Context",constructor:function(a){this.mixins["Voyant.data.store.VoyantStore"].constructor.apply(this,[a,{"proxy.extraParams.tool":"corpus.DocumentContexts","proxy.reader.rootProperty":"documentContexts.contexts","proxy.reader.totalProperty":"documentContexts.total"}])}});
...
target:void 0,term:void 0,query:void 0,label:["summary","docs","terms"]},glyph:"xf06e@FontAwesome"},config:{options:{xtype:"stoplistoption"},caStore:null,pcaStore:null,tsneStore:null,docSimStore:null,termStore:null,chartMenu:null,newTerm:null,termsTimeout:null,highlightData:{x:0,y:0,r:0},highlightTask:null},tokenFreqTipTemplate:null,docFreqTipTemplate:null,constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(){this.setCaStore(Ext.create("Voyant.data.store.CAAnalysis",
