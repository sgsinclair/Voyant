...
a.updateLayout();l=d;remaining=this.getCorpus().getDocuments().getTotalCount()-c-f.length;if(0<remaining){e=new Ext.Template(this.localize("moreDistinctiveWords"));var n=Ext.dom.Helper.append(b,{tag:"li",cls:"more",html:e.apply([l>remaining?remaining:l,remaining])},!0);n.on("click",function(){n.remove();this.showMoreDistinctiveWords()},this)}}}})}}});Ext.define("Voyant.panel.TextualArc",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.textualarc",statics:{i18n:{},api:{stopList:"auto",docIndex:0,speed:50},glyph:"xf06e@FontAwesome"},config:{corpus:void 0,options:{xtype:"stoplistoption"}},tokensFetch:500,constructor:function(){this.mixins["Voyant.util.Localization"].constructor.apply(this,arguments);Ext.apply(this,{title:this.localize("title"),html:'\x3ccanvas width\x3d"800" height\x3d"600"\x3e\x3c/canvas\x3e',dockedItems:[{dock:"bottom",
xtype:"toolbar",enableOverflow:!0,items:[{xtype:"documentselectorbutton",singleSelect:!0},{xtype:"slider",fieldLabel:this.localize("speed"),labelAlign:"right",labelWidth:40,width:100,increment:1,minValue:0,maxValue:100,value:30,listeners:{render:function(a){a.setValue(parseInt(this.getApiParam("speed")));Ext.tip.QuickTipManager.register({target:a.getEl(),text:this.localize("speedTip")})},changecomplete:function(a,b){this.setApiParam("speed",b);this.isReading=0!==b;this.draw()},scope:this}},{xtype:"tbfill"},
{xtype:"tbtext",html:this.localize("adaptation")}]}]});this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments);this.on("boxready",function(a){var b=this.getTargetEl().getWidth()-20-20,c=this.getTargetEl().getHeight()-20-20,d=Math.max(b,c),e=d/2,f=Math.min(b,c)/d,g=this.getTargetEl().dom.querySelector("canvas");g.getContext("2d");g.width=this.getTargetEl().getWidth();g.height=this.getTargetEl().getHeight();this.diam=d;this.perim=[];for(var h=parseInt(.75*d);this.perim.length<
d;)this.perim.push({x:20+b/2+e*(b>c?1:f)*Math.cos(2*Math.PI*h/d),y:20+c/2+e*(c>b?1:f)*Math.sin(2*Math.PI*h/d)}),h++==d&&(h=0);this.draw(g);g.addEventListener("mousemove",function(b){if(a.documentTerms){var c=g.getBoundingClientRect(),d=b.clientX-c.left,e=b.clientY-c.top,f={};a.documentTerms.each(function(a){var b=a.get("x"),c=a.get("y");if(b>d-15&&b<d+15&&c>e-15&&c<e+15)return f[a.getTerm()]=!0,!1});if(0!=Object.keys(a.currentTerms||{}).length||0!=Object.keys(f).length)a.currentTerms=f,a.draw(g)}},
!1)});this.on("loadedCorpus",function(a,b){this.setCorpus(b);this.loadDocument()},this);this.on("documentselected",function(a,b){this.setApiParam("docIndex",this.getCorpus().getDocument(b).getIndex());this.loadDocument()});this.on("resize",function(){})},draw:function(a,b){a=a||this.getTargetEl().dom.querySelector("canvas");b=b||a.getContext("2d");b.clearRect(0,0,a.width,a.height);b.fillStyle="rgba(0,0,0,.1)";this.perim.forEach(function(a,c){0==c%3&&b.fillRect(a.x-5,a.y,10,1)});if(this.documentTerms&&
(this.drawTerms(a,b),this.drawReading(a,b),this.isReading)){var c=this;setTimeout(function(){c.draw()},10)}},drawReading:function(a,b){b=b||this.getTargetEl().dom.querySelector("canvas").getContext("2d");var c=2E3-1999*parseInt(this.getApiParam("speed"))/100;if(this.isReading&&this.documentTerms){var d=parseInt(this.readingIndex*this.perim.length/this.lastToken);b.fillStyle="purple";b.fillRect(this.perim[d].x,this.perim[d].y,5,5);var e=void 0==this.readingStartTime;this.readingStartTime=this.readingStartTime||
(new Date).getTime();d=this.readingStartTime+c-(new Date).getTime();if(this.sourceTerm&&this.targetTerm){if(e||0>=d){this.previousBeziers=this.previousBeziers||[];var e=this.sourceTerm.get("x"),f=this.sourceTerm.get("y"),g=this.targetTerm.get("x"),h=this.targetTerm.get("y");this.previousTerm&&this.previousTerm.get("x");this.previousTerm&&this.previousTerm.get("y");var k=Math.max(100,.5*Math.abs(e-g)),m=Math.max(100,.5*Math.abs(f-h));this.previousBeziers.unshift([e,f,e>g?e-k:e+k,h>f?f+m:f-m,g,h]);
10<this.previousBeziers.length&&this.previousBeziers.pop()}for(e=0;e<this.previousBeziers.length;e++)b.strokeStyle="rgba(0,0,255,"+(1-.1*e)+")",this.drawBezierSplit.apply(this,Ext.Array.merge([b],this.previousBeziers[e],[e+1==this.previousBeziers.length?1-d/c:0],[0==e?1-d/c:1]));0>=d&&(this.readingStartTime=void 0,this.read())}e=this.readingIndex+1;for(f=this.tokens.getCount();e<f&&this.tokens.getAt(e).getTerm().toLowerCase()!=this.targetTerm.getTerm();e++);c=e-parseInt(d*(e-this.readingIndex)/c);
for(d=this.tokens.getCount();c<e&&!(c<d&&this.tokens.getAt(c).isWord());c++);c=this.tokens.getRange(c,f=Math.min(this.readingIndex+50,this.tokens.getCount())).map(function(a){return a.getTerm()});b.font="14px sans-serif";b.fillStyle="rgba(0,0,0,.5)";b.textAlign="left";b.fillText(c.join(""),a.width/4,a.height-5);b.clearRect(.75*a.width,a.height-20,a.width,30)}else this.documentTerms&&this.documentTerms.getCount()<this.documentTerms.getTotalCount()&&(c=a.width/4,b.strokeStyle="rgba(0,0,0,.5)",b.fillStyle=
"rgba(0,0,0,.2)",b.strokeRect(c,a.height-12,2*c,10),b.fillRect(c,a.height-12,this.documentTerms.getCount()*c*2/this.documentTerms.getTotalCount(),10))},drawTerms:function(a,b){a=a||this.getTargetEl().dom.querySelector("canvas");b=b||a.getContext("2d");b.textAlign="center";this.documentTerms&&this.perim&&this.documentTerms.each(function(c){var d=c.getRawFreq(),e=c.getTerm(),f=c.get("x"),g=c.get("y");isCurrentTerm=this.currentTerms&&e in this.currentTerms;isReadingTerm=this.sourceTerm&&this.sourceTerm.getTerm()==
e;b.font=Math.log(d)*(10*a.width/800)/Math.log(this.maxRawFreq)+(isCurrentTerm||isReadingTerm?10:5)+"px sans-serif";b.fillStyle=isCurrentTerm?"red":isReadingTerm?"blue":"rgba(0,0,0,"+(.9*d/this.maxRawFreq+.1)+")";if(isCurrentTerm||isReadingTerm)b.strokeStyle=isCurrentTerm?"rgba(255,0,0,.2)":"rgba(0,255,0,.4)",c.getDistributions().forEach(function(a,c){0<a&&this.perim[c]&&(b.beginPath(),b.moveTo(f,g),b.lineTo(this.perim[c].x,this.perim[c].y),b.stroke())},this);b.fillText(e,f,g)},this)},read:function(a){Ext.isNumber(a)?
this.readingIndex=a:this.readingIndex++;this.sourceTerm&&(this.previousTerm=this.sourceTerm);a=this.readingIndex;for(var b=this.tokens.getCount();a<b;a++){var c=this.tokens.getAt(a),c=c.getTerm().toLowerCase();if(c in this.termsMap&&(this.sourceTerm=this.termsMap[c],1<=this.sourceTerm.getRawFreq())){this.readingIndex=a;break}}a=this.readingIndex+1;for(b=this.tokens.getCount();a<b&&!(c=this.tokens.getAt(a),c=c.getTerm().toLowerCase(),c in this.termsMap&&(this.targetTerm=this.termsMap[c],1<=this.targetTerm.getRawFreq()));a++);
!this.tokensLoading&&this.tokens.getCount()-this.readingIndex<this.tokensFetch&&this.fetchMoreTokens();this.draw()},loadDocument:function(){this.documentTerms&&(this.documentTerms.destroy(),this.documentTerms=void 0);this.termsMap={};this.draw();var a=this.getCorpus().getDocument(parseInt(this.getApiParam("docIndex")));this.lastToken=parseInt(a.get("lastTokenStartOffset-lexical"));this.documentTerms=a.getDocumentTerms({proxy:{extraParams:{stopList:this.getApiParam("stopList"),bins:this.diam,withDistributions:"raw",
minRawFreq:2}}});this.fetchMoreDocumentTerms()},fetchMoreDocumentTerms:function(){this.documentTerms?this.documentTerms.load({params:{start:this.documentTerms.getCount(),limit:0==this.documentTerms.getCount()?10:250},callback:function(a){0<a.length?(this.maxRawFreq=this.documentTerms.max("rawFreq"),a.forEach(function(a){var c=y=0;a.get("distributions").forEach(function(a,b){c+=this.perim[b].x*a;y+=this.perim[b].y*a},this);a.set("x",c/a.getRawFreq());a.set("y",y/a.getRawFreq())},this),Ext.Function.defer(this.fetchMoreDocumentTerms,
0,this),this.draw()):(this.termsMap={},this.documentTerms.each(function(a){this.termsMap[a.getTerm()]=a},this),this.tokens&&this.tokens.removeAll(!0),this.fetchMoreTokens())},addRecords:!0,scope:this}):this.loadDocument()},fetchMoreTokens:function(){if(!this.tokens)this.tokens=this.getCorpus().getDocument(parseInt(this.getApiParam("docIndex"))).getTokens({proxy:{extraParams:{stripTags:"all"}}}),this.noMoreTokens=!1;else if(this.noMoreTokens)return;var a=0==this.tokens.getCount();this.tokensLoading=
!0;var b=parseInt(this.getApiParam("speed"));this.tokens.load({params:{start:this.tokens.getCount(),limit:50==b&&a?200:Math.pow(110-b,2)},callback:function(b){this.tokensLoading=!1;0<b.length?(b.forEach(function(a){"other"==a.getTokenType()&&a.set("term",a.getTerm().replace(/\s+/g," "))}),a&&(this.previousBeziers=[],this.isReading=!0,this.read(0))):this.noMoreTokens=!0},addRecords:!0,scope:this})},animatePathDrawing:function(a,b,c,d,e,f,g,h){var k=null,m=function(l){null===k&&(k=l);l=Math.min((l-
k)/h,1);a.clearRect(0,0,a.canvas.width,a.canvas.height);drawBezierSplit(a,b,c,d,e,f,g,0,l);1>l&&window.requestAnimationFrame(m)};window.requestAnimationFrame(m)},drawBezierSplit:function(a,b,c,d,e,f,g,h,k){a.beginPath();if(0==h&&1==k)a.moveTo(b,c),a.quadraticCurveTo(d,e,f,g);else if(h!=k){var m=h*h,l=1-h,n=l*l,q=2*h*l,s=n*b+q*d+m*f,p=n*c+q*e+m*g,m=k*k,l=1-k,n=l*l,q=2*k*l,l=n*b+q*d+m*f,m=n*c+q*e+m*g;b=this.lerp(this.lerp(b,d,h),this.lerp(d,f,h),k);c=this.lerp(this.lerp(c,e,h),this.lerp(e,g,h),k);a.moveTo(s,
