...
this.getCategoryFeature(a,"color");if(void 0!==b){b=this.hexToRgb(b);for(var c=this.getCategoryTerms(a),d=0;d<c.length;d++)this.colorTermAssociations.replace(c[d],b)}}},getCategoryExportData:function(){return{categories:this.getCategories(),features:this.getFeatures()}},loadCategoryData:function(a){var b=new Ext.Deferred;Ext.Ajax.request({url:this.getTromboneUrl(),params:{tool:"resource.StoredCategories",retrieveResourceId:a,failQuietly:!1,corpus:this.getCorpus()?this.getCorpus().getId():void 0}}).then(function(a){a=
Ext.decode(a.responseText).storedCategories.resource;0==a.length?b.reject():(a=Ext.decode(a),this.setCategories(a.categories),this.setFeatures(a.features),b.resolve(a))},function(){this.showError("Unable to load categories data: "+a);b.reject()},null,this);return b.promise},saveCategoryData:function(a){a=a||this.getCategoryExportData();var b=new Ext.Deferred;a=Ext.encode(a);Ext.Ajax.request({url:this.getTromboneUrl(),params:{tool:"resource.StoredResource",storeResource:a}}).then(function(a){a=Ext.util.JSON.decode(a.responseText).storedResource.id;
b.resolve(a)},function(a){b.reject()});return b.promise}});Ext.define("Voyant.util.DiacriticsRemover",{diacriticsMap:{},constructor:function(){var a=[{base:"A",letters:"A\u24b6\uff21\u00c0\u00c1\u00c2\u1ea6\u1ea4\u1eaa\u1ea8\u00c3\u0100\u0102\u1eb0\u1eae\u1eb4\u1eb2\u0226\u01e0\u00c4\u01de\u1ea2\u00c5\u01fa\u01cd\u0200\u0202\u1ea0\u1eac\u1eb6\u1e00\u0104\u023a\u2c6f"},{base:"AA",letters:"\ua732"},{base:"AE",letters:"\u00c6\u01fc\u01e2"},{base:"AO",letters:"\ua734"},{base:"AU",letters:"\ua736"},{base:"AV",letters:"\ua738\ua73a"},{base:"AY",letters:"\ua73c"},
...
"value"],data:b},name:"category",value:a},{width:10},{xtype:"tbspacer"},{xtype:"button",text:this.localize("edit"),ui:"default-toolbar",handler:function(){if(void 0===this.getBuilderWin()){var a=this.up("window").panel;a=Ext.create("Voyant.widget.CategoriesBuilder",{panel:a,height:.75*a.getApplication().getViewport().getHeight(),width:.75*a.getApplication().getViewport().getWidth()});a.on("close",function(a){a=a.getCategoriesId();if(void 0!==a){var b=this.down("combo");b.getStore().add({name:a,value:a});
b.setValue(a);this.up("window").panel.setApiParam("categories",a)}},this);this.setBuilderWin(a)}a=this.down("combo").getValue();this.getBuilderWin().setCategoriesId(a);this.getBuilderWin().show()},scope:this}]});this.callParent(arguments)}});
...
this.getBaseUrl()},getRelativeUrl:function(){for(var a="",b=0,c=window.location.pathname.substring(this.getBaseUrl().length).split("/").length-1;b<c;b++)a+="../";return a},getTools:function(){return[{type:"maximize"},{type:"help"}]},launch:function(){Ext.tip.QuickTipManager.init();Ext.apply(Ext.tip.QuickTipManager.getQuickTip(),{showDelay:50});this.callParent(arguments)},tromboneCall:function(a){a=a?a:{};Ext.applyIf(a,{url:this.getTromboneUrl()});a.success||a.failure||a.callback||Ext.applyIf(a,{url:this.getTromboneUrl(),
...
scope:this})},listeners:{loadedCorpus:function(a,b){this.setCorpus(b);this.getApiParam("categories")&&this.loadCategoryData(this.getApiParam("categories")).then(function(){this.setColorTermAssociations()},null,null,this);this.on("unhandledEvent",function(a,d,e){a=this.getBaseUrl()+"?corpus\x3d"+b.getAliasOrId();var c=this.getModifiedApiParams()||{};delete c.view;if("termsClicked"==d)if(Ext.isArray(e)&&"term"in e[0]&&"docIndex"in e[0]){var g={},k={};e.forEach(function(a){g[a.term]=!0;k[a.docIndex]=
!0});c.query=Object.keys(g);c.docIndex=Object.keys(k)}else c.query=e;else if("documentsClicked"==d){var h=[];e.forEach&&e.forEach(function(a){h.push(a.getIndex())});c.docIndex=h}else if("corpusTermsClicked"==d)e.map&&(c.query=e.map(function(a){return a.getTerm()}));else if("documentTermsClicked"==d)e.map&&(c.query=e.map(function(a){return a.getTerm()}),c.docIndex=e.map(function(a){return a.getDocIndex()}));else{console&&console.warn("Unhandled event: "+d,e);return}c.toolFlow&&(d=Ext.Array.from(c.toolFlow.split(",")),
c.view=d.shift(),0<d.length?c.toolFlow=d.join(","):delete c.toolFlow);a+="\x26"+Ext.Object.toQueryString(c);this.openUrl(a)})}}});Ext.define("Voyant.panel.DocumentClusters",{extend:"Ext.panel.Panel",alias:"widget.documentclusters",title:"Document Clusters"});
