...
{items:[{xtype:"menucheckitem",text:this.localize("autoExecuteOnLoad"),hidden:0==this.results.isVisible(),itemId:"toggleAutoExecute",checked:this.getAutoExecute(),handler:function(b,c){this.setAutoExecute(b.checked);b.up("menu").close();a.toggleCls("autoExecute",b.checked);this.up("notebook").setIsEdited(!0)},scope:this},{text:this.localize("codeMode"),glyph:"xf1c9@FontAwesome",listeners:{click:function(){var a=this,b=this.editor.getMode().split("/").pop();(new Ext.Window({title:this.localize("codeModeTitle"),
layout:"fit",width:200,items:[{xtype:"form",layout:{type:"vbox",align:"stretch"},bodyPadding:10,items:[{xtype:"fieldset",title:this.localize("modeCode"),items:{xtype:"radiofield",boxLabel:this.localize("modeJavascript"),name:"codeMode",inputValue:"javascript",flex:1,checked:"javascript"==b}},{xtype:"fieldset",title:this.localize("modeData"),items:[{items:{xtype:"radiofield",boxLabel:this.localize("modeJson"),name:"codeMode",inputValue:"json",flex:1,checked:"json"==b}},{items:{xtype:"radiofield",boxLabel:this.localize("modeText"),
name:"codeMode",inputValue:"text",flex:1,checked:"text"==b}},{items:{xtype:"radiofield",boxLabel:this.localize("modeHtml"),name:"codeMode",inputValue:"html",flex:1,checked:"html"==b}},{items:{xtype:"radiofield",boxLabel:this.localize("modeXml"),name:"codeMode",inputValue:"xml",flex:1,checked:"xml"==b}}]}]}],buttons:[{text:this.localize("ok"),handler:function(){var b=this.up("window");values=b.down("form").getForm().getValues();a.switchModes(values.codeMode);b.close()}},{text:this.localize("cancel"),
handler:function(){this.up("window").close()}}]})).show()},scope:this}}]}).showAt(c.pageX,c.pageY)}}]},{xtype:"toolbar",dock:"right",items:[{xtype:"notebookwrappercounter",order:a.order,name:a.cellId},{xtype:"notebookwrapperremove"},{xtype:"notebookwrappermoveup"},{xtype:"notebookwrappermovedown"}]}],items:[this.editor,this.results]});this.callParent(arguments)},initComponent:function(a){var b=this;b.on("afterrender",function(){this.editor&&"ace/mode/javavscript"!=this.editor.getMode()&&this.switchModes(this.editor.getMode(),
...
this.addCode(a)},this):a.input?0===a.input.indexOf("http")&&this.loadFromUrl(a.input,b):c?(this.loadFromId(c[1]),this.setStorageSolution("voyant")):d?(this.githubDialogs.loadFileFromId(a.githubId),this.setStorageSolution("github")):this.addNew();if(b){var e=this;Ext.defer(function(){e.runAll()},100)}},scope:this})},getBlock:function(a,b){a=a||0;b=b||{};var c=this.query("notebookcodeeditorwrapper"),d=this.getCurrentBlock().id,e=c.findIndex(function(a){return a.id==d});if(0>e+a||e+a>c.length-1)"failQuietly"in
b&&b.failQuietly||Ext.Msg.show({title:this.localize("error"),msg:this.localize("blockDoesNotExist"),buttons:Ext.MessageBox.OK,icon:Ext.MessageBox.ERROR});else return content=c[e+a].getContent(),content.input},addNew:function(){this.setMetadata(new Spyral.Metadata({title:"\x3ch1\x3eSpyral Notebook\x3c/h1\x3e"}));this.addText("\x3cp\x3eThis is a Spyral Notebook, a dynamic document that combines writing, code and data in service of reading, analyzing and interpreting digital texts.\x3c/p\x3e\x3cp\x3eSpyral Notebooks are composed of text blocks (like this one) and code blocks (like the one below). You can \x3cspan class\x3d'marker'\x3eclick on the blocks to edit\x3c/span\x3e them and add new blocks by clicking add icon that appears in the left column when hovering over a block.\x3c/p\x3e");
...
this.getHeaderHtml()+"\x3carticle class\x3d'spyralArticle'\x3e";this.getComponent("cells").items.each(function(b,d){var c=b.isXType("notebookcodeeditorwrapper")?"code":"text";d=b.getContent();var f=b.down("notebookwrappercounter");a+="\x3csection id\x3d'"+f.name+"' class\x3d'notebook-editor-wrapper "+b.xtype+"'\x3e\n\x3cdiv class\x3d'notebookwrappercounter'\x3e"+f.getTargetEl().dom.innerHTML+"\x3c/div\x3e";"code"==c?(c=b.down("notebookcodeeditor").getMode(),c=c.substring(c.lastIndexOf("/")+1),f=b.getAutoExecute()?
"autoexec":"",b=b.getTargetEl().query(".ace_text-layer")[0].cloneNode(!0),b.style.setProperty("height","auto"),a+="\x3cdiv class\x3d'notebook-code-editor ace-chrome'\x3e\n"+b.outerHTML+"\n\x3c/div\x3e\n\x3cpre class\x3d'notebook-code-editor-raw editor-mode-"+c+" "+f+"'\x3e"+d.input+"\x3c/pre\x3e\n\x3cdiv class\x3d'notebook-code-results'\x3e\n"+d.output+"\n\x3c/div\x3e\n"):a+="\x3cdiv class\x3d'notebook-text-editor'\x3e"+d+"\x3c/div\x3e\n";a+="\x3c/section\x3e\n"});return a+="\x3c/article\x3e\n\x3cfooter class\x3d'spyral-footer'\x3e"+
this.getInnerFooterHtml()+"\x3c/footer\x3e\x3c/body\x3e\n\x3c/html\x3e"},getHeaderHtml:function(){return"\x3cheader class\x3d'spyral-header'\x3e"+this.getInnerHeaderHtml()+"\x3c/header\x3e\n"},getInnerHeaderHtml:function(){var a="";this.getMetadata().title&&(a+="\x3cdiv class\x3d'title'\x3e"+this.getMetadata().title+"\x3c/div\x3e");this.getMetadata().author&&(a+="\x3cdiv class\x3d'author'\x3e"+this.getMetadata().author+"\x3c/div\x3e");return a},getInnerFooterHtml:function(){var a="",b=this.getMetadata();
if(b.author||b.license)a="\x26copy;",b.author&&(a+=" "+b.author),b.license&&(a+=" ("+b.license+")"),a+=". ";if(b.created||b.modified){var c=b.shortDate("created");b=b.shortDate("modified");c&&(a+=this.localize("created")+" "+c+".");b&&c!=b&&(a+=this.localize("modified")+" "+b+".")}return a},getExtraExportItems:function(){return[{inputValue:"html",boxLabel:this.localize("exportHtml")},{inputValue:"htmlDownload",boxLabel:'\x3ca href\x3d"#"\x3e'+this.localize("exportHtmlDownload")+"\x3c/a\x3e",listeners:{afterrender:function(a){var b=
