...
null!=e&&0!=e.length||c():c();h=2*Math.PI;e=0;a.freqCounts={};var k=a.terms,m=null!=this.lastClickedBubbles[b],l=0,n;for(n in k){var q=k[n];if(q){l++;this.SEPARATE_LINES_FOR_TERMS&&c();var r=0,p=q.color.join(",");this.ctx.strokeStyle="rgba("+p+", 1)";this.ctx.fillStyle="rgba("+p+", 0.35)";this.ctx.lineWidth=.25;for(var e=e+q.rawFreq,r=r+q.rawFreq,s=m&&this.lastClickedBubbles[b][n],t=0;t<q.pos.length;t++){var u=q.pos[t];if(0<u.radius){var x=!1;s&&this.lastClickedBubbles[b][n]==u.id&&(this.ctx.strokeStyle=
"rgba(0, 0, 0, 0.75)",this.ctx.fillStyle="rgba("+p+", 0.5)",this.ctx.lineWidth=1,x=!0);this.ctx.beginPath();this.ctx.arc(u.x+g,this.yIndex,u.radius,0,h,!0);this.ctx.closePath();this.ctx.fill();this.ctx.stroke();x&&(this.ctx.strokeStyle="rgba("+p+", 1)",this.ctx.fillStyle="rgba("+p+", 0.35)",this.ctx.lineWidth=.25)}}a.freqCounts[n]=r;this.SEPARATE_LINES_FOR_TERMS&&(this.ctx.fillStyle="rgba(0, 0, 0, 1.0)",this.ctx.font="10px Verdana",this.ctx.fillText(r,g+d+5,this.yIndex-4),this.yIndex+=this.maxRadius)}}this.SEPARATE_LINES_FOR_TERMS&&
...
0;var k=this.MIXED=1;this.getWordOrientation=function(){return k};this.setWordOrientation=function(a){k=a};this.UPDATE_RATE=25;this.COARSENESS=5;this.grid=[];var m=0,l;this.doingArrange=!1;this.wordsToArrange=5;var n=null,q=0,r=0,p=[];this.getWords=function(){return p};this.setWords=function(a){p=a};this.sizeAdjustment=100;this.minFontSize=12;this.largestWordSize=0;this.smallestWordSize=1E4;var s={};this.addWords=function(a){for(var b=!1,e=0;e<a.length;e++){var h=a[e],k;k=void 0==typeof h.color||
null==h.color||""==h.color?f.colors[Math.floor(Math.random()*f.colors.length)]:h.color;var m;m=void 0==typeof h.size||null==h.size||""==h.size?Math.floor(40*Math.random()):parseFloat(h.size);var l=h.word,n=h.label,q=h.value,h=!1;null==s[l]&&(s[l]=!0,m>g.largestWordSize&&(g.largestWordSize=m,h=!0),m<g.smallestWordSize&&(g.smallestWordSize=.8*m,h=!0),l=new Word(l,m,k,n,q),p.push(l));b=h||b}c();this.setRelativeSizes();this.calculateSizeAdjustment();b?this.resizeWords():d()};this.resetWordCoordinates=
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},g;this.grid=[];m=0;g=function(){var a,g,h,k,n=this.wordsToArrange-1,t=f.canvas.width,s=f.canvas.height,q=.5*t,r=.5*s;do{h=p[m];if(void 0!==h){for(var v=Math.random()*Math.PI,w=.25*Math.random()*h.size,A=.5*(Math.random()-.5),y=.5*h.width,z=.5*h.height;;){a=Math.floor((q+Math.cos(v)*w*this.ratio-y)/this.COARSENESS)*this.COARSENESS;g=Math.floor((r+Math.sin(v)*w-z)/this.COARSENESS)*this.COARSENESS;
k=a+y>=t||g+z>=s?!0:d(a,g,h.height,h.width,h.mask);if(!k)break;v+=A;w+=.05}c(a,g,h);if(f.useFadeEffect)for(a=h.alpha=0;a<m;a++)g=p[a],1>g.alpha&&b(g);else h.alpha=1,h.draw(f.context)}m++;if(m>=p.length){clearTimeout(l);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(n--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;l=setInterval(g,50)}else alert("Error: There are no words to arrange.")};
...
h)/100+"%\n"}else for(a=this.localize("caTitle")+"\n",k=["xAxis","yAxis","fill"],e=c.getDimensions(),f=0;f<e.length&&!(f>=d);f++)h=parseFloat(e[f].data),a+=this.localize("dimension")+" "+(f+1)+" ("+this.localize(k[f])+"): "+Math.round(100*h)/100+"%\n";var m=0,l=Number.MAX_VALUE,n=0,q=Number.MAX_VALUE;this.termStore.removeAll();var r=[],p=[];c.getTokens().forEach(function(a){var b=a.get("rawFreq"),c="part"===a.get("category");c||(b>m&&(m=b),b<l&&(l=b),this.termStore.findExact("term",-1===a.get("term"))&&
this.termStore.addSorted(a));if(3===d){var e=a.get("vector")[2];e<q&&(q=e);e>n&&(n=e)}a={term:a.get("term"),rawFreq:b,relativeFreq:a.get("relativeFreq"),cluster:a.get("cluster"),x:a.get("vector")[0],y:a.get("vector")[1],z:a.get("vector")[2],isDoc:c};c?p.push(a):r.push(a)},this);c=this.termStore.getCount();this.queryById("limit").setRawValue(c);this.setApiParam("limit",c);c=Ext.create("Ext.data.JsonStore",{fields:"term x y z rawFreq relativeFreq cluster isDoc".split(" "),data:r});f=Ext.create("Ext.data.JsonStore",
{fields:"term x y z rawFreq relativeFreq cluster isDoc".split(" "),data:p});a={itemId:"chart",xtype:"cartesian",interactions:["crosszoom","panzoom","itemhighlight"],plugins:{ptype:"chartitemevents"},axes:[{type:"numeric",position:"bottom",fields:["x"]},{type:"numeric",position:"left",fields:["y"]}],sprites:[{type:"text",text:a,hidden:0<this.labelsMode,x:70,y:70}],innerPadding:{top:25,right:25,bottom:25,left:25},series:[{type:"customScatter",xField:"x",yField:"y",store:c,tooltip:{trackMouse:!0,style:"background: #fff",
...
a=a.interpolate(c,l,m,2,20);b.radius=a}}},{type:"customScatter",xField:"x",yField:"y",store:f,tooltip:{trackMouse:!0,style:"background: #fff",renderer:function(a,c){this.setHtml(b.docFreqTipTemplate.apply([a.get("term"),a.get("rawFreq")]))}},marker:{type:"diamond"},highlight:{fillStyle:"yellow",strokeStyle:"black"},renderer:function(a,b,c,e){c=c.store.getAt(e);null!==c&&(c.get("cluster"),e=this.getParent().up("scatterplot"),a=.65,3===d&&(a=e.interpolate(c.get("z"),q,n,0,1)),c=e.getApplication().getColor(6),
b.fillStyle="rgba("+c.join(",")+","+a+")",b.strokeStyle="rgba("+c.join(",")+",1)",b.radius=5)}}],listeners:{itemclick:function(a,b,c){a=Ext.create("Voyant.data.model.CorpusTerm",b.record.data);this.getApplication().dispatchEvent("corpusTermsClicked",this,[a])},render:function(a){a.body.on("contextmenu",function(a,b){a.preventDefault();var c=a.getXY(),d=Ext.fly(b).getXY(),e=c[0]-d[0],d=c[1]-d[1],f=this.down("#chart").getItemForPoint(e,d);null!=f&&1!=f.record.get("isDoc")&&(e=this.down("#chart").getSeries(),
e[0].disableToolTips(),e[1].disableToolTips(),e=f.record.get("term"),d=(new Ext.Template(this.localize("removeTerm"))).apply([e]),this.chartMenu.queryById("remove").setText(d),d=(new Ext.Template(this.localize("nearbyTerm"))).apply([e]),this.chartMenu.queryById("nearby").setText(d),this.chartMenu.on("click",function(a,b){if(void 0!==b){var c=f.record.get("term");b.text===this.localize("remove")?this.removeTerm(c):this.getNearbyForTerm(c)}},this,{single:!0}),this.chartMenu.showAt(c))},this)},scope:this}};
2>this.labelsMode&&(a.series[0].label={field:"term",display:"over"},a.series[1].label={field:"term",display:"over"});a=Ext.create("Ext.chart.CartesianChart",a);this.queryById("chartParent").insert(0,a);null!==this.newTerm&&(this.selectTerm(this.newTerm),this.newTerm=null)},doLabels:function(){var a=this.queryById("chart"),b=a.getSeries(),c=a.getSurface("chart").getItems()[0];switch(this.labelsMode){case 0:b[0].getLabel().show();b[1].getLabel().show();c.show();break;case 1:b[0].getLabel().show();b[1].getLabel().show();
c.hide();break;case 2:b[0].getLabel().hide(),b[1].getLabel().hide(),c.hide()}a.redraw()},selectTerm:function(a){var b=this.down("#chart").getSeries()[0];a=b.getStore().findExact("term",a);if(-1!==a){var c=b.getStore().getAt(a),d=b.getSprites()[0];a={series:b,category:b.getItemInstancing()?"items":"markers",index:a,record:c,field:b.getYField(),sprite:d};b.setHighlightItem(a)}},getCurrentTerms:function(){var a=[];this.termStore.each(function(b){b.get("isDoc")||a.push(b.get("term"))});return a},getNearbyForTerm:function(a){var b=
...
var b={},c=this.getCurrentTerms();null!==this.newTerm&&(c.push(this.newTerm),this.setApiParam("limit",c.length));if(0<c.length){if(null!==this.newTerm||a)b.query=c.join(",");b.term=c}Ext.apply(b,this.getApiParams());"pca"===b.analysis?this.pcaStore.load({params:b}):this.caStore.load({params:b})},interpolate:function(a,b,c,d,e){return d+(e-d)*Math.max(0,Math.min(1,(a-b)/(c-b)))}});
