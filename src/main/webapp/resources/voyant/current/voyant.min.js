...
c=function(b,c,d,e){a(b,c,d);d.x=b;d.y=c;d.live=!0;e&&d.draw(f.context)},d=function(a,b,c,d,e){for(var f=0;f<=d;f+=this.COARSENESS)for(var g=0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},g;this.grid=[];k=0;g=function(){var a,g,h,l,n=this.wordsToArrange-1,s=f.canvas.width,q=f.canvas.height,r=.5*s,A=.5*q;do{h=p[k];if(void 0!==h){for(var u=Math.random()*Math.PI,v=.25*Math.random()*h.size,B=.5*(Math.random()-.5),y=.5*h.width,z=.5*h.height;;){a=
Math.floor((r+Math.cos(u)*v*this.ratio-y)/this.COARSENESS)*this.COARSENESS;g=Math.floor((A+Math.sin(u)*v-z)/this.COARSENESS)*this.COARSENESS;l=a+y>=s||g+z>=q?!0:d(a,g,h.height,h.width,h.mask);if(!l)break;u+=B;v+=.05}c(a,g,h);if(f.useFadeEffect)for(a=h.alpha=0;a<k;a++)g=p[a],1>g.alpha&&b(g);else h.alpha=1,h.draw(f.context)}k++;if(k>=p.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(n--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);
...
docFreqTip:{en:"\x3cb\x3e{0}\x3c/b\x3e\x3cbr/\x3e\x3cb\x3eWord Count\x3c/b\x3e\x3cbr/\x3e{1}\x3c/b\x3e"}},api:{analysis:"ca",limit:50,dimensions:3,bins:10,clusters:3,freqType:"raw",target:void 0,query:void 0},glyph:"xf06e@FontAwesome"},tokenFreqTipTemplate:null,docFreqTipTemplate:null,caStore:null,pcaStore:null,chartMenu:null,constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(){Ext.apply(this,{title:this.localize("title"),
caStore:Ext.create("Voyant.data.store.CAAnalysis"),pcaStore:Ext.create("Voyant.data.store.PCAAnalysis"),chartMenu:Ext.create("Ext.menu.Menu",{items:[{text:this.localize("nearby")}]}),layout:{type:"hbox",align:"stretch"},items:[{itemId:"terms",xtype:"grid",flex:1,columns:[{text:this.localize("terms"),dataIndex:"term",flex:1,sortable:!0},{text:this.localize("rawFreq"),dataIndex:"rawFreq",flex:1,sortable:!0}],selModel:Ext.create("Ext.selection.RowModel",{listeners:{selectionchange:{fn:function(a,b){this.getApplication().dispatchEvent("corpusTermsClicked",
this,b)},scope:this}},mode:"MULTI"}),store:Ext.create("Ext.data.JsonStore",{fields:[{name:"term"},{name:"rawFreq",type:"int"},{name:"relativeFreq",type:"number"}],sorters:[{property:"rawFreq",direction:"DESC"}]})}],tbar:[{text:this.localize("analysis"),itemId:"analysis",menu:{items:[{text:this.localize("pca"),itemId:"analysis_pca",group:"analysis",xtype:"menucheckitem"},{text:this.localize("ca"),itemId:"analysis_ca",group:"analysis",xtype:"menucheckitem"}],listeners:{click:function(a,b){void 0!==
b&&(b.text===this.localize("pca")?this.setApiParam("analysis","pca"):(this.setApiParam("analysis","ca"),this.getCorpus().getDocumentsCount()),this.loadFromApis())},scope:this}}},{text:this.localize("freqType"),itemId:"freqType",disabled:!0,menu:{items:[{text:this.localize("rawFreq"),itemId:"freqType_raw",group:"freqType",xtype:"menucheckitem"},{text:this.localize("relFreq"),itemId:"freqType_relative",group:"freqType",xtype:"menucheckitem"}],listeners:{click:function(a,b){void 0!==b&&(b.text===this.localize("rawFreq")?
this.setApiParam("freqType","raw"):this.setApiParam("freqType","relative"),this.loadFromApis())},scope:this}}},{text:this.localize("terms"),itemId:"limit",menu:{items:[{text:"10",itemId:"limit_10",group:"terms",xtype:"menucheckitem"},{text:"20",itemId:"limit_20",group:"terms",xtype:"menucheckitem"},{text:"30",itemId:"limit_30",group:"terms",xtype:"menucheckitem"},{text:"40",itemId:"limit_40",group:"terms",xtype:"menucheckitem"},{text:"50",itemId:"limit_50",group:"terms",xtype:"menucheckitem"},{text:"60",
itemId:"limit_60",group:"terms",xtype:"menucheckitem"},{text:"70",itemId:"limit_70",group:"terms",xtype:"menucheckitem"},{text:"80",itemId:"limit_80",group:"terms",xtype:"menucheckitem"},{text:"90",itemId:"limit_90",group:"terms",xtype:"menucheckitem"},{text:"100",itemId:"limit_100",group:"terms",xtype:"menucheckitem"}],listeners:{click:function(a,b){void 0!==b&&(this.setApiParam("limit",parseInt(b.text)),this.loadFromApis())},scope:this}}},{text:this.localize("clusters"),itemId:"clusters",menu:{items:[{text:"1",
...
b){var c=function(a){var b=this.getApiParam(a);this.getDockedItems("toolbar")[0].down("#"+a).down("#"+a+"_"+b).setChecked(!0)}.bind(this);c("analysis");c("freqType");c("limit");c("clusters");this.setCorpus(b);this.caStore.setCorpus(b);this.pcaStore.setCorpus(b);this.loadFromApis()},this);this.caStore.on("load",function(a,b){this.buildChart(a)},this);this.pcaStore.on("load",function(a,b){this.buildChart(a)},this);this.tokenFreqTipTemplate=new Ext.Template(this.localize("tokenFreqTip"));this.docFreqTipTemplate=
new Ext.Template(this.localize("docFreqTip"));this.callParent(arguments)},buildChart:function(a){var b=this,c=this.queryById("chart");null!==c&&c.remove();a=a.getAt(0).getTokens();var d=0,e=Number.MAX_VALUE,g=[];a.forEach(function(a){var b=parseInt(a.get("rawFreq"),10),c="part"===a.get("category");c||(b>d&&(d=b),b<e&&(e=b));a={term:a.get("term"),rawFreq:b,relativeFreq:a.get("relativeFreq"),cluster:a.get("cluster"),x:a.get("vector")[0],y:a.get("vector")[1]};a.isDoc=c;g.push(a)},this);this.queryById("terms").getStore().loadRawData(g,
!1);a={itemId:"chart",xtype:"cartesian",width:"50%",store:Ext.create("Ext.data.JsonStore",{fields:"term x y rawFreq cluster isDoc".split(" "),data:g}),interactions:["panzoom","itemhighlight"],plugins:{ptype:"chartitemevents"},axes:[{type:"numeric",position:"bottom",fields:["x"]},{type:"numeric",position:"left",fields:["y"]}],series:[{type:"scatter",xField:"x",yField:"y",label:{field:"term",display:"over"},tooltip:{trackMouse:!0,style:"background: #fff",renderer:function(a,c){a.get("isDoc")?this.setHtml(b.docFreqTipTemplate.apply([a.get("term"),
a.get("rawFreq")])):this.setHtml(b.tokenFreqTipTemplate.apply([a.get("term"),a.get("rawFreq"),a.get("relativeFreq")]))}},renderer:function(a,b,c,g){c=c.store.getAt(g);g=parseInt(c.get("cluster"),10);a=this.getParent().up("scatterplot");-1===g&&(g=0);g=a.getApplication().getColor(g);b.fillStyle="rgba("+g.join(",")+",0.5)";b.strokeStyle="rgba("+g.join(",")+",1)";c=c.get("rawFreq");a=a.interpolate(c,e,d,2,20);b.radius=a}}],listeners:{itemclick:function(a,b,c){a=Ext.create("Voyant.data.model.CorpusTerm",
b.record.data);this.getApplication().dispatchEvent("corpusTermsClicked",this,[a])},render:function(a){a.body.on("contextmenu",function(a,b){a.preventDefault();var c=a.getXY(),d=Ext.fly(b).getXY(),e=c[0]-d[0],d=c[1]-d[1],f=this.down("#chart").getItemForPoint(e,d);null!=f&&(this.chartMenu.on("click",function(a,b){if(void 0!==b&&b.text!==this.localize("remove")){var c=Math.max(2E3,Math.round(this.getCorpus().getWordTokensCount()/100)),d=this.getCurrentTerms();this.setApiParams({limit:c,target:f.record.get("term"),
query:d});this.loadFromApis();this.setApiParam("target",void 0)}},this,{single:!0}),this.chartMenu.showAt(c))},this)},scope:this}};a=Ext.create("Ext.chart.CartesianChart",a);this.insert(0,a)},getCurrentTerms:function(){var a=[];this.queryById("chart").getStore().each(function(b){b.get("isDoc")||a.push(b.get("term"))});return a},loadFromApis:function(){var a={};Ext.apply(a,this.getApiParams());delete a.stopList;"pca"===a.analysis?this.pcaStore.load({params:a}):this.caStore.load({params:a})},interpolate:function(a,
