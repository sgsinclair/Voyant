...
codeModeTitle:"Code Mode",codeModeTip:"Select from multiple code formats for this cell.",configureTip:"Configuration Options",autoExecuteOnLoad:"auto-run this cell on page load",ok:"OK",cancel:"Cancel"}},config:{isRun:!1,autoExecute:!1,isWarnedAboutPreviousCells:!1},layout:{type:"vbox",align:"stretch"},height:130,border:!1,EMPTY_RESULTS_TEXT:" ",constructor:function(a){this.results=this._getResultsComponent(Ext.Array.from(a.output).join(""));this.editor=Ext.create("Voyant.notebook.editor.CodeEditor",
{content:Ext.Array.from(a.input).join("\n"),docs:a.docs,mode:"ace/mode/"+(a.mode?a.mode:"javascript")});Ext.apply(this,{dockedItems:[{xtype:"toolbar",dock:"left",defaults:{textAlign:"left"},items:[{xtype:"notebookwrapperadd"},{xtype:"notebookwrapperrun",listeners:{click:{fn:this.run,scope:this}}},{glyph:"xf050@FontAwesome",tooltip:this.localize("runMultiple"),itemId:"runMultiple",listeners:{click:{fn:function(a,c){Ext.create("Ext.menu.Menu",{items:[{text:this.localize("runUntil"),tooltip:this.localize("runUntilTip"),
glyph:"xf049@FontAwesome",handler:function(){this.up("notebook").runUntil(this)},scope:this},{text:this.localize("runFrom"),tooltip:this.localize("runFromTip"),glyph:"xf050@FontAwesome",handler:function(){this.up("notebook").runFrom(this)},scope:this}]}).showAt(c.pageX,c.pageY)},scope:this}}},{xtype:"button",glyph:"xf013@FontAwesome",tooltip:this.localize("configureTip"),scope:this,cls:a.autoExecute?"autoExecute":"",handler:function(a,c){Ext.create("Ext.menu.Menu",{items:[{xtype:"menucheckitem",text:this.localize("autoExecuteOnLoad"),
hidden:0==this.results.isVisible(),itemId:"toggleAutoExecute",checked:this.getAutoExecute(),handler:function(b,c){this.setAutoExecute(b.checked);b.up("menu").close();a.toggleCls("autoExecute",b.checked);this.up("notebook").setIsEdited(!0)},scope:this},{text:this.localize("codeMode"),glyph:"xf1c9@FontAwesome",listeners:{click:function(){var a=this,b=this.editor.getMode().split("/").pop();(new Ext.Window({title:this.localize("codeModeTitle"),layout:"fit",width:200,items:[{xtype:"form",layout:{type:"vbox",
align:"stretch"},bodyPadding:10,items:[{xtype:"fieldset",title:this.localize("modeCode"),items:{xtype:"radiofield",boxLabel:this.localize("modeJavascript"),name:"codeMode",inputValue:"javascript",flex:1,checked:"javascript"==b}},{xtype:"fieldset",title:this.localize("modeData"),items:[{items:{xtype:"radiofield",boxLabel:this.localize("modeJson"),name:"codeMode",inputValue:"json",flex:1,checked:"json"==b}},{items:{xtype:"radiofield",boxLabel:this.localize("modeText"),name:"codeMode",inputValue:"text",
flex:1,checked:"text"==b}},{items:{xtype:"radiofield",boxLabel:this.localize("modeHtml"),name:"codeMode",inputValue:"html",flex:1,checked:"html"==b}},{items:{xtype:"radiofield",boxLabel:this.localize("modeXml"),name:"codeMode",inputValue:"xml",flex:1,checked:"xml"==b}}]}]}],buttons:[{text:this.localize("ok"),handler:function(){var b=this.up("window");values=b.down("form").getForm().getValues();a.switchModes(values.codeMode);b.close()}},{text:this.localize("cancel"),handler:function(){this.up("window").close()}}]})).show()},
scope:this}}]}).showAt(c.pageX,c.pageY)}}]},{xtype:"toolbar",dock:"right",items:[{xtype:"notebookwrappercounter",order:a.order,name:a.cellId},{xtype:"notebookwrapperremove"},{xtype:"notebookwrappermoveup"},{xtype:"notebookwrappermovedown"}]}],items:[this.editor,this.results]});void 0!==a.uiHtml&&this.items.push(this._getUIComponent(a.uiHtml));this.callParent(arguments)},initComponent:function(a){var b=this;b.on("afterrender",function(){this.editor&&"ace/mode/javavscript"!=this.editor.getMode()&&this.switchModes(this.editor.getMode(),
!0);this.getTargetEl().on("resize",function(a){var c=20;b.items.each(function(a){c+=a.getHeight()});b.setSize({height:c})})},this);b.callParent(arguments)},switchModes:function(a,b){var c=-1<a.indexOf("javascript");this.down("notebookwrapperrun").setVisible(c);this.queryById("runMultiple").setVisible(c);this.results.setVisible(c);b||this.editor.switchModes(a)},run:function(a){if("ace/mode/javascript"==this.editor.getMode()){if(!0===a||this.getIsWarnedAboutPreviousCells())return this._run();var b=
this.up("notebook");Ext.Array.each(b.query("notebookcodeeditorwrapper"),function(a){if(a==this)return this._run(),!1;if(a.editor&&"ace/mode/javascript"==a.editor.getMode()&&0==a.getIsRun())return Ext.Msg.confirm(this.localize("previousNotRunTitle"),this.localize("previousNotRun"),function(a){"yes"==a?b.runUntil(this):this._run()},this),this.setIsWarnedAboutPreviousCells(!0),!1},this)}},_run:function(){this.results.show();this.results.update(this.EMPTY_RESULTS_TEXT);this.results.mask("working\u2026");
var a=this.editor.getValue();Voyant.notebook.util.Show.TARGET=this.results.getEl();Voyant.notebook.Notebook.currentBlock=this;Voyant.notebook.Notebook.currentNotebook.setCurrentBlock(this);try{var b=eval.call(window,a)}catch(d){return this.results.unmask(),Voyant.notebook.util.Show.showError(d),this.getTargetEl().fireEvent("resize"),d}this.setIsRun(!0);if(void 0!==b)if(b.then&&b.catch&&b.finally){var c=this;b.then(function(a){c.results.unmask();void 0!==a&&c._showResult(a)}).catch(function(a){c.results.unmask();
Voyant.notebook.util.Show.showError(a)}).finally(function(){Ext.defer(function(){this.getTargetEl().fireEvent("resize")},50,c)})}else this.results.unmask(),this._showResult(b),this.getTargetEl().fireEvent("resize");else this.results.unmask(),this.getTargetEl().fireEvent("resize");return b},_getResultsComponent:function(a){return Ext.create("Ext.Component",{align:"stretch",cls:"notebook-code-results",html:a,getValue:function(){var a=this.getTargetEl(),c=a.dom.cloneNode(!0),d=c.innerHTML;c.style.height||
(d="\x3cdiv style\x3d'height: "+a.getHeight()+"px'\x3e"+d+"\x3c/div\x3e");return d}})},_getUIComponent:function(a){return Ext.create("Ext.Component",{align:"stretch",cls:"notebook-code-ui",padding:"20 10",html:a,getValue:function(){return this.getTargetEl().dom.cloneNode(!0).innerHTML}})},_showResult:function(a){this.results.getEl().dom.innerHTML===this.EMPTY_RESULTS_TEXT&&this.results.update(a.toString?a.toString():a)},clearResults:function(){this.results.show();var a=this.results.el.down(".x-panel");
if(a){var b=Ext.getCmp(a.id);b?b.destroy():a.destroy()}else this.results.update(" ");this.getTargetEl().fireEvent("resize")},tryToUnmask:function(){Spyral&&Spyral.promises&&Ext.defer(this.tryToUnmask,20,this);if(0===Voyant.application.getDeferredCount()){for(var a in window)"object"==typeof window[a]&&window[a]&&"opener"!=a&&window[a].isFulfilled&&window[a].isFulfilled()&&(window[a]=window[a].valueOf());this.results.unmask();0===this.results.getTargetEl().getHtml().trim().length&&this.results.hide();
this.getTargetEl().fireEvent("resize")}else Ext.defer(this.tryToUnmask,20,this)},getContent:function(){var a={input:this.editor.getValue(),mode:this.editor.getMode().split("/").pop()};if("javascript"===a.mode){a.output=this.results.getValue();var b=this.down('component[cls\x3d"notebook-code-ui"]');null!==b&&(a.ui=b.getValue())}return a},setContentAndMode:function(a,b,c){debugger;this.editor.setContent(a);this.switchModes(b||"javascript")}});Ext.define("Voyant.notebook.editor.TextEditor",{extend:"Ext.Component",mixins:["Voyant.util.Localization"],alias:"widget.notebooktexteditor",cls:"notebook-text-editor",config:{ckeditorConfig:{toolbar:[{name:"basicstyles",items:["Bold","Italic","-","RemoveFormat"]},{name:"paragraph",items:"NumberedList BulletedList - Justify Outdent Indent Blockquote JustifyLeft JustifyCenter JustifyRight".split(" ")},{name:"colors",items:["TextColor","BGColor"]},{name:"styles",items:["Styles","Format"]},{name:"links",
...
init:function(){window.Corpus=Spyral.Corpus;window.Table=Spyral.Table;window.loadCorpus=function(){return Spyral.Corpus.load.apply(Spyral.Corpus.load,arguments)};window.createTable=function(){return Spyral.Table.create.apply(Spyral.Table,arguments)};Ext.Ajax.request({url:this.getApplication().getBaseUrlFull()+"resources/spyral/docs/spyral.json",callback:function(a,b,c){b&&(this.spyralTernDocs=Ext.JSON.decode(c.responseText),this.spyralTernDocs.Corpus=this.spyralTernDocs.Spyral.Corpus,this.spyralTernDocs.Table=
this.spyralTernDocs.Spyral.Table,this.spyralTernDocs.loadCorpus=this.spyralTernDocs.Spyral.Corpus.load,this.spyralTernDocs.createTable=this.spyralTernDocs.Spyral.Table.create);a=Ext.Object.fromQueryString(document.location.search,!0);b=Ext.isDefined(a.run);c=/\/spyral\/([\w-]+)\/?$/.exec(location.pathname);var d=Ext.isDefined(a.githubId);"inputJsonArrayOfEncodedBase64"in a?Ext.decode(a.inputJsonArrayOfEncodedBase64).forEach(function(a){a=decodeURIComponent(atob(a));0==a.trim().indexOf("\x3c")?this.addText(a):
this.addCode(a)},this):a.input?0===a.input.indexOf("http")&&this.loadFromUrl(a.input,b):c?(this.loadFromId(c[1]),this.setStorageSolution("voyant")):d?(this.githubDialogs.loadFileFromId(a.githubId),this.setStorageSolution("github")):this.addNew();if(b){var e=this;Ext.defer(function(){e.runAll()},100)}},scope:this})},setBlock:function(a,b,c,d){a=a||"";b=b||1;d=d||{};var e=this.query("notebookcodeeditorwrapper"),f=this.getCurrentBlock().id,g=e.findIndex(function(a){return a.id==f});if(0>g+b||g+b>e.length)Ext.Msg.show({title:this.localize("error"),
msg:this.localize("blockDoesNotExist"),buttons:Ext.MessageBox.OK,icon:Ext.MessageBox.ERROR});else return e[g+b]&&e[g+b].remove(),this.addCode(Object.assign({},{input:a,mode:c||"text"},d),g+b)},getBlock:function(a,b){a=a||0;b=b||{};var c=this.query("notebookcodeeditorwrapper"),d=this.getCurrentBlock().id,e=c.findIndex(function(a){return a.id==d});if(0>e+a||e+a>c.length-1)"failQuietly"in b&&b.failQuietly||Ext.Msg.show({title:this.localize("error"),msg:this.localize("blockDoesNotExist"),buttons:Ext.MessageBox.OK,
icon:Ext.MessageBox.ERROR});else return content=c[e+a].getContent(),content.input},addNew:function(){this.setMetadata(new Spyral.Metadata({title:"\x3ch1\x3eSpyral Notebook\x3c/h1\x3e"}));this.addText("\x3cp\x3eThis is a Spyral Notebook, a dynamic document that combines writing, code and data in service of reading, analyzing and interpreting digital texts.\x3c/p\x3e\x3cp\x3eSpyral Notebooks are composed of text blocks (like this one) and code blocks (like the one below). You can \x3cspan class\x3d'marker'\x3eclick on the blocks to edit\x3c/span\x3e them and add new blocks by clicking add icon that appears in the left column when hovering over a block.\x3c/p\x3e");
