...
0==a.scrollTop?0:a.scrollHeight-a.scrollTop==a.clientHeight?1:(a.scrollTop+.5*a.clientHeight)/a.scrollHeight;a=$(a).find(".readerContainer").find(".word");var c=a.first()[0],d=a.last()[0];if(void 0!==c&&void 0!==d){a=this.getCorpus();var e=!1,c=Voyant.data.model.Token.getInfoFromElement(Ext.get(c)),d=Voyant.data.model.Token.getInfoFromElement(Ext.get(d));0!==c.position&&(e=!0);for(var f={},g=0,h=c.docIndex;h<=d.docIndex;){var k=a.getDocument(h).get("tokensCount-lexical");h===d.docIndex&&(k=d.position);
h===c.docIndex&&(k-=c.position);g+=k;f[h]=k;h++}g=Math.round(g*b);h=b=0;for(k=c.docIndex;k<=d.docIndex&&!(b=k,h+=f[k],h>=g);k++);d=f[b]-(h-g);e&&b===c.docIndex&&(d+=c.position);a=d/a.getDocument(b).get("tokensCount-lexical");e=this.query("cartesian")[b];a=e.getX()+e.getWidth()*a;Ext.get(this.getLocationMarker()).setX(a)}this.setLastLocationUpdate(new Date)},generateChart:function(a,b){function c(a,b){var c=[[116,116,181],[139,163,83],[189,157,60],[171,75,75],[174,61,155]],c=c[a%c.length];return"rgba("+
c[0]+","+c[1]+","+c[2]+","+b+")"}function d(d,e){var f=d.index,g=d.fraction,h=d.relativeHeight,k=c(f,.3),f=c(f,1);b.add({xtype:"cartesian",plugins:{ptype:"chartitemevents"},flex:g,height:"100%",insetPadding:0,background:{type:"linear",degrees:90,stops:[{offset:0,color:k},{offset:h,color:k},{offset:h,color:"white"},{offset:1,color:"white"}]},axes:[{type:"numeric",position:"left",fields:"distribution",hidden:!0},{type:"category",position:"bottom",fields:"bin",hidden:!0}],series:[{type:"line",xField:"bin",
yField:"distribution",style:{lineWidth:2,strokeStyle:f},tooltip:{corpus:a,trackMouse:!0,style:"background: #fff",showDelay:0,dismissDelay:500,hideDelay:5,renderer:function(b,c){this.setHtml(a.getDocument(b.get("docIndex")).getTitle()+"\x3cbr\x3e"+b.get("term")+": "+b.get("distribution"))}}}],store:Ext.create("Ext.data.ArrayStore",{fields:["docId","docIndex","bin","distribution","term"],data:[]}),listeners:{itemclick:function(a,b,c){a=b.record.data;a=this.getDocumentsStore().getAt(a.docIndex);this.getApplication().dispatchEvent("documentsClicked",
this,[a])},scope:e}}).body.on("click",function(a,b){var c=Ext.get(b),d=a.getX(),e=c.getBox(),d=(d-e.x)/e.width,c=c.parent(".x-panel"),e=c.parent(),c=Ext.toArray(e.dom.children).indexOf(c.dom),c=this.getDocumentsStore().getAt(c),e=c.get("tokensCount-lexical"),d=Math.floor(e*d)-this.getApiParam("limit")/2;this.setApiParams({skipToDocId:c.getId(),start:0>d?0:d});this.load(!0)},e)}b.removeAll();var e=a.getDocuments();if(50>e.getTotalCount()){for(var f=a.getWordTokensCount(),g=[],h=1E6,k=-1,l=0;l<e.getCount();l++){var m=
e.getAt(l),n=m.get("index"),m=m.get("tokensCount-lexical");m<h&&(h=m);m>k&&(k=m);g.push({index:n,count:m,fraction:m/f})}for(l=0;l<g.length;l++)m=g[l],m.relativeHeight=m.count==k?1:0+(m.count-h)/(k-h)*1,d(m,this)}},highlightKeywords:function(a,b){Ext.isArray(a)||(a=[a]);this.innerContainer.first().select("span[class*\x3dkeyword]").removeCls("keyword");var c=[],d=new RegExp("^"+a[0]+"$","i");this.innerContainer.first().select("span.word").each(function(a,b,g){a.dom.firstChild&&a.dom.firstChild.nodeValue.match(d)&&
(a.addCls("keyword"),c.push(a.dom))})},fetchPrevious:function(){var a=this.innerContainer.first(),b=a.first(".word");if(null!=b&&!1===b.hasCls("loading"))for(;b;){if(b.hasCls("word")){var a=Voyant.data.model.Token.getInfoFromElement(b),c=a.docIndex,a=a.position,d=this.getDocumentsStore().getAt(c),e=this.getApiParam("limit"),f=!1;if(0===c&&0===a)break;0<c&&0===a?(f=!0,c--,d=this.getDocumentsStore().getAt(c),c=d.get("tokensCount-lexical"),a=c-e,0>a&&(a=0,this.setApiParam("limit",c))):(e--,a-=e);0>a&&
(a=0);b.insertSibling("\x3cdiv class\x3d'loading'\x3e"+this.localize("loading")+"\x3c/div\x3e","before",!1).mask();f||b.destroy();b=d.getId();this.setApiParams({skipToDocId:b,start:a});this.setScrollDownwards(!1);this.load();this.setApiParam("limit",this.INITIAL_LIMIT);break}b.destroy();b=a.first()}},fetchNext:function(){var a=this.innerContainer.first(),b=a.last();if(!1===b.hasCls("loading"))for(var c=$(a.dom).contents().filter(function(){return 3===this.nodeType}).last();b;){if(b.hasCls("word")){var a=
Voyant.data.model.Token.getInfoFromElement(b),d=a.docIndex,e=a.position,f=this.getDocumentsStore().getAt(a.docIndex),g=f.getId(),f=f.get("tokensCount-lexical");if(e+this.getApiParam("limit")>=f&&d==this.getCorpus().getDocumentsCount()-1)if(d=f-e,1>=d)break;else this.setApiParam("limit",d);b.el.dom.nextSibling===c[0]&&c.remove();b.insertSibling("\x3cdiv class\x3d'loading'\x3e"+this.localize("loading")+"\x3c/div\x3e","after",!1).mask();b.destroy();this.setApiParams({skipToDocId:g,start:a.position});
this.setScrollDownwards(!0);this.load();this.setApiParam("limit",this.INITIAL_LIMIT);break}b.destroy();b=a.last()}},load:function(a,b){a&&(this.innerContainer.first().destroy(),this.innerContainer.setHtml('\x3cdiv class\x3d"readerContainer"\x3e\x3cdiv class\x3d"loading"\x3e'+this.localize("loading")+"\x3c/div\x3e\x3c/div\x3e"),this.innerContainer.first().first().mask());this.getTokensStore().load(Ext.apply(b||{},{params:Ext.apply(this.getApiParams(),{stripTags:"blocksOnly"})}))},updateText:function(a){var b=
this.innerContainer.down(".loading");b&&b.destroy();b=this.getScrollDownwards()?"beforeEnd":"afterBegin";this.innerContainer.first().insertHtml(b,a);a=this.down('panel[region\x3d"center"]').body.dom;this.updateLocationMarker(a)},updateChart:function(){}});Ext.define("Voyant.panel.ScatterPlot",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],requires:["Ext.chart.CartesianChart"],alias:"widget.scatterplot",config:{corpus:void 0,options:{xtype:"stoplistoption"}},statics:{i18n:{title:{en:"ScatterPlot"},analysis:{en:"Analysis"},ca:{en:"Correspondence Analysis"},pca:{en:"Principal Components Analysis"},rawFreq:{en:"Raw Frequency"},relFreq:{en:"Relative Frequency"},terms:{en:"Terms"},term:{en:"Term"},numTerms:{en:"Terms Count"},addTerm:{en:"Add Term"},
