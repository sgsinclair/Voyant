...
params:{params:{},type:a},fullPath:e.path,pathMin:e.min,pathMax:e.max,colour:d};c===this.lastSlippery&&(this.isSliderVisible&&this.sliderOverlayOff(c),this.lastSlippery=null);this.chart.select("g[class\x3dfrequency-line-"+c+"]").remove();this.overlayQueue.push(d);this.chartOverlayOn(d);this.isSliderVisible&&this.sliderOverlayOn(d)},doTermDeselect:function(a,b){var c=this.removeFilteredCharacters(a),d=this.getTermIndex(a);if(!0===b){var e=this.parent.query("[xtype\x3dlegend]")[0],d=e.getStore().findExact("name",
a);e.getStore().removeAt(d)}for(var e=this.parent.getApiParam("selectedWords"),f=0,g=e.length;f<g;f++)e[f]===c&&(e.splice(f,1),this.parent.setApiParams({selectedWords:e}));this.chartOverlayOff(c);this.overlayQueue.splice(d,1);this.isSliderVisible&&this.sliderOverlayOff(c);this.lastSticky=c},prepareFullPath:function(a){for(var b=[],c=this.absMaxFreq,d=this.absMinFreq,e=0,f=this.allData.length;e<f;e++){for(var g=foundB=0,h=this.allData[e].allDataInternal.length;g<h;g++)for(var k=0,l=this.allData[e].allDataInternal[g].frequencyArray.length;k<
