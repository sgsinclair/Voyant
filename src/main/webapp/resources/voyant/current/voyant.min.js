...
this.loadQueryTerms(d)},corpusTermsClicked:function(a,c){var d=[];c.forEach(function(a){a.getTerm()&&d.push(a.getTerm())});this.loadQueryTerms(d)},documentTermsClicked:function(a,c){var d=[];c.forEach(function(a){a.getTerm()&&d.push(a.getTerm())});this.loadQueryTerms(d)},documentsClicked:function(a,c,d){c&&(this.setApiParams({skipToDocId:c[0].getId(),start:0}),this.load(!0))},termLocationClicked:function(a,c){if(void 0!==c[0]){var d=c[0],e=d.get("docIndex"),f=d.get("position"),g=f-this.getApiParam("limit")/
2,h=this.getCorpus().getDocument(e);this.setApiParams({skipToDocId:h.getId(),start:0>g?0:g});this.load(!0,{callback:function(){var a=this.body.dom.querySelector("#_"+e+"_"+f);a&&a.scrollIntoView();this.highlightKeywords(d.get("term"),!1)},scope:this})}},scope:this}});this.callParent(arguments)},loadQueryTerms:function(a){a&&0<a.length&&this.getDocumentTermsStore().load({params:{query:a,docIndex:void 0,docId:void 0,page:void 0,start:void 0,limit:void 0}})},updateLocationMarker:function(a){var b;b=
0==a.scrollTop?0:a.scrollHeight-a.scrollTop==a.clientHeight?1:(a.scrollTop+.5*a.clientHeight)/a.scrollHeight;var c=$(a).find(".readerContainer").find(".word");a=c.first()[0];var d=c.last()[0];if(void 0!==a&&void 0!==d){var e=this.getCorpus(),c=!1,f=Voyant.data.model.Token.getInfoFromElement(Ext.get(a)),d=Voyant.data.model.Token.getInfoFromElement(Ext.get(d));0!==f.position&&(c=!0);a={};var g=0;if(f.docIndex===d.docIndex){var g=d.position-f.position,h=e.getDocument(f.docIndex).get("tokensCount-lexical");
a[f.docIndex]=h}else for(var k=f.docIndex;k<=d.docIndex;)h=e.getDocument(k).get("tokensCount-lexical"),c&&k===f.docIndex?(h-=f.position,g+=h):g=k===d.docIndex?g+d.position:g+h,a[k]=h,k++;e=Math.round(g*b);b=0;if(f.docIndex===d.docIndex)b=f.docIndex,c=e;else if(c&&0===e)b=f.docIndex,c=f.position;else{c=0;for(f=f.docIndex;f<=d.docIndex&&!(b=f,c+=a[f],c>=e);f++);c=a[b]-(c-e)}a=c/a[b];b=this.query("cartesian")[b];a=b.getX()+b.getWidth()*a;Ext.get(this.getLocationMarker()).setX(a)}this.setLastLocationUpdate(new Date)},
...
[],h=1E6,k=-1,m=0;m<e.getCount();m++){var l=e.getAt(m),n=l.get("index"),l=l.get("tokensCount-lexical");l<h&&(h=l);l>k&&(k=l);g.push({index:n,count:l,fraction:l/f})}for(m=0;m<g.length;m++)l=g[m],l.relativeHeight=l.count==k?1:0+(l.count-h)/(k-h)*1,d(l,this)}},highlightKeywords:function(a,b){Ext.isArray(a)||(a=[a]);this.innerContainer.first().select("span[class*\x3dkeyword]").removeCls("keyword");var c=[],d=new RegExp("^"+a[0]+"$","i");this.innerContainer.first().select("span.word").each(function(a,
b,g){a.dom.firstChild&&a.dom.firstChild.nodeValue.match(d)&&(a.addCls("keyword"),c.push(a.dom))})},fetchPrevious:function(){var a=this.innerContainer.first(),b=a.first(".word");if(null!=b&&!1===b.hasCls("loading"))for(;b;){if(b.hasCls("word")){var a=Voyant.data.model.Token.getInfoFromElement(b),c=a.docIndex,a=a.position,d=this.getDocumentsStore().getAt(c),e=this.getApiParam("limit"),f=!1;if(0===c&&0===a)break;0<c&&0===a?(f=!0,c--,d=this.getDocumentsStore().getAt(c),c=d.get("tokensCount-lexical"),
a=c-e,0>a&&(a=0,this.setApiParam("limit",c))):(e--,a-=e);0>a&&(a=0);b.insertSibling("\x3cdiv class\x3d'loading'\x3e"+this.localize("loading")+"\x3c/div\x3e","before",!1).mask();f||b.destroy();b=d.getId();this.setApiParams({skipToDocId:b,start:a});this.setScrollDownwards(!1);this.load();this.setApiParam("limit",this.INITIAL_LIMIT);break}b.destroy();b=a.first()}},fetchNext:function(){var a=this.innerContainer.first(),b=a.last();if(!1===b.hasCls("loading"))for(var c=$(a.dom).contents().filter(function(){return 3===
this.nodeType}).last();b;){if(b.hasCls("word")){var a=Voyant.data.model.Token.getInfoFromElement(b),d=a.docIndex,e=a.position,f=this.getDocumentsStore().getAt(a.docIndex),g=f.getId(),f=f.get("tokensCount-lexical");if(e+this.getApiParam("limit")>=f&&d==this.getCorpus().getDocumentsCount()-1)if(d=f-e,1>=d)break;else this.setApiParam("limit",d);b.el.dom.nextSibling===c[0]&&c.remove();b.insertSibling("\x3cdiv class\x3d'loading'\x3e"+this.localize("loading")+"\x3c/div\x3e","after",!1).mask();b.destroy();
this.setApiParams({skipToDocId:g,start:a.position});this.setScrollDownwards(!0);this.load();this.setApiParam("limit",this.INITIAL_LIMIT);break}b.destroy();b=a.last()}},load:function(a,b){a&&(this.innerContainer.first().destroy(),this.innerContainer.setHtml('\x3cdiv class\x3d"readerContainer"\x3e\x3cdiv class\x3d"loading"\x3e'+this.localize("loading")+"\x3c/div\x3e\x3c/div\x3e"),this.innerContainer.first().first().mask());this.getTokensStore().load(Ext.apply(b||{},{params:Ext.apply(this.getApiParams(),
