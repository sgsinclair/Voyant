...
a/30;this.MAX_LINE_WIDTH=a-this.maxRadius/2},calculateGraphHeights:function(){var a=.5*this.maxRadius;if(this.SEPARATE_LINES_FOR_TERMS){var b=[],c;for(c in this.currentTerms)b.push(c);this.cache.each(function(c,d,g){d=this.maxRadius*b.length;for(g=0;g<b.length;g++)c.terms[b[g]]||(d-=this.maxRadius);0==d&&(d=this.maxRadius);c.height=d+a},this)}else{var d=Math.max(this.maxRadius,this.MIN_GRAPH_SEPARATION);this.cache.each(function(b,c,g){b.height=d+a},this)}},findLongestDocument:function(a){a=a.getTotalWordTokens();
...
this.doDrag()},drawDocument:function(a,b,c){var d=a.lineLength,e=this.MAX_LABEL_WIDTH-a.titleWidth,f=5;this.ctx.textBaseline="top";this.ctx.font="bold 12px Verdana";if(null!=this.dragInfo&&this.dragInfo.oldIndex==b)this.ctx.fillStyle="rgba(128, 128, 128, 0.5)",f+=e,this.ctx.fillText(a.title,f,this.yIndex),this.SEPARATE_LINES_FOR_TERMS&&(this.yIndex+=a.height);else{c=function(){f=g.MAX_LABEL_WIDTH+g.maxRadius;g.ctx.strokeStyle="rgba(128, 128, 128, 1.0)";g.ctx.fillStyle="rgba(128, 128, 128, 1.0)";g.ctx.lineWidth=
.25;g.ctx.beginPath();g.ctx.moveTo(f,g.yIndex-6);g.ctx.lineTo(f,g.yIndex+6);g.ctx.stroke();g.ctx.beginPath();g.ctx.moveTo(f,g.yIndex);g.ctx.lineTo(f+d,g.yIndex);g.ctx.stroke();g.ctx.beginPath();g.ctx.moveTo(f+d,g.yIndex-6);g.ctx.lineTo(f+d,g.yIndex+6);g.ctx.stroke()};this.DRAW_TITLES&&(f+=e,this.ctx.fillStyle="rgba(128, 128, 128, 1.0)",e=a.title,this.DRAW_SHORT_TITLES&&(e=b+1+")"),this.ctx.fillText(e,f,this.yIndex));this.yIndex+=4;var g=this,e=[],h;for(h in this.currentTerms)e.push(h);this.SEPARATE_LINES_FOR_TERMS?
...
"rgba(0, 0, 0, 0.75)",this.ctx.fillStyle="rgba("+p+", 0.5)",this.ctx.lineWidth=1,x=!0);this.ctx.beginPath();this.ctx.arc(u.x+f,this.yIndex,u.radius,0,h,!0);this.ctx.closePath();this.ctx.fill();this.ctx.stroke();x&&(this.ctx.strokeStyle="rgba("+p+", 1)",this.ctx.fillStyle="rgba("+p+", 0.35)",this.ctx.lineWidth=.25)}}a.freqCounts[n]=s;this.SEPARATE_LINES_FOR_TERMS&&(this.ctx.fillStyle="rgba(0, 0, 0, 1.0)",this.ctx.font="10px Verdana",this.ctx.fillText(s,f+d+5,this.yIndex-4),this.yIndex+=this.maxRadius)}}this.SEPARATE_LINES_FOR_TERMS&&
0==l&&(c(),this.ctx.fillStyle="rgba(0, 0, 0, 1.0)",this.ctx.font="10px Verdana",this.ctx.fillText(0,f+d+5,this.yIndex-4),this.yIndex+=this.maxRadius);f+=d;this.SEPARATE_LINES_FOR_TERMS||(this.ctx.fillStyle="rgba(0, 0, 0, 1.0)",this.ctx.font="10px Verdana",this.ctx.fillText(e,f+5,this.yIndex-4))}this.yIndex=this.SEPARATE_LINES_FOR_TERMS?this.yIndex+.5*this.maxRadius:this.yIndex+a.height;this.yIndex-=4},drawLegend:function(){var a=this.MAX_LABEL_WIDTH+this.maxRadius;this.ctx.textBaseline="top";this.ctx.font=
...
c,e>=this.cache.getCount()?e=this.cache.getCount()-1:0>e&&(e=c+this.maxRadius>this.canvas.height?this.cache.getCount()-1:0),this.dragInfo.newIndex=e,document.body.style.cursor="move";else if(0<=e&&e<this.cache.getCount())if(0<=b){var f=!1,b=b-this.maxRadius,g=this.cache.get(e);if(b>=g.lineLength)this.overBubbles=[{docIndex:e,x:a.layerX+10,y:a.layerY+10}];else{var b=Math.round(b/(g.lineLength/50)),h=this.maxRadius;0<e&&(h=d-(this.cache.get(e).height-.75*this.maxRadius));var h=Math.round((c-h)/this.maxRadius),
k=0,m;for(m in g.terms)if(void 0!==this.currentTerms[m]){var l=g.terms[m];l?(this.SEPARATE_LINES_FOR_TERMS&&k==h||!this.SEPARATE_LINES_FOR_TERMS)&&l.pos[b]&&0<l.pos[b].radius&&(f=!0,this.overBubbles.push({label:m,type:l,docId:g.id,docIndex:e,xIndex:b,yIndex:h,freq:l.pos[b].freq,id:l.pos[b].id,tokenPositions:l.pos[b].tokenPositions,x:a.layerX+10,y:a.layerY+10})):this.SEPARATE_LINES_FOR_TERMS&&k--;k++}}document.body.style.cursor=f?"pointer":"auto"}else document.body.style.cursor="move";else document.body.style.cursor=
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];m=0;f=function(){var a,f,h,k,t=this.wordsToArrange-1,r=g.canvas.width,n=g.canvas.height,q=.5*r,s=.5*n;do{h=p[m];if(void 0!==h){for(var v=Math.random()*Math.PI,w=.25*Math.random()*h.size,A=.5*(Math.random()-.5),y=.5*h.width,z=.5*h.height;;){a=Math.floor((q+Math.cos(v)*w*this.ratio-y)/this.COARSENESS)*this.COARSENESS;f=Math.floor((s+Math.sin(v)*w-z)/this.COARSENESS)*this.COARSENESS;
k=a+y>=r||f+z>=n?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;v+=A;w+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<m;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}m++;if(m>=p.length){clearTimeout(l);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(t--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;l=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
this.bubblelines.drawGraph()},scope:this}]}],border:!1,layout:"fit",items:{layout:{type:"vbox",align:"stretch"},defaults:{border:!1},items:[{height:30,itemId:"termsView",xtype:"dataview",store:this.termStore,tpl:this.termTpl,itemSelector:"div.term",overItemCls:"over",selectedItemCls:"selected",focusCls:"",listeners:{beforeitemclick:function(a,c,d,e,f,g){f.preventDefault();f.stopPropagation();a.fireEvent("itemcontextmenu",a,c,d,e,f,g);return!1},selectionchange:function(a,c){for(var d=this.down("#termsView"),
e=[],f=d.el.query("div[class*\x3dterm]"),g=0;g<f.length;g++)Ext.fly(f[g]).addCls("unselected");var g;for(g=0;g<c.length;g++)f=c[g],e.push(f.get("term")),Ext.fly(d.getNode(f)).removeCls("unselected");for(g in this.lastClickedBubbles)for(term in d=this.lastClickedBubbles[g],d)-1==e.indexOf(type)&&delete this.lastClickedBubbles[g][term];this.setApiParams({typeFilter:e});this.bubblelines.setCanvasHeight();this.bubblelines.drawGraph()},itemcontextmenu:function(a,c,d,e,f){f.preventDefault();f.stopPropagation();
a.isSelected(d);(new Ext.menu.Menu({floating:!0,items:[{text:this.localize("removeTerm"),handler:function(){a.deselect(e);var c=this.termStore.getAt(e).get("term");this.termStore.removeAt(e);a.refresh();this.bubblelines.removeTerm(c);this.bubblelines.setCanvasHeight();this.bubblelines.drawGraph()},scope:this}]})).showAt(f.getXY())},scope:this}},{flex:1,xtype:"container",autoEl:"div",itemId:"canvasParent",layout:"fit",overflowY:"auto",overflowX:"hidden"}],listeners:{render:function(a){a=this.down("#canvasParent");
...
this.getNodes()||{},c=this.getLinks()||{},d=this.getApiParam("limit");a.forEach(function(a){var f={term:a.getKeyword(),type:"keyword",value:a.getKeywordRawFreq(),start:d},g=[f.term,f.type].join(";");b[g]||(b[g]=f);f={term:a.getContextTerm(),type:"context",value:a.getContextTermRawFreq(),start:0};a=[f.term,f.type].join(";");b[a]||(b[a]=f);f=[g,a].join("--");c[f]||(c[f]={source:b[g],target:b[a]})});this.updateNodesAndLinks(b,c)}},initGraph:function(){var a=this.getLayout().getRenderTarget();a.update("");
...
"link");link.exit().remove();this.setLink(link);node=this.getNode().data(b.nodes(),function(a){return a.term+a.type});var d=b.nodes().filter(function(a){return"keyword"==a.type}).map(function(a){return a.value}),e=b.nodes().filter(function(a){return"context"==a.type}).map(function(a){return a.value}),f=d3.min(d),d=d3.max(d),g=d3.min(e),e=d3.max(e),h=d3.scale.linear().domain([f<g?f:g,d>e?d:e]).range([8,20]),k=this.getCorpusColours();node.enter().append("text").attr("class",function(a){return"node "+
...
delete this.getNodes()[a.term+";"+a.type],this.prune(),this.updateNodesAndLinks(),this.unmask())},prune:function(){for(var a=this.getNodes()||{},b=this.getLinks()||{},c=Object.keys(this.getLinks()),d=[],e={},f=0,g=0,h=c.length;g<h;g++)parts=c[g].split("--"),a[parts[0]]&&a[parts[1]]?(e[parts[0]]=!0,e[parts[1]]=!0):(delete b[c[g]],f++,d.push(parts[0],parts[1]));if(0<f){g=0;for(h=d.length;g<h;g++)e[d[g]]||delete a[d[g]];this.prune()}},itemclick:function(a){},itemdblclick:function(a){this.getCorpus().getCorpusCollocates({autoLoad:!1}).load({params:Ext.apply(this.getApiParams(),
{query:a.term,start:a.start,limit:10}),callback:function(b,c,d){if(d){var e=this.getNodes()||{},f=this.getLinks()||{};this.getApiParam("limit");a.start+=10;var g=[a.term,a.type].join(";");b.forEach(function(a){var b={term:a.getContextTerm(),type:"context",value:a.getContextTermRawFreq(),start:0};a=[b.term,b.type].join(";");e[a]||(e[a]=b);b=[g,a].join("--");f[b]||(f[b]={source:e[g],target:e[a]})});this.updateNodesAndLinks(e,f)}},scope:this})},updateNodesAndLinks:function(a,b){a=a||this.getNodes()||
...
{xtype:"slider",minValue:5,value:5,maxValue:500,increment:10,width:50,listeners:{render:function(b){b.setValue(a.getApiParam("expand"))},changecomplete:function(b,c){a.setApiParam("expand",c);var d=a.getView(),e=a.plugins[0].recordsExpanded,f=d.getStore();for(id in e)if(e[id]){var g=f.getByInternalId(id),h=d.getRow(g);d.fireEvent("expandbody",h,g,h.parentNode.childNodes[1],{force:!0})}}}},{text:this.localize("corpus"),tooltip:this.localize("corpusTip"),itemId:"corpus",handler:function(a){a.hide();
...
showDelay:20,trackMouse:!0,autoShow:!0})},change:function(b,c){if(c){var d=b.up("form").getForm();if(d.isValid()){for(var e=b.fileInputEl.dom.files,f=/\.(png|gif|jpe?g|xls|mp[234a]|mpeg|exe|wmv|avi|ppt|mpg|tif|wav|mov|psd|wma|ai|bmp|pps|aif|pub|dwg|indd|swf|asf|mbd|dmg|flv)$/i,g=/\.(txt|pdf|html?|xml|docx?|rtf|pages|odt|zip|jar|tar|gz|ar|cpio|bzip2|bz2|gzip)$/i,h=[],k=[],m=0,l=e.length;m<l;m++){var n=e[m].name;f.test(n)?h.push(n.split("/").pop()):g.test(n)||k.push(n.split("/").pop())}0<h.length||
...
renderer:function(a){return Ext.util.Format.percent(a)},width:"autoSize"},{text:this.localize("language"),dataIndex:"language",hidden:!0,renderer:function(a,b,e,f,g,h,k){return k.ownerCt.getLanguage(a)},width:"autoSize"}],store:b,selModel:Ext.create("Ext.selection.RowModel",{listeners:{selectionchange:{fn:function(a,b){this.getApplication().dispatchEvent("documentsClicked",this,b,this.getStore().getCorpus())},scope:this}}}),dockedItems:[{dock:"bottom",xtype:"toolbar",items:[{xtype:"querysearchfield"},
...
c){var d=[];c.forEach(function(a){a.getTerm()&&d.push(a.getTerm())});this.loadQueryTerms(d)},documentTermsClicked:function(a,c){var d=[];c.forEach(function(a){a.getTerm()&&d.push(a.getTerm())});this.loadQueryTerms(d)},documentsClicked:function(a,c,d){c&&(this.setApiParams({skipToDocId:c[0].getId(),start:0}),this.load(!0))},termLocationClicked:function(a,c){if(void 0!==c[0]){var d=c[0],e=d.get("docIndex"),f=d.get("position"),g=f-this.getApiParam("limit")/2,h=this.getCorpus().getDocument(e);this.setApiParams({skipToDocId:h.getId(),
start:0>g?0:g});this.load(!0,{callback:function(){var a=this.body.dom.querySelector("#_"+e+"_"+f);a&&a.scrollIntoView();this.highlightKeywords(d.get("term"),!1)},scope:this})}},scope:this}});this.callParent(arguments)},loadQueryTerms:function(a){a&&0<a.length&&this.getDocumentTermsStore().load({params:{query:a,docIndex:void 0,docId:void 0,page:void 0,start:void 0,limit:void 0}})},updateLocationMarker:function(a){var b;b=0==a.scrollTop?0:a.scrollHeight-a.scrollTop==a.clientHeight?1:(a.scrollTop+.5*
a.clientHeight)/a.scrollHeight;var c=$(a).find(".readerContainer").find(".word");a=c.first()[0];var d=c.last()[0];if(void 0!==a&&void 0!==d){var e=this.getCorpus(),c=!1,f=Voyant.data.model.Token.getInfoFromElement(Ext.get(a)),d=Voyant.data.model.Token.getInfoFromElement(Ext.get(d));0!==f.position&&(c=!0);a={};var g=0;if(f.docIndex===d.docIndex){var g=d.position-f.position,h=e.getDocument(f.docIndex).get("tokensCount-lexical");a[f.docIndex]=h}else for(var k=f.docIndex;k<=d.docIndex;)h=e.getDocument(k).get("tokensCount-lexical"),
c&&k===f.docIndex?(h-=f.position,g+=h):g=k===d.docIndex?g+d.position:g+h,a[k]=h,k++;e=Math.round(g*b);b=0;if(f.docIndex===d.docIndex)b=f.docIndex,c=e;else if(c&&0===e)b=f.docIndex,c=f.position;else{c=0;for(f=f.docIndex;f<=d.docIndex&&!(b=f,c+=a[f],c>=e);f++);c=a[b]-(c-e)}a=c/a[b];b=this.query("cartesian")[b];a=b.getX()+b.getWidth()*a;Ext.get(this.getLocationMarker()).setX(a)}this.setLastLocationUpdate(new Date)},generateChart:function(a,b){function c(a,b){var c=[[116,116,181],[139,163,83],[189,157,
...
{fields:"term x y z rawFreq relativeFreq cluster isDoc docIndex".split(" "),data:s});g=Ext.create("Ext.data.JsonStore",{fields:"term x y z rawFreq relativeFreq cluster isDoc docIndex".split(" "),data:p});a={itemId:"chart",xtype:"cartesian",interactions:["crosszoom","panzoom","itemhighlight"],plugins:{ptype:"chartitemevents"},axes:[{type:"numeric",position:"bottom",fields:["x"]},{type:"numeric",position:"left",fields:["y"]}],sprites:[{type:"text",text:a,hidden:0<this.labelsMode,x:70,y:70}],innerPadding:{top:25,
...
c},prepareFullPath:function(a){for(var b=[],c=this.absMaxFreq,d=this.absMinFreq,e=0,f=this.allData.length;e<f;e++){for(var g=foundB=0,h=this.allData[e].allDataInternal.length;g<h;g++)for(var k=0,m=this.allData[e].allDataInternal[g].frequencyArray.length;k<m;k++)if(this.allData[e].allDataInternal[g].frequencyArray[k].wordString===a){var l=this.allData[e].allDataInternal[g].frequencyArray[k].freq;b.push({x:e,y:l});l<c&&(c=l);l>d&&(d=l);foundB=1}0===foundB&&(g=this.minFreq,b.push({x:e,y:g}),g<c&&(c=
...
api:{panels:void 0},glyph:"xf17a@FontAwesome"},constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},layout:"border",items:[{region:"west",flex:3,layout:"fit",moreTools:["cirrus","corpusterms"],xtype:"voyanttabpanel",split:{width:5},tabBarHeaderPosition:0,items:[{xtype:"cirrus"},{xtype:"corpusterms"}]},{region:"center",flex:3,layout:"fit",xtype:"voyanttabpanel",tabBarHeaderPosition:0,items:[{xtype:"reader"}]},{region:"east",flex:3,
layout:"fit",xtype:"voyanttabpanel",split:{width:5},tabBarHeaderPosition:0,moreTools:["trends","collocatesgraph","corpuscollocates"],items:[{xtype:"trends"},{xtype:"collocatesgraph"},{xtype:"corpuscollocates"}]},{region:"south",flex:2,split:{width:5},layout:"border",items:[{layout:"fit",region:"center",flex:1,xtype:"voyanttabpanel",split:{width:5},tabBarHeaderPosition:0,moreTools:["summary","documents","phrases"],items:[{xtype:"summary"},{xtype:"documents"},{xtype:"phrases"}]},{layout:"fit",region:"east",
