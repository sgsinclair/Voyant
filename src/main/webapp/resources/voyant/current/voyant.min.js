...
{title:this.localize("title"),store:Ext.create("Voyant.data.store.Contexts",{stripTags:"all",remoteSort:!1,sortOnLoad:!0,sorters:{property:"position",direction:"ASC"}}),selModel:Ext.create("Ext.selection.RowModel",{listeners:{selectionchange:{fn:function(a,c){this.getApplication().dispatchEvent("termLocationClicked",this,c)},scope:this}}}),plugins:[{ptype:"rowexpander",rowBodyTpl:new Ext.XTemplate("")}],dockedItems:[{dock:"bottom",xtype:"toolbar",items:[{xtype:"querysearchfield"},{xtype:"component",
itemId:"status",tpl:this.localize("matchingTerms"),style:"margin-right:5px"},this.localize("context"),{xtype:"slider",minValue:5,value:5,maxValue:50,increment:5,width:50,listeners:{render:function(b){b.setValue(a.getApiParam("context"))},changecomplete:function(b,c){a.setApiParam("context",b.getValue());a.getStore().loadPage(1,{params:a.getApiParams()})}}},this.localize("expand"),{xtype:"slider",minValue:5,value:5,maxValue:500,increment:10,width:50,listeners:{render:function(b){b.setValue(a.getApiParam("expand"))},
changecomplete:function(b,c){a.setApiParam("expand",c);var d=a.getView(),e=a.plugins[0].recordsExpanded,g=d.getStore();for(id in e)if(e[id]){var f=g.getByInternalId(id),h=d.getRow(f);d.fireEvent("expandbody",h,f,h.parentNode.childNodes[1],{force:!0})}}}}]}],columns:[{text:this.localize("document"),toolTip:this.localize("documentTip"),width:"autoSize",dataIndex:"docIndex",sortable:!1,renderer:function(a,c,d,e,g,f){return f.getCorpus().getDocument(a).getTinyLabel()}},{text:this.localize("left"),tooltip:this.localize("leftTip"),
...
c);this.loadFromApis()},this)},loadFromApis:function(){this.getStore().getCorpus()&&this.getStore().load({params:this.getApiParams()})},initComponent:function(){var a=Ext.create("Voyant.data.store.CorpusNgrams",{autoLoad:!1});a.on("totalcountchange",function(){this.down("#status").update({count:this.getStore().getTotalCount()})},this);a.on("beforesort",function(a,c,d){debugger},this);this.on("sortchange",function(a,c,d,e){this.setApiParam("sort",c.dataIndex);this.setApiParam("dir",d);a=this.getApiParams("stopList query docId docIndex sort dir minLength maxLength overlapFilter".split(" "));
c=this.getStore().getProxy();for(var g in a)c.setExtraParam(g,a[g])},this);Ext.apply(this,{title:this.localize("title"),store:a,selModel:Ext.create("Ext.selection.CheckboxModel",{listeners:{selectionchange:{fn:function(a,c){var d=[];this.getApiParam("context");c.forEach(function(a){d.push('"'+a.getTerm()+'"')});this.getApplication().dispatchEvent("termsClicked",this,d)},scope:this}}}),dockedItems:[{dock:"bottom",xtype:"toolbar",items:[{xtype:"querysearchfield"},{xtype:"component",itemId:"status",
tpl:this.localize("matchingTerms"),style:"margin-right:5px"},"-",{text:this.localize("length"),tooltip:"test",xtype:"label"},{xtype:"slider",minValue:2,values:[2,30],maxValue:30,increment:1,width:75,tooltip:this.localize("lengthTip"),listeners:{render:{fn:function(a){var c=a.getValues();a.setValue(0,parseInt(this.getApiParam("minLength",c[0])));a.setValue(1,parseInt(this.getApiParam("maxLength",c[1])))},scope:this},changecomplete:{fn:function(a,c){var d=a.getValues();this.setApiParam("minLength",
parseInt(d[0]));this.setApiParam("maxLength",parseInt(d[1]));this.getStore().load({params:this.getApiParams()})},scope:this}}},"-",{xtype:"button",text:this.localize("overlap"),tooltip:this.localize("overlapTip"),menu:{items:['\x3cb class\x3d"menu-title"\x3e'+this.localize("overlapMenu")+"\x3c/b\x3e",{text:this.localize("overlapNone"),checked:!0,group:"overlap",checkHandler:function(){this.setApiParam("overlapFilter","none");this.getStore().load({params:this.getApiParams()})},scope:this},{text:this.localize("overlapLength"),
checked:!1,group:"overlap",checkHandler:function(){this.setApiParam("overlapFilter","length");this.getStore().load({params:this.getApiParams()})},scope:this},{text:this.localize("overlapFreq"),checked:!1,group:"overlap",checkHandler:function(){this.setApiParam("overlapFilter","rawfreq");this.getStore().load({params:this.getApiParams()})},scope:this}]}}]}],columns:[{text:this.localize("term"),dataIndex:"term",tooltip:this.localize("termTip"),sortable:!0,flex:1},{text:this.localize("rawFreq"),dataIndex:"rawFreq",
tooltip:this.localize("termRawFreqTip"),sortable:!0,width:"autoSize"},{text:this.localize("length"),dataIndex:"length",tooltip:this.localize("lengthTip"),sortable:!0,width:"autoSize"},{xtype:"widgetcolumn",text:this.localize("trend"),tooltip:this.localize("trendTip"),width:120,dataIndex:"distributions",widget:{xtype:"sparklineline"}}],listeners:{termsClicked:{fn:function(a,c){if(this.getStore().getCorpus()){var d=[];c.forEach(function(a){a.term&&d.push(a.term)});d&&(this.setApiParams({docIndex:void 0,
docId:void 0,query:d}),this.isVisible()&&this.isVisible()&&this.getStore().loadPage(1,{params:this.getApiParams()}))}},scope:this}}});this.callParent(arguments);this.getStore().getProxy().setExtraParam("withDistributions",!0)}});Ext.define("Voyant.panel.CorpusTerms",{extend:"Ext.grid.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.corpusterms",statics:{i18n:{title:{en:"Corpus Terms"},helpTip:{en:"\x3cp\x3eCorpus Terms is a table view of terms that appear in the entire corpus. Features include:\x3c/p\x3e\x3cul\x3e\x3cli\x3ereordering by \x3ci\x3eterm\x3c/i\x3e and \x3ci\x3ecount\x3c/i\x3e (click on the column headers)\x3c/li\x3e\x3cli\x3ea sparkline graph of the term frequency trends across the corpus (if the corpus has multiple documents) or across the document (if the corpus has only one document)\x3c/li\x3e\x3cli\x3eadditional columns available (relative frequency, distribution peakedness and skew) by clicking on the arrow that appears when hovering over a header\x3c/li\x3e\x3cli\x3ea search box for queries (hover over the magnifying icon for help with the syntax)\x3c/li\x3e\x3c/ul\x3e"},
...
{text:this.localize("typeTokenRatioLexical"),dataIndex:"typeTokenRatio-lexical",renderer:function(a){return Ext.util.Format.percent(a)},width:"autoSize"},{text:this.localize("language"),dataIndex:"language",hidden:!0,renderer:function(a,b,e,g,f,h,k){return k.ownerCt.getLanguage(a)},width:"autoSize"}],store:b,selModel:Ext.create("Ext.selection.RowModel",{listeners:{selectionchange:{fn:function(a,b){this.getApplication().dispatchEvent("documentsClicked",this,b,this.getStore().getCorpus())},scope:this}}}),
