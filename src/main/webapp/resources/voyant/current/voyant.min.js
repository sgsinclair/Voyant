...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];m=0;f=function(){var a,f,h,k,s=this.wordsToArrange-1,n=g.canvas.width,q=g.canvas.height,H=.5*n,u=.5*q;do{h=p[m];if(void 0!==h){for(var C=Math.random()*Math.PI,v=.25*Math.random()*h.size,w=.5*(Math.random()-.5),E=.5*h.width,N=.5*h.height;;){a=Math.floor((H+Math.cos(C)*v*this.ratio-E)/this.COARSENESS)*this.COARSENESS;f=Math.floor((u+Math.sin(C)*v-N)/this.COARSENESS)*this.COARSENESS;
k=a+E>=n||f+N>=q?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;C+=w;v+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<m;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}m++;if(m>=p.length){clearTimeout(l);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(s--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;l=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
l=a;return b};b.showTokenExtra=function(a){if(!arguments.length)return n;n=a;return b};b.scaleLabels=function(a){if(!arguments.length)return q;q=a;return b};b.succeeded=function(){return C};b.sortFun=function(a){if(!arguments.length)return u;u=a;return b};b.nodeText=function(a){if(!arguments.length)return p;p=a;return b};b.tokenExtraText=function(a){if(!arguments.length)return s;s=a;return b};b.rectColor=function(a){if(!arguments.length)return H;H=a;return b};b.rectBorderColor=function(a){if(!arguments.length)return r;
r=a;return b};b.continuationColor=function(a){if(!arguments.length)return G;G=a;return b};b.basicStyles=function(a){if(!arguments.length)return L;Object.keys(L).forEach(function(b){b in a&&Object.keys(L[b]).forEach(function(c){c in a[b]&&(L[b][c]=a[b][c])})});return b};return b};doubletree.Tree=function(a,b,c,d,k,m,l,n,q,u,p,s,H,r,G){function L(a){a.children&&(a._children=a.children,a._children.forEach(L),a.children=null)}function C(a){a.parent&&a.parent.children.forEach(function(b){b!=a&&L(b)})}
...
c.setApiParams({stopList:b.stopList});for(var e=c.getViewport().query("panel,chart"),f=0;f<e.length;f++)e[f].setApiParams&&e[f].setApiParams({stopList:b.stopList});d?(c.dispatchEvent("loadedCorpus",this,d),this.fireEvent("loadedCorpus",this,d)):(c.dispatchEvent("apiParamsUpdated",this,b),this.fireEvent("apiParamsUpdated",this,b))}else d?this.fireEvent("loadedCorpus",this,d):this.fireEvent("apiParamsUpdated",this,b);a.up("window").close()},scope:a}]},bodyPadding:5}).show()}}:void 0,help:{glyph:"xf128@FontAwesome",
...
this.getProxy()&&this.getProxy().setExtraParam("corpus",Ext.isString(a)?a:a.getId());this.callParent(arguments)}});Ext.define("Voyant.data.store.CAAnalysis",{extend:"Ext.data.Store",model:"Voyant.data.model.StatisticalAnalysis",config:{corpus:void 0},constructor:function(a){a=a||{};Ext.apply(a,{proxy:{type:"ajax",url:Voyant.application.getTromboneUrl(),extraParams:{tool:"corpus.CA",corpus:a&&a.corpus?Ext.isString(a.corpus)?a.corpus:a.corpus.getId():void 0,withDistributions:!0},reader:{type:"json",rootProperty:"correspondenceAnalysis",totalProperty:"correspondenceAnalysis.totalTerms"},simpleSortMode:!0}});this.callParent([a])},
...
f=b?Math.min(d/Math.abs(b[1].x-d/2),d/Math.abs(b[0].x-d/2),e/Math.abs(b[1].y-e/2),e/Math.abs(b[0].y-e/2))/2:1,g=this.getVis().selectAll("text").data(a,function(a){return a.text});g.transition().duration(1E3).attr("transform",function(a){return"translate("+[a.x,a.y]+")rotate("+a.rotate+")"}).style("font-size",function(a){return a.fontSize+"px"});g.enter().append("text").attr("text-anchor","middle").attr("data-freq",function(a){return a.rawFreq}).attr("transform",function(a){return"translate("+[a.x,
a.y]+")rotate("+a.rotate+")"}).style("font-size","1px").transition().duration(1E3).style("font-size",function(a){return a.fontSize+"px"});g.style("font-family",function(a){return a.font}).style("fill",function(a){return c.getApplication().getColorForTerm(a.text,!0)}).text(function(a){return a.text}).on("click",function(a){c.dispatchEvent("termsClicked",c,[a.text])});g.exit().remove();this.getVis().transition().duration(1E3).attr("transform","translate("+d/2+","+e/2+")scale("+f+")")},map:function(a,
...
c.attr("d",function(a){return l(a.values)}).style("fill",function(a,c){return b(c)});c.enter().append("path").attr("d",function(a){return l(a.values)}).style("fill",function(a,c){return b(c)}).append("title").text(function(a){return a.name});c.exit().remove();this.getVis().selectAll("g.axis").remove();this.getVis().append("g").attr("class","axis x").attr("transform","translate(0,"+a+")").call(k);if(this.getMode()===this.MODE_CORPUS){var u=g/f,p=0;this.getCorpus().getDocuments().each(function(a){this.getVis().select("g.x").append("text").attr("text-anchor",
...
panel:this},listeners:{itemclick:{fn:this.handleClickedItem,scope:this}}})},this);a=Ext.create("Ext.data.JsonStore",{fields:d,data:c});d.shift();var g=this;this.buildChart({store:a,series:e,axes:[{type:"numeric",position:"left",majorTickSteps:"raw"==this.getApiParam("withDistributions")&&20>f?f:void 0,increment:1,title:{text:this.localize("raw"==this.getApiParam("withDistributions")?"rawFrequencies":"relativeFrequencies")}},{type:"category",position:"bottom",increment:1,fields:["index"],title:{text:this.localize(b==
this.MODE_DOCUMENT?"segments":"documents")},label:{rotate:{degrees:-30}},renderer:function(a,c){if(b==g.MODE_DOCUMENT)return parseInt(c)+1;var d=g.getCorpus().getDocument(c);if(d){var e=g.getTargetEl().getHeight();return 250>e?parseInt(c)+1:500>e?d.getTinyTitle():d.getShortTitle()}return"?"}}]})},reloadFromChart:function(){var a=this.down("chart");if(a){var b=[];a.series.forEach(function(a){b.push(a.getTitle())});this.fireEvent("termsClicked",this,b)}},buildChart:function(a){a.axes.forEach(function(a){Ext.applyIf(a,
{style:{opacity:.2},label:{opacity:.5}});Ext.applyIf(a.title,{fontSize:12})});Ext.applyIf(a,{plugins:{ptype:"chartitemevents",moveEvents:!0},legend:{docked:"top"},interactions:["itemhighlight","panzoom"],innerPadding:{top:5,right:5,bottom:5,left:5},border:!1,bodyBorder:!1});this.query("chart").forEach(function(a){this.remove(a)},this);a=Ext.create("Ext.chart.CartesianChart",a);this.add(a)},clearChart:function(){this.query("chart").forEach(function(a){this.remove(a)},this)},handleClickedItem:function(a,
b){var c=this.getApiParam("mode");if(c===this.MODE_DOCUMENT){if(c=this.getApiParam("docId")){var d=this.getCorpus().getDocument(c),d=d.get("tokensCount-lexical"),e=this.getApiParam("bins"),d=b.index/e*d;this.dispatchEvent("documentIndexTermsClicked",this,[{term:b.series.getTitle(),docId:c,position:d}])}}else c==this.MODE_CORPUS&&(d=this.getCorpus().getDocument(b.index),this.dispatchEvent("documentIndexTermsClicked",this,[{term:b.series.getTitle(),docIndex:b.index,docId:d.getId()}]))},isLastClickedItem:function(a){return this.lastClickedItem&&
this.lastClickedItem.term==a.term&&this.lastClickedItem.index==a.index},setMode:function(a){this.setApiParams({mode:a});this.getApiParam("mode")}});
Ext.override(Ext.chart.axis.sprite.Axis,{renderLabels:function(a,b,c,d){var e=this,f=e.attr,g=.5*f.lineWidth,h=f.position,k=f.matrix,m=f.textPadding,l=k.getXX(),n=k.getDX(),q=k.getYY(),u=k.getDY(),p=0,k=c.majorTicks,s=Math.max(f.majorTickSize,f.minorTickSize)+f.lineWidth,H=Ext.draw.Draw.isBBoxIntersect,r=e.getLabel(),G,L,C=null,F=0,z=0,I=c.segmenter,D=e.getRenderer(),A=e.getAxis(),J=(G=A.getTitle())&&""!==G.attr.text&&G.getBBox(),M,K=null,x,B,v;if(k&&r&&!r.attr.hidden){G=r.attr.font;b.font!==G&&(b.font=
G);r.setAttributes({translationX:0,translationY:0},!0);r.applyTransformations();M=r.attr.inverseMatrix.elements.slice(0);switch(h){case "left":b=J?J.x+J.width:0;switch(r.attr.textAlign){case "start":d=a.roundPixel(b+n)-g;break;case "end":d=a.roundPixel(d[2]-s+n)-g;break;default:d=a.roundPixel(b+(d[2]-b-s)/2+n)-g}r.setAttributes({translationX:d},!0);break;case "right":b=J?d[2]-J.x:0;switch(r.attr.textAlign){case "start":d=a.roundPixel(s+n)+g;break;case "end":d=a.roundPixel(d[2]-b+n)+g;break;default:d=
...
a:a.msg,response:b}))},showError:function(a){if(a.statusText&&a.responseText)return this.showResponseError({},a);if(a instanceof Voyant.util.ResponseError){var b=a.getResponse();Ext.apply(a,{message:a.getMsg()+" "+this.localize("serverResponseError")+"\x3cpre class\x3d'error'\x3e\n"+b.responseText.substring(0,b.responseText.indexOf("\n\t"))+"\u2026 \x3ca href\x3d'#' onclick\x3d\"window.open('').document.write(unescape('\x3cpre\x3e"+escape(b.responseText)+"\x3c/pre\x3e')); return false;\"\x3emore\x3c/a\x3e\x3c/pre\x3e"})}Ext.isString(a)&&
(a={message:a});Ext.applyIf(a,{title:this.localize("error"),buttons:Ext.Msg.OK,icon:Ext.MessageBox.ERROR,autoScroll:!0});Ext.Msg.show(a)},getToolConfigFromToolXtype:function(a){cls=Ext.ClassManager.getByAlias("widget."+a);return{xtype:a,title:this._localizeClass(cls,"title"),tooltip:{text:this._localizeClass(cls,"helpTip")},glyph:cls&&cls.glyph?cls.glyph:"xf12e@FontAwesome"}},colors:[[0,0,255],[51,197,51],[255,0,255],[121,51,255],[28,255,255],[255,174,0],[30,177,255],[182,242,58],[255,0,164],[51,
102,153],[34,111,52],[155,20,104],[109,43,157],[128,130,33],[111,76,10],[119,115,165],[61,177,169],[202,135,115],[194,169,204],[181,212,228],[182,197,174],[255,197,197],[228,200,124],[197,179,159]],rgbToHex:function(a){return"#"+(16777216+(a[0]<<16)+(a[1]<<8)+a[2]).toString(16).slice(1)},getColorPalette:function(a){if(a){a=[];for(var b=0;b<this.colors.length;b++)a.push(this.rgbToHex(this.colors[b]));return a}return this.colors},getColor:function(a,b){return b?this.rgbToHex(this.colors[a]):this.colors[a]},
colorTermAssociations:new Ext.util.MixedCollection,getColorForTerm:function(a,b){-1!=a.indexOf(":")&&(a=a.split(":")[1]);var c=this.colorTermAssociations.get(a);null==c&&(c=this.colorTermAssociations.getCount()%this.colors.length,c=this.colors[c],this.colorTermAssociations.add(a,c));b&&(c=this.rgbToHex(c));return c},openUrl:function(a){window.open(a)||Ext.Msg.show({title:"Popup Blocked",buttonText:{ok:"Close"},icon:Ext.MessageBox.INFO,message:"A popup window was blocked. \x3ca href\x3d'"+a+"' target\x3d'_blank' class\x3d'link'\x3eClick here\x3c/a\x3e to open the new window.",
...
this.on("unhandledEvent",function(a,d,e){a=this.getBaseUrl()+"?corpus\x3d"+b.getAliasOrId();var f=this.getModifiedApiParams()||{};delete f.view;if("termsClicked"==d)f.query=e;else if("documentsClicked"==d){var g=[];e.forEach&&e.forEach(function(a){g.push(a.getIndex())});f.docIndex=g}else if("corpusTermsClicked"==d)e.map&&(f.query=e.map(function(a){return a.getTerm()}));else if("documentTermsClicked"==d)e.map&&(f.query=e.map(function(a){return a.getTerm()}),f.docIndex=e.map(function(a){return a.getDocIndex()}));
else{console&&console.warn("Unhandled event: "+d,e);return}f.toolFlow&&(d=Ext.Array.from(f.toolFlow.split(",")),f.view=d.shift(),0<d.length?f.toolFlow=d.join(","):delete f.toolFlow);a+="\x26"+Ext.Object.toQueryString(f);this.openUrl(a)})}}});Ext.define("Voyant.VoyantDefaultApp",{extend:"Voyant.VoyantCorpusApp",mixins:["Voyant.util.Api"],name:"VoyantDefaultApp",constructor:function(){this.mixins["Voyant.util.Api"].constructor.apply(this,arguments);this.callParent(arguments)},statics:{i18n:{},api:{view:"corpusset",stopList:"auto",panels:void 0,rtl:void 0}},listeners:{loadedCorpus:function(a,b){this.viewport.down("voyantheader").collapse();this.viewport.down("#toolsContainer").setActiveItem(1);var c=this.getCorpusId&&this.getCorpusId()?
