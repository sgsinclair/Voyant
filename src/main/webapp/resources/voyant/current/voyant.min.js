...
d.distributions[k];n>l&&(l=n);f.push({id:Ext.id(null,"bub"),x:m,freq:n,radius:0,bin:k,tokenPositions:d.positions.slice(h,h+=n)});m+=g}a.terms[c].maxDistribution=l;a.terms[c].pos=f;l>this.maxFreq.value&&(b=!0,this.setMaxFreq({term:c,value:l}))}return b},calculateBubbleRadii:function(a,b){var c=Math.log(this.maxFreq.value),d=Math.log(2)/2,e;for(e in a.terms){var g=a.terms[e];if(g&&(null==b||e==b))for(var f=0;f<g.pos.length;f++){var h=g.pos[f];if(0<h.freq){var l=Math.max(Math.log(h.freq),d)/c*this.maxRadius;
...
c=function(b,c,d,e){a(b,c,d);d.x=b;d.y=c;d.live=!0;e&&d.draw(f.context)},d=function(a,b,c,d,e){for(var f=0;f<=d;f+=this.COARSENESS)for(var g=0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},g;this.grid=[];l=0;g=function(){var a,g,h,k,v=this.wordsToArrange-1,n=f.canvas.width,q=f.canvas.height,r=.5*n,A=.5*q;do{h=p[l];if(void 0!==h){for(var t=Math.random()*Math.PI,u=.25*Math.random()*h.size,B=.5*(Math.random()-.5),y=.5*h.width,z=.5*h.height;;){a=
Math.floor((r+Math.cos(t)*u*this.ratio-y)/this.COARSENESS)*this.COARSENESS;g=Math.floor((A+Math.sin(t)*u-z)/this.COARSENESS)*this.COARSENESS;k=a+y>=n||g+z>=q?!0:d(a,g,h.height,h.width,h.mask);if(!k)break;t+=B;u+=.05}c(a,g,h);if(f.useFadeEffect)for(a=h.alpha=0;a<l;a++)g=p[a],1>g.alpha&&b(g);else h.alpha=1,h.draw(f.context)}l++;if(l>=p.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(v--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);
...
showDelay:20,trackMouse:!0,autoShow:!0})},change:function(b,c){if(c){var d=b.up("form").getForm();if(d.isValid()){for(var e=b.fileInputEl.dom.files,g=/\.(png|gif|jpe?g|xls|mp[234a]|mpeg|exe|wmv|avi|ppt|mpg|tif|wav|mov|psd|wma|ai|bmp|pps|aif|pub|dwg|indd|swf|asf|mbd|dmg|flv)$/i,f=/\.(txt|pdf|html?|xml|docx?|rtf|pages|odt|zip|jar|tar|gz|ar|cpio|bzip2|bz2|gzip)$/i,h=[],l=[],m=0,k=e.length;m<k;m++){var n=e[m].name;g.test(n)?h.push(n.split("/").pop()):f.test(n)||l.push(n.split("/").pop())}0<h.length||
0<l.length?Ext.Msg.show({title:a.localize("fileTypesWarning"),icon:Ext.MessageBox.ERROR,message:a.localize("fileTypesMessage")+"\x3cul\x3e"+(0<h.length?"\x3cli\x3e"+a.localize("badFiles")+h.slice(0,5).join(", ")+(5<h.length?"\u2026":"")+"\x3c/li\x3e":"")+(0<l.length?"\x3cli\x3e"+a.localize("unknownFiles")+l.slice(0,5).join(", ")+(5<l.length?"\u2026":"")+"\x3c/li\x3e":"")+"\x3c/ul\x3e"+a.localize("sureContinue"),buttons:Ext.Msg.YESNO,fn:function(c){"yes"===c?a.loadForm(d):(b.reset(),b.fileInputEl.dom.setAttribute("multiple",
...
renderer:function(a){return Ext.util.Format.percent(a)},width:"autoSize"},{text:this.localize("language"),dataIndex:"language",hidden:!0,renderer:function(a,b,e,g,f,h,l){return l.ownerCt.getLanguage(a)},width:"autoSize"}],store:b,selModel:Ext.create("Ext.selection.RowModel",{listeners:{selectionchange:{fn:function(a,b){this.getApplication().dispatchEvent("documentsClicked",this,b,this.getStore().getCorpus())},scope:this}}}),dockedItems:[{dock:"bottom",xtype:"toolbar",items:[{xtype:"querysearchfield"},
...
constructor:function(){Ext.apply(this,{title:this.localize("title")});this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(){var a=Ext.create("Voyant.data.store.Tokens");a.on("load",function(a,c,d){if(d){var e="",g=this.localize("documentFrequency"),f=!1,h=-1,l=!1;c.forEach(function(a){0==a.getPosition()&&(e+="\x3ch3\x3e"+this.getDocumentsStore().getById(a.getDocId()).getFullLabel()+"\x3c/h3\x3e");a.getDocIndex()!=h&&(f=this.getDocumentsStore().getById(a.getDocId()).isPlainText(),
h=a.getDocIndex());if(a.isWord())l=!1,e+="\x3cspan class\x3d'word' id\x3d'"+a.getId()+"' data-qtip\x3d'"+g+" "+a.getDocumentRawFreq()+"'\x3e"+a.getTerm()+"\x3c/span\x3e";else{a=a.getTermWithLineSpacing(f);var b=0==a.indexOf("\x3cbr /\x3e");if(!l||!b&&0!=a.trim().length)e+=a,l=b}},this);this.updateText(e,!0);a=this.down("querysearchfield").getValue();""!=a&&this.highlightKeywords(a)}},this);this.setTokensStore(a);this.on("query",function(a,c){this.loadQueryTerms([c])},this);this.setDocumentTermsStore(Ext.create("Ext.data.Store",
