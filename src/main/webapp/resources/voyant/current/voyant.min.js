...
Ext.define("Ext.chart.series.CustomScatter",{extend:"Ext.chart.series.Scatter",alias:"series.customScatter",type:"customScatter",seriesType:"scatterSeries",tipsDisabled:!1,enableToolTips:function(){this.tipsDisabled=!1},disableToolTips:function(){this.tipsDisabled=!0},showTip:function(a,b){this.tipsDisabled||this.callParent(arguments)}});Ext.define("Voyant.panel.StreamGraph",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.streamgraph",statics:{i18n:{title:{en:"StreamGraph"},freqsMode:{en:"Frequencies"},freqsModeTip:{en:"Determines if frequencies are expressed as raw counts or as relative counts (per document or segment)."},rawFrequencies:{en:"Raw Frequencies"},relativeFrequencies:{en:"Relative Frequencies"},documentSegments:{en:"Document Segments"},documents:{en:"Documents"},clearTerms:{en:"Clear Terms"},segments:{en:"Segments"}},
api:{limit:5,stopList:"auto",query:void 0,withDistributions:"relative",bins:50,docIndex:void 0,docId:void 0},glyph:"xf1cb@FontAwesome"},config:{corpus:void 0,visLayout:void 0,vis:void 0,mode:void 0},graphMargin:{top:20,right:60,bottom:110,left:80},MODE_CORPUS:"corpus",MODE_DOCUMENT:"document",constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(){Ext.apply(this,{title:this.localize("title"),tbar:new Ext.Toolbar({enableOverflow:!0,
items:["-\x3e",{xtype:"legend",store:new Ext.data.JsonStore({fields:["name","mark","active"]}),listeners:{itemclick:function(a,b,c,d){a=Ext.fly(c.firstElementChild).hasCls("x-legend-inactive");b.set("active",a);b=this.getCurrentTerms();this.setApiParams({query:b,limit:b.length,stopList:void 0});this.loadFromCorpus()},scope:this}},"-\x3e"]}),bbar:{enableOverflow:!0,items:[{xtype:"querysearchfield"},{xtype:"corpusdocumentselector",singleSelect:!0},{text:this.localize("freqsMode"),glyph:"xf201@FontAwesome",
tooltip:this.localize("freqsModeTip"),menu:{items:[{text:this.localize("relativeFrequencies"),checked:!0,itemId:"relative",group:"freqsMode",checkHandler:function(a,b){b&&(this.setApiParam("withDistributions","relative"),this.loadFromCorpus())},scope:this},{text:this.localize("rawFrequencies"),checked:!1,itemId:"raw",group:"freqsMode",checkHandler:function(a,b){b&&(this.setApiParam("withDistributions","raw"),this.loadFromCorpus())},scope:this}]}},{xtype:"slider",itemId:"segmentsSlider",fieldLabel:this.localize("segments"),
...
b){f.push({x:b,y:a})},this);e.push({name:g,values:f});d.push({id:g,name:g,mark:b(c),active:!0})},this);c.loadData(d);var f=this.getVisLayout()(e),g;g=this.getMode()===this.MODE_DOCUMENT?this.getApiParam("bins"):this.getCorpus().getDocumentsCount();g--;var c=this.body.down("svg").getWidth()-this.graphMargin.left-this.graphMargin.right,h=d3.scale.linear().domain([0,g]).range([0,c]),k=d3.max(f,function(a){return d3.max(a.values,function(a){return a.y0+a.y})});a=this.body.down("svg").getHeight()-this.graphMargin.top-
this.graphMargin.bottom;var l=d3.scale.linear().domain([0,k]).range([a,0]),m=d3.svg.area().x(function(a){return h(a.x)}).y0(function(a){return l(a.y0)}).y1(function(a){return l(a.y0+a.y)}),k=d3.svg.axis().scale(h).orient("bottom");if(this.getMode()===this.MODE_CORPUS){for(var n=[],q=0;q<=g;q++)n.push(q);k.tickValues(n);k.tickFormat("")}n=d3.svg.axis().scale(l).orient("left");f=this.getVis().selectAll("path").data(f,function(a){return a.name});f.attr("d",function(a){return m(a.values)}).style("fill",
function(a,c){return b(c)});f.enter().append("path").attr("d",function(a){return m(a.values)}).style("fill",function(a,c){return b(c)}).append("title").text(function(a){return a.name});f.exit().remove();this.getVis().selectAll("g.axis").remove();this.getVis().append("g").attr("class","axis x").attr("transform","translate(0,"+a+")").call(k);if(this.getMode()===this.MODE_CORPUS){var s=c/g,p=0;this.getCorpus().getDocuments().each(function(a){this.getVis().select("g.x").append("text").attr("text-anchor",
"end").attr("transform","translate("+p+", 10) rotate(-45)").text(a.getTinyTitle());p+=s},this);g=this.localize("documents")}else g=this.localize("documentSegments");this.getVis().select("g.x").append("text").attr("text-anchor","middle").attr("transform","translate("+c/2+", "+(this.graphMargin.bottom-20)+")").text(g);this.getVis().append("g").attr("class","axis y").attr("transform","translate(0,0)").call(n);c="raw"===this.getApiParam("withDistributions")?this.localize("rawFrequencies"):this.localize("relativeFrequencies");
...
d=a.getWidth()-b,e=a.getHeight()-c;this.setVisLayout(d3.layout.stack().offset("silhouette").values(function(a){return a.values}));this.setVis(d3.select(a.dom).append("svg").attr("id","streamGraph").attr("width",d+b).attr("height",e+c).append("g").attr("transform","translate("+this.graphMargin.left+","+this.graphMargin.top+")"))}},updateMode:function(a){this.queryById("segmentsSlider").setHidden(a==this.MODE_CORPUS)}});Ext.define("Voyant.panel.Summary",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel","Voyant.util.SparkLine"],alias:"widget.summary",statics:{i18n:{title:{en:"Summary"},helpTip:{en:"\x3cp\x3eThe \x3ci\x3eSummary\x3c/i\x3e tool provides general information about the corpus. Many elements in the tool are links that trigger other views. Features include:\x3c/p\x3e\x3cul\x3e\x3cli\x3etotal words (tokens) and word forms (types) and age of the corpus\x3c/li\x3e\x3cli\x3emost frequent terms in the corpus\x3c/li\x3e\x3cli\x3efor corpora with more than one documen\x3cul\x3e\x3cli\x3edocuments ordered by length and vocabulary density\x3c/li\x3e\x3cli\x3edistinctive words for each document (by TF-IDF score)\x3c/li\x3e\x3c/ul\x3e\x3c/li\x3e\x3c/ul\x3e"},
