...
{i18n:"moreToolsTypeOther",glyph:"xf035@FontAwesome",items:["reader","summary"]}]},launch:function(){this.callParent(arguments);if(this.hasQueryToLoad()){var a=Ext.Object.fromQueryString(document.location.search);this.loadCorpusFromParams(a)}},loadCorpusFromParams:function(a){var b=this,c=b.getViewport();c.mask(this.localize("fetchingCorpus"));(new Corpus(a)).then(function(a){if(a.requiresPassword()){var e=a.getNoPasswordAccess(),f=[{text:"Validate"}];"NONCONSUMPTIVE"==e&&f.push({text:"Limited"});
var g=Ext.create("Ext.window.Window",{title:b.localize("passwordRequiredTitle"),layout:"fit",items:{padding:10,flex:1,width:300,layout:{type:"vbox",align:"stretch"},items:[{html:"\x3cp\x3e"+b.localize("passwordRequiredMessage")+"\x3c/p\x3e"+("NONCONSUMPTIVE"==e?"\x3cp\x3e"+b.localize("nonConsumptiveMessage")+"\x3c/p\x3e":"")+"\x3c/p\x3e"},{xtype:"textfield",fieldLabel:b.localize("password")}],bbar:{layout:{pack:"center"},items:[{text:b.localize("passwordValidateButton"),ui:"default",handler:function(){var e=
g.query("textfield")[0].getValue().trim();0==e.length?b.showError({message:b.localize("noPasswordGiven")}):(g.mask(),Ext.Ajax.request({url:b.getTromboneUrl(),params:{corpus:a.getId(),passwordForSession:e},method:"POST",success:function(e,f){g.unmask();var h=e.responseText;"ADMIN"==h||"ACCESS"==h?(g.close(),c.unmask(),b.dispatchEvent("loadedCorpus",this,a)):b.showError({message:b.localize("badPassword")})},failure:function(a,c){g.unmask();b.showError({message:b.localize("passwordValidationError")})}}))}},
{text:b.localize("nonConsumptiveButton"),handler:function(){g.mask();Ext.Ajax.request({url:b.getTromboneUrl(),params:{corpus:a.getId(),passwordForSessionRemove:!0},method:"POST",callback:function(e,f){g.unmask();g.close();c.unmask();b.dispatchEvent("loadedCorpus",b,a)}})}}]}}}).show()}else c.unmask(),b.dispatchEvent("loadedCorpus",this,a)}).fail(function(a,b){debugger;c.unmask()})},hasQueryToLoad:function(a){a||(a=Ext.Object.fromQueryString(document.location.search));return a.corpus||a.input},listeners:{loadedCorpus:function(a,
b){this.setCorpus(b);this.on("unhandledEvent",function(a,d,e){a=this.getBaseUrl()+"?corpus\x3d"+b.getId();var f=this.getModifiedApiParams()||{};delete f.view;if("termsClicked"==d)f.query=e;else if("documentsClicked"==d){var g=[];e.forEach&&e.forEach(function(a){g.push(a.getIndex())});f.docIndex=g}else if("corpusTermsClicked"==d)e.map&&(f.query=e.map(function(a){return a.getTerm()}));else if("documentTermsClicked"==d)e.map&&(f.query=e.map(function(a){return a.getTerm()}),f.docIndex=e.map(function(a){return a.getDocIndex()}));
...
api:{view:"corpusset",stopList:"auto",panels:void 0}},listeners:{loadedCorpus:function(a,b){this.viewport.down("voyantheader").collapse();this.viewport.down("#toolsContainer").setActiveItem(1);if(window.history.pushState){var c=b.getId(),d=Ext.Object.fromQueryString(document.location.search),e=this.getBaseUrl()+"?corpus\x3d"+c,f;for(f in d)if("corpus"!==f){var g=Ext.isString(d[f])?[d[f]]:d[f];Ext.isArray(g)&&g.forEach(function(a){e+="\x26"+f+"\x3d"+a})}window.history.pushState({corpus:c},"Corpus: "+
c,e)}}},getViewComponent:function(){return this.viewport.down("#toolsContainer-main")},launch:function(){var a=Ext.Object.fromQueryString(document.location.search)||{},b=this.getApiParam("view","CorpusSet"),c=b.toLowerCase();if(!Ext.ClassManager.getByAlias("widget."+c)||a.noskin)Ext.Msg.show({title:this.localize("noViewErrorTitle"),message:(new Ext.Template(this.localize(a.noskin?"noViewKnownErrorTpl":"noViewErrorTpl"))).apply({view:a.noskin?a.noskin:b,additional:a.noskin&&"convert"==a.noskin?this.localize(a.noskin+
"SkinMsg"):""}),buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR}),c="corpusset";this.viewport=Ext.create("Ext.container.Viewport",{layout:"border",items:[{xtype:"voyantheader",region:"north"},{region:"south",xtype:"voyantfooter"},{region:"center",layout:"card",itemId:"toolsContainer",activeItem:0,items:[{xtype:"container",layout:{type:"vbox",pack:"center",align:"center"},items:[{xtype:"corpuscreator"},{xtype:"container",html:this.localize("voyantIs")}]},{layout:"fit",itemId:"toolsContainer-main",items:{xtype:c}}]}]});
