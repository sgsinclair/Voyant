...
var show=Voyant.notebook.util.Show.show,showError=Voyant.notebook.util.Show.showError;Ext.define("Voyant.notebook.util.Embed",{transferable:["embed"],embed:function(){embed.apply(this,arguments)},statics:{i18n:{},embed:function(a,b){if(this.then)this.then(function(c){embed.call(c,a,b)});else if(!this.embeddable||a&&!Ext.isObject(a)||(Ext.isObject(a)&&(b=a),a=this.embeddable[0]),Ext.isString(a)&&(a=Ext.ClassManager.getByAlias("widget."+a)||Ext.ClassManager.get(a)),Ext.isFunction(a)){var c=a.getName();if(this.embeddable&&Ext.Array.each(this.embeddable,function(a){return a!=c},this))Voyant.notebook.util.Embed.showWidgetNotRecognized.call(this);
else{b=b||{};var d=new a;d.setApiParams(b);var e=d.getModifiedApiParams();d.destroy();e.corpus||("Voyant.data.model.Corpus"==Ext.getClassName(this)?e.corpus=this.getId():this.getCorpus&&(e.corpus=this.getCorpus().getId()));Ext.applyIf(b,{width:"100%",height:"300px"});d=Ext.Object.toQueryString(e);e=new Ext.XTemplate('\x3ciframe style\x3d"width: '+b.width+"; height: "+b.height+'" src\x3d"'+Voyant.application.getBaseUrl()+"tool/"+c.substring(c.lastIndexOf(".")+1)+'/?{0}"\x3e\x3c/iframe\x3e');if(1950>
d.length)show(e.apply([d]));else{showError("Long arguments are not yet implemented.");debugger}}}else Voyant.notebook.util.Embed.showWidgetNotRecognized.call(this)},showWidgetNotRecognized:function(){var a=Voyant.notebook.util.Embed.i18n.widgetNotRecognized;this.embeddable&&(a+=Voyant.notebook.util.Embed.i18n.tryWidget+"\x3cul\x3e"+this.embeddable.map(function(a){a=a.substring(a.lastIndexOf(".")+1).toLowerCase();return"\"\x3ca href\x3d'"+Voyant.application.getBaseUrl()+"/docs/#!/guide/"+a+"' target\x3d'voyantdocs'\x3e"+
...
Ext.define("Voyant.data.store.Tokens",{extend:"Ext.data.Store",mixins:["Voyant.data.store.TokensMixin"],constructor:function(a){a=a||{};this.mixins["Voyant.data.store.TokensMixin"].constructor.apply(this,[a]);this.callParent([a])}});Ext.define("Voyant.data.store.TokensBuffered",{extend:"Ext.data.BufferedStore",mixins:["Voyant.data.store.TokensMixin"],constructor:function(a){a=a||{};this.mixins["Voyant.data.store.TokensMixin"].constructor.apply(this,[a]);this.callParent([a])}});var Corpus=function(a,b){return Ext.create("Voyant.data.model.Corpus",a,b)};function loadCorpus(a,b){return new Corpus(a,b)}
Ext.define("Voyant.data.model.Corpus",{alternateClassName:["Corpus"],mixins:["Voyant.notebook.util.Embed","Voyant.notebook.util.Show","Voyant.util.Transferable","Voyant.util.Localization"],embeddable:["Voyant.panel.Summary","Voyant.panel.Cirrus","Voyant.panel.Documents","Voyant.panel.CorpusTerms"],requires:["Voyant.util.ResponseError","Voyant.data.store.CorpusTerms","Voyant.data.store.Documents"],extend:"Ext.data.Model",config:{documentsStore:void 0},statics:{i18n:{}},fields:[{name:"documentsCount",
type:"int"},{name:"lexicalTokensCount",type:"int"},{name:"lexicalTypesCount",type:"int"},{name:"createdTime",type:"int"},{name:"createdDate",type:"date",dateFormat:"c"}],constructor:function(a,b){this.callParent([b]);if(a){b=b||{};Ext.apply(b,{tool:"corpus.CorpusMetadata"});Ext.isString(a)||Ext.isArray(a)?b.input=a:Ext.isObject(a)&&Ext.apply(b,a);var c=Voyant.application.getDeferred(this),d=this;Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:b}).then(function(a){d.set(Ext.JSON.decode(a.responseText).corpus.metadata);
return d}).otherwise(function(a){Voyant.application.showResponseError(d.localize("failedCreateCorpus"),a)}).then(function(a){!("docsLimit"in b)||!1!==b.docsLimit&&0<b.docsLimit?d.getDocuments().load({params:{limit:"docsLimit"in b?b.docsLimit:d.getDocumentsCount()},callback:function(b,g,h){h?(d.setDocumentsStore(this),c.resolve(a)):c.reject(g)}}):c.resolve(a)});return c.promise}},getId:function(){return this.then?Voyant.application.getDeferredNestedPromise(this,arguments):this.get("id")},getCorpusTerms:function(a){return Ext.create("Voyant.data.store.CorpusTerms",
Ext.apply(a||{},{corpus:this}))},getCorpusCollocates:function(a){return Ext.create("Voyant.data.store.CorpusCollocates",Ext.apply(a||{},{corpus:this}))},getDocumentQueryMatches:function(a){return Ext.create("Voyant.data.store.DocumentQueryMatches",Ext.apply(a||{},{corpus:this}))},getDocumentTerms:function(a){return Ext.create("Voyant.data.store.DocumentTerms",Ext.apply(a||{},{corpus:this}))},getContexts:function(a){return Ext.create("Voyant.data.store.Contexts",Ext.apply(a||{},{corpus:this}))},getDocuments:function(a){return this.getDocumentsStore()?
...
this.getNoPasswordAccess();return"NONE"==a||"NONCONSUMPTIVE"==a},getNoPasswordAccess:function(){return this.then?Voyant.application.getDeferredNestedPromise(this,arguments):this.get("noPasswordAccess")},toString:function(a){a=this.getDocumentsCount();var b=this.localize("thisCorpus");if(0==a)b+=" "+this.localize("isEmpty")+".";else{var b=b+" ",b=1<a?b+(new Ext.XTemplate(this.localize("hasNdocuments"))).apply({count:Ext.util.Format.number(a,"0,000")}):b+this.localize("has1document"),b=b+(" "+(new Ext.XTemplate(this.localize("widthNwordsAndNTypes"))).apply({words:Ext.util.Format.number(this.getWordTokensCount(),
"0,000"),types:Ext.util.Format.number(this.getWordTypesCount(),"0,000")})+"."),b=b+" Created ",c=this.get("createdDate"),d=new Date;!0===Ext.Array.each([["year",Ext.Date.YEAR],["month",Ext.Date.MONTH],["day",Ext.Date.DAY],["hour",Ext.Date.HOUR],["minute",Ext.Date.MINUTE],["second",Ext.Date.SECOND]],function(a){if(Ext.Date.diff(c,d,a[1])>("second"==a[0]?1:0)){var f=Ext.Date.diff(c,d,a[1]);b+="\x3cspan class\x3d'info-tip' data-qtip\x3d'"+Ext.Date.format(c,"Y-m-d, H:i:s")+"'\x3e";b=1==f?b+(new Ext.XTemplate(this.localize(a[0]+
"Ago"))).apply({count:f,date:c}):b+(new Ext.XTemplate(this.localize(a[0]+"sAgo"))).apply({count:f,date:c});b+="\x3c/span\x3e";return!1}},this)&&(b+=this.localize("now"));b+=".";b+=""}return b}});Ext.define("Voyant.widget.CorpusSelector",{extend:"Ext.form.field.ComboBox",mixins:["Voyant.util.Localization"],alias:"widget.corpusselector",statics:{i18n:{}},config:{labelWidth:150,labelAlign:"right",fieldLabel:"Choose a corpus:",name:"corpus",queryMode:"local",store:[["shakespeare","Shakespeare's Plays"],["austen","Austen's Novels"]],forceSelection:!0},initComponent:function(a){this.callParent(arguments)}});Ext.define("Voyant.widget.StopListOption",{extend:"Ext.container.Container",mixins:["Voyant.util.Localization"],alias:"widget.stoplistoption",layout:"hbox",statics:{i18n:{}},initComponent:function(a){var b=this.up("window").panel.getApiParam("stopList"),c=[];"ar:stop.ar.arabic-lucene.txt bg:stop.bu.bulgarian-lucene.txt br:stop.br.breton-lucene.txt ca:stop.ca.catalan-lucene.txt ckb:stop.ckb-turkish-lucene.txt cn:stop.cn.chinese-lawrence.txt cz:stop.cz.czech-lucene.txt de:stop.de.german.txt el:stop.el.greek-lucene.txt en:stop.en.taporware.txt es:stop.es.spanish.txt eu:stop.eu.basque-lucene.txt fa:stop.fa.farsi-lucene.txt fr:stop.fr.veronis.txt ga:stop.ga-irish-lucene.txt gl:stop.ga.galician-lucene.txt hi:stop.hi.hindi-lucene.txt hu:stop.hu.hungarian.txt hy:stop.hy.armenian-lucene.txt id:stop.id.indonesian-lucene.txt it:stop.it.italian.txt ja:stop.ja.japanese-lucene.txt lv:stop.lv.latvian-lucene.txt lt:stop.lt.lithuanian-lucene.txt mu:stop.mu.multi.txt nl:stop.nl.dutch.txt no:stop.no.norwegian.txt ro:stop.ro.romanian-lucene.txt se:stop.se.swedish-long.txt th:stop.th.thai-lucene.txt tr:stop.tr.turkish-lucene.txt".split(" ").forEach(function(a){a=
...
tooltip:"Select an exsting corpus",hidden:void 0!=this.getCorpus(),handler:function(){Ext.create("Ext.window.Window",{title:"Open an Existing Corpus",layout:"fit",modal:!0,items:{xtype:"form",submitEmptyText:!1,margin:"5,5,5,5",items:{xtype:"corpusselector"},buttons:[{text:"Open",glyph:"xf00c@FontAwesome",handler:function(b){b.up("form").getForm();var c=b.up("form").getForm().getValues().corpus;""!=c?(a.loadCorpus({corpus:c}),b.up("window").close()):Ext.Msg.show({title:"Select a Corpus",message:"Please be sure to select a corpus.",
buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR})},flex:1},{text:"Cancel",glyph:"xf00d@FontAwesome",flex:1,handler:function(a){a.up("window").close()}}]}}).show()}},{xtype:"fileuploadfield",glyph:"xf093@FontAwesome",name:"upload",buttonOnly:!0,hideLabel:!0,ui:"default-toolbar",buttonText:"Upload",listeners:{render:function(a){a.fileInputEl.dom.setAttribute("multiple",!0);Ext.tip.QuickTipManager.register({target:a.getEl(),text:"Upload one or more documents from your computer"})},change:function(b,c){if(c){var d=
b.up("form").getForm();if(d.isValid()){for(var e=b.fileInputEl.dom.files,f=/\.(png|gif|jpe?g|xls|mp[234a]|mpeg|exe|wmv|avi|ppt|mpg|tif|wav|mov|psd|wma|ai|bmp|pps|aif|pub|dwg|indd|swf|asf|mbd|dmg|flv)$/i,g=/\.(txt|pdf|html?|xml|docx?|rtf|pages|odt|zip|jar|tar|gz|ar|cpio|bzip2|bz2|gzip|xlsx?)$/i,h=[],k=[],m=0,l=e.length;m<l;m++){var n=e[m].name;f.test(n)?h.push(n.split("/").pop()):g.test(n)||k.push(n.split("/").pop())}0<h.length||0<k.length?Ext.Msg.show({title:a.localize("fileTypesWarning"),icon:Ext.MessageBox.ERROR,
message:a.localize("fileTypesMessage")+"\x3cul\x3e"+(0<h.length?"\x3cli\x3e"+a.localize("badFiles")+h.slice(0,5).join(", ")+(5<h.length?"\u2026":"")+"\x3c/li\x3e":"")+(0<k.length?"\x3cli\x3e"+a.localize("unknownFiles")+k.slice(0,5).join(", ")+(5<k.length?"\u2026":"")+"\x3c/li\x3e":"")+"\x3c/ul\x3e"+a.localize("sureContinue"),buttons:Ext.Msg.YESNO,fn:function(c){"yes"===c?a.loadForm(d):(b.reset(),b.fileInputEl.dom.setAttribute("multiple",!0))},scope:d}):a.loadForm(d)}}}}},"-\x3e",{xtype:"button",scale:"large",
...
function(b){"yes"==b&&a.loadCorpus(Ext.apply(d,{input:c}))},a):a.loadCorpus(Ext.apply(d,{input:c}))}else Ext.Msg.show({title:"No Text Provided",message:"Please provide text in the text box (or choose open or upload).",buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR})}}]}],items:{xtype:"textareafield",width:800,height:100,itemId:"input",emptyText:this.localize("emptyInput")}});a.callParent(arguments)},loadForm:function(a){var b={tool:this.getCorpus()?"corpus.CorpusMetadata":"corpus.CorpusCreator"};this.getCorpus()&&
Ext.apply(b,{corpus:this.getCorpus().getId(),addDocuments:!0});var c=this.getApiParams();delete c.view;delete c.stopList;Ext.apply(b,c);var d=this.getApplication().getViewport();d.mask(this.localize("uploadingCorpus"));a.submit({url:this.getTromboneUrl(),params:b,failure:function(a,b){d.unmask();b.result&&(this.setCorpus(void 0),this.loadCorpus({corpus:b.result.corpus?b.result.corpus.metadata.id:b.result.stepEnabledCorpusCreator.storedId}))},scope:this})},loadCorpus:function(a){this.getCorpus()&&
...
items:[{xtype:"combo",fieldLabel:a.localize("inputFormat"),labelWidth:90,name:"inputFormat",queryMode:"local",store:[["",a.localize("inputFormatAuto")],["dtoc","DToC: Dynamic Table of Contexts"],["TEI","TEI: Text Encoding Initative"],["TEI","TEI Corpus"],["RSS","Really Simple Syndication: RSS"]],forceSelection:!0,value:""},{xtype:"container",html:"\x3cp\x3e\x3ci\x3e"+(new Ext.Template(a.localize("advancedOptionsText"))).applyTemplate([a.getBaseUrl()+"docs/#!/guide/corpuscreator-section-xml"])+"\x3c/i\x3e\x3c/p\x3e",
