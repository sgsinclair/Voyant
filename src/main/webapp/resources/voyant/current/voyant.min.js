...
this.lastClickedBubbles[b],m=0,n;for(n in k)if(-1!==this.termsFilter.indexOf(n)){var q=k[n];if(q){m++;this.SEPARATE_LINES_FOR_TERMS&&c();var r=0,p=q.color.join(",");this.ctx.strokeStyle="rgba("+p+", 1)";this.ctx.fillStyle="rgba("+p+", 0.35)";this.ctx.lineWidth=.25;for(var h=h+q.rawFreq,r=r+q.rawFreq,s=l&&this.lastClickedBubbles[b][n],v=0;v<q.pos.length;v++){var u=q.pos[v];if(0<u.radius){var G=!1;s&&this.lastClickedBubbles[b][n]==u.id&&(this.ctx.strokeStyle="rgba(0, 0, 0, 0.75)",this.ctx.fillStyle=
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];l=0;f=function(){var a,f,h,k,n=this.wordsToArrange-1,s=g.canvas.width,q=g.canvas.height,v=.5*s,r=.5*q;do{h=p[l];if(void 0!==h){for(var B=Math.random()*Math.PI,w=.25*Math.random()*h.size,D=.5*(Math.random()-.5),M=.5*h.width,N=.5*h.height;;){a=Math.floor((v+Math.cos(B)*w*this.ratio-M)/this.COARSENESS)*this.COARSENESS;f=Math.floor((r+Math.sin(B)*w-N)/this.COARSENESS)*this.COARSENESS;
k=a+M>=s||f+N>=q?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;B+=D;w+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<l;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}l++;if(l>=p.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(n--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;m=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
l=a;return b};b.handlers=function(a){if(!arguments.length)return m;m=a;return b};b.showTokenExtra=function(a){if(!arguments.length)return n;n=a;return b};b.scaleLabels=function(a){if(!arguments.length)return q;q=a;return b};b.succeeded=function(){return B};b.sortFun=function(a){if(!arguments.length)return r;r=a;return b};b.nodeText=function(a){if(!arguments.length)return p;p=a;return b};b.tokenExtraText=function(a){if(!arguments.length)return s;s=a;return b};b.rectColor=function(a){if(!arguments.length)return v;
v=a;return b};b.rectBorderColor=function(a){if(!arguments.length)return u;u=a;return b};b.continuationColor=function(a){if(!arguments.length)return G;G=a;return b};b.basicStyles=function(a){if(!arguments.length)return L;Object.keys(L).forEach(function(b){b in a&&Object.keys(L[b]).forEach(function(c){c in a[b]&&(L[b][c]=a[b][c])})});return b};return b};doubletree.Tree=function(a,b,c,d,k,l,m,n,q,r,p,s,v,u,G){function L(a){a.children&&(a._children=a.children,a._children.forEach(L),a.children=null)}function B(a){a.parent&&
...
!1));b&&x.update(a)}function H(a){return k?-a:a}var C=q,A=s;b=b-20-20;c=c-20-20;var K=0,I;l||(l=doubletree.sortByStrFld("token"));var K=0,J=d3.tree().size([c,b]).nodeSize([40,40]),F=d3.linkHorizontal().x(function(a){return H(a.y)}).y(function(a){return a.x});a=a.append("g").attr("transform","translate(20,20)");this.readJSONTree=function(a){I=d3.hierarchy(a);I.x0=0;I.y0=0;I.children.forEach(L);this.update(I)};this.update=function(b){var c=J(I).descendants();c.forEach(function(a){var b=a.textSize;if(void 0==
...
d3.forceY(c/2)).on("tick",function(){this.getLinks().attr("x1",function(a){return a.source.x}).attr("y1",function(a){return a.source.y}).attr("x2",function(a){return a.target.x}).attr("y2",function(a){return a.target.y});this.getNodes().attr("transform",function(a){var b=a.x,c=a.y;if(this.getNetworkMode()===this.DEFAULT_MODE||"keyword"!==a.type)b-=.5*a.bbox.width,c-=.5*a.bbox.height;return"translate("+b+","+c+")"}.bind(this));.075>this.getVisLayout().alpha()&&this.getVisLayout().alpha(-1)}.bind(this)).on("end",
...
b[f]&&f!=g&&b[f].collocates.push({term:g,value:e})}this.setMaxCollocateValue(c);a=[];for(f in b)a.push(b[f]);return a},buildVisFromData:function(a){var b=this;if(this.getVis()){a.push({term:"$$$root$$$",collocates:[],rawFreq:1});var c=d3.stratify().id(function(a){return a.term}).parentId(function(a){return"$$$root$$$"!==a.term?"$$$root$$$":""})(a).sort(function(a,b){return a.rawFreq<b.rawFreq?1:a.rawFreq>b.rawFreq?-1:0}).sum(function(a){return Math.pow(a.rawFreq,1/b.getScalingFactor())});this.getVisLayout()&&
this.getVisLayout()(c);c=this.getVis().selectAll(".node").data(c.descendants());c.attr("transform",function(a){return"translate("+a.x+","+a.y+")"});c.selectAll("circle").attr("r",function(a){return a.r});var d=d3.scalePow().exponent(1/3).domain([0,this.getMaxCollocateValue()]).range(["#fff","#bd3163"]),e;e=this.getMode()===this.MODE_DISTINCT?d3.scalePow().exponent(.2).domain([this.getMinFillValue(),this.getMaxFillValue()]).range(["#dedede","#fff"]):d3.scaleLinear().domain([this.getMaxFillValue(),
this.getMinFillValue()]).range(["#dedede","#fff"]);var f=this.getVisLayout().size(),f=Math.min(f[0],f[1])/2;a=Math.sqrt(Math.PI*f*f/a.length/Math.PI)/3;var f=Math.abs(this.getScalingFactor()-(this.MAX_SCALING+1)),f=Math.max(1,f-1),f=a*f,g=d3.scaleLinear().domain([this.getMinRawFreq(),this.getMaxRawFreq()]).range([a,f]);a=c.enter().append("g").attr("class","node").style("visibility",function(a){return 0<a.depth?"visible":"hidden"}).attr("transform",function(a){return"translate("+a.x+","+a.y+")"}).on("click",
function(a){b.dispatchEvent("termsClicked",b,[a.data.term])}).on("mouseover",function(a,c){b.setCurrentNode(a);b.getVis().selectAll("circle").style("stroke-width",1).style("stroke","#111").style("fill",function(a){return e(a.data.fillValue)});d3.select(this).select("circle").style("fill","#89e1c2").style("stroke","#26926c").style("stroke-opacity",b.MAX_STROKE_OPACITY);var f;f=b.getMode()===b.MODE_DISTINCT?b.localize("tfidf"):b.localize("inDocs");if(!b.getContextMenu().isVisible()){f="\x3cb\x3e"+a.data.term+
"\x3c/b\x3e ("+a.data.rawFreq+")\x3cbr/\x3e"+f+": "+a.data.fillValue;var g=b.getTip();g.update(f);g.show()}for(c=0;c<a.data.collocates.length;c++){var n=a.data.collocates[c];f=b.getVis().selectAll(".node").filter(function(a){return a.data.term===n.term});f.select("circle").style("fill",function(a){return d(n.value)}).style("stroke","#bd3163").style("stroke-opacity",b.MAX_STROKE_OPACITY);f.select("tspan.value").text(function(a){return n.value})}}).on("mousemove",function(){b.getTip().setPosition(d3.event.pageX+
...
50)});a.append("circle").attr("id",function(a){return a.data.term.replace(/\W/g,"_")}).attr("r",function(a){return a.r}).style("fill",function(a){return e(a.data.fillValue)}).style("stroke","#111").style("stroke-opacity",b.MIN_STROKE_OPACITY).style("stroke-width",1);a.append("clipPath").attr("id",function(a){return"clip-"+a.data.term.replace(/\W/g,"_")}).append("use").attr("xlink:href",function(a){return"#"+a.data.term.replace(/\W/g,"_")});a=a.append("text").attr("clip-path",function(a){return"url(#clip-"+
a.data.term.replace(/\W/g,"_")+")"}).style("font-family",function(a){return b.getApplication().getFeatureForTerm("font",a.data.term)}).style("text-anchor","middle").style("cursor","default");a.append("tspan").attr("class","term").style("font-size",function(a){return g(a.data.rawFreq)}).attr("x",0).attr("y",function(a){return g(a.data.rawFreq)/4}).text(function(a){return a.data.term});a.append("tspan").attr("class","value").style("font-size",function(a){return.75*g(a.data.rawFreq)}).attr("x",0).attr("y",
function(a){return g(a.data.rawFreq)+1});c.exit().remove()}},initVisLayout:function(){var a=this.getLayout().getRenderTarget();a.update("");var b=a.getWidth(),c=a.getHeight();this.setVisLayout(d3.pack().size([b,c]).padding(1.5));a=d3.select(a.dom).append("svg").attr("id",this.getVisId()).attr("width",b).attr("height",c);this.setVis(a.append("g"));this.setVisInfo(a.append("text").attr("x",10).attr("y",10));void 0===this.getTip()&&this.setTip(Ext.create("Ext.tip.Tip",{}))}});Ext.define("Voyant.panel.TermsRadio",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.termsradio",config:{options:[{xtype:"stoplistoption"}],speed:50,termsRadio:void 0},statics:{i18n:{},api:{withDistributions:!0,bins:5,visibleBins:5,docIdType:null,limit:50,mode:null,position:0,selectedWords:[],stopList:"auto",query:null,yAxisScale:"log",speed:50,slider:void 0},glyph:"xf201@FontAwesome"},constructor:function(a){var b=function(a,b,e,f,g){this.setApiParams({mode:a});this.getTermsRadio().loadRecords(e);
...
Ext.override(Ext.chart.axis.sprite.Axis,{renderLabels:function(a,b,c,d){var e=this,f=e.attr,g=.5*f.lineWidth,h=f.position,k=f.matrix,l=f.textPadding,m=k.getXX(),n=k.getDX(),q=k.getYY(),r=k.getDY(),p=0,k=c.majorTicks,s=Math.max(f.majorTickSize,f.minorTickSize)+f.lineWidth,v=Ext.draw.Draw.isBBoxIntersect,u=e.getLabel(),G,L,B=null,E=0,z=0,H=c.segmenter,C=e.getRenderer(),A=e.getAxis(),K=(G=A.getTitle())&&""!==G.attr.text&&G.getBBox(),I,J=null,F,x,w;if(k&&u&&!u.attr.hidden){G=u.attr.font;b.font!==G&&(b.font=
...
function(b,d,f){void 0!==d&&(w=C?Ext.callback(C,null,[A,d,c,B],0,A):H.renderer(d,c,B),B=d,u.setAttributes({text:String(w),translationY:a.roundPixel(b*q+r)},!0),u.applyTransformations(),p=Math.max(p,u.getBBox().width+s),p<=e.thickness&&(x=Ext.draw.Matrix.fly(u.attr.matrix.elements.slice(0)),F=x.prepend.apply(x,I).transformBBox(u.getBBox(!0)),!J||v(F,J,l)))&&(a.renderSprite(u),J=F,E+=F.height,z++)});else if("top"===h||"bottom"===h)e.iterate(k,function(b,d,f){void 0!==d&&(w=C?Ext.callback(C,null,[A,
d,c,B],0,A):H.renderer(d,c,B),B=d,u.setAttributes({text:String(w),translationX:a.roundPixel(b*m+n)},!0),u.applyTransformations(),p=Math.max(p,u.getBBox().height+s),p<=e.thickness&&(x=Ext.draw.Matrix.fly(u.attr.matrix.elements.slice(0)),F=x.prepend.apply(x,I).transformBBox(u.getBBox(!0)),J&&v(F,J,l),a.renderSprite(u),J=F,E+=F.width,z++))});else if("radial"===h)e.iterate(k,function(b,d,e){void 0!==d&&(w=C?Ext.callback(C,null,[A,d,c,B],0,A):H.renderer(d,c,B),B=d,"undefined"!==typeof w&&(u.setAttributes({text:String(w),
translationX:f.centerX-a.roundPixel(b)/f.max*f.length*Math.cos(f.baseRotation+Math.PI/2),translationY:f.centerY-a.roundPixel(b)/f.max*f.length*Math.sin(f.baseRotation+Math.PI/2)},!0),u.applyTransformations(),F=u.attr.matrix.transformBBox(u.getBBox(!0)),!J||v(F,J)))&&(a.renderSprite(u),J=F,E+=F.width,z++)});else if("angular"===h)L=f.majorTickSize+.5*f.lineWidth+(parseInt(u.attr.fontSize,10)||10)/2,e.iterate(k,function(b,d,e){void 0!==d&&(w=C?Ext.callback(C,null,[A,d,c,B],0,A):H.renderer(d,c,B),B=d,
p=Math.max(p,Math.max(f.majorTickSize,f.minorTickSize)+("butt"!==f.lineCap?.5*f.lineWidth:0)),"undefined"!==typeof w&&(b=b/(f.max+1)*Math.PI*2+f.baseRotation,u.setAttributes({text:String(w),translationX:f.centerX+(f.length+L)*Math.cos(b),translationY:f.centerY+(f.length+L)*Math.sin(b)},!0),u.applyTransformations(),F=u.attr.matrix.transformBBox(u.getBBox(!0)),!J||v(F,J)))&&(a.renderSprite(u),J=F,E+=F.width,z++)});else if("gauge"===h){var D=e.getGaugeAngles();e.iterate(k,function(b,d,e){void 0!==d&&
(w=C?Ext.callback(C,null,[A,d,c,B],0,A):H.renderer(d,c,B),B=d,"undefined"!==typeof w&&(b=(b-f.min)/(f.max-f.min+1)*f.totalAngle-f.totalAngle+D.start,u.setAttributes({text:String(w),translationX:f.centerX+(f.length+10)*Math.cos(b),translationY:f.centerY+(f.length+10)*Math.sin(b)},!0),u.applyTransformations(),F=u.attr.matrix.transformBBox(u.getBBox(!0)),!J||v(F,J)))&&(a.renderSprite(u),J=F,E+=F.width,z++)})}f.enlargeEstStepSizeByText&&z&&(E/=z,E+=s,E*=2,f.estStepSize<E&&(f.estStepSize=E));if(1<Math.abs(e.thickness-
...
"method"!=c.tagname||(b.docs[a][c.name]={"!type":"fn()","!url":"","!doc":c.short_doc+h.apply(["-"+c.id])})});b.docsLoading--},function(a,b){console.log("server-side failure with status code "+a.status)})},this);Ext.apply(a,{title:this.localize("title"),includeTools:{help:!0,save:!0,saveIt:{tooltip:this.localize("saveItTip"),callback:function(){b.mask(b.localize("saving"));Ext.Ajax.request({url:this.getTromboneUrl(),params:{tool:"notebook.NotebookManager",jsonData:Ext.encode(b.getExportAllJson())},
scope:this}).then(function(a,c){b.unmask();var f=Ext.decode(a.responseText);if(f&&f.notebook&&f.notebook.notebook){var g=b.getBaseUrl()+"spiral/"+f.notebook.notebook,h=b.getApplication().getModifiedApiParams();h&&(g+="?"+Ext.Object.toQueryString(h));window.history.pushState({url:g},"Spiral Notebook: "+f.notebook.notebook,g);b.toastInfo({html:b.localize("savedUrlChanged"),anchor:"tr"})}else b.showError(b.localize("savedFailed"))},function(a,c){b.unmask();b.showResponseError("Unable to save this notebook.",
...
"Spiral Notebook: "+a,this.getBaseUrl()+"spiral/?input\x3d"+a);c.mask(this.localize("fetchingNotebook"));Ext.Ajax.request({url:this.getTromboneUrl(),params:{tool:"notebook.NotebookManager",notebook:a,noCache:!0},scope:this}).then(function(d,e){c.unmask();var h=Ext.decode(d.responseText),k=c.getBlocksFromString(h.notebook.jsonData);k.metadata=k.metadata||{};k.metadata.previousNotebook=a;Ext.applyIf(k.metadata,{originalUrl:a});if(k&&k.metadata&&k.metadata.url){var l;try{l=new URL(k.metadata.url)}catch(m){}if(l&&
