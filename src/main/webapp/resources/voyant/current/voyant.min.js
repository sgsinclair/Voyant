/* This file created by JSCacher. Last modified: Sun Feb 11 14:12:40 EST 2018 */
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];l=0;f=function(){var a,f,h,k,n=this.wordsToArrange-1,q=g.canvas.width,u=g.canvas.height,r=.5*q,v=.5*u;do{h=p[l];if(void 0!==h){for(var B=Math.random()*Math.PI,w=.25*Math.random()*h.size,D=.5*(Math.random()-.5),M=.5*h.width,N=.5*h.height;;){a=Math.floor((r+Math.cos(B)*w*this.ratio-M)/this.COARSENESS)*this.COARSENESS;f=Math.floor((v+Math.sin(B)*w-N)/this.COARSENESS)*this.COARSENESS;
k=a+M>=q||f+N>=u?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;B+=D;w+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<l;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}l++;if(l>=p.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(n--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;m=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
h,l,m);H=new doubletree.Trie(g,h,l,m);g=c.length;for(h=0;h<g;h++){l=f?f[h]:h;m=c[h];var n=a[h].slice(),q=d[h].slice();n.push(m);n.reverse();q.unshift(m);k?(H.addNgram(n,l),z.addNgram(q,l)):(z.addNgram(n,l),H.addNgram(q,l))}b.setupFromTries(z,H);return b};b.filteredIDs=function(){return K};b.search=function(a){C.search(a);A.search(a);a=d3.select(f[0]);var b=a.selectAll("text.foundText");if(b.empty())return 0;b=b[0].length;null!=a.selectAll("text.rtNdText.foundText").node()&&b--;return b};b.clearSearch=
...
"-"+a.target}).join("; ")}});Ext.define("Voyant.data.util.Geonames",{mixins:["Voyant.util.Localization"],config:{data:{},queries:void 0,corpus:void 0},constructor:function(a,b){a=a||{};this.callParent([a]);this.setCorpus(a.corpus)},load:function(a){this.setData({});var b=Voyant.application.getDeferred(this),c=this,d={corpus:this.getCorpus().getAliasOrId(),queries:this.getQueries(),tool:"corpus.Geonames"};Ext.apply(d,a||{});Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:d,scope:this}).then(function(a){(a=Ext.JSON.decode(a.responseText))&&
a.geonames&&(c.setData(a.geonames),b.resolve(c))},function(a){Voyant.application.showResponseError(c.localize("failedToFetchGeonames"),a)});return b.promise},eachCity:function(a,b){var c=this.getData().cities,d=[],e;for(e in c)d.push(Ext.apply(c[e],{id:e}));d.sort(function(a,b){return a.rawFreq==b.rawFreq?a.label-b.label:b.rawFreq-a.rawFreq});d.forEach(function(c){a.call(b,c)})},eachConnection:function(a,b){var c=this.getData().connectionCounts,d=this.getData().cities;orderedConnections=[];for(var e in c){var f=
e.split("-");orderedConnections.push({source:Ext.apply({id:f[0]},d[f[0]]),target:Ext.apply({id:f[1]},d[f[1]]),rawFreq:c[e]})}orderedConnections.sort(function(a,b){return b.rawFreq-a.rawFreq});orderedConnections.forEach(function(c){a.call(b,c)})},eachConnectionOccurrence:function(a,b){var c=this.getData().cities;this.getData().connections.connections.forEach(function(d){Ext.apply(d.source,c[d.source.id]);Ext.apply(d.target,c[d.target.id]);a.call(b,d)})}});Ext.define("Voyant.data.model.Corpus",{alternateClassName:["Corpus"],mixins:["Voyant.notebook.util.Embed","Voyant.notebook.util.Show","Voyant.util.Transferable","Voyant.util.Localization","Voyant.util.Assignable"],transferable:"loadCorpusTerms loadTokens getPlainText getText getWords getString getLemmasArray".split(" "),embeddable:"Voyant.panel.Summary Voyant.panel.Cirrus Voyant.panel.Documents Voyant.panel.CorpusTerms Voyant.panel.Reader Voyant.panel.Trends Voyant.panel.TermsRadio Voyant.panel.DocumentTerms Voyant.panel.TermsBerry Voyant.panel.CollocatesGraph Voyant.panel.Contexts Voyant.panel.WordTree Voyant.panel.Veliza Voyant.panel.ScatterPlot Voyant.panel.Topics".split(" "),
...
api:{stopList:"auto",author:void 0,title:void 0,keyword:void 0,pubDate:void 0,minPopulation:15E3},glyph:"xf124@FontAwesome"},config:{map:void 0,overlay:void 0,contentEl:void 0,isOpenLayersLoaded:!1,isArcLoaded:!1,maxResults:400,zoomTreshold:80,sizeRatio:5E4,totalEntries:0,nbOfEntries:[],pointsPerMs:.3,pointsPerArc:100,delayBetweenVectors:100/.3,filterCount:0,timedEvents:[[]],cities:[],coordinatesSequence:[],travels:[],texts:[],colors:"rgb(230, 25, 75);rgb(0,92,49);rgb(145, 30, 180);rgb(128, 0, 0);rgb(0, 0, 128);rgb(60, 180, 75);rgb(143,124,0);rgb(157,204,0)".split(";"),
filters:[],corpus:void 0},html:'\x3cdiv class\x3d"map"\x3e\x3c/div\x3e\x3cdiv class\x3d"ol-popup"\x3e\x3ca href\x3d"#" class\x3d"ol-popup-closer"\x3e\x3c/a\x3e\x3cdiv class\x3d"popup-content"\x3e\x3c/div\x3e\x3c/div\x3e',cls:"dreamscape",constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments);Ext.Loader.loadScript({url:this.getBaseUrl()+"resources/openlayers/ol.js",onLoad:function(){void 0===ol.Map.prototype.getLayer&&(ol.Map.prototype.getLayer=
function(a){var c=void 0;this.getLayers().forEach(function(d){a===d.get("id")&&(c=d)});return c});this.setIsOpenLayersLoaded(!0)},scope:this});Ext.Loader.loadScript({url:this.getBaseUrl()+"resources/dreamscape/arc.js",onLoad:function(){this.setIsArcLoaded(!0)},scope:this})},initComponent:function(){Ext.apply(this,{title:this.localize("title"),dockedItems:[{dock:"bottom",xtype:"toolbar",overflowHandler:"scroller",items:[{text:this.localize("show"),tooltip:this.localize("showTip"),menu:{defaults:{xtype:"menucheckitem",
checkHandler:function(a,b){this.getMap().getLayer(a.getItemId()+"0").setVisible(b)},scope:this,checked:!0},items:[{text:this.localize("cities"),itemId:"cities"},{text:this.localize("connections"),itemId:"layer"}]}},{text:this.localize("filter"),tooltip:this.localize("filterTip"),menu:{defaults:{xtype:"querysearchfield",labelWidth:60,labelAlign:"right",width:250,maxWidth:250,listeners:{scope:this,change:function(a,b){filters=this.getFilters();filter=filters[0];filter[a.getItemId()]=b[0];console.warn(a,
b)}}},items:[{fieldLabel:this.localize("authorLabel"),tokenType:"author",itemId:"author"},{fieldLabel:this.localize("titleLabel"),itemId:"title",tokenType:"title"},{fieldLabel:this.localize("keywordLabel"),hidden:!0},{xtype:"multislider",fieldLabel:this.localize("pubDateLabel"),itemId:"pubDate",hidden:!0},{xtype:"button",text:"filter",scope:this,handler:function(){var a=this.getFilters()[0];this.filter(0,a.author?a.author:"",a.title?a.title:"",a.yearBegin?a.yearBegin:"",a.yearEnd?a.yearEnd:"")}}]}}]}]});
this.on("loadedCorpus",function(a,b){(new Voyant.data.store.CorpusTerms({corpus:b})).load({params:{tokenType:"author",limit:1},callback:function(a,b,e){e&&a&&1==a.length?this.down("toolbar").getComponent("author").show():this.toastInfo(this.localize("noauthor"))},scope:this});(new Voyant.data.store.CorpusTerms({corpus:b})).load({params:{tokenType:"pubDate",sort:"TERMASC",limit:1},callback:function(a,d,e){if(e&&!a&&1==a.length){var f=parseInt(a[0].getTerm());(new Voyant.data.store.CorpusTerms({corpus:b})).load({params:{tokenType:"pubDate",
sort:"TERMDESC",limit:1},callback:function(a,b,c){c&&!a&&1==a.length&&(a=parseInt(a[0].getTerm()),b=this.down("toolbar").getComponent("pubDate"),b.setMinValue(f),b.setMaxValue(a),b.show())},scope:this})}else this.toastInfo(this.localize("nopubDate"))},scope:this});this.tryInit()},this);this.callParent()},tryInit:function(){if(this.getIsOpenLayersLoaded()&&this.getIsArcLoaded()){var a=this.body.down(".map").setId(Ext.id()),b=this.body.down(".ol-popup"),c=new ol.Overlay({element:b.dom,autoPan:!0,autoPanAnimation:{duration:250}});
this.setOverlay(c);var d=b.down(".ol-popup-closer").dom;d.onclick=function(){c.setPosition(void 0);d.blur();return!1};this.setContentEl(b.down(".popup-content"));var e=new ol.Map({layers:[new ol.layer.Tile({preload:Infinity,source:new ol.source.Stamen({layer:"watercolor"})}),new ol.layer.Tile({preload:Infinity,source:new ol.source.Stamen({cacheize:2048,layer:"toner-hybrid"})})],target:a.getId(),loadTilesWhileInteracting:!0,overlays:[c],view:new ol.View({center:[15E5,6E6],minResolution:50,zoom:0})}),
f=this;e.on("singleclick",function(a){var b=e.getFeaturesAtPixel(a.pixel);texts=f.getTexts();if(b){var c="\x3cul\x3e";b.forEach(function(b){if("Circle"===b.getGeometry().getType()&&b.get("selected")){var d=b.get("occurences");d.forEach(function(a){c+="\x3cli\x3e"+a.name+" : "+texts[a.docIndex].authors+', \x3ca href\x3d"#" onclick\x3d"console.log(\'triggerEventHere\'+'+a.offset+');return false;"\x3e'+texts[a.docIndex].title+"\x3c/a\x3e, "+texts[a.docIndex].year+" "+a.offset+"\x3c/li\x3e"});d=b.get("text");
c+="\x3c/ul\x3e";0<b.get("alternates").length&&(d+=" ("+b.get("alternates")+")");b=a.coordinate;f.getContentEl().setHtml("\x3ch3\x3e"+d+"\x3c/h3\x3e"+c);f.getOverlay().setPosition(b)}else b.get("selected")&&(d=b.get("occurences"),d.forEach(function(a){c+="\x3cli\x3efrom : "+a.from.name+" : "+texts[a.from.docIndex].authors+', \x3ca href\x3d"#" onclick\x3d"console.log(\'triggerEventHere\'+'+a.from.offset+');return false;"\x3e'+texts[a.from.docIndex].title+"\x3c/a\x3e, "+texts[a.from.docIndex].year+
", to: "+a.to.name+" : "+texts[a.to.docIndex].authors+' , \x3ca href\x3d"#" onclick\x3d"console.log(\'triggerEventHere\'+'+a.to.offset+');return false;"\x3e'+texts[a.to.docIndex].title+"\x3c/a\x3e, "+texts[a.to.docIndex].year+"\x3c/li\x3e"}),d=b.get("text"),c+="\x3c/ul\x3e",b=a.coordinate,f.getContentEl().setHtml("\x3ch3\x3e"+d+"\x3c/h3\x3e"+c),f.getOverlay().setPosition(b))})}});var g=new ol.layer.Vector({map:e,source:new ol.source.Vector({wrapX:!1,useSpatialIndex:!1}),zIndex:10,selected:!0,style:function(a){return"Circle"===
a.getGeometry().getType()?f.cityStyleFunction(a,e.getView().getResolution()):f.travelStyleFunction(a,e.getView().getResolution())},updateWhileAnimating:!0,updateWhileInteracting:!0});e.on("pointermove",function(a){g.getSource().clear();var b=a.coordinate;if(a=e.getFeaturesAtPixel(a.pixel)){for(var c=0;a[c].get("selected")&&(c++,c!==a.length););if(c<a.length){c=a[c];a={font:"12px Calibri,sans-serif",textAlign:"center",offsetY:-15,fill:new ol.style.Fill({color:[0,0,0,1]}),stroke:new ol.style.Stroke({color:[255,
255,255,.5],width:4})};a.text=c.get("text");a=new ol.style.Style({text:new ol.style.Text(a),zIndex:1});var d=new ol.Feature({text:c.get("text"),geometry:c.getGeometry(),occurences:c.get("occurences"),alternates:c.get("alternates"),selected:!0,filterId:c.get("filterId"),coordinates:c.get("coordinates"),width:c.get("width"),visible:c.get("visible")});g.getSource().addFeature(d);b=c.getGeometry().getClosestPoint(b);b=new ol.Feature({geometry:new ol.geom.Point(b),selected:!0});b.setStyle(a);g.getSource().addFeature(b)}}});
this.setMap(e);this.addFilter()}else Ext.defer(this.tryInit,500,this)},travelStyleFunction:function(a,b){var c="rgb(255, 0, 0)";a.get("selected")?c="rgb(0, 0, 255)":a.get("color")&&(c=a.get("color"));var d=.5+Math.sqrt(a.get("occurences").length/parseFloat(this.getTotalEntries())*this.getSizeRatio()*.2),c=new ol.style.Stroke({color:c,width:d}),d=[new ol.style.Style({stroke:c})],e=a.getGeometry(),f=e.getLastCoordinate(),e=e.getCoordinateAt(.9),e=Math.atan2(f[1]-e[1],f[0]-e[0]),g=new ol.geom.LineString([f,
[f[0]-10*b,f[1]+10*b]]);g.rotate(e,f);var h=new ol.geom.LineString([f,[f[0]-10*b,f[1]-10*b]]);h.rotate(e,f);d.push(new ol.style.Style({geometry:g,stroke:c}));d.push(new ol.style.Style({geometry:h,stroke:c}));return d},cityStyleFunction:function(a,b){var c="rgb(255, 0, 0)";a.get("selected")?c="rgb(0, 0, 255)":a.get("color")&&(c=a.get("color"));return a.get("visible")?new ol.style.Style({stroke:new ol.style.Stroke({color:c,width:a.get("width")})}):!1},animationStyleFunction:function(a){var b="rgb(255, 0, 0)";
a.get("color")&&(b=a.get("color"));return new ol.style.Style({stroke:new ol.style.Stroke({color:b,width:6})})},addLater:function(a,b,c){var d=this.getMap();b=window.setTimeout(function(){a.set("start",(new Date).getTime());d.getLayer("animation"+c).getSource().addFeature(a)},b);this.getTimedEvents()[c].push(b)},animateTravels:function(a,b){var c=this.getMap(),d=this,e=a.vectorContext,f=a.frameState;c.getLayer("animation"+b).getSource().getFeatures().forEach(function(a){if(!a.get("finished"))if("Circle"===
a.getGeometry().getType())a.set("finished",!0);else{var b=a.getGeometry().getCoordinates(),c=(f.time-a.get("start"))*d.getPointsPerMs();c>=b.length&&a.set("finished",!0);c=Math.min(c,b.length);b=new ol.geom.LineString(b.slice(0,c));e.setStyle(this.animationStyleFunction(a));e.drawGeometry(b)}},this);c.render()},fetchDataFromStaticFile:function(){var a=this.getBaseUrl()+"resources/dreamscape/datafile.json";return fetch(a).then(function(a){return a.json()})},fetchDataFromServer:function(a,b,c,d,e){var f=
this,g={};Ext.apply(g,{minPopulation:this.getApiParam("minPopulation")});var h=[];a&&h.push("author:"+b);b&&h.push("title:"+b);c&&d&&h.push("pubDate:["+c+"-"+d+"]");h&&(g.query=h);e&&(g.limit=e);return(new Voyant.data.util.Geonames({corpus:this.getCorpus()})).load(g).then(function(a){var b=[];f.getCorpus().each(function(a){b.push({locs:[],filename:a.get("location"),URL:"",title:a.getTitle(),year:parseInt(a.getTitle()),authors:a.getAuthor()})});a.eachConnectionOccurrence(function(a){b[a.docIndex].locs.push({"long":a.source.longitude,
lat:a.source.latitude,conf:Math.random(),offset:a.source.position,name:a.source.label})});return{AnnotatedCollection:b}})},serverSideFiltering:function(a,b,c,d,e){return this.fetchDataFromServer(a,b,c,d,e).then(function(f){var g=[],h=[],k=[],l=0;f.AnnotatedCollection.forEach(function(e){e.authors.toLowerCase().includes(a.toLowerCase())&&e.title.toLowerCase().includes(b.toLowerCase())&&(""===c||e.year>=c)&&(""===d||e.year<=d)&&(k[l]={authors:e.authors,title:e.title,year:e.year},e.locs.forEach(function(a){coordinates=
[a.lat,a["long"]];if(g[coordinates]){var b=g[coordinates];b.occurences.push({name:a.name,coordinates:b.coordinates,docIndex:l,offset:a.offset,conf:a.conf});a.name!==b.name&&0>b.alternates.indexOf(a.name)&&b.alternates.push(a.name)}else g[coordinates]={name:a.name,alternates:[],coordinates:coordinates,occurences:[{name:a.name,coordinates:coordinates,docIndex:l,offset:a.offset,conf:a.conf}]}}));l++});f=[];for(coords in g)f.push(g[coords]);f.sort(function(a,b){return b.occurences.length-a.occurences.length});
f=f.splice(0,e);var m=[];f.forEach(function(a){a.occurences.forEach(function(a){h.push(a);m[a.docIndex]||(m[a.docIndex]=k[a.docIndex])})});h.sort(function(a,b){var c=a.docIndex-b.docIndex;return 0==c?a.offset-b.offset:c});return resultsJson=JSON.stringify({locations:f,entries:h,texts:m})})},filter:function(a,b,c,d,e){b="undefined"!==typeof b?b:"";c="undefined"!==typeof c?c:"";d="undefined"!==typeof d?d:"";e="undefined"!==typeof e?e:"";var f=this.getTimedEvents(),g=this.getCities(),h=this.getMap(),
k=this.getColors();f[a].forEach(function(a){window.clearTimeout(a)});g[a]=[];var l=h.getLayer("cities"+a);l.getSource().clear();f=h.getLayer("layer"+a);f.setVisible(!0);f.getSource().clear();var m=this;h.getView().on("change:resolution",function(){l.getSource().getFeatures().forEach(function(a){var b=h.getView().getZoom(),c=5+Math.sqrt(a.get("occurences").length/parseFloat(m.getTotalEntries())*m.getSizeRatio());a.set("width",c);a.set("visible",c*b>m.getZoomTreshold())});m.generateTravels(a)});this.serverSideFiltering(b,
c,d,e,this.getMaxResults()).then(function(a){return JSON.parse(a)}).then(function(b){console.log(b);m.setTexts(b.texts);nbOfEntries=m.getNbOfEntries();nbOfEntries[a]=b.entries.length;var c=0;nbOfEntries.forEach(function(a){c+=a});m.setTotalEntries(c);coordinatesSequence=m.getCoordinatesSequence();coordinatesSequence[a]=b.entries;b.locations.forEach(function(b){var c=[parseFloat(b.coordinates[1]),parseFloat(b.coordinates[0])],d=new ol.geom.Circle(c);d.transform(ol.proj.get("EPSG:4326"),ol.proj.get("EPSG:3857"));
b=new ol.Feature({geometry:d,description:b.name,alternates:b.alternates,text:b.name+"("+b.occurences.length+")",finished:!0,occurences:b.occurences,color:k[a],filterId:a,coordinates:c});l.getSource().addFeature(b);g[a][c]=b})}).then(function(){var b=h.getView().getResolution()+1;h.getView().setResolution(b);0<l.getSource().getFeatures().length&&h.getView().fit(l.getSource().getExtent());m.animateLayer(a)})},addFilter:function(){var a=this.getTimedEvents(),b=this.getFilterCount();map=this.getMap();
this.getFilters()[b]={};a[b]=[];a=new ol.layer.Vector({source:new ol.source.Vector({wrapX:!1,useSpatialIndex:!0}),id:"layer"+b,visible:!1,opacity:.4,style:this.travelStyleFunction.bind(this),updateWhileAnimating:!1,updateWhileInteracting:!1});map.addLayer(a);a=new ol.layer.Vector({source:new ol.source.Vector({wrapX:!1,useSpatialIndex:!0}),id:"cities"+b,opacity:.7,style:this.cityStyleFunction.bind(this)});map.addLayer(a);a=new ol.layer.Vector({source:new ol.source.Vector({wrapX:!1,useSpatialIndex:!0}),
id:"animation"+b,style:null});map.addLayer(a);this.setFilterCount(b+1);this.filter(b)},generateTravels:function(a){var b=this.getCities();if(0<Object.keys(b[a]).length){var c=map.getLayer("layer"+a);c.getSource().clear();var d=this.getTravels();d[a]=[];for(var e=void 0,f=!1,g=void 0,h=0;h<coordinatesSequence[a].length;h++){var k=coordinatesSequence[a][h];if(f){var l=[k.coordinates[1],k.coordinates[0]],m=[e,l];if((e[0]!==l[0]||e[1]!==l[1])&&b[a][l].get("visible"))if(d[a][m]){var n=d[a][m].get("occurences");
n.push({from:g,to:k});n=d[a][m].get("description")+"("+n.length+")";d[a][m].set("text",n)}else{var n=b[a][e].get("description"),q=b[a][l].get("description"),r=n+"-"+q;(new arc.GreatCircle({x:e[0],y:e[1]},{x:l[0],y:l[1]})).Arc(this.getPointsPerArc(),{offset:100}).geometries.forEach(function(b){b=new ol.geom.LineString(b.coords);b.transform(ol.proj.get("EPSG:4326"),ol.proj.get("EPSG:3857"));var f=this.getColors()[a];b=new ol.Feature({geometry:b,description:r,text:r+"(1)",finished:!0,occurences:[{from:g,
to:k}],color:f,filterId:a,start:e,end:l});c.getSource().addFeature(b);d[a][m]=b},this);e=l;g=k}}else e=[k.coordinates[1],k.coordinates[0]],b[a][e].get("visible")&&(g=k,f=!0)}}},animateLayer:function(a){var b=this.getTimedEvents(),c=this.getMap();b[a].forEach(function(a){window.clearTimeout(a)});var d=0,e=!1,f=0;c.getLayer("layer"+a).getSource().getFeatures().forEach(function(b){b=new ol.Feature({geometry:b.getGeometry(),color:b.get("color"),finished:!1});var c=this.getDelayBetweenVectors();b.getGeometry().getCoordinates().length<
this.getPointsPerArc()?e?(e=!1,this.addLater(b,f,a),d++):(this.addLater(b,d*c,a),e=!0,f=d*c+b.getGeometry().getCoordinates().length/this.getPointsPerMs()):(this.addLater(b,d*c,a),d++)},this);c.on("postcompose",function(b){this.animateTravels(b,a)},this)}});Ext.define("Voyant.panel.Knots",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.knots",statics:{i18n:{},api:{query:null,stopList:"auto",docId:void 0,audio:!1},glyph:"xf06e@FontAwesome"},config:{knots:void 0,termStore:void 0,docTermStore:void 0,tokensStore:void 0,options:[{xtype:"stoplistoption"},{xtype:"colorpaletteoption"}],refreshInterval:100,startAngle:315,angleIncrement:15,currentTerm:void 0},termTpl:new Ext.XTemplate('\x3ctpl for\x3d"."\x3e','\x3cdiv class\x3d"term" style\x3d"color: rgb({color});float: left;padding: 3px;margin: 2px;"\x3e{term}\x3c/div\x3e',
