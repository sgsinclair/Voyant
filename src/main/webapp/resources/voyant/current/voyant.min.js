...
this,b)}},loadCorpusTerms:function(a){if(2>this.getCorpus().getDocumentsCount()||"document"==this.getApiParam("mode")||0<Array.from(this.getApiParam("docIndex")||"").length)return this.loadDocumentTerms();if(this.getApiParam("query")){debugger;a=a||{};var b=this.getApiParam("withDistributions");Ext.applyIf(a,{bins:this.getCorpus().getDocumentsCount(),limit:1E3,stopList:""});var c=this.getCorpus().map(function(a){return a.getTinyTitle()});Ext.Array.unique(c).length<c.length&&(c=c.map(function(a,b){return b+
1+")"+a}));Ext.applyIf(a,this.getApiParams());this.getCorpus().getCorpusTerms().load({params:a,callback:function(a,e,f){var g=[],h=[],k=this.getApiParam("chartType");a.forEach(function(a,d){var e=a.get("term");a.get("docIndex");var f=this.getApplication().getColorForTerm(e,!0);a.get("distributions").forEach(function(a,f){g[f]||(g[f]={index:c[f]});g[f]["_"+d]="relative"==b?a.toFixed(7):a;g[f]["term"+d]=e},this);"bar"!=k&&("barline"==k?["bar","line"]:[k]).forEach(function(a){h.push({type:"stacked"==
a?"bar":a,title:e,xField:"index",yField:"_"+d,term:e,colors:[f],label:"barline"==k&&"bar"==a?{display:"none"}:{field:"term"+d}})},this)},this);e=a.map(function(a){return a.getTerm()});f=e.map(function(a){return this.getApplication().getColorForTerm(a,!0)},this);"bar"==k&&h.push({type:"bar",title:e,colors:f,xField:"index",yField:0<g.length?Object.keys(g[0]).filter(function(a){return"_"==a.charAt(0)}):void 0,label:{field:a.map(function(a,b){return"term"+b})}});a=Ext.create("Ext.data.JsonStore",{fields:0<
g.length?Object.keys(g[0]):void 0,data:g});this.buildChart({store:a,series:h,axes:[{type:"numeric",position:"left",increment:1,title:{text:this.localize(this.getApiParam("withDistributions")+"Title")}},{type:"category",position:"bottom",title:{text:this.localize("corpusTitle")}}]})},scope:this})}else this.getCorpus().getCorpusTerms().load({params:{limit:5,stopList:this.getApiParam("stopList")},callback:function(a,b,c){0==a.length?b&&b.error?this.showError(this.localize("noResults")+"\x3cp style\x3d'color: red'\x3e"+
b.error+"\x3c/p\x3e"):this.showError(this.localize("noResults")):(this.setApiParam("query",a.map(function(a){return a.getTerm()})),this.loadCorpusTerms())},scope:this})},getItemToolTip:function(a,b,c){var d=c.field.split("_"),e=2==d.length?c.index:d[2],d=parseInt(d[1]),f=c.series.getTitle(),f=Ext.isArray(f)?f[d]:f,g=c.series.getColors();b="\x3cspan class\x3d'x-legend-item-marker' style\x3d'background:"+(1==g.length?g[0]:g[d])+"; left: 2px;'\x3e\x3c/span\x3e \x3cspan style\x3d'padding-left: 1.2em; font-weight: bold;'\x3e"+
...
this.getCorpus().map(function(a){return a.getTinyTitle()});var c;c=1==this.getCorpus().getDocumentsCount()?this.getCorpus().getDocument(0):this.getCorpus().getDocument(this.getApiParam("docIndex"));Ext.applyIf(a,this.getApiParams());this.getCorpus().getDocumentTerms().load({params:a,callback:function(a,e,f){var g=[],h=[],k=this.getApiParam("chartType");c||a.sort(function(a,b){return a.getTerm()==b.getTerm()?a.getDocIndex()-b.getDocIndex():a.getTerm().localeCompare(b.getTerm())});a.forEach(function(a,
d){var e=a.get("term"),f=a.get("docIndex"),l=c?this.getApplication().getColorForTerm(e,!0):this.getApplication().getColor(f,!0),f=a.get("docIndex");a.get("distributions").forEach(function(a,c){g[c]||(g[c]={docIndex:f,index:c+1});g[c]["_"+d+"_"+f]="relative"==b?a.toFixed(7):a;g[c]["term"+d]=e},this);"bar"!=k&&("barline"==k?["bar","line"]:[k]).forEach(function(a){h.push({type:"stacked"==a?"bar":a,title:c?e:f+1+") "+e,xField:"index",yField:"_"+d+"_"+f,term:e,colors:[l],label:"barline"==k&&"bar"==a?{display:"none"}:
{field:"term"+d}})},this)},this);if("bar"==k){var l=Ext.Array.unique(a.map(function(a){return a.getTerm()})).length;e=a.map(function(a){return 1+a.get("docIndex")+") "+a.getTerm()});a=a.map(function(a){return l?this.getApplication().getColor(a.get("docIndex"),!0):this.getApplication().getColorForTerm(a.getTerm(),!0)},this);h.push({type:"bar",title:0<e.length?e:this.localize("noResults"),colors:a,xField:"index",yField:0<g.length?Object.keys(g[0]).filter(function(a){return"_"==a.charAt(0)}):void 0,
label:{field:e}})}a=Ext.create("Ext.data.JsonStore",{fields:0<g.length?Object.keys(g[0]):void 0,data:g});this.buildChart({store:a,series:h,axes:[{type:"numeric",position:"left",title:{text:this.localize(this.getApiParam("withDistributions")+"Title")}},{type:"category",position:"bottom",title:{text:this.localize("segmentsTitle")+(c?" ("+c.getShortTitle()+")":"")}}]})},scope:this})}else this.getCorpus().getCorpusTerms().load({params:{limit:2>this.getCorpus().getDocumentsCount()?5:2,stopList:this.getApiParam("stopList")},
