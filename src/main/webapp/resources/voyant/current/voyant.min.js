...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];l=0;f=function(){var a,f,h,k,n=this.wordsToArrange-1,s=g.canvas.width,t=g.canvas.height,q=.5*s,r=.5*t;do{h=p[l];if(void 0!==h){for(var v=Math.random()*Math.PI,w=.25*Math.random()*h.size,A=.5*(Math.random()-.5),y=.5*h.width,z=.5*h.height;;){a=Math.floor((q+Math.cos(v)*w*this.ratio-y)/this.COARSENESS)*this.COARSENESS;f=Math.floor((r+Math.sin(v)*w-z)/this.COARSENESS)*this.COARSENESS;
k=a+y>=s||f+z>=t?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;v+=A;w+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<l;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}l++;if(l>=p.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(n--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;m=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
"facet.authorTitle":{en:"Authors"},"facet.languageTitle":{en:"Languages"},"facet.titleTitle":{en:"Titles"},"facet.keywordTitle":{en:"Keywords"},"facet.pubDateTitle":{en:"Publication Dates"},"facet.publisherTitle":{en:"Publishers"},"facet.pubPlaceTitle":{en:"Publication Locations"},lexicalTitle:{en:"Terms"},noMatches:{en:new Ext.Template("No matches (out of {0} documents).",{compiled:!0})},queryMatches:{en:new Ext.Template("{0} matching documents (out of {1}).",{compiled:!0})},clickToOpenCorpus:{en:new Ext.Template('\x3ca href\x3d"{0}" target\x3d"_blank" class\x3d"link"\x3eClick here\x3c/a\x3e to access your new corpus',
{compiled:!0})},"export":{en:"Export"},exportTip:{en:"Create a new Voyant corpus with the selected documents."}},api:{config:void 0,stopList:"auto",facet:["facet.title","facet.author","facet.language"]},glyph:"xf1ea@FontAwesome"},config:{corpus:void 0,facets:{},matchingDocIds:[]},constructor:function(a){Ext.apply(this,{title:this.localize("title"),layout:"hbox",items:[{layout:"vbox",height:"100%",align:"stretch",itemId:"facets",defaults:{width:250,flex:1,xtype:"facet",margin:5,border:!0,frame:!0},
items:[]},{xtype:"panel",html:"",flex:1,itemId:"results",height:"100%",align:"stretch",scrollable:!0,margin:5,getCorpus:function(){return this.findParentByType("panel").getCorpus()},listeners:{query:function(a,c){this.findParentByType("panel").updateResults([c])}},bbar:[{itemId:"export",text:this.localize("export"),tooltip:this.localize("exportTip"),disabled:!0,handler:function(){this.mask(this.localize("exportInProgress"));var a=this;Ext.Ajax.request({url:this.getApplication().getTromboneUrl(),params:{corpus:this.getCorpus().getId(),
tool:"corpus.CorpusManager",keepDocuments:!0,docId:this.getMatchingDocIds()},success:function(c,d){a.unmask();var e=Ext.JSON.decode(c.responseText),e=a.getBaseUrl()+"?corpus\x3d"+e.corpus.id,f=window.open(e);f||(f=Ext.Msg.show({buttons:Ext.MessageBox.OK,buttonText:{ok:"Close"},icon:Ext.MessageBox.INFO,message:a.localize("clickToOpenCorpus",[e])}),Ext.Msg.getEl().dom.querySelector("a").addEventListener("click",function(){f.close()}))},failure:function(c,d){a.unmask();me.showError(c)}})},scope:this},
{xtype:"querysearchfield",width:200,flex:1},{itemId:"status",xtype:"tbtext"}]}]});this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments);this.on("loadedCorpus",function(a,c){this.setCorpus(c);this.queryById("status").update(this.localize("noMatches",[c.getDocumentsCount()]));this.query("facet").forEach(function(a){a.setCorpus(c)})});this.on("afterrender",function(a){var c=this.getApiParam("facet");Ext.isString(c)&&(c=[c]);var d=this.queryById("facets");c.forEach(function(c){var f=
d.add({title:a.localize(c+"Title"),facet:c,bbar:[{xtype:"querysearchfield",tokenType:c.replace("facet.",""),inDocumentsCountOnly:!0}]});f.getSelectionModel().on("selectionchange",function(d,f){var k=[];f.forEach(function(a){k.push({facet:a.getFacet(),label:a.getLabel()})});a.getFacets()[c]=k;a.updateResults()});f.on("query",function(c,d){a.getFacets()[f.facet]=[];a.updateResults()})});d.add({title:a.localize("lexicalTitle"),store:Ext.create("Voyant.data.store.CorpusTerms",{parentPanel:this}),facet:"lexical",
columns:[{renderer:function(a,b,c){return"("+c.getRawFreq()+") "+c.getTerm()},flex:1}],bbar:[{xtype:"querysearchfield"}]}).getSelectionModel().on("selectionchange",function(c,d){var g=[];d.forEach(function(a){g.push({facet:"lexical",label:a.getTerm()})});a.getFacets().lexical=g;a.updateResults()})})},updateResults:function(a){var b=this.getFacets();if(!a){a=[];for(facet in b)b[facet].forEach(function(b){a.push(b.facet+":"+b.label)});if(a)return this.updateResults(a)}var c=this.queryById("results").getTargetEl();
c.update("");this.queryById("export").setDisabled(!0);if(0<a.length){this.mask(this.localize("loading"));var d=this.getCorpus().getDocumentQueryMatches();d.load({params:{query:a,includeDocIds:!0},callback:function(a,f,g){this.unmask();if(a&&0<a.length){this.queryById("status").setHtml(a.length);var h="\x3cul\x3e",k=[];a.forEach(function(a){a.getDocIds().forEach(function(a){k.push(a);a=d.getCorpus().getDocument(a);var c;c="\x3cli\x3e"+("\x3ci\x3e"+a.getTitle()+"\x3c/i\x3e");for(facet in b)if(0!=b[facet].length){var e=
"";if("facet.title"!=facet){var f=facet.replace(/^.+?\./,""),g=a.get(f);if(g){var l=Ext.isArray(g);l?e+="\x3cli\x3e"+f+"\x3cul\x3e":g=[g];g.forEach(function(a){var c=!1;b[facet].forEach(function(b){b.label==a?c=!0:-1==b.facet.indexOf("facet")&&b.label.split(/\W+/).forEach(function(b){0<b.trim().length&&-1<a.toLowerCase().indexOf(b.toLowerCase())&&(c=!0)})});e+="\x3cli\x3e"+(l?"":f+": ")+(c?'\x3cspan class\x3d"keyword"\x3e'+a+"\x3c/span\x3e":a)+"\x3c/li\x3e"});l&&(e+="\x3c/ul\x3e\x3c/li\x3e")}}e&&
(c+="\x3cul\x3e"+e+"\x3c/ul\x3e")}h+=c+"\x3c/li\x3e"})});h+="\x3c/ul\x3e";c.update(h);this.queryById("status").update(this.localize("queryMatches",[k.length,this.getCorpus().getDocumentsCount()]));this.setMatchingDocIds(k);0<k.length&&this.queryById("export").setDisabled(!1)}},scope:this})}}});Ext.define("Voyant.panel.Cirrus",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.cirrus",statics:{i18n:{title:{en:"Cirrus"},helpTip:{en:"\x3cp\x3eCirrus provides a wordcloud view of the most frequently occurring words in the corpus or document \u2013\u00a0this provides a convenient (though reductive) overview of the content. Features include\x3c/p\x3e\x3cul\x3e\x3cli\x3eterm frequency appears when hovering over words\x3c/li\x3e\x3cli\x3eclicking on terms may produce results in other tools if any are displayed\x3c/li\x3e\x3c/ul\x3e"},
...
tokenFreqTip:{en:"\x3cb\x3e{0}\x3c/b\x3e\x3cbr/\x3e\x3cb\x3eRaw Frequency\x3c/b\x3e\x3cbr/\x3e{1}\x3c/b\x3e\x3cbr/\x3e\x3cb\x3eRelative Frequency\x3c/b\x3e\x3cbr/\x3e{2}\x3c/b\x3e"},docFreqTip:{en:"\x3cb\x3e{0}\x3c/b\x3e\x3cbr/\x3e\x3cb\x3eWord Count\x3c/b\x3e\x3cbr/\x3e{1}\x3c/b\x3e"},noTermSelected:{en:"No term selected."}},api:{analysis:"ca",limit:50,dimensions:3,bins:10,clusters:3,stopList:"auto",target:void 0,query:void 0},glyph:"xf06e@FontAwesome"},tokenFreqTipTemplate:null,docFreqTipTemplate:null,
caStore:null,pcaStore:null,termStore:Ext.create("Ext.data.JsonStore",{fields:[{name:"term"},{name:"rawFreq",type:"int"},{name:"relativeFreq",type:"number"},{name:"coordinates",mapping:"vector"}],sorters:[{property:"rawFreq",direction:"DESC"}]}),newTerm:null,termsTimeout:null,chartMenu:null,labelsMode:0,highlightData:{x:0,y:0,r:0},highlightTask:null,constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(){Ext.apply(this,
{title:this.localize("title"),caStore:Ext.create("Voyant.data.store.CAAnalysis"),pcaStore:Ext.create("Voyant.data.store.PCAAnalysis"),termStore:this.termStore,chartMenu:Ext.create("Ext.menu.Menu",{items:[{text:this.localize("remove"),itemId:"remove",glyph:"xf068@FontAwesome"},{text:this.localize("nearby"),itemId:"nearby",glyph:"xf0b2@FontAwesome"}],listeners:{hide:function(){var a=this.down("#chart").getSeries();a[0].enableToolTips();a[1].enableToolTips()},scope:this}}),layout:"border",autoDestroy:!0,
items:[{itemId:"chartParent",region:"center",layout:"fit",tbar:{enableOverflow:!0,items:[{text:this.localize("analysis"),itemId:"analysis",glyph:"xf1ec@FontAwesome",enableOverflow:!0,menu:{items:[{text:this.localize("pca"),itemId:"analysis_pca",group:"analysis",xtype:"menucheckitem"},{text:this.localize("ca"),itemId:"analysis_ca",group:"analysis",xtype:"menucheckitem"}],listeners:{click:function(a,b){void 0!==b&&(b.text===this.localize("pca")?this.setApiParam("analysis","pca"):(this.setApiParam("analysis",
"ca"),this.getCorpus().getDocumentsCount()),this.loadFromApis(!0))},scope:this}}},{text:this.localize("clusters"),itemId:"clusters",glyph:"xf192@FontAwesome",menu:{items:[{text:"1",itemId:"clusters_1",group:"clusters",xtype:"menucheckitem"},{text:"2",itemId:"clusters_2",group:"clusters",xtype:"menucheckitem"},{text:"3",itemId:"clusters_3",group:"clusters",xtype:"menucheckitem"},{text:"4",itemId:"clusters_4",group:"clusters",xtype:"menucheckitem"},{text:"5",itemId:"clusters_5",group:"clusters",xtype:"menucheckitem"}],
listeners:{click:function(a,b){void 0!==b&&(this.setApiParam("clusters",parseInt(b.text)),this.loadFromApis(!0))},scope:this}}},{text:this.localize("dimensions"),itemId:"dimensions",glyph:"xf1b2@FontAwesome",menu:{items:[{text:"2",itemId:"dimensions_2",group:"dimensions",xtype:"menucheckitem"},{text:"3",itemId:"dimensions_3",group:"dimensions",xtype:"menucheckitem"}],listeners:{click:function(a,b){void 0!==b&&(this.setApiParam("dimensions",parseInt(b.text)),this.loadFromApis(!0))},scope:this}}},{text:this.localize("labels"),
itemId:"labels",glyph:"xf02b@FontAwesome",handler:function(){this.labelsMode++;2<this.labelsMode&&(this.labelsMode=0);this.doLabels()},scope:this}]}},{itemId:"terms",xtype:"grid",region:"east",width:225,split:!0,bbar:{enableOverflow:!0,items:[{xtype:"button",text:this.localize("nearby"),glyph:"xf0b2@FontAwesome",handler:function(a){var b=this.down("#terms").getSelection()[0];void 0===b?this.toastError({html:this.localize("noTermSelected"),anchor:a.up("panel").getTargetEl()}):(a=b.get("term"),this.getNearbyForTerm(a))},
scope:this},{xtype:"button",text:this.localize("remove"),glyph:"xf068@FontAwesome",handler:function(a){var b=this.down("#terms").getSelection()[0];void 0===b?this.toastError({html:this.localize("noTermSelected"),anchor:a.up("panel").getTargetEl()}):(a=b.get("term"),this.removeTerm(a))},scope:this}]},dockedItems:[{dock:"top",xtype:"toolbar",enableOverflow:!0,items:[{fieldLabel:this.localize("numTerms"),labelAlign:"right",labelWidth:40,itemId:"limit",xtype:"combo",width:100,store:Ext.create("Ext.data.ArrayStore",
...
emptyText:this.localize("addTerm"),width:90}]}],columns:[{text:this.localize("term"),dataIndex:"term",flex:1,sortable:!0},{text:this.localize("rawFreq"),dataIndex:"rawFreq",width:"autoSize",sortable:!0},{text:this.localize("relFreq"),dataIndex:"relativeFreq",width:"autoSize",sortable:!0,hidden:!0}],selModel:Ext.create("Ext.selection.RowModel",{listeners:{selectionchange:{fn:function(a,b){var c=b[0];void 0!==c&&(c=c.get("term"),this.selectTerm(c))},scope:this}},mode:"SINGLE"}),store:this.termStore,
listeners:{query:function(a,b){void 0!==b&&-1===this.termStore.findExact("term",b)?(this.newTerm=b,this.loadFromApis()):this.newTerm=null},scope:this}}]});this.on("loadedCorpus",function(a,b){var c=function(a){var b=this.getApiParam(a);this.queryById("chartParent").getDockedItems("toolbar")[0].down("#"+a).down("#"+a+"_"+b).setChecked(!0)}.bind(this);c("analysis");c("clusters");c("dimensions");this.setCorpus(b);this.caStore.setCorpus(b);this.pcaStore.setCorpus(b);this.loadFromApis()},this);this.caStore.on("load",
function(a,b){this.buildChart(a)},this);this.pcaStore.on("load",function(a,b){this.buildChart(a)},this);this.tokenFreqTipTemplate=new Ext.Template(this.localize("tokenFreqTip"));this.docFreqTipTemplate=new Ext.Template(this.localize("docFreqTip"));this.callParent(arguments)},buildChart:function(a){var b=this,c=this.queryById("chart");null!==c&&this.queryById("chartParent").remove(c);var c=a.getAt(0),d=this.getApiParam("dimensions");a="";if("pca"==this.getApiParam("analysis")){for(var e=c.getPrincipalComponents(),
f=0,g=0;g<e.length;g++)var h=e[g],f=f+parseFloat(h.get("eigenValue"));a=this.localize("pcTitle")+"\n";for(var k=["xAxis","yAxis","fill"],g=0;g<e.length&&!(g>=d);g++)h=e[g],h=parseFloat(h.get("eigenValue"))/f*100,a+=this.localize("pc")+" "+(g+1)+" ("+this.localize(k[g])+"): "+Math.round(100*h)/100+"%\n"}else for(a=this.localize("caTitle")+"\n",k=["xAxis","yAxis","fill"],e=c.getDimensions(),g=0;g<e.length&&!(g>=d);g++)h=parseFloat(e[g].data),a+=this.localize("dimension")+" "+(g+1)+" ("+this.localize(k[g])+
"): "+Math.round(100*h)/100+"%\n";var l=0,m=Number.MAX_VALUE,n=0,q=Number.MAX_VALUE;this.termStore.removeAll();var r=[],p=[];c.getTokens().forEach(function(a){var b=a.get("rawFreq"),c="part"===a.get("category");c||(b>l&&(l=b),b<m&&(m=b),this.termStore.findExact("term",-1===a.get("term"))&&this.termStore.addSorted(a));if(3===d){var e=a.get("vector")[2];e<q&&(q=e);e>n&&(n=e)}b={term:a.get("term"),rawFreq:b,relativeFreq:a.get("relativeFreq"),cluster:a.get("cluster"),x:a.get("vector")[0],y:a.get("vector")[1],
z:a.get("vector")[2],isDoc:c};c?(b.docIndex=a.get("docIndex"),a=this.getCorpus().getDocument(b.docIndex),b.term=a.getTinyTitle(),b.title=a.getTitle(),p.push(b)):r.push(b)},this);c=this.termStore.getCount();this.queryById("limit").setRawValue(c);this.setApiParam("limit",c);c=Ext.create("Ext.data.JsonStore",{fields:"term x y z rawFreq relativeFreq cluster isDoc docIndex".split(" "),data:r});g=Ext.create("Ext.data.JsonStore",{fields:"term x y z rawFreq relativeFreq cluster isDoc docIndex".split(" "),
data:p});a={itemId:"chart",xtype:"cartesian",interactions:["crosszoom","panzoom","itemhighlight"],plugins:{ptype:"chartitemevents"},axes:[{type:"numeric",position:"bottom",fields:["x"]},{type:"numeric",position:"left",fields:["y"]}],sprites:[{type:"text",text:a,hidden:0<this.labelsMode,x:70,y:70}],innerPadding:{top:25,right:25,bottom:25,left:25},series:[{type:"customScatter",xField:"x",yField:"y",store:c,tooltip:{trackMouse:!0,style:"background: #fff",renderer:function(a,c){this.setHtml(b.tokenFreqTipTemplate.apply([a.get("term"),
a.get("rawFreq"),a.get("relativeFreq")]))}},marker:{type:"circle"},highlight:{fillStyle:"yellow",strokeStyle:"black"},renderer:function(a,b,c,e){c=c.store.getAt(e);if(null!==c){var f=c.get("cluster");a=this.getParent().up("scatterplot");-1===f&&(f=0);e=.65;3===d&&(e=a.interpolate(c.get("z"),q,n,0,1));f=a.getApplication().getColor(f);b.fillStyle="rgba("+f.join(",")+","+e+")";b.strokeStyle="rgba("+f.join(",")+",1)";c=c.get("rawFreq");a=a.interpolate(c,m,l,2,20);b.radius=a}}},{type:"customScatter",xField:"x",
yField:"y",store:g,tooltip:{trackMouse:!0,style:"background: #fff",renderer:function(a,c){this.setHtml(b.docFreqTipTemplate.apply([a.get("title"),a.get("rawFreq")]))}},marker:{type:"diamond"},highlight:{fillStyle:"yellow",strokeStyle:"black"},renderer:function(a,b,c,e){c=c.store.getAt(e);null!==c&&(c.get("cluster"),e=this.getParent().up("scatterplot"),a=.65,3===d&&(a=e.interpolate(c.get("z"),q,n,0,1)),c=e.getApplication().getColor(6),b.fillStyle="rgba("+c.join(",")+","+a+")",b.strokeStyle="rgba("+
c.join(",")+",1)",b.radius=5)}}],listeners:{itemclick:function(a,b,c){a=b.record.data;a.isDoc?(a=this.getCorpus().getDocument(a.docIndex),this.getApplication().dispatchEvent("documentsClicked",this,[a])):(a=Ext.create("Voyant.data.model.CorpusTerm",a),this.getApplication().dispatchEvent("corpusTermsClicked",this,[a]))},render:function(a){a.body.on("contextmenu",function(a,b){a.preventDefault();var c=a.getXY(),d=Ext.fly(b).getXY(),e=c[0]-d[0],d=c[1]-d[1],f=this.down("#chart").getItemForPoint(e,d);
null!=f&&1!=f.record.get("isDoc")&&(e=this.down("#chart").getSeries(),e[0].disableToolTips(),e[1].disableToolTips(),e=f.record.get("term"),d=(new Ext.Template(this.localize("removeTerm"))).apply([e]),this.chartMenu.queryById("remove").setText(d),d=(new Ext.Template(this.localize("nearbyTerm"))).apply([e]),this.chartMenu.queryById("nearby").setText(d),this.chartMenu.on("click",function(a,b){if(void 0!==b){var c=f.record.get("term");b.text===this.localize("remove")?this.removeTerm(c):this.getNearbyForTerm(c)}},
this,{single:!0}),this.chartMenu.showAt(c))},this)},scope:this}};2>this.labelsMode&&(a.series[0].label={field:"term",display:"over"},a.series[1].label={field:"term",display:"over"});a=Ext.create("Ext.chart.CartesianChart",a);this.queryById("chartParent").insert(0,a);null!==this.newTerm&&(this.selectTerm(this.newTerm),this.newTerm=null)},doLabels:function(){var a=this.queryById("chart"),b=a.getSeries(),c=a.getSurface("chart").getItems()[0];switch(this.labelsMode){case 0:b[0].getLabel().show();b[1].getLabel().show();
c.show();break;case 1:b[0].getLabel().show();b[1].getLabel().show();c.hide();break;case 2:b[0].getLabel().hide(),b[1].getLabel().hide(),c.hide()}a.redraw()},selectTerm:function(a){var b=this.down("#chart").getSeries()[0];a=b.getStore().findExact("term",a);if(-1!==a){var c=b.getStore().getAt(a),d=b.getSprites()[0],c={series:b,category:b.getItemInstancing()?"items":"markers",index:a,record:c,field:b.getYField(),sprite:d};b.setHighlightItem(c);b=this.getPointFromIndex(b,a);this.highlightData={x:b[0],
y:b[1],r:50};null==this.highlightTask&&(this.highlightTask=Ext.TaskManager.newTask({run:this.doHighlight,scope:this,interval:25,repeat:this.highlightData.r}));this.highlightTask.restart()}},getPointFromIndex:function(a,b){var c=a.getSprites()[0];return c.attr.matrix.clone().prependMatrix(c.surfaceMatrix).transformPoint([c.attr.dataX[b],c.attr.dataY[b]])},doHighlight:function(){var a=this.down("#chart");if(0<this.highlightData.r){for(var b=a.getSurface(),c=null,d=b.getItems(),e=0;e<d.length;e++){var f=
d[e];if("customHighlight"==f.id){c=f;break}}null==c?b.add({id:"customHighlight",type:"circle",strokeStyle:"red",fillStyle:"none",radius:this.highlightData.r,x:this.highlightData.x,y:this.highlightData.y}):(c.setAttributes({x:this.highlightData.x,y:this.highlightData.y,radius:this.highlightData.r}),this.highlightData.r-=1.5,0>=this.highlightData.r&&(this.highlightData.r=0,b.remove(c,!0)));a.redraw()}},getCurrentTerms:function(){var a=[];this.termStore.each(function(b){b.get("isDoc")||a.push(b.get("term"))});
return a},getNearbyForTerm:function(a){var b=Math.max(2E3,Math.round(this.getCorpus().getWordTokensCount()/100));this.setApiParams({limit:b,target:a});this.loadFromApis();this.setApiParam("target",void 0)},removeTerm:function(a){var b=this.down("#chart").getSeries()[0],c=b.getStore().findExact("term",a);b.getStore().removeAt(c);c=this.termStore.findExact("term",a);this.termStore.removeAt(c);a=this.termStore.getCount();this.queryById("limit").setRawValue(a)},loadFromApis:function(a){var b=this.down("#chart");
null!==b&&b.mask(this.localize("loading"));var b={},c=this.getCurrentTerms();null!==this.newTerm&&(c.push(this.newTerm),this.setApiParam("limit",c.length));if(0<c.length){if(null!==this.newTerm||a)b.query=c.join(",");b.term=c}Ext.apply(b,this.getApiParams());"pca"===b.analysis?this.pcaStore.load({params:b}):this.caStore.load({params:b})},interpolate:function(a,b,c,d,e){return d+(e-d)*Math.max(0,Math.min(1,(a-b)/(c-b)))}});
Ext.define("Ext.chart.series.CustomScatter",{extend:"Ext.chart.series.Scatter",alias:"series.customScatter",type:"customScatter",seriesType:"scatterSeries",tipsDisabled:!1,enableToolTips:function(){this.tipsDisabled=!1},disableToolTips:function(){this.tipsDisabled=!0},showTip:function(a,b){this.tipsDisabled||this.callParent(arguments)}});Ext.define("Voyant.panel.DocumentSimilarity",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],requires:["Ext.chart.CartesianChart"],alias:"widget.documentsimilarity",config:{corpus:void 0,options:{xtype:"stoplistoption"}},statics:{i18n:{title:{en:"Document Similarity"},loading:{en:"Loading"},helpTip:{en:""},freqsMode:{en:"Frequencies"},freqsModeTip:{en:"Determines if frequencies are expressed as relative counts or as TF-IDF."},options:{en:"Options"},tfidf:{en:"TF-IDF"},relativeFrequencies:{en:"Relative Frequencies"},
tokenFreqTip:{en:"\x3cb\x3e{0}\x3c/b\x3e\x3cbr/\x3e\x3cb\x3eRaw Frequency\x3c/b\x3e\x3cbr/\x3e{1}\x3c/b\x3e\x3cbr/\x3e\x3cb\x3eRelative Frequency\x3c/b\x3e\x3cbr/\x3e{2}\x3c/b\x3e"},docFreqTip:{en:"\x3cb\x3e{0}\x3c/b\x3e\x3cbr/\x3e\x3cb\x3eWord Count\x3c/b\x3e\x3cbr/\x3e{1}\x3c/b\x3e"},noTermSelected:{en:"No term selected."}},api:{limit:50,dimensions:3,clusters:3,stopList:"auto",docId:void 0,comparisonType:"relative"},glyph:"xf06e@FontAwesome"},tokenFreqTipTemplate:null,docFreqTipTemplate:null,chartMenu:null,
labelsMode:0,constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(){Ext.apply(this,{title:this.localize("title"),docSimStore:Ext.create("Voyant.data.store.DocSimAnalysis"),autoDestroy:!0,layout:"fit",items:{layout:"fit",itemId:"chartParent"},bbar:new Ext.Toolbar({items:[{xtype:"documentselectorbutton"},{text:this.localize("freqsMode"),glyph:"xf201@FontAwesome",tooltip:this.localize("freqsModeTip"),menu:{items:[{text:this.localize("relativeFrequencies"),
checked:!0,itemId:"relative",group:"freqsMode",checkHandler:function(a,b){b&&(this.setApiParam("comparisonType","relative"),this.loadFromApis())},scope:this},{text:this.localize("tfidf"),checked:!1,itemId:"tfidf",group:"freqsMode",checkHandler:function(a,b){b&&(this.setApiParam("comparisonType","tfidf"),this.loadFromApis())},scope:this}]}}]})});this.on("loadedCorpus",function(a,b){this.setCorpus(b);this.docSimStore.setCorpus(b);this.loadFromApis()},this);this.on("documentsSelected",function(a,b){this.setApiParam("docId",
b);this.loadFromApis()},this);this.docSimStore.on("load",function(a,b){this.buildChart(a)},this);this.tokenFreqTipTemplate=new Ext.Template(this.localize("tokenFreqTip"));this.docFreqTipTemplate=new Ext.Template(this.localize("docFreqTip"));this.callParent(arguments)},buildChart:function(a){var b=this,c=this.queryById("chart");null!==c&&this.queryById("chartParent").remove(c);a=a.getAt(0);var d=0,e=Number.MAX_VALUE,f=0,g=Number.MAX_VALUE,h=[],k=[];a.getTokens().forEach(function(a){var b=a.get("rawFreq"),
c="part"===a.get("category");c||(b>d&&(d=b),b<e&&(e=b));var q=a.get("vector")[2];q<g&&(g=q);q>f&&(f=q);b={term:a.get("term"),rawFreq:b,relativeFreq:a.get("relativeFreq"),cluster:a.get("cluster"),x:a.get("vector")[0],y:a.get("vector")[1],z:a.get("vector")[2],isDoc:c};c?(b.docIndex=a.get("docIndex"),a=this.getCorpus().getDocument(b.docIndex),b.term=a.getTinyTitle(),b.title=a.getTitle(),k.push(b)):h.push(b)},this);a=Ext.create("Ext.data.JsonStore",{fields:"term x y z rawFreq relativeFreq cluster isDoc docIndex".split(" "),
data:h});c=Ext.create("Ext.data.JsonStore",{fields:"term x y z rawFreq relativeFreq cluster isDoc docIndex".split(" "),data:k});a={itemId:"chart",xtype:"cartesian",interactions:["crosszoom","panzoom","itemhighlight"],plugins:{ptype:"chartitemevents"},axes:[{type:"numeric",position:"bottom",fields:["x"]},{type:"numeric",position:"left",fields:["y"]}],innerPadding:{top:25,right:25,bottom:25,left:25},series:[{type:"customScatter",xField:"x",yField:"y",store:a,tooltip:{trackMouse:!0,style:"background: #fff",
renderer:function(a,c){this.setHtml(b.tokenFreqTipTemplate.apply([a.get("term"),a.get("rawFreq"),a.get("relativeFreq")]))}},marker:{type:"circle"},highlight:{fillStyle:"yellow",strokeStyle:"black"},renderer:function(a,b,c,h){c=c.store.getAt(h);if(null!==c){var k=c.get("cluster");a=this.getParent().up("documentsimilarity");-1===k&&(k=0);h=.65;h=a.interpolate(c.get("z"),g,f,0,1);k=a.getApplication().getColor(k);b.fillStyle="rgba("+k.join(",")+","+h+")";b.strokeStyle="rgba("+k.join(",")+",1)";c=c.get("rawFreq");
a=a.interpolate(c,e,d,2,20);b.radius=a}}},{type:"customScatter",xField:"x",yField:"y",store:c,tooltip:{trackMouse:!0,style:"background: #fff",renderer:function(a,c){this.setHtml(b.docFreqTipTemplate.apply([a.get("title"),a.get("rawFreq")]))}},marker:{type:"diamond"},highlight:{fillStyle:"yellow",strokeStyle:"black"},renderer:function(a,b,c,d){c=c.store.getAt(d);null!==c&&(c.get("cluster"),d=this.getParent().up("documentsimilarity"),a=.65,a=d.interpolate(c.get("z"),g,f,0,1),c=d.getApplication().getColor(6),
b.fillStyle="rgba("+c.join(",")+","+a+")",b.strokeStyle="rgba("+c.join(",")+",1)",b.radius=5)}}],listeners:{itemclick:function(a,b,c){a=b.record.data;a.isDoc?(a=this.getCorpus().getDocument(a.docIndex),this.getApplication().dispatchEvent("documentsClicked",this,[a])):(a=Ext.create("Voyant.data.model.CorpusTerm",a),this.getApplication().dispatchEvent("corpusTermsClicked",this,[a]))},scope:this}};a.series[0].label={field:"term",display:"over"};a.series[1].label={field:"term",display:"over"};a=Ext.create("Ext.chart.CartesianChart",
a);this.queryById("chartParent").insert(0,a)},loadFromApis:function(a){a=this.down("#chart");null!==a&&a.mask(this.localize("loading"));a={};Ext.apply(a,this.getApiParams());this.docSimStore.load({params:a})},interpolate:function(a,b,c,d,e){return d+(e-d)*Math.max(0,Math.min(1,(a-b)/(c-b)))}});
...
function(a,c){return b(c)});f.enter().append("path").attr("d",function(a){return m(a.values)}).style("fill",function(a,c){return b(c)}).append("title").text(function(a){return a.name});f.exit().remove();this.getVis().selectAll("g.axis").remove();this.getVis().append("g").attr("class","axis x").attr("transform","translate(0,"+a+")").call(k);if(this.getMode()===this.MODE_CORPUS){var r=c/g,p=0;this.getCorpus().getDocuments().each(function(a){this.getVis().select("g.x").append("text").attr("text-anchor",
"end").attr("transform","translate("+p+", 10) rotate(-45)").text(a.getTinyTitle());p+=r},this);g=this.localize("documents")}else g=this.localize("documentSegments");this.getVis().select("g.x").append("text").attr("text-anchor","middle").attr("transform","translate("+c/2+", "+(this.graphMargin.bottom-20)+")").text(g);this.getVis().append("g").attr("class","axis y").attr("transform","translate(0,0)").call(n);c="raw"===this.getApiParam("withDistributions")?this.localize("rawFrequencies"):this.localize("relativeFrequencies");
