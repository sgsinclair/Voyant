...
a.addLayer(f);f=new ol.layer.Vector({source:new ol.source.Vector({wrapX:!1,useSpatialIndex:!0}),id:d.getId()+"-cities",opacity:.5,style:this.cityStyleFunction.bind(this)});a.addLayer(f);d=new ol.layer.Vector({source:new ol.source.Vector({wrapX:!1,useSpatialIndex:!0}),id:d.getId()+"-animation",style:this.animationStyleFunction.bind(this)});a.addLayer(d)},scope:this}]}]});this.on("loadedCorpus",function(a,c){this.tryInit()},this);this.on("resize",function(){var a=this.body.down(".ticker");a.setTop(this.getTargetEl().getHeight()-
a.getHeight()-4)},this);this.callParent()},tryInit:function(){if(this.getIsOpenLayersLoaded()&&this.getIsArcLoaded()){var a=this.body.down(".map").setId(Ext.id()),b=this,c=new ol.Map({layers:[new ol.layer.Tile({preload:Infinity,source:new ol.source.Stamen({layer:"watercolor"})}),new ol.layer.Tile({preload:Infinity,source:new ol.source.Stamen({cacheize:2048,layer:"toner-hybrid"})})],target:a.getId(),loadTilesWhileInteracting:!0,view:new ol.View({center:[15E5,6E6],maxResolution:4.007501668557849E7/
b.body.dom.offsetWidth,zoom:0})});c.on("singleclick",function(a){(a=c.getFeaturesAtPixel(a.pixel))&&a.forEach(function(a){"Circle"===a.getGeometry().getType()&&a.get("selected")&&b.dispatchEvent("termsClicked",b,[a.get("forms").join("|")])},this)});var d=new ol.layer.Vector({map:c,source:new ol.source.Vector({wrapX:!1,useSpatialIndex:!1}),zIndex:10,selected:!0,style:function(a){return"Circle"===a.getGeometry().getType()?b.cityStyleFunction(a,c.getView().getResolution()):b.travelStyleFunction(a,c.getView().getResolution())},
updateWhileAnimating:!0,updateWhileInteracting:!0});c.on("pointermove",function(a){d.getSource().clear();var f=a.coordinate;if(a=c.getFeaturesAtPixel(a.pixel)){for(var g=0;a[g].get("selected")&&(g++,g!==a.length););if(g<a.length){var h=a[g];a={font:"12px Calibri,sans-serif",textAlign:"center",offsetY:-15,fill:new ol.style.Fill({color:[0,0,0,1]}),stroke:new ol.style.Stroke({color:[255,255,255,.5],width:4})};a.text=h.get("text");a=new ol.style.Style({text:new ol.style.Text(a),zIndex:1});g=new ol.Feature({text:h.get("text"),
geometry:h.getGeometry(),forms:h.get("forms"),selected:!0,coordinates:h.get("coordinates"),width:h.get("width"),visible:h.get("visible"),color:h.get("color")});d.getSource().addFeature(g);g=h.getGeometry();f=g.getClosestPoint(f);f=new ol.Feature({geometry:new ol.geom.Point(f),selected:!0});f.setStyle(a);d.getSource().addFeature(f);"Circle"===g.getType()&&b.getMap().getLayers().forEach(function(a){var b=a.get("id");b&&-1!==b.indexOf("connections")&&(connections=a.getSource().getFeatures(),connections.forEach(function(a){a.get("target")!==
h.get("cityId")&&a.get("source")!==h.get("cityId")||d.getSource().addFeature(new ol.Feature({geometry:a.getGeometry(),selected:!0,coordinates:a.get("coordinates"),width:a.get("width"),visible:a.get("visible"),color:a.get("color")}))}))})}}});this.setMap(c);this.getDockedComponent("bottomToolbar").getComponent("addFilter").click()}else Ext.defer(this.tryInit,500,this)},travelStyleFunction:function(a,b){var c=new ol.style.Stroke({color:a.get("color"),width:a.get("width")}),d=[new ol.style.Style({stroke:c})],
...
this.animate()},scope:this}]},{xtype:"checkbox",checked:!0,checkHandler:function(a,b){c.setKeepAnimationInFrame(b)},boxLabel:"Keep animation in frame"}]},{xtype:"container",text:"\x26nbsp;"},{xtype:"button",text:"remove",scope:this,handler:function(a){this.fireEvent("removeFilterWidget",this);a.ownerCt.ownerCmp.destroy()}}]}});this.callParent([a]);this.on("resize",function(a){});this.on("afterrender",function(a){a.getTargetEl().down(".x-btn-glyph").setStyle("color",this.getColor());var b=a.up("panel");
b.mask(a.localize("loading"));a.loadGeonames().then(function(c){b.unmask();if(1==b.getFilterWidgets().getCount()){var g=c.getCitiesCount(),h=c.getTotalCitiesCount(),k=c.getConnectionsCount();c=c.getTotalConnectionsCount();message=a.localize(g==h&&k==c?"loadedAll":"loadedSome")+"\x3cbr\x3e\x3cbr\x3e"+a.localize("disclaimer");b.toastInfo({closable:!0,maxWidth:"90%",html:(new Ext.Template(a.localize("disclaimer"))).apply({currentCitiesCount:g,totalCitiesCount:h,currentConnectionsCount:k,totalConnectionsCount:c,
url:b.getBaseUrl()+"docs/#!/guide/dreamscape"})})}})},this)},loadGeonames:function(a){var b=this;a=a||{};var c=[];this.query("querysearchfield").forEach(function(a){var b=a.getItemId();a.getValue().forEach(function(a){c.push(b+":"+a)})});var d=this.down("multislider");if(d&&d.isVisible()){var e=d.getValue();c.push(d.tokenType+":["+e[0]+"-"+e[1]+"]")}0<c.length&&!("query"in a)&&(a.query=c);(d=this.up("panel"))&&d.getApiParams&&Ext.applyIf(a,d.getApiParams(["minPopulation","connectionsMaxCount","connectionsMinFreq"]));
this.mask("\u2026");var f=this.getGeonames();return f.load(a).then(function(){b.unmask();b.fireEvent("filterUpdate",b,f);return f})},clearAnimation:function(){this.getTimeout()&&clearTimeout(this.getTimeout())},animate:function(){this.clearAnimation();var a=this.getCurrentConnectionOccurrence();if(a){var b=this.getAnimationLayer();b||(b=this.up("panel").getMap().getLayer(this.getId()+"-animation"),this.setAnimationLayer(b));if(b){var c=b.getSource().getFeatures();if(0==c.length){if(c=this.up("panel")){for(var d=
c.getMap().getLayer(this.getId()+"-connections").getSource().getFeatures(),e=!1,f=0,g=d.length;f<g;f++)if(a.source.id==d[f].get("source")&&a.target.id==d[f].get("target")){e=d[f];break}if(e){this.getKeepAnimationInFrame()&&(g=c.getMap(),d=c.getMap().getView().calculateExtent(g.getSize()),ol.extent.containsExtent(d,e.getGeometry().getExtent())||g.getView().setZoom(g.getView().getZoom()-1));var e=(new arc.GreatCircle({x:a.source.longitude,y:a.source.latitude},{x:a.target.longitude,y:a.target.latitude})).Arc(100,
{offset:100}),h=a.source.label+" -\x3e "+a.target.label;e.geometries.forEach(function(a){var c=new ol.geom.LineString([]);c.transform(ol.proj.get("EPSG:4326"),ol.proj.get("EPSG:3857"));a=new ol.Feature({geometry:c,allcoords:a.coords,text:h,color:this.getColor(),width:5,start:(new Date).getTime()});b.getSource().addFeature(a)},this);c.body.down(".ticker");c.body.down(".ticker").setHtml(a.source.left+" \x3cspan class\x3d'keyword'\x3e"+a.source.middle+"\x3c/span\x3e "+a.source.right+" \u2026 "+a.target.left+
" \x3cspan class\x3d'keyword'\x3e"+a.target.middle+"\x3c/span\x3e "+a.target.right);return this.setTimeout(setTimeout(this.animate.bind(this),1))}if(this.getGeonames())return a=this.getGeonames().getConnectionOccurrence(a.index+1),this.setCurrentConnectionOccurrence(a),b.getSource().clear(),this.setTimeout(setTimeout(this.animate.bind(this),1))}}else if(0<c.length){g=c[0].getGeometry().getCoordinates();e=c[0].get("allcoords");if(g.length<e.length)return g=((new Date).getTime()-c[0].get("start"))*
e.length/this.getMillisPerAnimation(),line=new ol.geom.LineString(e.slice(0,g)),line.transform(ol.proj.get("EPSG:4326"),ol.proj.get("EPSG:3857")),c[0].set("geometry",line),this.setTimeout(setTimeout(this.animate.bind(this),25));a=this.getGeonames().getConnectionOccurrence(a.index+1);if(!this.getStepByStepMode())return this.setCurrentConnectionOccurrence(a),b.getSource().clear(),this.setTimeout(setTimeout(this.animate.bind(this),1))}}}else if(this.getGeonames()&&(a=this.getGeonames().getConnectionOccurrence(0)))return this.setCurrentConnectionOccurrence(a),
