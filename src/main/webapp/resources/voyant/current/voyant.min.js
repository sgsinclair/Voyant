...
{text:(new Ext.Template(this.localize("runIterations"))).apply([100]),glyph:"xf04b@FontAwesome",tooltip:this.localize("runIterationsTip"),handler:this.runIterations,scope:this},{xtype:"tbtext",itemId:"done"},{xtype:"tbfill"},{xtype:"tbtext",html:this.localize("adaptation")}]}});this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments);this.resetData();var b=parseInt(this.getApiParam("numTopics"));this.setTokensPerTopic(this.zeros(b));this.setTopicWeights(this.zeros(b));
this.on("loadedCorpus",function(a,b){this.setCorpus(b);if(this.rendered)this.initialize();else this.on("afterrender",function(){this.initialize()},this)});this.on("query",function(a,b){this.setApiParam("query",b);this.updateSearchResults()})},loadStopwords:function(){this.mask(this.localize("loadingStopWords"));Ext.Ajax.request({url:this.getTromboneUrl(),params:{tool:"resource.KeywordsManager",stopList:this.getApiParam("stopList"),corpus:this.getCorpus()?this.getCorpus().getAliasOrId():void 0},success:function(a,
b){this.unmask();var c={};Ext.util.JSON.decode(a.responseText).keywords.keywords.forEach(function(a){c[a]=1});this.setStopwords(c);this.loadDocuments()},scope:this})},loadDocuments:function(){this.mask(this.localize("loadingDocuments"));var a=this.getCorpus();a&&Ext.Ajax.request({url:this.getTromboneUrl(),params:{tool:"corpus.DocumentTokens",corpus:a.getAliasOrId(),outputFormat:"text",template:"docTokens2text",limit:0,noOthers:!0,perDocLimit:1==a.getDocumentsCount()?void 0:parseInt(this.getApiParam("perDocLimit"))},
success:function(b,c){this.unmask();this.mask(this.localize("parsingDocuments"));var d=b.responseText.trim().split(/(\r\n|\r|\n)/);if(1==d.length){if(d=d[0].split(": "),1<d.length)for(var e=a.getDocument(d[0]),f=d[1].split(" "),e=e.getLexicalTokensCount(),e=10>e?e:1E3>e?10:100,e=10,g=Math.floor(f.length/e),h=0;h<e;h++){var k=f.slice(h*g,h*g+g).join(" ");this.parseLine(d[0]+"-"+h+"\t"+d[0]+"-"+h+"\t"+k)}}else d.forEach(function(b){b=b.split(": ");if(1<b.length){var c=a.getDocument(b[0]);this.parseLine(b[0]+
"\t"+c.getTinyLabel()+"\t"+b[1])}},this);this.unmask();this.runIterations()},scope:this})},resetData:function(){var a=parseInt(this.getApiParam("numTopics"));this.setVocabularyCounts({});this.setSumDocSortSmoothing(this.getDocSortSmoothing()*a);this.setRequestedSweeps(0);this.setWordTopicCounts({});this.setTopicWordCounts([]);this.setTokensPerTopic([]);this.setTopicWeights(Array(a));this.setDocuments([]);this.setTotalIterations(0)},loadFromExistingDocuments:function(){var a=this.getDocuments().map(function(a){return a.id+
"\t"+a.date+"\t"+a.originalText});this.resetData();a.forEach(function(a){this.parseLine(a)},this);this.runIterations()},runIterations:function(){var a=this.getRequestedSweeps(),b=parseInt(this.getApiParam("iterations")),a=a+parseInt(b);this.setTotalIterations(this.getTotalIterations()+b);this.setRequestedSweeps(a);this.getProgress()||(b=Ext.MessageBox.show({title:this.localize("runningIterations"),message:(new Ext.Template(this.localize("runningIterationsCount"))).apply([a]),progress:!0,progressText:"",
target:a,customUpdateProgress:function(a){this.updateProgress((this.config.target-a)*this.config.target," iterations")}}),this.setProgress(b),this.updateProgress(a));this.sweep()},updateProgress:function(a){var b=this.getProgress();a=this.getRequestedSweeps();if(b){var c=b.cfg.target;a=(c-a)/c;b.updateProgress(a,100*parseInt(a)+"% done")}},sweep:function(){Date.now();for(var a=parseInt(this.getApiParam("numTopics")),b=this.getVocabularySize(),c=this.getTopicWordSmoothing(),d=this.getTokensPerTopic(),
e=this.getDocuments(),f=this.getWordTopicCounts(),g=this.getTopicWeights(),h=this.getDocumentTopicSmoothing(),k=this.zeros(a),l=0;l<a;l++)k[l]=1/(b*c+d[l]);for(var m=0;m<e.length;m++)for(var n=e[m],q=n.topicCounts,s=0;s<n.tokens.length;s++){var p=n.tokens[s];if(!p.isStopword){d[p.topic]--;var r=f[p.word];r[p.topic]--;q[p.topic]--;k[p.topic]=1/(b*c+d[p.topic]);for(var w=0,l=0;l<a;l++)g[l]=r[l]?(h+q[l])*(c+r[l])*k[l]:(h+q[l])*c*k[l],w+=g[l];l=w*Math.random();w=0;for(l-=g[w];0<l;)w++,l-=g[w];p.topic=
w;d[p.topic]++;r[p.topic]=r[p.topic]?r[p.topic]+1:1;q[p.topic]++;k[p.topic]=1/(b*c+d[p.topic])}}a=this.getRequestedSweeps()-1;this.setRequestedSweeps(a);b=this.getProgress();0<a?(this.updateProgress(a),Ext.defer(this.sweep,0,this)):(b.close(),this.setProgress(void 0),this.postSweep())},postSweep:function(){this.sortTopicWords();var a=this.getStore(),b=(new Ext.Template(this.localize("totalDone"))).apply([this.getTotalIterations()]);this.down("#done").setHtml(b);for(var b=parseInt(this.getApiParam("numTopics")),
c=this.getTopicWordCounts(),d=[],e=this.getDocuments(),f=this.getDocSortSmoothing(),g=this.getSumDocSortSmoothing(),h=0;h<b;h++){var k=e.map(function(a,b){return(a.topicCounts[h]+f)/(a.tokens.length+g)});d.push({topic:this.topNWords(c[h],10),scores:k})}a.loadData(d)},topNWords:function(a,b){return a.slice(0,b).map(function(a){return a.word}).join(" ")},displayTopicWords:function(){for(var a=parseInt(this.getApiParam("numTopics")),b=this.getTopicWordCounts(),c=[],d=0;d<a;d++)c.push(this.topNWords(b[d],
10));var a=d3.select("#"+this.body.getId()+" div#topics").selectAll("div.topicwords").data(c),e=this;a.enter().append("div").attr("class","topicwords").on("click",function(a,b){e.toggleTopicDocuments(b)});a.transition().text(function(a,b){return"["+b+"] "+a});return b},sortTopicWords:function(){for(var a=this.getTopicWordCounts(),b=parseInt(this.getApiParam("numTopics")),c=this.getWordTopicCounts(),a=[],d=0;d<b;d++)a[d]=[];for(var e in c)for(d in c[e])a[d].push({word:e,count:c[e][d]});for(d=0;d<b;d++)a[d].sort(this.byCountDescending);
this.setTopicWordCounts(a)},byCountDescending:function(a,b){return b.count-a.count},initialize:function(){this.loadStopwords()},parseLine:function(a){var b=this.getStopwords(),c=this.getVocabularyCounts(),d=this.getTokensPerTopic(),e=this.getVocabularySize(),c=this.getVocabularyCounts(),f=this.getDocuments(),g=parseInt(this.getApiParam("numTopics")),f=this.getDocuments(),h=this.getWordTopicCounts();if(""!=a){var k=f.length,l="",m=a.split("\t");a=m[0];3==m.length&&(k=m[0],l=m[1],a=m[2]);var n=[],m=
a.toLowerCase().split(" ");if(null!=m){var q=this.zeros(g);m.forEach(function(a){if(""!==a){var f=Math.floor(Math.random()*g);2>=a.length&&(b[a]=1);var k=b[a];k?c[a]=c[a]?c[a]+1:1:(d[f]++,h[a]||(h[a]={},e++,c[a]=0),h[a][f]||(h[a][f]=0),h[a][f]+=1,c[a]+=1,q[f]+=1);n.push({word:a,topic:f,isStopword:k})}});this.setVocabularySize(e);f.push({originalOrder:f.length,id:k,date:l,originalText:a,tokens:n,topicCounts:q})}}}});Ext.define("Voyant.VoyantApp",{extend:"Ext.app.Application",mixins:["Voyant.util.Deferrable","Voyant.util.Localization","Voyant.util.Api"],requires:["Voyant.util.ResponseError"],name:"VoyantApp",statics:{i18n:{},api:{palette:"default",lang:void 0,debug:void 0}},config:{baseUrl:void 0,tromboneUrl:void 0},constructor:function(a){this.setBaseUrl(this.config.baseUrl);this.setTromboneUrl(this.config.baseUrl+"trombone");Voyant.application=this;this.mixins["Voyant.util.Api"].constructor.apply(this,arguments);
