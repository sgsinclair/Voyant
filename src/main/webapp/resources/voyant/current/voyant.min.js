...
b){this.store.setCorpus(b);this.store.load({params:this.getApiParams()});"NONCONSUMPTIVE"==b.getNoPasswordAccess()&&this.queryById("modifyButton").hide()});this.on("query",function(a,b){this.setApiParam("query",b);this.store.load({params:this.getApiParams()})});a.embedded&&("Voyant.data.model.Corpus"==Ext.getClass(a.embedded).getName()?a.corpus=a.embedded:"Voyant.data.store.Documents"==Ext.getClass(a.embedded).getName()&&(this.store.setRecords(a.embedded.getData()),a.corpus=a.embedded.getCorpus()));
a.corpus&&this.fireEvent("loadedCorpus",this,a.corpus)},keepRemoveReorderHandler:function(a){var b=a.up("window").down("documents"),c=b.getSelection(),d=b.getStore().getCorpus().getDocumentsCount(),e=a.getItemId();if("reorder"==e){if(b.getStore().getCount()<d)return Ext.Msg.show({title:this.localize("error"),message:this.localize("reorderFilteredError"),buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});docIndex=[];b.getStore().each(function(a){docIndex.push(a.getIndex())},this);for(a=1;a<docIndex.length;a++)if(docIndex[a-
1]>docIndex[a])return Ext.Msg.confirm(b.localize("newCorpus"),(new Ext.Template(b.localize(e+"Documents"))).applyTemplate([c.length]),function(a){"yes"===a&&(docIndex=[],this.getStore().each(function(a){docIndex.push(a.getIndex())},this),a={docIndex:docIndex},a[e+"Documents"]=!0,this.editCorpus(a))},b);return Ext.Msg.show({title:this.localize("error"),message:this.localize("reorderOriginalError"),buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR})}return 0<c.length?c.length==d?1==d?Ext.Msg.show({title:this.localize("error"),
message:this.localize("onlyOneError"),buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR}):Ext.Msg.show({title:this.localize("error"),message:this.localize("allSelectedError"),buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR}):Ext.Msg.confirm(this.localize("newCorpus"),(new Ext.Template(this.localize(e+"SelectedDocuments"))).applyTemplate([c.length]),function(a){"yes"===a&&(docIndex=[],c.forEach(function(a){docIndex.push(a.getIndex())}),a={docIndex:docIndex},a[e+"Documents"]=!0,this.editCorpus(a))},b):b.getApiParam("query")&&
b.getStore().getCount()<d?Ext.Msg.confirm(this.localize("newCorpus"),(new Ext.Template(this.localize(e+"FilteredDocuments"))).applyTemplate([c.length]),function(a){"yes"===a&&(docIndex=[],this.getStore().each(function(a){docIndex.push(a.getIndex())},this),a={docIndex:docIndex},a[e+"Documents"]=!0,this.editCorpus(a))},b):Ext.Msg.show({title:this.localize("error"),message:this.localize("selectOrFilterError"),buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR})},editCorpus:function(a){Ext.apply(a,{tool:"corpus.CorpusManager",
corpus:this.getStore().getCorpus().getId()});var b=this.getApplication(),c=b.getViewport();c.mask(this.localize("Creating new corpus\u2026"));Ext.Ajax.request({url:this.getApplication().getTromboneUrl(),method:"POST",params:a,success:function(a){c.unmask();a=Ext.decode(a.responseText);b.openUrl(b.getBaseUrl()+"?corpus\x3d"+a.corpus.id)}});(a=this.up("window"))&&a.isFloating()&&a.close()}});Ext.define("Voyant.panel.DocumentsFinder",{extend:"Ext.grid.Panel",require:["Voyant.data.store.DocumentQueryMatches","Ext.grid.plugin.CellEditing"],mixins:["Voyant.panel.Panel"],alias:"widget.documentsfinder",statics:{i18n:{}},constructor:function(a){this.cellEditing=Ext.create("Ext.grid.plugin.CellEditing",{clicksToEdit:1});this.cellEditing.on("edit",this.onEditComplete,this);var b=this;Ext.apply(this,{title:this.localize("title"),emptyText:this.localize("emptyText"),plugins:[this.cellEditing],bbar:[{text:this.localize("addRow"),
...
(this.documentTerms.destroy(),this.documentTerms=void 0);this.termsMap={};this.draw();var a=this.getCorpus().getDocument(parseInt(this.getApiParam("docIndex")));this.setTitle(this.localize("title")+" \x3cspan class\x3d'subtitle'\x3e"+a.getFullLabel()+"\x3c/span\x3e");this.lastToken=parseInt(a.get("lastTokenStartOffset-lexical"));this.documentTerms=a.getDocumentTerms({proxy:{extraParams:{stopList:this.getApiParam("stopList"),bins:this.getDiam(),withDistributions:"raw",minRawFreq:parseInt(this.getApiParam("minRawFreq"))}}});
