/* This file created by JSCacher. Last modified: Thu Apr 07 11:14:27 EDT 2016 */
...
stopList:"auto",facet:["facet.title","facet.author","facet.language"]},glyph:"xf1ea@FontAwesome"},config:{corpus:void 0,facets:{},matchingDocIds:[]},constructor:function(a){Ext.apply(this,{title:this.localize("title"),layout:"hbox",items:[{layout:"vbox",height:"100%",align:"stretch",itemId:"facets",defaults:{width:250,flex:1,xtype:"facet",margin:5,border:!0,frame:!0},items:[]},{xtype:"panel",html:"",flex:1,itemId:"results",height:"100%",align:"stretch",scrollable:!0,margin:5,getCorpus:function(){return this.findParentByType("panel").getCorpus()},
listeners:{query:function(a,c){this.findParentByType("panel").updateResults([c])}},bbar:[{itemId:"export",text:this.localize("export"),tooltip:this.localize("exportTip"),disabled:!0,handler:function(){this.mask(this.localize("exportInProgress"));var a=this;Ext.Ajax.request({url:this.getApplication().getTromboneUrl(),params:{corpus:this.getCorpus().getId(),tool:"corpus.CorpusManager",keepDocuments:!0,docId:this.getMatchingDocIds()},success:function(c,d){a.unmask();var e=Ext.JSON.decode(c.responseText),
e=a.getBaseUrl()+"?corpus\x3d"+e.corpus.id;if(!window.open(e)){var f=Ext.create("Ext.window.MessageBox",{makeButton:function(c){return new Ext.button.Button({handler:this.btnCallback,scope:this,text:a.localize("cancel"),ui:"default-toolbar",minWidth:75})}}).show({title:a.localize("export"),buttons:Ext.MessageBox.CANCEL,icon:Ext.MessageBox.INFO,message:a.localize("clickToOpenCorpus",[e])}),e=f.getTargetEl().dom.querySelector("a");e.addEventListener("click",function(){f.close()});Ext.get(e).frame().frame()}},
failure:function(c,d){a.unmask();me.showError(c)}})},scope:this},{xtype:"querysearchfield",width:200,flex:1},{itemId:"status",xtype:"tbtext"}]}]});this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments);this.on("loadedCorpus",function(a,c){this.setCorpus(c);this.queryById("status").update(this.localize("noMatches",[c.getDocumentsCount()]));this.query("facet").forEach(function(a){a.setCorpus(c)})});this.on("afterrender",function(a){var c=this.getApiParam("facet");
Ext.isString(c)&&(c=c.split(","));var d=this.queryById("facets"),e='\x3cspan style\x3d"font-size: smaller;"\x3e(\x3cspan class\x3d"info-tip" data-qtip\x3d"'+a.localize("matchingDocuments")+'"\x3e{inDocumentsCount}\x3c/span\x3e)\x3c/span\x3e {term}\x3cspan style\x3d"font-size: smaller;"\x3e (\x3cspan class\x3d"info-tip" data-qtip\x3d"'+a.localize("rawFreqs")+'"\x3e{rawFreq}\x3c/span\x3e)\x3c/span\x3e';c.forEach(function(c){var h=a.localize(c+"Title");h=="["+c+"Title]"&&(h=c.replace(/^facet\./,"").replace(/^extra./,
""));a.localize("matchingDocuments");var k=d.add({title:h,collapsible:!0,facet:c,columns:[{renderer:function(a,b,c){return'\x3cspan style\x3d"font-size: smaller;"\x3e(\x3cspan class\x3d"info-tip" data-qtip\x3d"'+f.localize("matchingDocuments")+'"\x3e'+c.getInDocumentsCount()+"\x3c/span\x3e) \x3c/span\x3e"+c.getLabel()},flex:1}],bbar:[{xtype:"querysearchfield",width:"100%",tokenType:c.replace("facet.",""),itemTpl:e}]});k.getSelectionModel().on("selectionchange",function(d,e){var f=[];e.forEach(function(a){f.push({facet:a.getFacet(),
label:a.getLabel()})});a.getFacets()[c]=f;a.updateResults()});k.on("query",function(c,d){a.getFacets()[k.facet]=[];a.updateResults()})});var f=this;d.add({title:a.localize("lexicalTitle"),collapsible:!0,store:Ext.create("Voyant.data.store.CorpusTerms",{parentPanel:this,proxy:{extraParams:{stopList:this.getApiParam("stopList")}}}),facet:"lexical",columns:[{renderer:function(a,b,c){return'\x3cspan style\x3d"font-size: smaller;"\x3e(\x3cspan class\x3d"info-tip" data-qtip\x3d"'+f.localize("matchingDocuments")+
'"\x3e'+c.getInDocumentsCount()+"\x3c/span\x3e) \x3c/span\x3e"+c.getTerm()+'\x3cspan style\x3d"font-size: smaller;"\x3e (\x3cspan class\x3d"info-tip" data-qtip\x3d"'+f.localize("rawFreqs")+'"\x3e'+c.getRawFreq()+"\x3c/span\x3e)\x3c/span\x3e"},flex:1}],bbar:[{xtype:"querysearchfield",itemTpl:e,grow:!1,growMax:10}]}).getSelectionModel().on("selectionchange",function(c,d){var e=[];d.forEach(function(a){e.push({facet:"lexical",label:a.getTerm()})});a.getFacets().lexical=e;a.updateResults()})})},updateResults:function(a){var b=
this.getFacets();if(!a){a=[];for(facet in b)b[facet].forEach(function(b){a.push(b.facet+":"+b.label)});if(a)return this.updateResults(a)}var c=this.queryById("results").getTargetEl();c.update("");this.queryById("export").setDisabled(!0);if(0<a.length){this.mask(this.localize("loading"));var d=this.getCorpus().getDocumentQueryMatches();d.load({params:{query:a,includeDocIds:!0},callback:function(a,f,g){this.unmask();if(a&&0<a.length){this.queryById("status").setHtml(a.length);var h="\x3cul\x3e",k=[];
a.forEach(function(a){a.getDocIds().forEach(function(a){k.push(a);var e=d.getCorpus().getDocument(a);a="\x3cli id\x3d'"+c.getId()+"_"+a+"' class\x3d'cataloguedoc'\x3e";a+="\x3ci\x3e"+e.getTitle()+"\x3c/i\x3e";for(facet in b)if(0!=b[facet].length){var f="";if("facet.title"!=facet){var g=facet.replace(/^.+?\./,""),l=e.get(g);if(l){var r=Ext.isArray(l);r?f+="\x3cli\x3e"+g+"\x3cul\x3e":l=[l];l.forEach(function(a){var c=!1;b[facet].forEach(function(b){b.label==a?c=!0:-1==b.facet.indexOf("facet")&&b.label.split(/\W+/).forEach(function(b){0<
b.trim().length&&-1<a.toLowerCase().indexOf(b.toLowerCase())&&(c=!0)})});f+="\x3cli\x3e"+(r?"":g+": ")+(c?'\x3cspan class\x3d"keyword"\x3e'+a+"\x3c/span\x3e":a)+"\x3c/li\x3e"});r&&(f+="\x3c/ul\x3e\x3c/li\x3e")}}f&&(a+="\x3cul\x3e"+f+"\x3c/ul\x3e")}h+=a+"\x3c/li\x3e"})});h+="\x3c/ul\x3e";c.update(h);this.queryById("status").update(this.localize("queryMatches",[k.length,this.getCorpus().getDocumentsCount()]));this.setMatchingDocIds(Ext.Array.clone(k));0<k.length&&this.queryById("export").setDisabled(!1);
b.lexical&&(a=k.splice(0,5),this.loadSnippets(a,c.first().first()),k&&0<k.length&&this.loadSnippets(k))}},scope:this})}},loadSnippets:function(a,b){var c=this.queryById("results").getTargetEl(),d=this.getFacets();if(d.lexical){var d=d.lexical.map(function(a){return a.facet+":"+a.label}),e=this.getCorpus().getContexts({buffered:!1});b&&b.mask(this.localize("loadingSnippets"));e.load({method:"POST",params:{stripTags:"all",query:d,docId:a,perDocLimit:3,limit:100,accurateTotalNotNeeded:!0},scope:this,
callback:function(a,d,e){b&&b.unmask();if(e&&Ext.isArray(a)&&0<a.length){var k={};a.forEach(function(a){k[a.getDocIndex()]||(k[a.getDocIndex()]=[]);k[a.getDocIndex()].push(a)});for(docIndex in k)d=this.getCorpus().getDocument(docIndex).getId(),a='\x3cli style\x3d"list-style-type: none; font-size: smaller;"\x3e'+k[docIndex].map(function(a){return a.getHighlightedContext()}).join(" \u2026 ")+"\x3c/li\x3e",d=c.down("#"+c.getId()+"_"+d),d.query("ul")&&(a="\x3cul\x3e"+a+"\x3c/ul\x3e"),d.insertHtml("beforeEnd",
a)}}})}}});Ext.define("Voyant.panel.Cirrus",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.cirrus",statics:{i18n:{title:{en:"Cirrus"},helpTip:{en:"\x3cp\x3eCirrus provides a wordcloud view of the most frequently occurring words in the corpus or document \u2013\u00a0this provides a convenient (though reductive) overview of the content. Features include\x3c/p\x3e\x3cul\x3e\x3cli\x3eterm frequency appears when hovering over words\x3c/li\x3e\x3cli\x3eclicking on terms may produce results in other tools if any are displayed\x3c/li\x3e\x3c/ul\x3e"},
