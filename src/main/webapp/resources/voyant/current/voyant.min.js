...
null==h.color||""==h.color?g.colors[Math.floor(Math.random()*g.colors.length)]:h.color;var m;m=void 0==typeof h.size||null==h.size||""==h.size?Math.floor(40*Math.random()):parseFloat(h.size);var l=h.word,n=h.label,q=h.value,h=!1;null==s[l]&&(s[l]=!0,m>f.largestWordSize&&(f.largestWordSize=m,h=!0),m<f.smallestWordSize&&(f.smallestWordSize=.8*m,h=!0),l=new Word(l,m,k,n,q),p.push(l));b=h||b}c();this.setRelativeSizes();this.calculateSizeAdjustment();b?this.resizeWords():d()};this.resetWordCoordinates=
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];m=0;f=function(){var a,f,h,k,s=this.wordsToArrange-1,n=g.canvas.width,G=g.canvas.height,q=.5*n,u=.5*G;do{h=p[m];if(void 0!==h){for(var C=Math.random()*Math.PI,v=.25*Math.random()*h.size,w=.5*(Math.random()-.5),E=.5*h.width,N=.5*h.height;;){a=Math.floor((q+Math.cos(C)*v*this.ratio-E)/this.COARSENESS)*this.COARSENESS;f=Math.floor((u+Math.sin(C)*v-N)/this.COARSENESS)*this.COARSENESS;
k=a+E>=n||f+N>=G?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;C+=w;v+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<m;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}m++;if(m>=p.length){clearTimeout(l);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(s--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;l=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
s=function(a){return doubletree.fieldText(a,"POS")},G=function(a){return"rgba(255,255,255,1)"},r=function(a){return"rgba(255,255,255,1)"},H=function(a){return"red"},L={node:{fill:"white",stroke:"steelblue","stroke-width":"1.5px"},branch:{stroke:"#aaa","stroke-width":"1.5px"}},C=!1,F=d3.dispatch("idsUpdated");F.on("idsUpdated",function(){this==D?(A.setIds(D.continuationIDs),A.updateContinuations()):this==A&&(D.setIds(A.continuationIDs),D.updateContinuations())});var z,I,D,A,J,M;b.init=function(a){d3.select(d3.selectAll(a)).call(this);
return b};b.redraw=function(){b.setupFromTries(z,I);return b};b.setupFromTries=function(d,k){z=d.getUniqRoot();I=k.getUniqRoot();var B=z.toTree(m.left),v=I.toTree(m.right),w=!0;0<Object.keys(v.pruned).length&&(a(v,v.pruned,w),a(B,v.pruned,w),w=!1);0<Object.keys(B.pruned).length&&(a(B,B.pruned,w),a(v,B.pruned,w));var w={},E;for(E in v.info)"continuations"!=E&&"ids"!=E&&"count"!=E&&(w[E]=v.info[E]);w["right continuations"]=v.info.continuations;w["left continuations"]=B.info.continuations;w.ids={};c(w.ids,
v.info.ids);c(w.ids,B.info.ids);w.count=Object.keys(w.ids).length;J=Object.keys(w.ids);if(v.info.origIDs||B.info.origIDs)w.origIDs={},c(w.origIDs,v.info.origIDs),c(w.origIDs,B.info.origIDs),w.origCount=Object.keys(w.origIDs).length;v.info=w;B.info=w;E=Math.max(B.maxChildren,v.maxChildren);if(isNaN(E)||0==E)return C=!1,b;q?M=d3.scale.log().range([8,14]):(M=function(){return 14},M.domain=function(){});E=Math.min(B.minCount,v.minCount);M.domain([E,B.info.count]);var N=g-20-20,O=h-20-20;f[0].forEach(function(a,
b){function c(){var a=d3.event.scale,b=d3.event.translate;d3.select(d).select("svg \x3e g").attr("transform","translate("+b+") scale("+a+")")}var d=a,e;e=d3.select(d).select("svg");null==e[0][0]?(e=d3.select(d).append("svg").attr("width",N+20+20).attr("height",O+20+20).attr("cursor","move").call(d3.behavior.zoom().scaleExtent([1,1]).on("zoom",c)),e.append("g")):(e.attr("width",N+20+20).attr("height",O+20+20),e.selectAll("g *").remove());D=new doubletree.Tree(e.select("g"),g,h,B,!0,u,F,M,n,p,s,G,r,
H,L);A=new doubletree.Tree(e.select("g"),g,h,v,!1,u,F,M,n,p,s,G,r,H,L)});D.handleAltPress=l.alt;A.handleAltPress=l.alt;D.handleShiftPress=l.shift;A.handleShiftPress=l.shift;D.handleClick=l.click;A.handleClick=l.click;C=!0;return b};b.setupFromArrays=function(a,c,d,f,g,h,m,l){void 0==g&&z&&(g=z.caseSensitive());void 0==h&&z&&(h=z.fieldNames());void 0==m&&z&&(m=z.fieldDelim());void 0==l&&z&&(l=z.distinguishingFieldsArray());z=new doubletree.Trie(g,h,m,l);I=new doubletree.Trie(g,h,m,l);g=c.length;for(h=
0;h<g;h++){m=f?f[h]:h;l=c[h];var s=a[h].slice(),n=d[h].slice();s.push(l);s.reverse();n.unshift(l);k?(I.addNgram(s,m),z.addNgram(n,m)):(z.addNgram(s,m),I.addNgram(n,m))}b.setupFromTries(z,I);return b};b.filteredIDs=function(){return J};b.search=function(a){D.search(a);A.search(a);a=d3.select(f[0][0]);var b=a.selectAll("text.foundText");if(b.empty())return 0;b=b[0].length;null!=a.selectAll("text.rtNdText.foundText")[0][0]&&b--;return b};b.clearSearch=function(){D.clearSearch();A.clearSearch();return b};
b.updateTokenExtras=function(){D.showTokenExtras(n);A.showTokenExtras(n);var a=d3.select(f[0][0]).select('.tokenExtra[display\x3d"inline"]');a.empty()||"0px"==a.style("height")&&b.redraw();return b};b.visWidth=function(a){if(!arguments.length)return g;g=a;return b};b.visHeight=function(a){if(!arguments.length)return h;h=a;return b};b.prefixesOnRight=function(a){if(!arguments.length)return k;k=a;return b};b.filters=function(a){if(!arguments.length)return m;m=a;return b};b.handlers=function(a){if(!arguments.length)return l;
l=a;return b};b.showTokenExtra=function(a){if(!arguments.length)return n;n=a;return b};b.scaleLabels=function(a){if(!arguments.length)return q;q=a;return b};b.succeeded=function(){return C};b.sortFun=function(a){if(!arguments.length)return u;u=a;return b};b.nodeText=function(a){if(!arguments.length)return p;p=a;return b};b.tokenExtraText=function(a){if(!arguments.length)return s;s=a;return b};b.rectColor=function(a){if(!arguments.length)return G;G=a;return b};b.rectBorderColor=function(a){if(!arguments.length)return r;
r=a;return b};b.continuationColor=function(a){if(!arguments.length)return H;H=a;return b};b.basicStyles=function(a){if(!arguments.length)return L;Object.keys(L).forEach(function(b){b in a&&Object.keys(L[b]).forEach(function(c){c in a[b]&&(L[b][c]=a[b][c])})});return b};return b};doubletree.Tree=function(a,b,c,d,k,m,l,n,q,u,p,s,G,r,H){function L(a){a.children&&(a._children=a.children,a._children.forEach(L),a.children=null)}function C(a){a.parent&&a.parent.children.forEach(function(b){b!=a&&L(b)})}
function F(a,b){d3.event.altKey?E.handleAltPress(a,b):d3.event.shiftKey?E.handleShiftPress(a,b):(E.handleClick(a,b),a.parent&&(E.continuationIDs!=a.info.ids&&(E.setIds(a.info.ids),E.clickedNode=a.id,l.idsUpdated.apply(E)),C(a),z(a,!0)))}function z(a,b){a.children?(a.children&&1==a.children.length&&z(a.children[0],!0),a._children=a.children,a.children=null):(a.children=a._children,a._children=null,a.children&&1==a.children.length&&z(a.children[0],!1));b&&E.update(a)}function I(a){return k?J/2-a:J/
2+a}var D=q,A=s,J=b-20-20,M=c-20-20,K=0,x,B;m||(m=doubletree.sortByStrFld("token"));var v=d3.layout.tree().nodeSize([40,40]).sort(m),w=d3.svg.diagonal().projection(function(a){return[I(a.y),a.x-B]});a=a.append("g").attr("transform","translate(20,20)");this.readJSONTree=function(a){x=a;x.x0=M/2;x.y0=J/2;x.children.forEach(L);this.update(x)};var E=this;this.update=function(b){b||(b=x);var c=v.nodes(x).reverse();B=x.x-M/2;c.forEach(function(a){for(var b=0,c=a.parent;null!=c;)b+=Ext.draw.TextMeasurer.measureText(c.name,
"arial").width,c=c.parent;a.y=25*a.depth+b});var d=a.selectAll("g.node_"+k).data(c,function(a){return a.id||(a.id=++K)}),f=d.enter().append("g").attr("class","node node_"+k).attr("cursor","pointer").attr("transform",function(a){return"translate("+I(b.y0)+","+(b.x0-B)+")"}).on("click",F);f.append("title").text(function(a){return doubletree.infoToText(a.info)});f.append("circle").attr("r",1E-6).style("fill",function(a){return a._children?"#fff":H.node.fill}).style("stroke",function(a){return H.node.stroke});
...
{x:b.x0,y:b.y0};return w({source:a,target:a})}).style("fill","none").style("stroke",H.branch.stroke).style("stroke-width",H.branch["stroke-width"]);f.transition().duration(200).attr("d",w);f.exit().transition().duration(200).attr("d",function(a){a={x:b.x,y:b.y};return w({source:a,target:a})}).remove();c.forEach(function(a){a.x0=a.x;a.y0=a.y});this.updateContinuations()};this.setIds=function(a){E.continuationIDs=a};this.updateContinuations=function(){a.selectAll("g.node_"+k+" text").classed("continuation",
function(a){var b;a:{a=a.info.ids;var c=E.continuationIDs||{};for(b in a)if(b in c){b=!0;break a}b=!1}return b}).style("fill",function(a){return d3.select(this).classed("continuation")?r(a.info):"#444"})};this.search=function(b){a.selectAll("g.node text").classed("foundText",function(a){return b.test(u(a.info))})};this.clearSearch=function(){a.selectAll("g.node text").classed("foundText",!1)};this.showTokenExtras=function(a){if(0==arguments.length)return D;D=a;this.drawRects();return this};this.setRectColor=
...
(function(){doubletree.Trie=function(a,b,c,d,e){function f(a,b,c){this.id=b;this.count=c;this.info={count:c,ids:{}};if(null==a)this.item=k;else{this.item=a;this.info.ids={};this.info.ids[b]=!0;a=a.split(n);for(var d in a)this.info[l[d]]=[a[d]]}this.nodes={};this.addNgram=function(a,b,c){c||(c=1);var d,e;if(0<a.length){d=a.shift();var g=d.split(n);e=u&&this.item==k?"":g.filter(function(a,b){return-1<q.indexOf(l[b])}).map(function(a){return m?a.toLocaleLowerCase():a}).join(n)}else e=d=h;var s;if(e in
this.nodes&&this.nodes[e]instanceof f){s=this.nodes[e];s.info.count+=c;s.info.ids[b]=!0;for(var r in g)e=g[r],-1==s.info[l[r]].indexOf(e)&&s.info[l[r]].push(e)}else s=new f(d,b,c),this.nodes[e]=s;d!=h&&s.addNgram(a,b,c)};this.getUniqRoot=function(){if(this.item==k){var a=Object.keys(this.nodes);if(1==a.length)return this.nodes[a[0]]}return this};this.toTree=function(a){function b(a,c,d){var e={children:[]};e.name=d.item;e.info={};for(var f in d.info)if("Object"===typeof d.info[f]){e.info[f]={};for(var h in d.info[f])e.info[f][h]=
...
a+="\x3ci\x3e"+e.getTitle()+"\x3c/i\x3e";for(facet in b)if(0!=b[facet].length){var f="";if("facet.title"!=facet){var g=facet.replace(/^.+?\./,""),m=e.get(g);if(m){var s=Ext.isArray(m);s?f+="\x3cli\x3e"+g+"\x3cul\x3e":m=[m];m.forEach(function(a){var c=!1;b[facet].forEach(function(b){b.label==a?c=!0:-1==b.facet.indexOf("facet")&&b.label.split(/\W+/).forEach(function(b){0<b.trim().length&&-1<a.toLowerCase().indexOf(b.toLowerCase())&&(c=!0)})});f+="\x3cli\x3e"+(s?"":g.replace("extra.","")+": ")+(c?'\x3cspan class\x3d"keyword"\x3e'+
...
background:"rgba(255,255,255,1)"},scaling:{label:{min:10,max:30}}},edgeOptions:{color:{color:"rgba(0,0,0,0.1)",highlight:"black",hover:"red"},labelHighlightBold:!1},highlightOptions:{font:{color:"white"},color:{background:"black",hover:{border:"#CB157F",background:"#EB42A5"}}},physicsOptions:{defaultPhysics:{barnesHut:{gravitationalConstant:-1500,centralGravity:6,damping:.5,avoidOverlap:.5}},centralizedPhysics:{barnesHut:{gravitationalConstant:-1500,centralGravity:1,damping:.5,avoidOverlap:.5}}},
keywordColor:"green",contextColor:"maroon",constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(){var a=this;Ext.apply(a,{title:this.localize("title"),dockedItems:[{dock:"bottom",xtype:"toolbar",enableOverflow:!0,items:[{xtype:"querysearchfield"},{text:a.localize("clearTerms"),glyph:"xf014@FontAwesome",handler:function(){this.getNodeDataSet().clear();this.getEdgeDataSet().clear();this.queryById("contextSlider").enable();
this.getNetwork().setOptions({physics:this.physicsOptions.defaultPhysics})},scope:a},this.localize("context"),{xtype:"slider",itemId:"contextSlider",minValue:3,value:5,maxValue:30,increment:2,width:50,listeners:{render:function(b){b.setValue(a.getApiParam("context"))},changecomplete:{fn:function(a,c){this.setApiParam("context",a.getValue());var d=this.getNodeDataSet().map(function(a){return a.label});0<d.length&&(this.getNodeDataSet().clear(),this.loadFromQuery(d))},scope:a}}}]}]});this.setContextMenu(Ext.create("Ext.menu.Menu",
...
{xtype:"button",text:"Centralize",style:"margin: 5px;",handler:function(a,c){var d=this.getNetwork().getSelectedNodes();if(null!=d[0]){this.queryById("contextSlider").disable();d=this.getNodeDataSet().get(d[0]);d.fixed=!0;this.getNodeDataSet().clear();this.getEdgeDataSet().clear();this.getNodeDataSet().add(d);this.getNetwork().setOptions({physics:this.physicsOptions.centralizedPhysics});var e=this.getApiParam("limit");this.setApiParam("limit",100);this.itemdblclick(d);this.setApiParam("limit",e);
this.getContextMenu().hide()}},scope:this},{xtype:"button",text:"Remove",style:"margin: 5px;",handler:function(a,c){var d=this.getNetwork().getSelectedNodes();null!=d[0]&&(this.getNodeDataSet().remove(d[0]),a.up("menu").hide(),this.forceUpdate())},scope:this}]}));this.on("loadedCorpus",function(a,c){this.setCorpus(c);this.isVisible()&&this.initLoad()},this);this.on("activate",function(){this.getCorpus()&&Ext.Function.defer(this.initLoad,100,this)},this);this.on("query",function(a,c){this.loadFromQuery(c)},
this);this.on("resize",function(a,c,d){a=this.getLayout().getRenderTarget();c=this.getDockedItems();for(var e=0,f=0;f<c.length;f++)e+=c[f].getHeight();a.setHeight(d-e);a.setWidth(a.getWidth());this.setGraphHeight(a.getHeight());this.setGraphWidth(a.getWidth());void 0!==this.getNetwork()&&this.getNetwork().fit()},this);a.callParent(arguments)},initLoad:function(){this.getCorpus()&&(this.initGraph(),this.getCorpus().getCorpusTerms({leadingBufferZone:0,autoLoad:!1}).load({callback:function(a,b,c){c&&
this.loadFromCorpusTermRecords(a)},scope:this,params:{limit:3,stopList:this.getApiParam("stopList")}}))},loadFromQuery:function(a){this.setApiParams({mode:"corpus"});var b=this.getApiParams();(Ext.isString(a)?[a]:a).forEach(function(a){this.getCorpus().getCorpusCollocates({autoLoad:!1}).load({params:Ext.apply(Ext.clone(b),{query:a}),callback:function(a,b,c){c&&this.loadFromCorpusCollocateRecords(a)},scope:this})},this)},loadFromCorpusTermRecords:function(a){if(Ext.isArray(a)&&0<a.length){var b=[];
a.forEach(function(a){b.push(a.getTerm())});this.loadFromQuery(b)}},loadFromCorpusCollocateRecords:function(a,b){if(Ext.isArray(a)){var c=this.getApiParam("limit"),d=this.getNodeDataSet(),e=this.getEdgeDataSet(),f={};d.forEach(function(a){f[a.id]=!0});var g=[],h=[];a.forEach(function(a,m){var l=a.getTerm(),n=a.getContextTerm(),q=a.getKeywordRawFreq(),u=a.getContextTermRawFreq();0==m&&(void 0===b&&(b=l),void 0!==f[b]?d.update({id:b,value:q,title:l+" ("+q+")",type:"keyword",font:{color:this.keywordColor}}):
(f[b]=!0,g.push({id:l,label:l,title:l+" ("+q+")",type:"keyword",value:q,start:c,font:{color:this.keywordColor}})));if(l!=n){void 0===f[n]&&(f[n]=!0,g.push({id:n,label:n,title:n+" ("+u+")",type:"context",value:u,start:0,font:{color:this.contextColor}}));var p=null;e.forEach(function(a){if(a.from==b&&a.to==n||a.from==n&&a.to==b)p=a});l=a.getContextTermRawFreq();null===p?h.push({from:b,to:n,value:l}):p.value<l&&e.update({id:p.id,value:l})}},this);d.add(g);e.add(h);this.forceUpdate();this.getNetwork().fit()}},
initGraph:function(){var a=this.getLayout().getRenderTarget();a.setWidth(a.getWidth());a.setHeight(a.getHeight());this.setGraphHeight(a.getHeight());this.setGraphWidth(a.getWidth());if(void 0===this.getNetwork()){var b={autoResize:!0,interaction:{hover:!0,hoverConnectedEdges:!0,multiselect:!1},physics:this.physicsOptions.defaultPhysics,nodes:this.nodeOptions,edges:this.edgeOptions},a=new vis.Network(a.dom,{nodes:this.getNodeDataSet(),edges:this.getEdgeDataSet()},b);this.setNetwork(a);this.getNodeDataSet().on("remove",
...
Ext.override(Ext.chart.axis.sprite.Axis,{renderLabels:function(a,b,c,d){var e=this,f=e.attr,g=.5*f.lineWidth,h=f.position,k=f.matrix,m=f.textPadding,l=k.getXX(),n=k.getDX(),q=k.getYY(),u=k.getDY(),p=0,k=c.majorTicks,s=Math.max(f.majorTickSize,f.minorTickSize)+f.lineWidth,G=Ext.draw.Draw.isBBoxIntersect,r=e.getLabel(),H,L,C=null,F=0,z=0,I=c.segmenter,D=e.getRenderer(),A=e.getAxis(),J=(H=A.getTitle())&&""!==H.attr.text&&H.getBBox(),M,K=null,x,B,v;if(k&&r&&!r.attr.hidden){H=r.attr.font;b.font!==H&&(b.font=
H);r.setAttributes({translationX:0,translationY:0},!0);r.applyTransformations();M=r.attr.inverseMatrix.elements.slice(0);switch(h){case "left":b=J?J.x+J.width:0;switch(r.attr.textAlign){case "start":d=a.roundPixel(b+n)-g;break;case "end":d=a.roundPixel(d[2]-s+n)-g;break;default:d=a.roundPixel(b+(d[2]-b-s)/2+n)-g}r.setAttributes({translationX:d},!0);break;case "right":b=J?d[2]-J.x:0;switch(r.attr.textAlign){case "start":d=a.roundPixel(s+n)+g;break;case "end":d=a.roundPixel(d[2]-b+n)+g;break;default:d=
a.roundPixel(s+(d[2]-s-b)/2+n)+g}r.setAttributes({translationX:d},!0);break;case "top":b=J?J.y+J.height:0;r.setAttributes({translationY:a.roundPixel(b+(d[3]-b-s)/2)-g},!0);break;case "bottom":b=J?d[3]-J.y:0;r.setAttributes({translationY:a.roundPixel(s+(d[3]-s-b)/2)+g},!0);break;case "radial":r.setAttributes({translationX:f.centerX},!0);break;case "angular":r.setAttributes({translationY:f.centerY},!0);break;case "gauge":r.setAttributes({translationY:f.centerY},!0)}if("left"===h||"right"===h)e.iterate(k,
function(b,d,f){void 0!==d&&(v=D?Ext.callback(D,null,[A,d,c,C],0,A):I.renderer(d,c,C),C=d,r.setAttributes({text:String(v),translationY:a.roundPixel(b*q+u)},!0),r.applyTransformations(),p=Math.max(p,r.getBBox().width+s),p<=e.thickness&&(B=Ext.draw.Matrix.fly(r.attr.matrix.elements.slice(0)),x=B.prepend.apply(B,M).transformBBox(r.getBBox(!0)),!K||G(x,K,m)))&&(a.renderSprite(r),K=x,F+=x.height,z++)});else if("top"===h||"bottom"===h)e.iterate(k,function(b,d,f){void 0!==d&&(v=D?Ext.callback(D,null,[A,
d,c,C],0,A):I.renderer(d,c,C),C=d,r.setAttributes({text:String(v),translationX:a.roundPixel(b*l+n)},!0),r.applyTransformations(),p=Math.max(p,r.getBBox().height+s),p<=e.thickness&&(B=Ext.draw.Matrix.fly(r.attr.matrix.elements.slice(0)),x=B.prepend.apply(B,M).transformBBox(r.getBBox(!0)),K&&G(x,K,m),a.renderSprite(r),K=x,F+=x.width,z++))});else if("radial"===h)e.iterate(k,function(b,d,e){void 0!==d&&(v=D?Ext.callback(D,null,[A,d,c,C],0,A):I.renderer(d,c,C),C=d,"undefined"!==typeof v&&(r.setAttributes({text:String(v),
translationX:f.centerX-a.roundPixel(b)/f.max*f.length*Math.cos(f.baseRotation+Math.PI/2),translationY:f.centerY-a.roundPixel(b)/f.max*f.length*Math.sin(f.baseRotation+Math.PI/2)},!0),r.applyTransformations(),x=r.attr.matrix.transformBBox(r.getBBox(!0)),!K||G(x,K)))&&(a.renderSprite(r),K=x,F+=x.width,z++)});else if("angular"===h)L=f.majorTickSize+.5*f.lineWidth+(parseInt(r.attr.fontSize,10)||10)/2,e.iterate(k,function(b,d,e){void 0!==d&&(v=D?Ext.callback(D,null,[A,d,c,C],0,A):I.renderer(d,c,C),C=d,
p=Math.max(p,Math.max(f.majorTickSize,f.minorTickSize)+("butt"!==f.lineCap?.5*f.lineWidth:0)),"undefined"!==typeof v&&(b=b/(f.max+1)*Math.PI*2+f.baseRotation,r.setAttributes({text:String(v),translationX:f.centerX+(f.length+L)*Math.cos(b),translationY:f.centerY+(f.length+L)*Math.sin(b)},!0),r.applyTransformations(),x=r.attr.matrix.transformBBox(r.getBBox(!0)),!K||G(x,K)))&&(a.renderSprite(r),K=x,F+=x.width,z++)});else if("gauge"===h){var w=e.getGaugeAngles();e.iterate(k,function(b,d,e){void 0!==d&&
(v=D?Ext.callback(D,null,[A,d,c,C],0,A):I.renderer(d,c,C),C=d,"undefined"!==typeof v&&(b=(b-f.min)/(f.max-f.min+1)*f.totalAngle-f.totalAngle+w.start,r.setAttributes({text:String(v),translationX:f.centerX+(f.length+10)*Math.cos(b),translationY:f.centerY+(f.length+10)*Math.sin(b)},!0),r.applyTransformations(),x=r.attr.matrix.transformBBox(r.getBBox(!0)),!K||G(x,K)))&&(a.renderSprite(r),K=x,F+=x.width,z++)})}f.enlargeEstStepSizeByText&&z&&(F/=z,F+=s,F*=2,f.estStepSize<F&&(f.estStepSize=F));if(1<Math.abs(e.thickness-
...
{msg:Ext.isString(a)?a:a.msg,response:b}))},showError:function(a,b){Ext.isString(a)&&(a={message:a});debugger;!b&&(a.error||a.responseText||a instanceof Voyant.util.ResponseError)&&(b=a);if(b){a=a||{};a.message=a.message||"";if(b.error){var c=b.error.split(/(\r\n|\r|\n)/);0<c.length&&(a.message=this.localize("serverResponseError"),a.message+="\x3cpre class\x3d'error'\x3e\n"+c[0]+"\x3c/p\x3e",1<c.length&&(a.message+="\u2026  \x3ca href\x3d'#' onclick\x3d\"window.open('').document.write(unescape('\x3cpre\x3e"+
