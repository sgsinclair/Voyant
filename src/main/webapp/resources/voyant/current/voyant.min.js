...
Ext.define("Voyant.data.store.CorpusFacets",{extend:"Ext.data.Store",mixins:["Voyant.data.store.CorpusFacetsMixin"],constructor:function(a){a=a||{};this.mixins["Voyant.data.store.CorpusFacetsMixin"].constructor.apply(this,[a]);this.callParent([a])}});Ext.define("Voyant.data.store.CorpusFacetsBuffered",{extend:"Ext.data.BufferedStore",mixins:["Voyant.data.store.CorpusFacetsMixin"],constructor:function(a){a=a||{};this.mixins["Voyant.data.store.CorpusFacetsMixin"].constructor.apply(this,[a]);this.callParent([a])}});Ext.define("Voyant.data.store.CorpusTermsMixin",{mixins:["Voyant.data.store.VoyantStore"],model:Voyant.data.model.CorpusTerm,statics:{i18n:{getString:"This store has {0} terms with a total of {1} occurrences."}},constructor:function(a){this.mixins["Voyant.data.store.VoyantStore"].constructor.apply(this,[a,{"proxy.extraParams.tool":"corpus.CorpusTerms","proxy.reader.rootProperty":"corpusTerms.terms","proxy.reader.totalProperty":"corpusTerms.total"}])},show:function(a){show(this.getString(a))}});
...
series:e,axes:[{type:"numeric",position:"left",majorTickSteps:"raw"==this.getApiParam("withDistributions")&&20>f?f:void 0,increment:1,title:{text:this.localize("raw"==this.getApiParam("withDistributions")?"rawFrequencies":"relativeFrequencies")}},{type:"category",position:"bottom",fields:["index"],title:{text:this.localize(b==this.MODE_DOCUMENT?"segments":"documents")},label:{rotate:{degrees:-30},textAlign:"end"},renderer:function(a,c){if(b==g.MODE_DOCUMENT)return parseInt(c)+1;var d=g.getCorpus().getDocument(c);
return d?d.getTinyLabel():"?"}}]})},reloadFromChart:function(){var a=this.down("chart");if(a){var b=[];a.series.forEach(function(a){b.push(a.getTitle())});this.fireEvent("termsClicked",this,b)}},buildChart:function(a){a.axes.forEach(function(a){Ext.applyIf(a,{style:{opacity:.2},label:{opacity:.5}});Ext.applyIf(a.title,{fontSize:12})});Ext.applyIf(a,{plugins:{ptype:"chartitemevents",moveEvents:!0},legend:{docked:"top"},interactions:["itemhighlight","panzoom"],innerPadding:{top:5,right:5,bottom:5,left:5},
border:!1,bodyBorder:!1});this.query("chart").forEach(function(a){this.remove(a)},this);a=Ext.create("Ext.chart.CartesianChart",a);this.add(a)},clearChart:function(){this.query("chart").forEach(function(a){this.remove(a)},this)},handleClickedItem:function(a,b){var c=this.getApiParam("mode");if(c===this.MODE_DOCUMENT){if(c=this.getApiParam("docId")){var d=this.getCorpus().getDocument(c),d=d.get("tokensCount-lexical"),e=this.getApiParam("bins"),d=b.index/e*d;this.dispatchEvent("documentIndexTermsClicked",
this,[{term:b.series.getTitle(),docId:c,position:d}])}}else c==this.MODE_CORPUS&&(d=this.getCorpus().getDocument(b.index),this.dispatchEvent("documentIndexTermsClicked",this,[{term:b.series.getTitle(),docIndex:b.index,docId:d.getId()}]))},isLastClickedItem:function(a){return this.lastClickedItem&&this.lastClickedItem.term==a.term&&this.lastClickedItem.index==a.index},setMode:function(a){this.setApiParams({mode:a});this.getApiParam("mode")}});Ext.define("Voyant.panel.NoTool",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.notool",statics:{i18n:{},api:{tool:void 0}},config:{html:void 0,notYetImplemented:"Centroid DocumentInputAdd DocumentTypeCollocateFrequenciesGrid EntitiesBrowser Equalizer FeatureClusters Flowerbed KwicsTagger Lava Mandala MicroSearch NetVizApplet PaperDrill RezoViz Sunburst Termometer Ticker TokensViz ToolBrowser ToolBrowserLarge VoyeurTagline WordCloud VoyeurTagline WordCountFountain".split(" ")},
