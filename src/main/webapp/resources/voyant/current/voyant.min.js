...
a||0>b||a>c.getWidth()||b>c.getHeight()},zoomToFit:function(a,b){var c=this.getVis().node().getBBox(),d=c.width,e=c.height,f=c.x+d/2,g=c.y+e/2,c=this.getVis().node().parentElement,h=c.clientWidth,k=c.clientHeight,d=(a||.8)/Math.max(d/h,e/k),f=[h/2-d*f,k/2-d*g];d3.select(c).transition().duration(b||500).call(this.getZoom().transform,d3.zoomIdentity.translate(f[0],f[1]).scale(d))},applyNodeStyle:function(a,b){var c=void 0===b?"normal":b;a.style("fill",function(a){a=a.type+"Node";return this.getGraphStyle()[a][c].fill}.bind(this));
a.style("stroke",function(a){a=a.type+"Node";return this.getGraphStyle()[a][c].stroke}.bind(this))},applyLinkStyle:function(a,b){var c=void 0===b?"normal":b;a.style("stroke",function(a){return this.getGraphStyle().link[c].stroke}.bind(this));a.style("stroke-opacity",function(a){return this.getGraphStyle().link[c].strokeOpacity}.bind(this))},linkMouseOver:function(a){this.getVis().selectAll("line").call(this.applyLinkStyle.bind(this));this.getVis().select("#"+a.id).call(this.applyLinkStyle.bind(this),
"highlight")},linkMouseOut:function(a){this.getVis().selectAll("line").call(this.applyLinkStyle.bind(this))},nodeMouseOver:function(a){this.setCurrentNode(a);this.getVis().selectAll("rect").call(this.applyNodeStyle.bind(this));this.getLinks().each(function(b){var c;b.source.id==a.id?c=b.target.id:b.target.id==a.id&&(c=b.source.id);void 0!==c&&(this.getVis().select("#"+c+" rect").call(this.applyNodeStyle.bind(this),"highlight"),this.getVis().select("#"+b.id).call(this.applyLinkStyle.bind(this),"highlight"))}.bind(this));
this.getVis().select("#"+a.id+" rect").style("stroke-width",3).call(this.applyNodeStyle.bind(this),"highlight")},nodeMouseOut:function(a){this.getContextMenu().isVisible()||this.setCurrentNode(void 0);this.getVis().selectAll("rect").style("stroke-width",1).call(this.applyNodeStyle.bind(this));this.getVis().selectAll("line").call(this.applyLinkStyle.bind(this))},fetchCollocatesForNode:function(a){var b=this.getApiParam("limit");this.getCorpus().getCorpusCollocates({autoLoad:!1}).load({params:Ext.apply(this.getApiParams(),
{query:a.term,start:a.start,limit:b}),callback:function(c,d,e){e&&(this.updateDataForNode(a.id,{start:a.start+b}),this.loadFromCorpusCollocateRecords(c,a.id))},scope:this})}});Ext.define("Voyant.panel.Contexts",{extend:"Ext.grid.Panel",mixins:["Voyant.panel.Panel"],requires:["Voyant.data.store.Contexts"],alias:"widget.contexts",isConsumptive:!0,statics:{i18n:{},api:{query:void 0,docId:void 0,docIndex:void 0,stopList:"auto",context:5,expand:50},glyph:"xf0ce@FontAwesome"},constructor:function(){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(){var a=this;Ext.apply(a,{title:this.localize("title"),emptyText:this.localize("emptyText"),
...
b){this.store.setCorpus(b);this.store.load({params:this.getApiParams()});0==this.hasCorpusAccess(b)&&(this.queryById("modifyButton").hide(),this.queryById("downloadButton").hide())});this.on("query",function(a,b){this.setApiParam("query",b);this.store.load({params:this.getApiParams()})});a.embedded&&("Voyant.data.model.Corpus"==Ext.getClass(a.embedded).getName()?a.corpus=a.embedded:"Voyant.data.store.Documents"==Ext.getClass(a.embedded).getName()&&(this.store.setRecords(a.embedded.getData()),a.corpus=
a.embedded.getCorpus()));a.corpus&&this.fireEvent("loadedCorpus",this,a.corpus)},keepRemoveReorderHandler:function(a){var b=a.up("window").down("documents"),c=b.getSelection(),d=b.getStore().getCorpus().getDocumentsCount(),e=a.getItemId();if("reorder"==e){if(b.getStore().getCount()<d)return Ext.Msg.show({title:this.localize("error"),message:this.localize("reorderFilteredError"),buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});docIndex=[];b.getStore().each(function(a){docIndex.push(a.getIndex())},this);for(a=
1;a<docIndex.length;a++)if(docIndex[a-1]>docIndex[a])return Ext.Msg.confirm(b.localize("newCorpus"),(new Ext.Template(b.localize(e+"Documents"))).applyTemplate([c.length]),function(a){"yes"===a&&(docIndex=[],this.getStore().each(function(a){docIndex.push(a.getIndex())},this),a={docIndex:docIndex},a[e+"Documents"]=!0,this.editCorpus(a))},b);return Ext.Msg.show({title:this.localize("error"),message:this.localize("reorderOriginalError"),buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR})}return 0<c.length?c.length==
d?1==d?Ext.Msg.show({title:this.localize("error"),message:this.localize("onlyOneError"),buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR}):Ext.Msg.show({title:this.localize("error"),message:this.localize("allSelectedError"),buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR}):Ext.Msg.confirm(this.localize("newCorpus"),(new Ext.Template(this.localize(e+"SelectedDocuments"))).applyTemplate([c.length]),function(a){"yes"===a&&(docIndex=[],c.forEach(function(a){docIndex.push(a.getIndex())}),a={docIndex:docIndex},a[e+"Documents"]=
!0,this.editCorpus(a))},b):b.getApiParam("query")&&b.getStore().getCount()<d?Ext.Msg.confirm(this.localize("newCorpus"),(new Ext.Template(this.localize(e+"FilteredDocuments"))).applyTemplate([c.length]),function(a){"yes"===a&&(docIndex=[],this.getStore().each(function(a){docIndex.push(a.getIndex())},this),a={docIndex:docIndex},a[e+"Documents"]=!0,this.editCorpus(a))},b):Ext.Msg.show({title:this.localize("error"),message:this.localize("selectOrFilterError"),buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR})},
...
SCROLL_UP:-1,SCROLL_EQ:0,SCROLL_DOWN:1,LOCATION_UPDATE_FREQ:100,DETAILED_GRAPH_DOC_LIMIT:25,INITIAL_LIMIT:1E3,constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(a){var b=Ext.create("Voyant.data.store.Tokens"),c=this;b.on("beforeload",function(a){return c.hasCorpusAccess(a.getCorpus())});b.on("load",function(a,b,c){if(c){var g="",h=this.localize("documentFrequency"),k=!1,l=-1,m=!1;b.forEach(function(a){0==a.getPosition()&&
(g+="\x3ch3\x3e"+this.getDocumentsStore().getById(a.getDocId()).getFullLabel()+"\x3c/h3\x3e");a.getDocIndex()!=l&&(k=this.getDocumentsStore().getById(a.getDocId()).isPlainText(),l=a.getDocIndex());if(a.isWord())m=!1,g+="\x3cspan class\x3d'word' id\x3d'"+a.getId()+"' data-qtip\x3d'"+h+" "+a.getDocumentRawFreq()+"'\x3e"+a.getTerm()+"\x3c/span\x3e";else{a=a.getTermWithLineSpacing(k);var b=0==a.indexOf("\x3cbr /\x3e");if(!m||!b&&0!=a.trim().length)g+=a,m=b}},this);this.updateText(g);this.down("querysearchfield").getValue()}},
this);this.setTokensStore(b);this.on("query",function(a,b){this.loadQueryTerms(b)},this);this.setDocumentTermsStore(Ext.create("Ext.data.Store",{model:"Voyant.data.model.DocumentTerm",autoLoad:!1,remoteSort:!1,proxy:{type:"ajax",url:Voyant.application.getTromboneUrl(),extraParams:{tool:"corpus.DocumentTerms",withDistributions:!0,withPositions:!0,bins:25},reader:{type:"json",rootProperty:"documentTerms.terms",totalProperty:"documentTerms.total"},simpleSortMode:!0},listeners:{load:function(a,b,c,g){a.sort("docIndex",
"ASC");var h={},k=0,l;a.each(function(a){var b=[],c=a.get("distributions"),d=a.get("docId"),e=a.get("docIndex");l=a.get("term");for(a=0;a<c.length;a++){var f=a,g=c[a];g>k&&(k=g);b.push([d,e,f,g,l])}h[e]=b},this);this.highlightKeywords(l);if(this.getIsDetailedGraph())for(a=this.query("cartesian"),b=0;b<a.length;b++)c=a[b],g=h[b],void 0!==g?(c.getAxes()[0].setMaximum(k),c.getStore().loadData(g)):c.getStore().removeAll()},scope:this}}));this.on("afterrender",function(){var a=this.down('panel[region\x3d"center"]');
this.setInnerContainer(a.getLayout().getRenderTarget());a.body.on("scroll",function(a,b){var c=this.getLastScrollTop()<b.scrollTop?this.SCROLL_DOWN:this.getLastScrollTop()>b.scrollTop?this.SCROLL_UP:this.SCROLL_EQ;if(c==this.SCROLL_UP&&1>b.scrollTop)this.fetchPrevious(!0);else if(c==this.SCROLL_DOWN&&b.scrollHeight-b.scrollTop<1.5*b.offsetHeight)this.fetchNext(!1);else{var d;d=0==b.scrollTop?0:b.scrollHeight-b.scrollTop==b.clientHeight?1:(b.scrollTop+.5*b.clientHeight)/b.scrollHeight;(new Date-this.getLastLocationUpdate()>
...
{xtype:"tbseparator"},{xtype:"querysearchfield"}]}],listeners:{loadedCorpus:function(a,b){this.getTokensStore().setCorpus(b);this.getDocumentTermsStore().getProxy().setExtraParam("corpus",b.getId());var c=b.getDocuments();this.setDocumentsStore(c);this.setIsDetailedGraph(c.getTotalCount()<this.DETAILED_GRAPH_DOC_LIMIT);c=this.down('panel[region\x3d"south"]');void 0==this.getLocationMarker()&&this.setLocationMarker(Ext.DomHelper.append(c.el,{tag:"div",style:"background-color: #157fcc; height: 100%; width: 2px; position: absolute; top: 0; left: 0;"}));
this.generateChart(b,c);this.rendered&&(this.load(),0==this.hasCorpusAccess(b)&&this.mask(this.localize("limitedAccess"),"mask-no-spinner"),(c=this.getApiParam("query"))&&this.loadQueryTerms(Ext.isString(c)?[c]:c))},termsClicked:function(a,b){var c=[];b.forEach(function(a){Ext.isString(a)?c.push(a):a.term?c.push(a.term):a.getTerm&&c.push(a.getTerm())});0<c.length&&this.loadQueryTerms(c)},corpusTermsClicked:function(a,b){var c=[];b.forEach(function(a){a.getTerm()&&c.push(a.getTerm())});this.loadQueryTerms(c)},
documentTermsClicked:function(a,b){var c=[];b.forEach(function(a){a.getTerm()&&c.push(a.getTerm())});this.loadQueryTerms(c)},documentSelected:function(a,b){var c=this.getTokensStore().getCorpus().getDocument(b);this.setApiParams({skipToDocId:c.getId(),start:0});this.load(!0)},documentsClicked:function(a,b,c){0<b.length&&(this.setApiParams({skipToDocId:b[0].getId(),start:0}),this.load(!0))},termLocationClicked:function(a,b){if(void 0!==b[0]){var c=b[0],g=c.get("docIndex"),h=c.get("position"),k=h-this.getApiParam("limit")/
2,l=this.getCorpus().getDocument(g);this.setApiParams({skipToDocId:l.getId(),start:0>k?0:k});this.load(!0,{callback:function(){var a=this.body.dom.querySelector("#_"+g+"_"+h);a&&a.scrollIntoView();this.highlightKeywords(c.get("term"),!1)},scope:this})}},documentIndexTermsClicked:function(a,b){if(void 0!==b[0]){var c=Ext.create("Voyant.data.model.Token",b[0]);this.fireEvent("termLocationClicked",this,[c])}},scope:this}});this.callParent(arguments)},loadQueryTerms:function(a){a&&0<a.length&&this.getDocumentTermsStore().load({params:{query:a}})},
updateLocationMarker:function(a,b){var c=Ext.DomQuery.select(".word",this.getInnerContainer().down(".readerContainer",!0)),d=c[0],e=c[c.length-1];if(void 0!==d&&void 0!==e){var c=this.getCorpus(),f=!1,g=Voyant.data.model.Token.getInfoFromElement(Ext.get(d)),e=Voyant.data.model.Token.getInfoFromElement(Ext.get(e));0!==g.position&&(f=!0);for(var h={},d=0,k=g.docIndex;k<=e.docIndex;){var l=c.getDocument(k).get("tokensCount-lexical");k===e.docIndex&&(l=e.position);k===g.docIndex&&(l-=g.position);d+=l;
h[k]=l;k++}for(var k=Math.round(d*a),l=d=0,m=g.docIndex;m<=e.docIndex&&!(d=m,l+=h[m],l>=k);m++);e=h[d]-(l-k);f&&d===g.docIndex&&(e+=g.position);f=e/c.getDocument(d).get("tokensCount-lexical");c=Ext.get(this.getLocationMarker());g=c.getX();this.getIsDetailedGraph()?(e=this.query("cartesian")[d])&&(g=e.getX()+e.getWidth()*f):(e=this.down("cartesian"),g=e.getWidth()/this.getCorpus().getDocuments().getCount(),g=e.getX()+g*d+g*f);null!=b&&(f=c.getX(),b==this.SCROLL_DOWN&&f>g||b==this.SCROLL_UP&&f<g)&&
