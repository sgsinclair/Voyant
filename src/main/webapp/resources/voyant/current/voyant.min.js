...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];m=0;f=function(){var a,f,h,k,r=this.wordsToArrange-1,n=g.canvas.width,z=g.canvas.height,q=.5*n,s=.5*z;do{h=p[m];if(void 0!==h){for(var G=Math.random()*Math.PI,w=.25*Math.random()*h.size,u=.5*(Math.random()-.5),v=.5*h.width,L=.5*h.height;;){a=Math.floor((q+Math.cos(G)*w*this.ratio-v)/this.COARSENESS)*this.COARSENESS;f=Math.floor((s+Math.sin(G)*w-L)/this.COARSENESS)*this.COARSENESS;
k=a+v>=n||f+L>=z?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;G+=u;w+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<m;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}m++;if(m>=p.length){clearTimeout(l);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(r--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;l=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
r=function(a){return doubletree.fieldText(a,"POS")},z=function(a){return"rgba(255,255,255,1)"},E=function(a){return"rgba(255,255,255,1)"},C=function(a){return"red"},F={node:{fill:"white",stroke:"steelblue","stroke-width":"1.5px"},branch:{stroke:"#aaa","stroke-width":"1.5px"}},G=!1,K=d3.dispatch("idsUpdated");K.on("idsUpdated",function(){this==A?(B.setIds(A.continuationIDs),B.updateContinuations()):this==B&&(A.setIds(B.continuationIDs),A.updateContinuations())});var y,D,A,B,J,H;b.init=function(a){d3.select(d3.selectAll(a)).call(this);
return b};b.redraw=function(){b.setupFromTries(y,D);return b};b.setupFromTries=function(d,k){y=d.getUniqRoot();D=k.getUniqRoot();var x=y.toTree(m.left),w=D.toTree(m.right),u=!0;0<Object.keys(w.pruned).length&&(a(w,w.pruned,u),a(x,w.pruned,u),u=!1);0<Object.keys(x.pruned).length&&(a(x,x.pruned,u),a(w,x.pruned,u));var u={},v;for(v in w.info)"continuations"!=v&&"ids"!=v&&"count"!=v&&(u[v]=w.info[v]);u["right continuations"]=w.info.continuations;u["left continuations"]=x.info.continuations;u.ids={};c(u.ids,
w.info.ids);c(u.ids,x.info.ids);u.count=Object.keys(u.ids).length;J=Object.keys(u.ids);if(w.info.origIDs||x.info.origIDs)u.origIDs={},c(u.origIDs,w.info.origIDs),c(u.origIDs,x.info.origIDs),u.origCount=Object.keys(u.origIDs).length;w.info=u;x.info=u;v=Math.max(x.maxChildren,w.maxChildren);if(isNaN(v)||0==v)return G=!1,b;q?H=d3.scale.log().range([8,14]):(H=function(){return 14},H.domain=function(){});v=Math.min(x.minCount,w.minCount);H.domain([v,x.info.count]);var L=g-20-20,M=h-20-20;f[0].forEach(function(a,
b){function c(){var a=d3.event.scale,b=d3.event.translate;d3.select(d).select("svg \x3e g").attr("transform","translate("+b+") scale("+a+")")}var d=a,e;e=d3.select(d).select("svg");null==e[0][0]?(e=d3.select(d).append("svg").attr("width",L+20+20).attr("height",M+20+20).attr("cursor","move").call(d3.behavior.zoom().scaleExtent([1,1]).on("zoom",c)),e.append("g")):(e.attr("width",L+20+20).attr("height",M+20+20),e.selectAll("g *").remove());A=new doubletree.Tree(e.select("g"),g,h,x,!0,s,K,H,n,p,r,z,E,
C,F);B=new doubletree.Tree(e.select("g"),g,h,w,!1,s,K,H,n,p,r,z,E,C,F)});A.handleAltPress=l.alt;B.handleAltPress=l.alt;A.handleShiftPress=l.shift;B.handleShiftPress=l.shift;A.handleClick=l.click;B.handleClick=l.click;G=!0;return b};b.setupFromArrays=function(a,c,d,f,g,h,m,l){void 0==g&&y&&(g=y.caseSensitive());void 0==h&&y&&(h=y.fieldNames());void 0==m&&y&&(m=y.fieldDelim());void 0==l&&y&&(l=y.distinguishingFieldsArray());y=new doubletree.Trie(g,h,m,l);D=new doubletree.Trie(g,h,m,l);g=c.length;for(h=
0;h<g;h++){m=f?f[h]:h;l=c[h];var r=a[h].slice(),n=d[h].slice();r.push(l);r.reverse();n.unshift(l);k?(D.addNgram(r,m),y.addNgram(n,m)):(y.addNgram(r,m),D.addNgram(n,m))}b.setupFromTries(y,D);return b};b.filteredIDs=function(){return J};b.search=function(a){A.search(a);B.search(a);a=d3.select(f[0][0]);var b=a.selectAll("text.foundText");if(b.empty())return 0;b=b[0].length;null!=a.selectAll("text.rtNdText.foundText")[0][0]&&b--;return b};b.clearSearch=function(){A.clearSearch();B.clearSearch();return b};
...
l=a;return b};b.showTokenExtra=function(a){if(!arguments.length)return n;n=a;return b};b.scaleLabels=function(a){if(!arguments.length)return q;q=a;return b};b.succeeded=function(){return G};b.sortFun=function(a){if(!arguments.length)return s;s=a;return b};b.nodeText=function(a){if(!arguments.length)return p;p=a;return b};b.tokenExtraText=function(a){if(!arguments.length)return r;r=a;return b};b.rectColor=function(a){if(!arguments.length)return z;z=a;return b};b.rectBorderColor=function(a){if(!arguments.length)return E;
E=a;return b};b.continuationColor=function(a){if(!arguments.length)return C;C=a;return b};b.basicStyles=function(a){if(!arguments.length)return F;Object.keys(F).forEach(function(b){b in a&&Object.keys(F[b]).forEach(function(c){c in a[b]&&(F[b][c]=a[b][c])})});return b};return b};doubletree.Tree=function(a,b,c,d,k,m,l,n,q,s,p,r,z,E,C){function F(a){a.children&&(a._children=a.children,a._children.forEach(F),a.children=null)}function G(a){a.parent&&a.parent.children.forEach(function(b){b!=a&&F(b)})}
function K(a,b){d3.event.altKey?v.handleAltPress(a,b):d3.event.shiftKey?v.handleShiftPress(a,b):(v.handleClick(a,b),a.parent&&(v.continuationIDs!=a.info.ids&&(v.setIds(a.info.ids),v.clickedNode=a.id,l.idsUpdated.apply(v)),G(a),y(a,!0)))}function y(a,b){a.children?(a.children&&1==a.children.length&&y(a.children[0],!0),a._children=a.children,a.children=null):(a.children=a._children,a._children=null,a.children&&1==a.children.length&&y(a.children[0],!1));b&&v.update(a)}function D(a){return k?J/2-a:J/
2+a}var A=q,B=r,J=b-20-20,H=c-20-20,N=0,I,x;m||(m=doubletree.sortByStrFld("token"));var w=d3.layout.tree().nodeSize([40,40]).sort(m),u=d3.svg.diagonal().projection(function(a){return[D(a.y),a.x-x]});a=a.append("g").attr("transform","translate(20,20)");this.readJSONTree=function(a){I=a;I.x0=H/2;I.y0=J/2;I.children.forEach(F);this.update(I)};var v=this;this.update=function(b){b||(b=I);var c=w.nodes(I).reverse();x=I.x-H/2;c.forEach(function(a){for(var b=0,c=a.parent;null!=c;)b+=Ext.draw.TextMeasurer.measureText(c.name,
"arial").width,c=c.parent;a.y=25*a.depth+b});var d=a.selectAll("g.node_"+k).data(c,function(a){return a.id||(a.id=++N)}),f=d.enter().append("g").attr("class","node node_"+k).attr("cursor","pointer").attr("transform",function(a){return"translate("+D(b.y0)+","+(b.x0-x)+")"}).on("click",K);f.append("title").text(function(a){return doubletree.infoToText(a.info)});f.append("circle").attr("r",1E-6).style("fill",function(a){return a._children?"#fff":C.node.fill}).style("stroke",function(a){return C.node.stroke});
...
function(a){var b=this.parentElement.getBBox().width;return a.parent?k?-.5*b:0:-.33333*b}).style("stroke-opacity",1).style("stroke-width",1).style("stroke",function(a){return z(a.info)}).style("fill",function(a){return B(a.info)}).style("fill-opacity",function(a){return 1})};try{this.drawRects()}catch(g){}f=d.transition().duration(200).attr("transform",function(a){return"translate("+D(a.y)+","+(a.x-x)+")"});f.select("circle").attr("r",function(a){return a.children||a._children?1E-6:4.5}).style("fill",
function(a){return a._children?"#fff":C.node.fill}).style("stroke-width",C.node["stroke-width"]);f.selectAll("tspan").style("fill-opacity",1);f=d.exit().transition().duration(200).attr("transform",function(a){return"translate("+D(b.y)+","+(b.x-x)+")"}).remove();f.select("circle").attr("r",1E-6);f.selectAll("tspan").style("fill-opacity",1E-6);f=a.selectAll("path.link_"+k).data(w.links(c),function(a){return a.target.id});f.enter().insert("path","g").attr("class","link link_"+k).attr("d",function(a){a=
{x:b.x0,y:b.y0};return u({source:a,target:a})}).style("fill","none").style("stroke",C.branch.stroke).style("stroke-width",C.branch["stroke-width"]);f.transition().duration(200).attr("d",u);f.exit().transition().duration(200).attr("d",function(a){a={x:b.x,y:b.y};return u({source:a,target:a})}).remove();c.forEach(function(a){a.x0=a.x;a.y0=a.y});this.updateContinuations()};this.setIds=function(a){v.continuationIDs=a};this.updateContinuations=function(){a.selectAll("g.node_"+k+" text").classed("continuation",
function(a){var b;a:{a=a.info.ids;var c=v.continuationIDs||{};for(b in a)if(b in c){b=!0;break a}b=!1}return b}).style("fill",function(a){return d3.select(this).classed("continuation")?E(a.info):"#444"})};this.search=function(b){a.selectAll("g.node text").classed("foundText",function(a){return b.test(s(a.info))})};this.clearSearch=function(){a.selectAll("g.node text").classed("foundText",!1)};this.showTokenExtras=function(a){if(0==arguments.length)return A;A=a;this.drawRects();return this};this.setRectColor=
