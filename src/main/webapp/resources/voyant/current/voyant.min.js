...
this.lastClickedBubbles[b],m=0,n;for(n in k)if(-1!==this.termsFilter.indexOf(n)){var r=k[n];if(r){m++;this.SEPARATE_LINES_FOR_TERMS&&c();var s=0,p=r.color.join(",");this.ctx.strokeStyle="rgba("+p+", 1)";this.ctx.fillStyle="rgba("+p+", 0.35)";this.ctx.lineWidth=.25;for(var h=h+r.rawFreq,s=s+r.rawFreq,q=l&&this.lastClickedBubbles[b][n],y=0;y<r.pos.length;y++){var D=r.pos[y];if(0<D.radius){var F=!1;q&&this.lastClickedBubbles[b][n]==D.id&&(this.ctx.strokeStyle="rgba(0, 0, 0, 0.75)",this.ctx.fillStyle=
...
(d=90,c.ty=0):(d=-90,c.ty=c.height,c.tx=c.width));c.size=Math.max(c.height,c.width);c.rotation=Math.PI/180*d;g.context.fillStyle=g.canvas.style.backgroundColor;g.context.fillRect(0,0,g.canvas.width,g.canvas.height);c.draw(g.context);d=g.context.getImageData(c.x,c.y,c.width,c.height);for(var e=[],h=0;h<c.width;h++){var k=Math.floor(h/f.COARSENESS)*f.COARSENESS;null==e[k]&&(e[k]={});for(var l=0;l<c.height;l++){var m=Math.floor(l/f.COARSENESS)*f.COARSENESS,q;q=4*(h+l*d.width);q=[d.data[q],d.data[q+1],
d.data[q+2],d.data[q+3]];"rgb("+q[0]+", "+q[1]+", "+q[2]+")"!=g.canvas.style.backgroundColor&&(e[k][m]=!0);e[k][m]&&(l=m+f.COARSENESS)}}c.mask=e}}function e(){g.clear();for(var a=p.length;a--;){var b=p[a];b.alpha=1;b.live&&b.draw(g.context)}a=$(g.canvas);if(null!=n){a.css("cursor","pointer");g.context.save();g.context.textBaseline="alphabetic";g.context.font="12px Arial";var b=g.context.measureText(n.text).width,c=g.context.measureText(n.value).width,b=(b>c?b:c)+20,c=r+15,d=s+25,e=a.width(),a=a.height();
...
0;var k=this.MIXED=1;this.getWordOrientation=function(){return k};this.setWordOrientation=function(a){k=a};this.UPDATE_RATE=25;this.COARSENESS=5;this.grid=[];var l=0,m;this.doingArrange=!1;this.wordsToArrange=5;var n=null,r=0,s=0,p=[];this.getWords=function(){return p};this.setWords=function(a){p=a};this.sizeAdjustment=100;this.minFontSize=12;this.largestWordSize=0;this.smallestWordSize=1E4;var q={};this.addWords=function(a){for(var b=!1,e=0;e<a.length;e++){var h=a[e],k;k=void 0==typeof h.color||
null==h.color||""==h.color?g.colors[Math.floor(Math.random()*g.colors.length)]:h.color;var l;l=void 0==typeof h.size||null==h.size||""==h.size?Math.floor(40*Math.random()):parseFloat(h.size);var m=h.word,n=h.label,r=h.value,h=!1;null==q[m]&&(q[m]=!0,l>f.largestWordSize&&(f.largestWordSize=l,h=!0),l<f.smallestWordSize&&(f.smallestWordSize=.8*l,h=!0),m=new Word(m,l,k,n,r),p.push(m));b=h||b}c();this.setRelativeSizes();this.calculateSizeAdjustment();b?this.resizeWords():d()};this.resetWordCoordinates=
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];l=0;f=function(){var a,f,h,k,q=this.wordsToArrange-1,n=g.canvas.width,y=g.canvas.height,r=.5*n,M=.5*y;do{h=p[l];if(void 0!==h){for(var s=Math.random()*Math.PI,C=.25*Math.random()*h.size,w=.5*(Math.random()-.5),t=.5*h.width,u=.5*h.height;;){a=Math.floor((r+Math.cos(s)*C*this.ratio-t)/this.COARSENESS)*this.COARSENESS;f=Math.floor((M+Math.sin(s)*C-u)/this.COARSENESS)*this.COARSENESS;
k=a+t>=n||f+u>=y?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;s+=w;C+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<l;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}l++;if(l>=p.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(q--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;m=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
w.info.ids);c(t.ids,v.info.ids);t.count=Object.keys(t.ids).length;J=Object.keys(t.ids);if(w.info.origIDs||v.info.origIDs)t.origIDs={},c(t.origIDs,w.info.origIDs),c(t.origIDs,v.info.origIDs),t.origCount=Object.keys(t.origIDs).length;w.info=t;v.info=t;u=Math.max(v.maxChildren,w.maxChildren);if(isNaN(u)||0==u)return C=!1,b;r?G=d3.scale.log().range([8,14]):(G=function(){return 14},G.domain=function(){});h=Math.max(200,u*(I-2));u=Math.max(v.maxLen,w.maxLen);var L=Math.max(80,.6*u*I);200<L&&(L=200);u=Math.min(v.minCount,
...
function(a){if(!arguments.length)return n;n=a;return b};b.scaleLabels=function(a){if(!arguments.length)return r;r=a;return b};b.succeeded=function(){return C};b.sortFun=function(a){if(!arguments.length)return s;s=a;return b};b.nodeText=function(a){if(!arguments.length)return p;p=a;return b};b.tokenExtraText=function(a){if(!arguments.length)return q;q=a;return b};b.rectColor=function(a){if(!arguments.length)return y;y=a;return b};b.rectBorderColor=function(a){if(!arguments.length)return D;D=a;return b};
b.continuationColor=function(a){if(!arguments.length)return F;F=a;return b};b.basicStyles=function(a){if(!arguments.length)return B;Object.keys(B).forEach(function(b){b in a&&Object.keys(B[b]).forEach(function(c){c in a[b]&&(B[b][c]=a[b][c])})});return b};return b};doubletree.Tree=function(a,c,d,h,k,l,m,n,r,s,p,q,y,D,F,B){function C(a){a.children&&(a._children=a.children,a._children.forEach(C),a.children=null)}function K(a){a.parent&&a.parent.children.forEach(function(b){b!=a&&C(b)})}function x(a,
...
a.children||a._children?l?"end":"start":l?"start":"end":"middle"}).style("font-size",function(a){return r(a.info.count)+"pt"});g.append("tspan").attr("dy",".35em").attr("class","tokenText").text(function(a){return p(a.info,1>a.depth)}).style("fill-opacity",1E-6);g.append("tspan").attr("dx",".35em").attr("class","tokenExtra").text(function(a){return q(a.info,1>a.depth)}).style("fill-opacity",1E-6);this.drawRects=function(){var b=A?"inline":"none";a.selectAll(".tokenExtra").attr("display",b);f.selectAll("rect").remove();
...
(function(){doubletree.Trie=function(a,b,c,d,e){function f(a,b,c){this.id=b;this.count=c;this.info={count:c,ids:{}};if(null==a)this.item=k;else{this.item=a;this.info.ids={};this.info.ids[b]=!0;a=a.split(n);for(var d in a)this.info[m[d]]=[a[d]]}this.nodes={};this.addNgram=function(a,b,c){c||(c=1);var d,e;if(0<a.length){d=a.shift();var g=d.split(n);e=s&&this.item==k?"":g.filter(function(a,b){return-1<r.indexOf(m[b])}).map(function(a){return l?a.toLocaleLowerCase():a}).join(n)}else e=d=h;var q;if(e in
this.nodes&&this.nodes[e]instanceof f){q=this.nodes[e];q.info.count+=c;q.info.ids[b]=!0;for(var p in g)e=g[p],-1==q.info[m[p]].indexOf(e)&&q.info[m[p]].push(e)}else q=new f(d,b,c),this.nodes[e]=q;d!=h&&q.addNgram(a,b,c)};this.getUniqRoot=function(){if(this.item==k){var a=Object.keys(this.nodes);if(1==a.length)return this.nodes[a[0]]}return this};this.toTree=function(a){function b(a,c,d){var e={children:[]};e.name=d.item;e.info={};for(var f in d.info)if("Object"===typeof d.info[f]){e.info[f]={};for(var h in d.info[f])e.info[f][h]=
...
arguments):this.get("id")},getCorpusTerms:function(a){return Ext.create("Voyant.data.store.CorpusTerms",Ext.apply(a||{},{corpus:this}))},getCorpusCollocates:function(a){return Ext.create("Voyant.data.store.CorpusCollocates",Ext.apply(a||{},{corpus:this}))},getDocumentQueryMatches:function(a){return Ext.create("Voyant.data.store.DocumentQueryMatches",Ext.apply(a||{},{corpus:this}))},getDocumentTerms:function(a){return Ext.create("Voyant.data.store.DocumentTerms",Ext.apply(a||{},{corpus:this}))},getContexts:function(a){return Ext.create("Voyant.data.store.Contexts",
...
arguments):this.get("createdTime")},requiresPassword:function(){var a=this.getNoPasswordAccess();return"NONE"==a||"NONCONSUMPTIVE"==a},getNoPasswordAccess:function(){return this.then?Voyant.application.getDeferredNestedPromise(this,arguments):this.get("noPasswordAccess")},show:function(a){return this.then?Voyant.application.getDeferredNestedPromise(this,arguments):this.getShow().show()},getShow:function(a){if(this.then)return Voyant.application.getDeferredNestedPromise(this,arguments);var b=this.getDocumentsCount(),
...
config:{tokenType:"lexical",inDocumentsCountOnly:void 0},constructor:function(a){a=a||{};Ext.applyIf(a,{minWidth:100,matchFieldWidth:!1,minChars:2,displayField:"term",valueField:"term",filterPickList:!0,createNewOnEnter:!0,createNewOnBlur:!1,tpl:Ext.create("Ext.XTemplate",'\x3cul class\x3d"x-list-plain"\x3e\x3ctpl for\x3d"."\x3e','\x3cli role\x3d"option" class\x3d"x-boundlist-item" style\x3d"white-space: nowrap;"\x3e'+(a.itemTpl?a.itemTpl:"{term} ({rawFreq})")+"\x3c/li\x3e","\x3c/tpl\x3e\x3c/ul\x3e"),
triggers:{help:{weight:2,cls:"fa-trigger form-fa-help-trigger",handler:"onHelpClick",scope:"this"}}});this.callParent(arguments)},initComponent:function(a){var b=this;b.on("beforequery",function(a){if(a.query){a.query=a.query.trim();"^"==a.query.charAt(0)&&(a.query=a.query.substring(1),a.cancel=0==a.query.length);"*"==a.query.charAt(a.query.length-1)&&(a.query=a.query.substring(0,a.query.length-1),a.cancel=0==a.query.length);try{new RegExp(a.query)}catch(b){a.cancel=!0}-1<a.query.indexOf('"')&&(-1==
a.query.indexOf(" ")&&(a.cancel=!0),2!=(a.query.match(/"/)||[]).length&&(a.cancel=!0));a.query=a.query+"*"+(-1==a.query.indexOf(" ")?",^"+a.query+"*":"")}},b);b.on("change",function(a,d){b.up("panel").fireEvent("query",b,d)});b.up("panel").on("loadedCorpus",function(a,d){var e=void 0;if(this.getApiParam)e=this.getApiParam("stopList");else{var f=this.up("panel");f&&f.getApiParam&&(e=f.getApiParam("stopList"))}b.setStore(d.getCorpusTerms({corpus:d,proxy:{extraParams:{limit:10,tokenType:b.tokenType,
inDocumentsCountOnly:b.inDocumentsCountOnly,stopList:e}}}))});b.on("afterrender",function(a){a.triggers&&a.triggers.help&&Ext.QuickTips.register({target:a.triggers.help.getEl(),text:a.localize("querySearchTip"),enabled:!0,showDelay:20,trackMouse:!0,autoShow:!0});this.suggest=Ext.create("Ext.tip.ToolTip",{target:this.inputEl,autoShow:!1,hidden:!0,html:""})});b.callParent(arguments)}});Ext.define("Voyant.widget.TotalPropertyStatus",{extend:"Ext.Component",mixins:["Voyant.util.Localization"],alias:"widget.totalpropertystatus",statics:{i18n:{totalPropertyStatus:{en:'{count:number("0,000")}'}}},initComponent:function(){Ext.applyIf(this,{tpl:this.localize("totalPropertyStatus"),itemId:"totalpropertystatus",style:"margin-right:5px",listeners:{afterrender:function(a){var b=a.up("grid");if(b){var c=b.getStore();a.updateStatus(c.getTotalCount());b.getStore().on("totalcountchange",a.updateStatus,
...
void 0!==this.getNetwork()&&this.getNetwork().fit()},this);a.callParent(arguments)},initLoad:function(){this.getCorpus()&&(this.initGraph(),this.getCorpus().getCorpusTerms({leadingBufferZone:0,autoLoad:!1}).load({callback:function(a,b,c){c&&this.loadFromCorpusTermRecords(a)},scope:this,params:{limit:3,stopList:this.getApiParam("stopList")}}))},loadFromQuery:function(a){var b=this.getCorpus().getCorpusCollocates({autoLoad:!1});this.setApiParams({mode:"corpus"});var c=this.getApiParams();c.query=a;
b.load({params:c,callback:function(a,b,c){c&&this.loadFromCorpusCollocateRecords(a)},scope:this})},loadFromCorpusTermRecords:function(a){if(Ext.isArray(a)&&0<a.length){var b=[];a.forEach(function(a){b.push(a.getTerm())});this.loadFromCorpusTermStringsArray(b)}},loadFromCorpusTermStringsArray:function(a){a.forEach(function(a){this.loadFromQuery(a)},this);this.setApiParams({query:a,mode:"corpus"})},loadFromCorpusCollocateRecords:function(a,b){if(Ext.isArray(a)){var c={};this.getNodeDataSet().forEach(function(a){c[a.id]=
!0});var d=[],e=[],f=this.getEdgeDataSet(),g=this.getApiParam("limit");a.forEach(function(a){if(a.getContextTerm()!=a.getTerm()){if(void 0===b){var k={id:a.getKeyword()+".keyword",label:a.getKeyword(),title:a.getKeyword()+" ("+a.getKeywordRawFreq()+")",type:"keyword",value:a.getKeywordRawFreq(),start:g,font:{color:"green"}};b=k.id;void 0===c[b]&&(c[b]=!0,d.push(k))}a={id:a.getContextTerm()+".context",label:a.getContextTerm(),title:a.getContextTerm()+" ("+a.getContextTermRawFreq()+")",type:"context",
value:a.getContextTermRawFreq(),start:0,font:{color:"maroon"}};var l=a.id;void 0===c[l]&&(c[l]=!0,d.push(a));var m=!1;f.forEach(function(a){if(a.from==b&&a.to==l||a.from==l&&a.to==b)m=!0});m||e.push({from:b,to:l})}});this.getNodeDataSet().add(d);f.add(e);this.forceUpdate();this.getNetwork().fit()}},initGraph:function(){var a=this.getLayout().getRenderTarget();a.setWidth(a.getWidth());a.setHeight(a.getHeight());this.setGraphHeight(a.getHeight());this.setGraphWidth(a.getWidth());if(void 0===this.getNetwork()){var b=
{autoResize:!0,interaction:{hover:!0,hoverConnectedEdges:!0,multiselect:!1},physics:{barnesHut:{gravitationalConstant:-1500,centralGravity:6,damping:.5,avoidOverlap:.5}},nodes:this.nodeOptions,edges:this.edgeOptions},a=new vis.Network(a.dom,{nodes:this.getNodeDataSet(),edges:this.getEdgeDataSet()},b);this.setNetwork(a);this.getNodeDataSet().on("remove",function(a,b,e){var f=b.items[0];a=this.getEdgeDataSet().get({filter:function(a){return a.from==f||a.to==f}});this.getEdgeDataSet().remove(a);var g=
[];this.getNodeDataSet().forEach(function(a){0==this.getEdgeDataSet().get({filter:function(b){return b.from==a.id||b.to==a.id}}).length&&g.push(a.id)}.bind(this));this.getNodeDataSet().remove(g)}.bind(this));a.on("dragStart",function(a){a=a.nodes[0];null!=a&&this.getNodeDataSet().update({id:a,fixed:!1})}.bind(this));a.on("dragging",function(a){null!=a.nodes[0]&&(this.isMasked()?this.isOffCanvas(a.pointer.DOM)||this.unmask():this.isOffCanvas(a.pointer.DOM)&&this.mask(this.localize("releaseToRemove")))}.bind(this));
a.on("dragEnd",function(a){var b=a.nodes[0];null!=b&&(this.isOffCanvas(a.pointer.DOM)?(this.unmask(),this.mask("cleaning"),this.getNodeDataSet().remove(b),this.forceUpdate(),this.unmask()):this.getNodeDataSet().update({id:b,fixed:!0}))}.bind(this));a.on("click",function(a){this.getContextMenu().hide()}.bind(this));a.on("doubleClick",function(a){a=a.nodes[0];null!=a&&(a=this.getNodeDataSet().get(a),this.itemdblclick(a))}.bind(this));a.on("oncontext",function(a){a.event.preventDefault();var b=this.getNetwork().getNodeAt(a.pointer.DOM);
...
c)},scope:this,params:this.getApiParams("docId docIndex limit stopList query withDistributions bins".split(" "))}))},loadFromRecords:function(a){var b=d3.scale.category10(),c=this.down("[xtype\x3dlegend]").getStore(),d=[],e=[];a.forEach(function(a,c){var f=[],g=a.getTerm();a.get("distributions").forEach(function(a,b){f.push({x:b,y:a})},this);e.push({name:g,values:f});d.push({id:g,name:g,mark:b(c),active:!0})},this);c.loadData(d);var f=this.getVisLayout()(e),g;g=this.getMode()===this.MODE_DOCUMENT?
this.getApiParam("bins"):this.getCorpus().getDocumentsCount();g--;var c=this.body.down("svg").getWidth()-this.graphMargin.left-this.graphMargin.right,h=d3.scale.linear().domain([0,g]).range([0,c]),k=d3.max(f,function(a){return d3.max(a.values,function(a){return a.y0+a.y})});a=this.body.down("svg").getHeight()-this.graphMargin.top-this.graphMargin.bottom;var l=d3.scale.linear().domain([0,k]).range([a,0]),m=d3.svg.area().x(function(a){return h(a.x)}).y0(function(a){return l(a.y0)}).y1(function(a){return l(a.y0+
a.y)}),k=d3.svg.axis().scale(h).orient("bottom");if(this.getMode()===this.MODE_CORPUS){for(var n=[],r=0;r<=g;r++)n.push(r);k.tickValues(n);k.tickFormat("")}n=d3.svg.axis().scale(l).orient("left");f=this.getVis().selectAll("path").data(f,function(a){return a.name});f.attr("d",function(a){return m(a.values)}).style("fill",function(a,c){return b(c)});f.enter().append("path").attr("d",function(a){return m(a.values)}).style("fill",function(a,c){return b(c)}).append("title").text(function(a){return a.name});
f.exit().remove();this.getVis().selectAll("g.axis").remove();this.getVis().append("g").attr("class","axis x").attr("transform","translate(0,"+a+")").call(k);if(this.getMode()===this.MODE_CORPUS){var s=c/g,p=0;this.getCorpus().getDocuments().each(function(a){this.getVis().select("g.x").append("text").attr("text-anchor","end").attr("transform","translate("+p+", 10) rotate(-45)").text(a.getTinyTitle());p+=s},this);g=this.localize("documents")}else g=this.localize("documentSegments");this.getVis().select("g.x").append("text").attr("text-anchor",
"middle").attr("transform","translate("+c/2+", "+(this.graphMargin.bottom-20)+")").text(g);this.getVis().append("g").attr("class","axis y").attr("transform","translate(0,0)").call(n);c="raw"===this.getApiParam("withDistributions")?this.localize("rawFrequencies"):this.localize("relativeFrequencies");this.getVis().select("g.y").append("text").attr("text-anchor","middle").attr("transform","translate(-"+(this.graphMargin.left-20)+", "+a/2+") rotate(-90)").text(c)},getCurrentTerms:function(){var a=[];
this.down("[xtype\x3dlegend]").getStore().each(function(b){b.get("active")&&a.push(b.get("name"))},this);return a},initGraph:function(){if(void 0===this.getVisLayout()){var a=this.getLayout().getRenderTarget(),b=this.graphMargin.left+this.graphMargin.right,c=this.graphMargin.top+this.graphMargin.bottom,d=a.getWidth()-b,e=a.getHeight()-c;this.setVisLayout(d3.layout.stack().offset("silhouette").values(function(a){return a.values}));this.setVis(d3.select(a.dom).append("svg").attr("id","streamGraph").attr("width",
d+b).attr("height",e+c).append("g").attr("transform","translate("+this.graphMargin.left+","+this.graphMargin.top+")"))}},updateMode:function(a){this.queryById("segmentsSlider").setHidden(a==this.MODE_CORPUS)}});Ext.define("Voyant.panel.Summary",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel","Voyant.util.SparkLine"],alias:"widget.summary",statics:{i18n:{title:{en:"Summary"},helpTip:{en:"\x3cp\x3eThe \x3ci\x3eSummary\x3c/i\x3e tool provides general information about the corpus. Many elements in the tool are links that trigger other views. Features include:\x3c/p\x3e\x3cul\x3e\x3cli\x3etotal words (tokens) and word forms (types) and age of the corpus\x3c/li\x3e\x3cli\x3emost frequent terms in the corpus\x3c/li\x3e\x3cli\x3efor corpora with more than one documen\x3cul\x3e\x3cli\x3edocuments ordered by length and vocabulary density\x3c/li\x3e\x3cli\x3edistinctive words for each document (by TF-IDF score)\x3c/li\x3e\x3c/ul\x3e\x3c/li\x3e\x3c/ul\x3e"},
corpusType:{en:'\x3ctpl for\x3d"types"\x3e\x3ca href\x3d"#" onclick\x3d"return false" class\x3d"corpus-type keyword" voyant:recordId\x3d"{id}"\x3e{type}\x3c/a\x3e ({val})\x3ctpl if\x3d"xindex \x26lt; xcount"\x3e, \x3c/tpl\x3e\x3c/tpl\x3e'},documentType:{en:'\x3ctpl for\x3d"types"\x3e\x3ca href\x3d"#" onclick\x3d"return false" class\x3d"document-type keyword" voyant:recordId\x3d"{id}" voyant:val\x3d"{docId}:{type}"\x3e{type}\x3c/a\x3e ({val})\x3ctpl if\x3d"xindex \x26lt; xcount"\x3e, \x3c/tpl\x3e\x3c/tpl\x3e'},
mostFrequentWords:{en:"Most \x3cb\x3efrequent words\x3c/b\x3e in the corpus: "},docsLength:{en:'\x3cb\x3eDocument Length\x3c/b\x3e ({spark})\x3cul\x3e\x3cli\x3elongest: \x3ctpl for\x3d"longestDocs"\x3e\x3ca href\x3d"#" onclick\x3d"return false" class\x3d"document-id" voyant:val\x3d"{id}"\x3e{title}\x3c/a\x3e ({totalWordTokens})\x3c/a\x3e\x3ctpl if\x3d"xindex \x26lt; xcount"\x3e; \x3c/tpl\x3e\x3c/tpl\x3e\x3c/li\x3e\x3cli\x3eshortest: \x3ctpl for\x3d"shortestDocs"\x3e\x3ca href\x3d"#" onclick\x3d"return false" class\x3d"document-id" voyant:val\x3d"{id}"\x3e{title}\x3c/a\x3e ({totalWordTokens})\x3c/a\x3e\x3ctpl if\x3d"xindex \x26lt; xcount"\x3e; \x3c/tpl\x3e\x3c/tpl\x3e\x3c/li\x3e\x3c/ul\x3e'},
docsDensity:{en:'\x3cb\x3eVocabulary Density\x3c/b\x3e ({spark})\x3cul\x3e\x3cli\x3ehighest: \x3ctpl for\x3d"highestDocs"\x3e\x3ca href\x3d"#" onclick\x3d"return false" class\x3d"document-id" voyant:val\x3d"{id}"\x3e{title}\x3c/a\x3e ({wordDensity})\x3c/a\x3e\x3ctpl if\x3d"xindex \x26lt; xcount"\x3e; \x3c/tpl\x3e\x3c/tpl\x3e\x3c/li\x3e\x3cli\x3elowest: \x3ctpl for\x3d"lowestDocs"\x3e\x3ca href\x3d"#" onclick\x3d"return false" class\x3d"document-id" voyant:val\x3d"{id}"\x3e{title}\x3c/a\x3e ({wordDensity})\x3c/a\x3e\x3ctpl if\x3d"xindex \x26lt; xcount"\x3e; \x3c/tpl\x3e\x3c/tpl\x3e\x3c/li\x3e\x3c/ul\x3e'},
distinctiveWords:{en:"\x3cb\x3eDistinctive words\x3c/b\x3e (compared to the rest of the corpus): "},moreDistinctiveWords:{en:'\x3ca href\x3d"#" onclick\x3d"return false"\x3eNext {0} of {1} remaining\x3c/a\x3e'},seeAll:{en:"All\x26hellip;"},more:{en:"More\x26hellip;"},items:{en:"items"}},api:{stopList:"auto",start:0,limit:5,numberOfDocumentsForDistinctiveWords:10},glyph:"xf1ea@FontAwesome"},config:{corpus:void 0,options:{xtype:"stoplistoption"}},autoScroll:!0,cls:"corpus-summary",corpusTermsStore:null,
docsStore:null,documentTermsStore:null,summaryListParent:null,constructor:function(a){Ext.apply(this,{title:this.localize("title"),dockedItems:[{dock:"bottom",xtype:"toolbar",enableOverflow:!0,items:[{fieldLabel:this.localize("items"),labelWidth:40,width:120,xtype:"slider",increment:5,minValue:5,maxValue:59,listeners:{afterrender:function(a){a.setValue(this.getApiParam("visible"))},changecomplete:function(a,c){this.setApiParams({limit:c});this.loadSummary()},scope:this}}]}]});this.callParent(arguments);
this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments);this.on("afterrender",function(){this.addLinksHandler()});this.on("loadedCorpus",function(a,c){this.setCorpus(c);this.corpusTermsStore=c.getCorpusTerms();this.docStore=Ext.create("Ext.data.Store",{model:"Voyant.data.model.Document",autoLoad:!1,remoteSort:!1,corpus:this.getCorpus(),proxy:{type:"ajax",url:Voyant.application.getTromboneUrl(),extraParams:{tool:"corpus.DocumentsMetadata",corpus:this.getCorpus().getId()},reader:{type:"json",
rootProperty:"documentsMetadata.documents",totalProperty:"documentsMetadata.total"},simpleSortMode:!0},listeners:{load:function(a,b,c,g){var h=this.getCorpus();b.forEach(function(a){a.set("corpus",h)})},scope:this}});this.documentTermsStore=Ext.create("Ext.data.Store",{model:"Voyant.data.model.DocumentTerm",autoLoad:!1,remoteSort:!1,corpus:this.getCorpus(),proxy:{type:"ajax",url:Voyant.application.getTromboneUrl(),extraParams:{tool:"corpus.DocumentTerms",corpus:this.getCorpus().getId()},reader:{type:"json",
rootProperty:"documentTerms.terms",totalProperty:"documentTerms.total"},simpleSortMode:!0}});this.rendered?this.loadSummary():Ext.defer(this.fireEvent,100,this)});this.on("activate",function(){this.getCorpus()&&this.dispatchEvent("ensureCorpusView",this,this.getCorpus())},this);a.embedded&&"Voyant.data.model.Corpus"==Ext.getClass(a.embedded).getName()&&(a.corpus=a.embedded);a&&a.corpus&&this.fireEvent("loadedCorpus",this,a.corpus)},loadSummary:function(){var a=this.getLayout().getRenderTarget();a.update("");
this.summaryListParent=Ext.dom.Helper.append(a,"\x3cul\x3e\x3c/ul\x3e");Ext.dom.Helper.append(this.summaryListParent,"\x3cli\x3e"+this.getCorpus().getShow()+"\x3c/li\x3e");a=this.getCorpus().getDocumentsCount();1<a&&this.showLongestDocuments();1<a&&this.showMostDenseDocuments();this.showMostFrequentWords();1<a&&this.showDistinctiveWords();a=Ext.apply({},{limit:null},this.getApiParams());this.docStore.load({params:a})},showLongestDocuments:function(){var a=Ext.dom.Helper.append(this.summaryListParent,
"\x3cli\x3e\x3c/li\x3e");this.docStore.on("load",function(b,c,d){c=b.getTotalCount();if(1<c){b.sort("index","ASC");var e=[];b.each(function(a){e.push(a.get("tokensCount-lexical"))});b.sort("tokensCount-lexical","DESC");d=parseInt(this.getApiParam("limit"));b={shortest:b.getRange(c-d,c).reverse(),longest:b.getRange(0,d),all:b.getRange(0,c)};for(var f in b)if(f)for(c=b[f],d=0;d<c.length;d++){var g=c[d];b[f][d]={title:g.getShortTitle(),id:g.get("id"),totalWordTokens:Ext.util.Format.number(g.get("tokensCount-lexical"),
"0,000"),docsLen:c.length}}out=(new Ext.XTemplate(this.localize("docsLength"))).apply({spark:this.getSparkLine(e),longestDocs:b.longest,shortestDocs:b.shortest});Ext.dom.Helper.append(a,out)}},this)},showMostDenseDocuments:function(){var a=Ext.dom.Helper.append(this.summaryListParent,"\x3cli\x3e\x3c/li\x3e");this.docStore.on("load",function(b,c,d){c=b.getTotalCount();if(1<c){b.sort("index","ASC");var e=[];b.each(function(a){e.push(a.get("typeTokenRatio-lexical"))});b.sort("typeTokenRatio-lexical",
"DESC");d=parseInt(this.getApiParam("limit"));b={lowest:b.getRange(c-d,c).reverse(),highest:b.getRange(0,d),all:b.getRange(0,c)};for(var f in b)if(f)for(c=b[f],d=0;d<c.length;d++){var g=c[d];b[f][d]={title:g.getShortTitle(),id:g.get("id"),wordDensity:Ext.util.Format.number(g.get("typeTokenRatio-lexical"),"0.000"),docsLen:c.length}}out=(new Ext.XTemplate(this.localize("docsDensity"))).apply({spark:this.getSparkLine(e),highestDocs:b.highest,lowestDocs:b.lowest});Ext.dom.Helper.append(a,out)}},this)},
showMostFrequentWords:function(){var a=Ext.dom.Helper.append(this.summaryListParent,"\x3cli\x3e\x3c/li\x3e"),b=this.getApiParams();this.corpusTermsStore.load({params:b,scope:this,callback:function(b){var d=this.localize("mostFrequentWords"),e=[],f=b.length;b.forEach(function(a,b,c){e.push({id:a.getId(),type:a.getTerm(),val:Ext.util.Format.number(a.get("rawFreq"),"0,000"),len:f})});d+=(new Ext.XTemplate(this.localize("corpusType"))).applyTemplate({types:e})+".";Ext.dom.Helper.append(a,d)}})},showDistinctiveWords:function(){Ext.dom.Helper.append(this.summaryListParent,
'\x3cli id\x3d"distinctiveWords"\x3e'+this.localize("distinctiveWords")+"\x3col\x3e\x3c/ol\x3e\x3c/li\x3e",!0);this.showMoreDistinctiveWords()},showMoreDistinctiveWords:function(){var a=Ext.dom.Query.selectNode("#distinctiveWords \x3e ol",this.summaryListParent),b=Ext.dom.Query.select("li:not(.more)",a).length,c=parseInt(this.getApiParam("numberOfDocumentsForDistinctiveWords")),d=this.getCorpus().getDocumentsStore().getRange(b,b+c-1);if(d&&Ext.isArray(d)){var e=[];d.forEach(function(a){e.push(a.getIndex())});
0<e.length&&this.documentTermsStore.load({addRecords:!0,params:{docIndex:e,perDocLimit:parseInt(this.getApiParam("limit")),limit:c*parseInt(this.getApiParam("limit")),sort:"TFIDF",dir:"DESC"},scope:this,callback:function(d,g,h){var k={};if(h&&d&&Ext.isArray(d)&&(d.forEach(function(a,b,c){b=a.getDocIndex();b in k||(k[b]=[]);k[b].push({id:a.getId(),type:a.getTerm(),val:Ext.util.Format.number(a.get("rawFreq"),"0,000"),docId:a.get("docId")})}),e.forEach(function(b){var c=this.getCorpus().getDocument(b);
len=k[b].length;Ext.dom.Helper.append(a,{tag:"li","voyant:index":String(b),html:'\x3ca href\x3d"#" onclick\x3d"return false" class\x3d"document-id document-id-distinctive" voyant:val\x3d"'+c.get("id")+'"\x3e'+c.getShortTitle()+"\x3c/a\x3e"+this.localize("colon")+(new Ext.XTemplate(this.localize("documentType"))).apply({types:k[b]})+"."})},this),len=c,remaining=this.getCorpus().getDocumentsStore().getTotalCount()-b-e.length,0<remaining)){d=new Ext.Template(this.localize("moreDistinctiveWords"));var l=
Ext.dom.Helper.append(a,{tag:"li",cls:"more",html:d.apply([len>remaining?remaining:len,remaining])},!0);l.on("click",function(){l.remove();this.showMoreDistinctiveWords()},this)}}})}},addLinksHandler:function(){this.body.addListener("click",function(a){if((a=a.getTarget(null,null,!0))&&"A"==a.dom.tagName){var b={};a.hasCls("corpus-documents")?(a.hasCls("corpus-documents-length")?b.sortBy="totalWordTokens":a.hasCls("corpus-documents-density")&&(b.sortBy="wordDensity"),b.sortDirection="DESC"):a.hasCls("document-id")?
(a=a.getAttribute("val","voyant"),a=this.docStore.getById(a),this.dispatchEvent("documentsClicked",this,[a])):a.hasCls("corpus-type")?(a=a.getAttribute("recordId","voyant"),a=this.corpusTermsStore.getById(a),this.dispatchEvent("corpusTermsClicked",this,[a])):a.hasCls("corpus-types")?(b.query="",a.hasCls("corpus-types-peaks")?Ext.apply(b,{sortBy:"RELATIVEDISTRIBUTIONKURTOSIS",sortDirection:"DESC",extendedSortZscoreMinimum:1}):Ext.apply(b,{sortBy:"rawFreq",sortDirection:"DESC",extendedSortZscoreMinimum:null})):
a.hasCls("document-type")&&(a=a.getAttribute("recordId","voyant"),a=this.documentTermsStore.getById(a),this.dispatchEvent("documentTermsClicked",this,[a]))}},this)}});Ext.define("Voyant.panel.TopicContexts",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.topiccontexts",statics:{i18n:{title:{en:"Topic Contexts"},helpTip:{en:"Topic Contexts"},reset:{en:"reset"}},api:{},glyph:"xf06e@FontAwesome"},constructor:function(a){Ext.apply(this,{title:this.localize("title"),cls:"twic_body"});this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},listeners:{loadedCorpus:function(a,b){this.loadFromCorpus(b)}},loadFromCorpus:function(a){a=
...
helpTip:{en:"This is the default, general-purpose corpus view."}},api:{panels:void 0},glyph:"xf17a@FontAwesome"},constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},layout:"border",header:!1,items:[{region:"west",flex:3,layout:"fit",moreTools:["cirrus","corpusterms"],xtype:"voyanttabpanel",split:{width:5},tabBarHeaderPosition:0,items:[{xtype:"cirrus"},{xtype:"corpusterms"}]},{region:"center",flex:3,layout:"fit",xtype:"voyanttabpanel",
tabBarHeaderPosition:0,items:[{xtype:"reader"}]},{region:"east",flex:3,layout:"fit",xtype:"voyanttabpanel",split:{width:5},tabBarHeaderPosition:0,moreTools:["trends","collocatesgraph","corpuscollocates"],items:[{xtype:"trends"},{xtype:"collocatesgraph"},{xtype:"corpuscollocates"}]},{region:"south",flex:2,split:{width:5},layout:"border",items:[{layout:"fit",region:"center",flex:1,xtype:"voyanttabpanel",split:{width:5},tabBarHeaderPosition:0,moreTools:["summary","documents","phrases"],items:[{xtype:"summary"},
...
"/wikis.sub.uni-hamburg.de/lhn/index.php/").replace("/hup.sub.uni-hamburg.de/","/wikis.sub.uni-hamburg.de/")}));(new Corpus(a)).then(function(a){if(a.requiresPassword()&&!b.getViewport().query("panel").every(function(a){return!a.isConsumptive})){var e=a.getNoPasswordAccess(),f=[{text:"Validate"}];"NONCONSUMPTIVE"==e&&f.push({text:"Limited"});var g=Ext.create("Ext.window.Window",{title:b.localize("passwordRequiredTitle"),layout:"fit",items:{padding:10,flex:1,width:300,layout:{type:"vbox",align:"stretch"},
items:[{html:"\x3cp\x3e"+b.localize("passwordRequiredMessage")+"\x3c/p\x3e"+("NONCONSUMPTIVE"==e?"\x3cp\x3e"+b.localize("nonConsumptiveMessage")+"\x3c/p\x3e":"")+"\x3c/p\x3e"},{xtype:"textfield",fieldLabel:b.localize("password")}],bbar:{layout:{pack:"center"},items:[{text:b.localize("passwordValidateButton"),ui:"default",handler:function(){var e=g.query("textfield")[0].getValue().trim();0==e.length?b.showError({message:b.localize("noPasswordGiven")}):(g.mask(),Ext.Ajax.request({url:b.getTromboneUrl(),
params:{corpus:a.getId(),passwordForSession:e},method:"POST",success:function(e,f){g.unmask();var h=e.responseText;"ADMIN"==h||"ACCESS"==h?(g.close(),c.unmask(),b.dispatchEvent("loadedCorpus",this,a)):b.showError({message:b.localize("badPassword")})},failure:function(a,c){g.unmask();b.showError({message:b.localize("passwordValidationError")})}}))}},{text:b.localize("nonConsumptiveButton"),handler:function(){g.mask();Ext.Ajax.request({url:b.getTromboneUrl(),params:{corpus:a.getId(),passwordForSessionRemove:!0},
method:"POST",callback:function(e,f){g.unmask();g.close();c.unmask();b.dispatchEvent("loadedCorpus",b,a)}})}}]}}}).show()}else c.unmask(),b.dispatchEvent("loadedCorpus",this,a)}).fail(function(a,b){debugger;c.unmask()})},hasQueryToLoad:function(a){a||(a=Ext.Object.fromQueryString(document.location.search));return a.corpus||a.input||this.getCorpusId&&this.getCorpusId()},listeners:{loadedCorpus:function(a,b){this.setCorpus(b);this.on("unhandledEvent",function(a,d,e){a=this.getBaseUrl()+"?corpus\x3d"+
b.getId();var f=this.getModifiedApiParams()||{};delete f.view;if("termsClicked"==d)f.query=e;else if("documentsClicked"==d){var g=[];e.forEach&&e.forEach(function(a){g.push(a.getIndex())});f.docIndex=g}else if("corpusTermsClicked"==d)e.map&&(f.query=e.map(function(a){return a.getTerm()}));else if("documentTermsClicked"==d)e.map&&(f.query=e.map(function(a){return a.getTerm()}),f.docIndex=e.map(function(a){return a.getDocIndex()}));else{console&&console.warn("Unhandled event: "+d,e);return}a+="\x26"+
Ext.Object.toQueryString(f);this.openUrl(a)})}}});Ext.define("Voyant.VoyantDefaultApp",{extend:"Voyant.VoyantCorpusApp",mixins:["Voyant.util.Api"],name:"VoyantDefaultApp",constructor:function(){this.mixins["Voyant.util.Api"].constructor.apply(this,arguments);this.callParent(arguments)},statics:{i18n:{noViewErrorTitle:{en:"View Error"},noViewErrorTpl:{en:'No view was found with the name "\x3ci\x3e{view}\x3c/i\x3e". The default view will be used instead.'},noViewKnownErrorTpl:{en:"The selected view ({view}) has not been migrated from the previous version of Voyant (and probably won't be). {additional} The default view will be used instead."},
