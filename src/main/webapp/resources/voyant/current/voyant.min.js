...
f.canvas.height);c.draw(f.context);d=f.context.getImageData(c.x,c.y,c.width,c.height);for(var e=[],h=0;h<c.width;h++){var k=Math.floor(h/g.COARSENESS)*g.COARSENESS;null==e[k]&&(e[k]={});for(var m=0;m<c.height;m++){var l=Math.floor(m/g.COARSENESS)*g.COARSENESS,n;n=4*(h+m*d.width);n=[d.data[n],d.data[n+1],d.data[n+2],d.data[n+3]];"rgb("+n[0]+", "+n[1]+", "+n[2]+")"!=f.canvas.style.backgroundColor&&(e[k][l]=!0);e[k][l]&&(m=l+g.COARSENESS)}}c.mask=e}}function e(){f.clear();for(var a=p.length;a--;){var b=
...
this.tokenFreqTipTemplate=new Ext.Template(this.localize("tokenFreqTip"));this.docFreqTipTemplate=new Ext.Template(this.localize("docFreqTip"));this.on("afterrender",function(a){debugger;var b=this.down("#chart");if(null!==b){debugger;a=Math.min(a.body.getHeight(),a.body.getWidth());b.setSize(a,a).redraw();b.on("render",function(a){a.body.on("click",function(a,b){var c=a.getXY(),f=Ext.fly(b).getXY(),h=c[0]-f[0],c=c[1]-f[1],h=this.down("#chart").getItemForPoint(h,c);null!=h&&(h=Ext.create("Voyant.data.model.CorpusTerm",
h.record.data),this.getApplication().dispatchEvent("corpusTermsClicked",this,[h]))},this);a.body.on("contextmenu",function(a,b){a.preventDefault();var c=a.getXY(),f=Ext.fly(b).getXY(),h=c[0]-f[0],f=c[1]-f[1],k=this.down("#chart").getItemForPoint(h,f);null!=k&&(this.chartMenu.on("click",function(a,b){if(void 0!==b&&b.text!==this.localize("remove")){var c=Math.max(2E3,Math.round(this.getCorpus().getWordTokensCount()/100));this.setApiParams({limit:c,target:k.record.get("term")});this.loadFromApis();
this.setApiParam("target",void 0)}},this,{single:!0}),this.chartMenu.showAt(c))},this)},this)}},this);this.callParent(arguments)},buildChart:function(a){var b=this,c=this.queryById("chart");null!==c&&c.remove();a=a.getAt(0).getTokens();var d=0,e=Number.MAX_VALUE,g=[];a.forEach(function(a){var b=parseInt(a.get("@rawFreq"),10),c="part"===a.get("@category");c||(b>d&&(d=b),b<e&&(e=b));a={term:a.get("@term"),rawFreq:b,relativeFreq:a.get("@relativeFreq"),cluster:a.get("@cluster"),x:a.get("vector")[0],y:a.get("vector")[1]};
a.isDoc=c;g.push(a)});a={itemId:"chart",xtype:"cartesian",store:Ext.create("Ext.data.JsonStore",{fields:"term x y rawFreq cluster isDoc".split(" "),data:g}),interactions:["crosszoom","itemhighlight"],axes:[{type:"numeric",position:"bottom",fields:["x"]},{type:"numeric",position:"left",fields:["y"]}],series:[{type:"scatter",xField:"x",yField:"y",label:{field:"term",display:"over"},tooltip:{trackMouse:!0,style:"background: #fff",renderer:function(a,c){a.get("isDoc")?this.setHtml(b.docFreqTipTemplate.apply([a.get("term"),
