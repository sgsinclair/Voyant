...
c=this;Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:a}).then(function(b){c.set(Ext.JSON.decode(b.responseText).corpus.metadata);a.corpusTitle||a.corpusSubTitle?(c.set("title",a.corpusTitle),c.set("subTitle",a.corpusSubTitle),Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:{tool:"resource.StoredResource",storeResource:Ext.encode({title:a.corpusTitle,subTitle:a.corpusSubTitle}),resourceId:"titles-"+c.getId()}})):Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),
params:{tool:"resource.StoredResource",verifyResourceId:"titles-"+c.getId()}}).then(function(a){(a=Ext.util.JSON.decode(a.responseText))&&a.storedResource&&a.storedResource.id&&Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:{tool:"resource.StoredResource",retrieveResourceId:"titles-"+c.getId()}}).then(function(a){a=Ext.util.JSON.decode(a.responseText).storedResource.resource;a=Ext.decode(a);c.set(a)})});return c}).otherwise(function(a){Voyant.application.showResponseError(c.localize("failedCreateCorpus"),
...
loadForm:function(a){var b={tool:this.getCorpus()?"corpus.CorpusMetadata":"corpus.CorpusCreator"};this.getCorpus()&&Ext.apply(b,{corpus:this.getCorpus().getId(),addDocuments:!0});var c=this.getApiParams();delete c.view;delete c.stopList;Ext.apply(b,c);var d=this.getApplication().getViewport();d.mask(this.localize("uploadingCorpus"));a.submit({url:this.getTromboneUrl(),params:b,failure:function(a,b){d.unmask();b.result&&(this.setCorpus(void 0),this.loadCorpus({corpus:b.result.corpus?b.result.corpus.metadata.id:
b.result.stepEnabledCorpusCreator.storedId}))},scope:this})},loadCorpus:function(a){this.getCorpus()&&Ext.apply(a,{corpus:this.getCorpus().getId(),addDocuments:!0});var b=this.up("window");b&&b.isFloating()&&b.close();this.getApplication().loadCorpusFromParams(a)},showOptionsClick:function(a){void 0===a.optionsWin&&(a.optionsWin=Ext.create("Ext.window.Window",{title:a.localize("gearWinTitle"),closeAction:"hide",layout:"fit",bodyPadding:10,items:[{xtype:"form",defaultType:"textfield",maxHeight:a.getApplication().getViewport().getHeight()-
