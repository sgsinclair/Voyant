...
xtype:"toolbar",overflowHandler:"scroller",groupId:"tbar",items:[{text:"Create New Voyant Corpus",handler:function(){var a=this.down("grid").getSelectionModel().getSelection().map(function(a){return a.get("cocoa")});if(0==a.length)return this.showError({title:"Error",msg:"No items are selected in the grid (on the right)."});itemsMap={};a.forEach(function(a){itemsMap[a]=!0});for(var b=this.getEditor().getValue(),a=/<(\/)?(\w+)(\s(\w+))?>/g,c,g={};null!=(c=a.exec(b));){var k="/"==c[1],h=c[2],l=c[4],
m=c[4]?c[2]+" "+c[4]:c[2];k||m in itemsMap?(h in g&&(g[h][g[h].length-1].end=c.index),k||(h in g||(g[h]=[]),g[h].push({start:c.index+c[0].length,tag:h,attr:l}))):m in itemsMap&&h in g&&(g[h][g[h].length-1].end=c.index)}isGroupItems=this.getDockedComponent(1).down("checkbox").checked;var n=[];if(isGroupItems){var p={};for(h in g)g[h].forEach(function(a,c){var d=b.substring(a.start,a.end).replace(/<\/?\w+(\s+\w)*>/g,"").trim();if(d){var f=a.tag+(a.attr?" "+a.attr:"");p[f]=f in p?p[f]+d:d}});for(m in p)n.push({title:m,
