...
if(null!=b){this.getNetwork().selectNodes([b]);var b=this.getNodeDataSet().get(b),e=this.getContextMenu();e.queryById("label").setText(b.label);e.queryById("fixed").setChecked(b.fixed);e.showAt(a.event.pageX,a.event.pageY)}}.bind(this))}},forceUpdate:function(){var a=this.getNodeDataSet().map(function(a){return{id:a.id}});this.getNodeDataSet().update(a);a=this.getEdgeDataSet().map(function(a){return{id:a.id}});this.getEdgeDataSet().update(a)},isOffCanvas:function(a){return 0>a.x||0>a.y||a.x>this.getGraphWidth()||
