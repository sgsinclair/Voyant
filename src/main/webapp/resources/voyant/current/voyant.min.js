...
changecomplete:function(b,c){a.setApiParam("expand",c);var d=a.getView(),e=a.plugins[0].recordsExpanded,g=d.getStore();for(id in e)if(e[id]){var f=g.getByInternalId(id),h=d.getRow(f);d.fireEvent("expandbody",h,f,h.parentNode.childNodes[1],{force:!0})}}}}]}],columns:[{text:"#",width:30,dataIndex:"docIndex",sortable:!0,renderer:function(a){return a+1}},{text:this.localize("left"),tooltip:this.localize("leftTip"),align:"right",dataIndex:"left",sortable:!0,flex:1},{text:this.localize("term"),tooltip:this.localize("termTip"),
...
(e.push(a.term),d[a.term]=!0);g[a.docIndex]||(f.push(a.docIndex),g[a.docIndex]=!0)});this.setApiParams({docId:void 0,docIndex:f,query:e});this.isVisible()&&this.getStore().loadPage(1,{params:this.getApiParams()})},scope:this},afterrender:function(a){a.getView().on("expandbody",function(c,d,e,g){if(""==e.innerText||g&&g.force){c=Ext.create("Voyant.data.store.Contexts",{stripTags:"all",corpus:a.getStore().getCorpus()});var f=d.getData();c.load({params:{query:f.query,docIndex:f.docIndex,position:f.position,
limit:1,context:a.getApiParam("expand")},callback:function(a,b,c){c&&1==a.length&&(f=a[0].getData(),b.expandRow.firstElementChild.firstElementChild.innerHTML=f.left+" \x3cspan class\x3d'word keyword'\x3e"+f.middle+"\x3c/span\x3e "+f.right)},expandRow:e})}})}}});a.on("loadedCorpus",function(b,c){this.getStore().setCorpus(c);Ext.create("Voyant.data.store.CorpusTerms",{corpus:c}).load({callback:function(a,b,c){c&&0<a.length&&(this.setApiParam("query",a[0].getTerm()),this.getStore().load({params:this.getApiParams()}))},
scope:a,params:{limit:1,stopList:this.getApiParam("stopList")}})});a.on("query",function(a,c){this.setApiParam("query",c);this.getStore().load({params:this.getApiParams()})},a);a.on("documentTermsClicked",function(a,c){var d=[];c.forEach(function(a){d.push({term:a.getTerm(),docIndex:a.getDocIndex()})});this.fireEvent("documentIndexTermsClicked",this,d)});a.on("termsClicked",function(a,c){var d=[];Ext.isString(c)&&(c=[c]);c.forEach(function(a){void 0!==a.docIndex&&d.push({term:a.term,docIndex:a.docIndex})});
