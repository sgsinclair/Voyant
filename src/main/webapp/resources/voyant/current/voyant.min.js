...
requires:["Voyant.util.ResponseError","Voyant.data.store.CorpusTerms","Voyant.data.store.Documents"],extend:"Ext.data.Model",config:{documentsStore:void 0},statics:{i18n:{}},fields:[{name:"documentsCount",type:"int"},{name:"lexicalTokensCount",type:"int"},{name:"lexicalTypesCount",type:"int"},{name:"createdTime",type:"int"},{name:"createdDate",type:"date",dateFormat:"c"},{name:"title",type:"string"},{name:"subTitle",type:"string"}],constructor:function(a,b){a=a||{};b=b||{};this.callParent([]);var c=
Voyant.application.getDeferred(this);if(Ext.isString(a))0==/\s/.test(a)&&-1==a.indexOf(":")?Ext.apply(b,{corpus:a}):Ext.apply(b,{input:a});else if(Ext.isArray(a))Ext.apply(b,{input:a});else if(Ext.isObject(a))Ext.apply(b,a);else return Voyant.application.showError(this.localize("badDataTypeCorpus")+": ("+typeof a+") "+a),Ext.defer(function(){c.reject(this.localize("badDataTypeCorpus")+": ("+typeof a+") "+a)},50,this),c.promise;if(Ext.isObject(b)){if(!b.corpus&&!b.input)return Voyant.application.showError(this.localize("noCorpusOrInput")+
": "+b),Ext.defer(function(){c.reject(this.localize("noCorpusOrInput")+": "+b)},50,this),c.promise;Ext.apply(b,{tool:"corpus.CorpusMetadata"});var d=this;Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:b}).then(function(a){d.set(Ext.JSON.decode(a.responseText).corpus.metadata);var c="titles-"+d.getId();b.title||b.subTitle?(d.set("title",b.title),d.set("subTitle",b.subTitle),Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:{tool:"resource.StoredResource",storeResource:Ext.encode({title:b.title,
subTitle:b.subTitle}),resourceId:c}})):Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:{tool:"resource.StoredResource",verifyResourceId:c}}).then(function(a){(a=Ext.util.JSON.decode(a.responseText))&&a.storedResource&&a.storedResource.id&&""!=a.storedResource.id&&Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:{tool:"resource.StoredResource",retrieveResourceId:c}}).then(function(a){a=Ext.util.JSON.decode(a.responseText).storedResource.resource;a=Ext.decode(a);d.set(a)})});
return d}).otherwise(function(a){Voyant.application.showResponseError(d.localize("failedCreateCorpus"),a)}).then(function(a){!("docsLimit"in b)||!1!==b.docsLimit&&0<b.docsLimit?d.getDocuments().load({params:{limit:"docsLimit"in b?b.docsLimit:d.getDocumentsCount()},callback:function(b,g,h){h?(d.setDocumentsStore(this),c.resolve(a)):c.reject(g)}}):c.resolve(a)})}else Voyant.application.showError(this.localize("badDataTypeCorpus")+": ("+typeof b+") "+b),Ext.defer(function(){c.reject(this.localize("badDataTypeCorpus")+
...
arguments);var b=Voyant.application.getDeferred(this);Ext.applyIf(a,{template:"docTokens2lemmas",withPosLemmas:!0});this.getWords(a).then(function(a){a=a.split(" ").map(function(a){return a.substring(0,a.indexOf("/"))});b.resolve(a)});return b.promise},getString:function(a){var b=this.getDocumentsCount(),c=this.localize("thisCorpus");if(0==b)c+=" "+this.localize("isEmpty")+".";else{var c=c+" ",c=1<b?c+(new Ext.XTemplate(this.localize("hasNdocuments"))).apply({count:Ext.util.Format.number(b,"0,000")}):
...
{store:{data:l,fields:e,proxy:{type:"memory",reader:{record:"tbody tr",type:"xml"}}},columns:f,width:c,height:m});this.callParent([b]);!1!==b.remove&&l.parentNode.removeChild(l)},doDestroy:function(){this.table.remove();this.tabl=null;this.callParent()}});Ext.define("Voyant.widget.CategoriesBuilder",{extend:"Ext.window.Window",requires:["Voyant.widget.FontFamilyOption"],mixins:["Voyant.util.Localization","Voyant.util.Api","Voyant.util.CategoriesManager"],alias:"widget.categoriesbuilder",statics:{i18n:{title:"Categories Builder",terms:"Terms",term:"Term",rawFreq:"Count",relativeFreq:"Relative",categories:"Categories",addCategory:"Add Category",removeCategory:"Remove Category",removeTerms:"Remove Selected Terms",categoryName:"Category Name",add:"Add",
cancel:"Cancel",exists:"Category already exists",confirmRemove:"Are you sure you want to remove the category?",done:"Done",features:"Features",category:"Category",color:"Color",font:"Font"},api:{stopList:"auto",query:void 0},features:{color:{xtype:"colorfield",format:"#hex6"},font:{xtype:"combobox",queryMode:"local",displayField:"name",valueField:"value",store:{fields:["name","value"],data:Voyant.widget.FontFamilyOption.fonts}}}},config:{corpus:void 0,categoriesManager:void 0,categoryWin:void 0,parentPanel:void 0},
modal:!0,height:250,width:500,constructor:function(a){a=a||{};a.panel&&(this.panel=a.panel);var b=a.categoriesManager?a.categoriesManager:Ext.create("Voyant.util.CategoriesManager");this.setCategoriesManager(b);this.mixins["Voyant.util.Api"].constructor.apply(this,arguments);this.callParent(arguments)},initComponent:function(){Ext.apply(this,{title:this.localize("title"),layout:"card",items:[{layout:"border",items:[{title:this.localize("terms"),split:!0,width:250,region:"west",layout:"fit",items:{itemId:"terms",
xtype:"grid",store:Ext.create("Voyant.data.store.CorpusTermsBuffered",{parentPanel:this}),viewConfig:{plugins:{ptype:"gridviewdragdrop",ddGroup:"terms",copy:!0,enableDrop:!1,dragZone:{getDragText:function(){var a="";this.dragData.records.forEach(function(b){a+=b.get("term")+", "});return a.substr(0,a.length-2)}}}},selModel:{mode:"MULTI"},columns:[{text:this.localize("term"),dataIndex:"term",flex:1,sortable:!0},{text:this.localize("rawFreq"),dataIndex:"rawFreq",width:"autoSize",sortable:!0},{text:this.localize("relativeFreq"),
dataIndex:"relativeFreq",renderer:function(a){return Ext.util.Format.number(1E6*a,"0,000")},width:"autoSize",hidden:!0,sortable:!0}],dockedItems:[{dock:"bottom",xtype:"toolbar",overflowHandler:"scroller",items:[{xtype:"querysearchfield"}]}],listeners:{query:function(a,b){this.setApiParam("query",b);var c=this.queryById("terms").getStore();c.removeAll();c.load()},scope:this}}},{title:this.localize("categories"),itemId:"categories",region:"center",xtype:"panel",layout:{type:"hbox",align:"stretch"},
scrollable:"x",dockedItems:[{dock:"bottom",xtype:"toolbar",overflowHandler:"scroller",items:[{text:this.localize("addCategory"),handler:function(){this.getCategoryWin().show()},scope:this},{text:this.localize("removeTerms"),handler:function(){this.queryById("categories").query("grid").forEach(function(a){a.getStore().remove(a.getSelection())},this)},scope:this}]}],items:[]}]},{layout:"fit",itemId:"features",title:this.localize("features"),xtype:"grid",scrollable:"y",store:Ext.create("Ext.data.JsonStore",
{fields:["category"]}),columns:[{text:this.localize("category"),dataIndex:"category",sortable:!1,hideable:!1,flex:1}]}],buttons:[{text:this.localize("features"),handler:function(a){var b=a.up("panel").getLayout();b.getNext()?(b.next(),a.setText(this.localize("categories"))):(b.prev(),a.setText(this.localize("features")))},scope:this},{text:this.localize("done"),handler:function(a){a.up("window").close()}}]});this.setCategoryWin(Ext.create("Ext.window.Window",{title:this.localize("addCategory"),modal:!0,
closeAction:"hide",layout:"fit",items:{xtype:"form",width:300,defaults:{labelAlign:"right"},items:[{xtype:"textfield",fieldLabel:this.localize("categoryName"),name:"categoryName",allowBlank:!1,validator:function(a){return void 0===this.getCategoriesManager().getCategoryTerms(a)?!0:this.localize("exists")}.bind(this),enableKeyEvents:!0,listeners:{keypress:function(a,b){b.getKeyName()},scope:this}}],buttons:[{text:this.localize("cancel"),handler:function(a){a.up("window").close()}},{text:this.localize("add"),
handler:function(a){var b=a.up("form");b.isValid()&&(b=b.getValues().categoryName,this.addCategory(b),a.up("window").close())},scope:this}]},listeners:{show:function(a){a=a.down("form").getForm();a.reset();a.clearInvalid()}}}));this.on("afterrender",function(a){a.on("loadedCorpus",function(a,c){this.setCorpus(c);this.queryById("terms").getStore().load()},a);this.panel?(this.panel.on("loadedCorpus",function(b,c){a.fireEvent("loadedCorpus",b,c)},a),this.panel.getCorpus&&this.panel.getCorpus()?a.fireEvent("loadedCorpus",
a,this.panel.getCorpus()):this.panel.getStore&&this.panel.getStore()&&this.panel.getStore().getCorpus&&this.panel.getStore().getCorpus()&&a.fireEvent("loadedCorpus",a,this.panel.getStore().getCorpus())):window.console&&console.warn("couldn't find parent panel for CategoriesBuilder");this.buildCategories();this.addFeature("font");this.addFeature("color")},this);this.on("beforeclose",function(a){a=this.panel.getApplication();var b=this.getCategoriesManager(),c;for(c in b.getCategories()){var d=b.getCategoryFeature(c,
"color");if(void 0!==d)for(var d=a.hexToRgb(d),e=b.getCategoryTerms(c),f=0;f<e.length;f++)a.colorTermAssociations.replace(e[f],d)}},this);this.callParent(arguments)},addCategory:function(a){this.queryById("features").getStore().loadRawData([{category:a}],!0);var b=this.queryById("categories"),c,d=[],e=this.getCategoriesManager();c=e.getCategoryTerms(a);if(void 0===c)e.addCategory(a),e.addFeature("color"),c=b.query("grid").length,c=this.panel.getApplication().getColor(c,!0),e.setCategoryFeature(a,
"color",c);else{for(var f=0;f<c.length;f++)d.push({term:c[f]});c=e.getCategoryFeature(a,"color")}return b.add({xtype:"grid",category:a,title:a,frame:!0,width:150,margin:"10 0 10 10",layout:"fit",tools:[{type:"close",tooltip:this.localize("removeCategory"),callback:function(b){Ext.Msg.confirm(this.localize("removeCategory"),this.localize("confirmRemove"),function(b){"yes"===b&&this.removeCategory(a)},this)},scope:this}],store:Ext.create("Ext.data.JsonStore",{data:d,fields:["term"]}),viewConfig:{plugins:{ptype:"gridviewdragdrop",
ddGroup:"terms",dragZone:{getDragText:function(){var a="";this.dragData.records.forEach(function(b){a+=b.get("term")+", "});return a.substr(0,a.length-2)}}}},selModel:{mode:"MULTI"},columns:[{dataIndex:"term",flex:1,sortable:!0}],listeners:{drop:function(b,c){c.view.getSelectionModel().deselectAll();this.getSelectionModel().deselectAll();for(var d=this.up("categoriesbuilder").getCategoriesManager(),e=[],f=0;f<c.records.length;f++)e.push(c.records[f].get("term"));d.addTerms(a,e);f=c.view.up("grid");
f.category&&d.removeTerms(f.category,e)}}})},removeCategory:function(a){var b=this.queryById("categories"),c=b.queryBy(function(b){return b.category&&b.category==a?!0:!1});b.remove(c[0]);b=this.queryById("features").getStore();b.removeAt(b.findExact("category",a));this.getCategoriesManager().removeCategory(a)},addFeature:function(a){this.getCategoriesManager().addFeature(a);this.buildFeatures()},buildFeatures:function(){var a=["category"],b=[{sortable:!1,text:this.localize("category"),dataIndex:"category",
flex:1}],c=[],d=this.getCategoriesManager(),e;for(e in d.getCategories())c.push({category:e});var f=d.getFeatures(),g=Ext.ClassManager.getClass(this).features,h;for(h in f)for(e in a.push(h),f=Ext.apply({feature:h,listeners:{change:function(a,b){if(a.rendered){var c=a.up("gridview").indexOf(a.el.up("table")),c=a.up("grid").getStore().getAt(c).get("category");this.getCategoriesManager().setCategoryFeature(c,a.feature,b)}},scope:this}},g[h]),b.push({sortable:!1,hideable:!1,text:this.localize(h),dataIndex:h,
flex:.5,xtype:"widgetcolumn",widget:f}),d.getCategories())for(var f=d.getCategoryFeature(e,h),k=0;k<c.length;k++)if(c[k].category==e){c[k][h]=f;break}a=Ext.create("Ext.data.JsonStore",{fields:a,data:c});this.queryById("features").reconfigure(a,b)},buildCategories:function(){var a=this.getCategoriesManager().getCategories(),b;for(b in a)this.addCategory(b)}});Ext.define("Voyant.widget.VoyantNetworkGraph",{extend:"Ext.panel.Panel",mixins:["Voyant.util.Localization","Voyant.util.Api","Voyant.notebook.util.Embed"],embeddable:["Voyant.widget.VoyantNetworkGraph"],alias:"widget.voyantnetworkgraph",statics:{i18n:{},api:{jsonData:void 0,docId:void 0,docIndex:void 0,json:void 0}},config:{json:void 0},constructor:function(a){a=a||{};this.mixins["Voyant.util.Api"].constructor.apply(this,arguments);this.callParent(arguments);this.on("boxready",function(a,c){this.buildGraph()},
...
scope:this}]},bodyPadding:5}).show()}});Ext.define("Voyant.panel.Cirrus",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.cirrus",statics:{i18n:{},api:{stopList:"auto",whiteList:void 0,limit:500,visible:50,terms:void 0,docId:void 0,docIndex:void 0,inlineData:void 0,fontFamily:'"Palatino Linotype", "Book Antiqua", Palatino, serif',cirrusForceFlash:!1,background:"0xffffff",fade:!0,smoothness:2,diagonals:"none"},glyph:"xf06e@FontAwesome"},config:{mode:void 0,options:[{xtype:"stoplistoption"},{xtype:"listeditor",name:"whiteList"},
{xtype:"numberfield",name:"label",fieldLabel:"Max words",labelAlign:"right",value:500,minValue:50,step:50,listeners:{afterrender:function(a){var b=a.up("window");b&&b.panel&&a.setFieldLabel(b.panel.localize("maxTerms"))}}},{xtype:"fontfamilyoption"},{xtype:"colorpaletteoption"}],records:void 0,terms:void 0,cirrusId:void 0,visLayout:void 0,vis:void 0,tip:void 0,sizeAdjustment:100,minFontSize:12,largestWordSize:0,smallestWordSize:1E6},MODE_CORPUS:"corpus",MODE_DOCUMENT:"mode_document",layout:"fit",
constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments);this.setCirrusId(Ext.id(null,"cirrus_"))},initComponent:function(a){Ext.apply(this,{title:this.localize("title"),dockedItems:[{dock:"bottom",xtype:"toolbar",overflowHandler:"scroller",items:[{xtype:"corpusdocumentselector",singleSelect:!0},{fieldLabel:this.localize("visibleTerms"),labelWidth:40,width:120,xtype:"slider",increment:25,minValue:25,maxValue:500,listeners:{afterrender:function(a){a.maxValue=
this.getApiParam("limit");a.increment=parseInt(a.maxValue/50);a.setValue(this.getApiParam("visible"))},changecomplete:function(a,c){this.setApiParams({visible:c});this.loadFromTermsRecords()},scope:this}}]}]});this.callParent(arguments)},listeners:{afterrender:function(){var a=this.getApiParam("inlineData");if(void 0!==a&&(a=Ext.decode(a,!0),null!==a)){this.setApiParam("inlineData",a);var b;(new Corpus).then(function(a){b=a;a=this.getApplication();a.dispatchEvent("loadedCorpus",a,b)},null,null,this)}},
resize:function(a,b,c){this.getVisLayout()&&this.getCorpus()&&(this.setAdjustedSizes(),a=this.getLayout().getRenderTarget(),b=a.getWidth(),c=a.getHeight(),a.down("svg").set({width:b,height:c}),this.getTerms()&&this.getVisLayout().size([b,c]).stop().words(this.getTerms()).start())},loadedCorpus:function(a,b){this.initVisLayout(!0);this.loadFromCorpus(b)},corpusSelected:function(a,b){this.loadFromCorpus(b)},documentSelected:function(a,b){if(b){var c=this.getCorpus();b=c.getDocument(b);this.setApiParam("docId",
b.getId());c=b.getDocumentTerms({autoload:!1,corpus:c,pageSize:this.getApiParam("maxVisible"),parentPanel:this});this.loadFromDocumentTerms(c)}},ensureCorpusView:function(a,b){this.getMode()!=this.MODE_CORPUS&&this.loadFromCorpus(b)},boxready:function(){this.initVisLayout()}},loadFromCorpus:function(a){var b=this.getApiParam("inlineData");if(void 0===b)this.setApiParams({docId:void 0,docIndex:void 0}),this.loadFromCorpusTerms(a.getCorpusTerms({autoload:!1,pageSize:this.getApiParam("maxVisible"),parentPanel:this}));
else if(void 0!==b){a=[];for(var c=0;c<b.length;c++){var d=Ext.create("Voyant.data.model.CorpusTerm",b[c]);a.push(d)}this.setRecords(a);this.setMode(this.MODE_CORPUS);this.loadFromTermsRecords()}},loadFromDocumentTerms:function(a){a.load({callback:function(a,c,d){this.setMode(this.MODE_DOCUMENT);this.setRecords(c.getRecords());this.loadFromTermsRecords()},scope:this,params:this.getApiParams()})},loadFromCorpusTerms:function(a){a.load({callback:function(a,c,d){this.setMode(this.MODE_CORPUS);this.setRecords(c.getRecords());
this.loadFromTermsRecords()},scope:this,params:this.getApiParams()})},loadFromTermsRecords:function(){var a=this.getRecords(),b=this.getApiParam("visible");b>a.length&&(b=a.length);for(var c=[],d=0;d<b;d++)c.push({text:a[d].get("term"),rawFreq:a[d].get("rawFreq")});this.setTerms(c);this.buildFromTerms()},initVisLayout:function(a){if(a||void 0==this.getVisLayout())if(a=this.getApiParam("cirrusForceFlash"),"true"==a||!0===a){this.setApiParam("cirrusForceFlash",!0);a=this.getCirrusId();for(var b={id:a},
...
function(a){return"#"+a.term.replace(/\W/g,"_")});e=e.append("text").attr("clip-path",function(a){return"url(#clip-"+a.term.replace(/\W/g,"_")+")"}).style("text-anchor","middle").style("cursor","default");e.append("tspan").attr("class","term").style("font-size",function(a){return g(a.rawFreq)}).attr("x",0).attr("y",function(a){return g(a.rawFreq)/4}).text(function(a){return a.term});e.append("tspan").attr("class","value").style("font-size",function(a){return.75*g(a.rawFreq)}).attr("x",0).attr("y",
function(a){return g(a.rawFreq)+1});a.exit().remove()},initVisLayout:function(){var a=this.getLayout().getRenderTarget();a.update("");var b=a.getWidth(),c=a.getHeight(),d=this;this.setVisLayout(d3.layout.pack().sort(function(a,b){return a.rawFreq<b.rawFreq?1:a.rawFreq>b.rawFreq?-1:0}).value(function(a){return Math.pow(a.rawFreq,1/d.getScalingFactor())}).size([b,c]).padding(1.5));a=d3.select(a.dom).append("svg").attr("id",this.getVisId()).attr("width",b).attr("height",c);this.setVis(a.append("g"));
this.setVisInfo(a.append("text").attr("x",10).attr("y",10));void 0===this.getTip()&&this.setTip(Ext.create("Ext.tip.Tip",{}))}});Ext.define("Voyant.panel.TermsRadio",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.termsradio",config:{options:[{xtype:"stoplistoption"}],speed:50,termsRadio:void 0},statics:{i18n:{},api:{withDistributions:!0,bins:5,visibleBins:5,docIdType:null,limit:50,mode:null,position:0,selectedWords:[],stopList:"auto",query:null,yAxisScale:"log",speed:50,slider:void 0},glyph:"xf201@FontAwesome"},constructor:function(a){var b=function(a,b,e,f,g){this.setApiParams({mode:a});this.getTermsRadio().loadRecords(e);
...
"\x3e").replace(/<.+?>/g,"")),e+f<b&&(d+=c[h],e+=f);a.update(d);Ext.tip.QuickTipManager.register({target:a.getTargetEl().dom.querySelector(".privacy"),text:this.localize("privacyMsg")})},beforedestroy:function(a){Ext.tip.QuickTipManager.unregister(a.getTargetEl().dom.querySelector(".privacy"))}}});Ext.define("Voyant.panel.VoyantHeader",{extend:"Ext.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.voyantheader",statics:{i18n:{categoriesBuilder:"Categories Builder"}},constructor:function(a){Ext.apply(this,{id:"voyantheader",title:"",layout:"fit",html:'\x3cdiv id\x3d"logo-container"\x3e\x3c/div\x3e',collapseMode:void 0,collapsible:!0,animCollapse:!1,titleCollapse:!1,floatable:!1,header:!0,hideCollapseTool:!0,listeners:{collapse:this.onCollapse},titleAlign:"center"});this.callParent(arguments);
Ext.applyIf(a,{moreTools:["corpusset","scatterplot","termsradio"],includeTools:{save:!0,plus:!0,help:!0,language:this.getLanguageToolMenu(),home:{type:"home",tooltip:this.localize("homeTip"),xtype:"toolmenu",glyph:"xf015@FontAwesome",handler:function(a){var c=this.up("panel");Ext.Msg.confirm(c.localize("home"),c.localize("homeConfirm"),function(a){"yes"==a&&(document.location.href=c.getBaseUrl())},this)}},categories:{type:"categories",tooltip:this.localize("categoriesBuilder"),xtype:"toolmenu",glyph:"xf02c@FontAwesome",
handler:function(a){Ext.create("Voyant.widget.CategoriesBuilder",{panel:this,categoriesManager:this.getApplication().getCategoriesManager(),height:.5*this.getApplication().getViewport().getHeight(),width:.75*this.getApplication().getViewport().getWidth()}).show()},scope:this}}});this.mixins["Voyant.panel.Panel"].constructor.call(this,a)},onCollapse:function(a){Ext.defer(function(){this.setTitle("\x3cimg src\x3d'"+this.getBaseUrl()+"resources/images/voyant-logo-tiny.png' style\x3d'vertical-align: middle' alt\x3d'Voyant Tools' /\x3e "+
...
void 0,void 0,!0,JSON&&JSON.stringify?JSON.stringify(a,void 0,4):Ext.encode(a))},wrap:function(a){if((a=a||"")&&80<a.length){for(var b=[],c=0,d=Math.ceil(a.length/80);c<d;c++)b.push(a.substr(80*c,80));a=b}return a}});Ext.define("Voyant.VoyantApp",{extend:"Ext.app.Application",mixins:["Voyant.util.Deferrable","Voyant.util.Localization","Voyant.util.Api"],requires:["Voyant.util.ResponseError","Voyant.util.CategoriesManager"],name:"VoyantApp",statics:{i18n:{},api:{palette:"default",lang:void 0,debug:void 0}},config:{baseUrl:void 0,tromboneUrl:void 0,categoriesManager:void 0},constructor:function(a){this.setBaseUrl(this.config.baseUrl);this.setTromboneUrl(this.config.baseUrl+"trombone");this.setCategoriesManager(new Ext.create("Voyant.util.CategoriesManager"));
Voyant.application=this;this.mixins["Voyant.util.Api"].constructor.apply(this,arguments);this.callParent(arguments);var b=d3.scale.category10().range().map(function(a){return this.hexToRgb(a)},this),c=d3.scale.category20().range().map(function(a){return this.hexToRgb(a)},this),d=d3.scale.category20b().range().map(function(a){return this.hexToRgb(a)},this),e=d3.scale.category20c().range().map(function(a){return this.hexToRgb(a)},this);this.addColorPalette("d3_cat10",b);this.addColorPalette("d3_cat20a",
c);this.addColorPalette("d3_cat20b",d);this.addColorPalette("d3_cat20c",e);b=Ext.create("Ext.chart.theme.Base").getColors().map(function(a){return this.hexToRgb(a)},this);this.addColorPalette("extjs",b)},getRelativeUrl:function(){for(var a="",b=0,c=window.location.pathname.substring(this.getBaseUrl().length).split("/").length-1;b<c;b++)a+="../";return a},getTools:function(){return[{type:"maximize"},{type:"help"}]},launch:function(){Ext.tip.QuickTipManager.init();Ext.apply(Ext.tip.QuickTipManager.getQuickTip(),
{showDelay:50});this.callParent(arguments)},tromboneCall:function(a){a=a?a:{};Ext.applyIf(a,{url:this.getTromboneUrl()});a.success||a.failure||a.callback||Ext.applyIf(a,{url:this.getTromboneUrl(),scope:this,callback:function(b,c,d){this.dispatchEvent(a.tool+"Loaded",b,c,d)}});Ext.Ajax.request(a)},getViewport:function(){return Ext.ComponentQuery.query("viewport")[0]},dispatchEvent:function(a,b){var c=this.getViewport().query("panel,chart"),d=!1;this.hasListener&&this.hasListener(a)&&(this.fireEvent.apply(this,
arguments),d=!0);for(var e=0;e<c.length;e++)!c[e].hasListener||!c[e].hasListener(a)||b&&b.getId&&c[e].getId&&b.getId()==c[e].getId()||(d=!0,c[e].fireEvent.apply(c[e],arguments));if(!d){c=["unhandledEvent",b,a];for(e=2;e<arguments.length;e++)c.push(arguments[e]);this.fireEvent.apply(this,c)}},showResponseError:function(a,b){this.showError(Ext.create("Voyant.util.ResponseError",{msg:Ext.isString(a)?a:a.msg,response:b}))},showError:function(a,b){if(a instanceof Voyant.util.ResponseError)a={message:a.getMsg()+
"\x3cp class\x3d'error'\x3e\n"+a.getError()+" \u2026 \x3ca href\x3d'#' onclick\x3d\"window.open('').document.write(unescape('\x3cpre\x3e"+escape(a.getDetails())+"\x3c/pre\x3e')); return false;\"\x3emore\x3c/a\x3e\x3c/p\x3e"};else if(Ext.isString(a))a={message:a};else if(Ext.isObject(a)&&a.responseText)return this.showResponseError(a.statusText,a);Ext.applyIf(a,{title:this.localize("error"),buttons:Ext.Msg.OK,icon:Ext.MessageBox.ERROR,autoScroll:!0});Ext.Msg.show(a)},getToolConfigFromToolXtype:function(a){cls=
Ext.ClassManager.getByAlias("widget."+a);return{xtype:a,title:this._localizeClass(cls,"title"),tooltip:{text:this._localizeClass(cls,"helpTip")},glyph:cls&&cls.glyph?cls.glyph:"xf12e@FontAwesome"}},palettes:{"default":[[0,0,255],[51,197,51],[255,0,255],[121,51,255],[28,255,255],[255,174,0],[30,177,255],[182,242,58],[255,0,164],[51,102,153],[34,111,52],[155,20,104],[109,43,157],[128,130,33],[111,76,10],[119,115,165],[61,177,169],[202,135,115],[194,169,204],[181,212,228],[182,197,174],[255,197,197],
[228,200,124],[197,179,159]]},rgbToHex:function(a){return"#"+(16777216+(a[0]<<16)+(a[1]<<8)+a[2]).toString(16).slice(1)},hexToRgb:function(a){a=a.replace(/^#?([a-f\d])([a-f\d])([a-f\d])$/i,function(a,c,d,e){return c+c+d+d+e+e});return(a=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(a))?[parseInt(a[1],16),parseInt(a[2],16),parseInt(a[3],16)]:null},addColorPalette:function(a,b){this.palettes[a]=b},getColorPalette:function(a,b){var c=a||this.getApiParam("palette")||"default",c=this.palettes[c];void 0===
c&&(c=[]);if(b){for(var d=[],e=0;e<c.length;e++)d.push(this.rgbToHex(c[e]));return d}return c},getColor:function(a,b){var c=this.getApiParam("palette")||"default",c=this.palettes[c];a>=c.length&&(a%=c.length);return b?this.rgbToHex(c[a]):c[a]},colorTermAssociations:new Ext.util.MixedCollection,getColorForTerm:function(a,b){-1!=a.indexOf(":")&&(a=a.split(":")[1]);var c=this.colorTermAssociations.get(a);if(null==c){var c=this.getApiParam("palette")||"default",c=this.palettes[c],d=this.colorTermAssociations.getCount()%
...
params:{tool:"resource.StoredResource",retrieveResourceId:b,corpus:a},success:function(a,d){var e=Ext.util.JSON.decode(a.responseText).storedResource.resource,e=Ext.decode(e);this.addColorPalette(b,e)},failure:function(a){this.setApiParam("palette",void 0)},scope:this})},listeners:{loadedCorpus:function(a,b){this.setCorpus(b);this.colorTermAssociations.clear();this.on("unhandledEvent",function(a,d,e){a=this.getBaseUrl()+"?corpus\x3d"+b.getAliasOrId();var f=this.getModifiedApiParams()||{};delete f.view;
if("termsClicked"==d)f.query=e;else if("documentsClicked"==d){var g=[];e.forEach&&e.forEach(function(a){g.push(a.getIndex())});f.docIndex=g}else if("corpusTermsClicked"==d)e.map&&(f.query=e.map(function(a){return a.getTerm()}));else if("documentTermsClicked"==d)e.map&&(f.query=e.map(function(a){return a.getTerm()}),f.docIndex=e.map(function(a){return a.getDocIndex()}));else{console&&console.warn("Unhandled event: "+d,e);return}f.toolFlow&&(d=Ext.Array.from(f.toolFlow.split(",")),f.view=d.shift(),
0<d.length?f.toolFlow=d.join(","):delete f.toolFlow);a+="\x26"+Ext.Object.toQueryString(f);this.openUrl(a)})}}});Ext.define("Voyant.panel.DocumentClusters",{extend:"Ext.panel.Panel",alias:"widget.documentclusters",title:"Document Clusters"});Ext.define("Voyant.panel.RezoViz",{extend:"Ext.panel.Panel",alias:"widget.rezoviz",title:"RezoViz"});
