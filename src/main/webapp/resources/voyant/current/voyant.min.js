...
"\x26"+Ext.Object.toQueryString(a)},helpToolClick:function(a){var b=a.localize("help",{"default":!1})||a.localize("helpTip");b==a._localizeClass(Ext.ClassManager.get("Voyant.util.Toolable"),"helpTip")?a.openUrl("http://docs.voyant-tools.org/"):Ext.Msg.alert(a.localize("title"),b)},replacePanel:function(a){var b=this.getApplication().getCorpus();this.getInitialConfig();var c;this.isXType("voyantheader")&&this.getApplication().getViewComponent?(c=this.getApplication().getViewComponent(),c.removeAll(!0),
a=c.add({xtype:a}),b&&this.getApplication().dispatchEvent("loadedCorpus",c,b)):(c=this.isXType("voyantheader")&&this.getApplication().getViewComponent?this.getApplication().getViewComponent():this.up("component"),c.remove(this,!0),a=c.add({xtype:a}),b&&a.fireEvent("loadedCorpus",a,b))}});
...
fr:{en:"French"},hu:{en:"Hungarian"},it:{en:"Italian"},no:{en:"Norwegian"},se:{en:"Swedish"},mu:{en:"Multilingual"},ok:{en:"Save"},cancel:{en:"Cancel"},editStopListTitle:{en:"Edit Stoplist"},editStopListMessage:{en:"This is the stoplist, one term per line."}}},initComponent:function(a){var b=this.up("window").panel.getApiParam("stopList"),c=[{name:this.localize("en"),value:"stop.en.taporware.txt"},{name:this.localize("de"),value:"stop.de.german.txt"},{name:this.localize("es"),value:"stop.es.spanish.txt"},
{name:this.localize("fr"),value:"stop.fr.veronis.txt"},{name:this.localize("hu"),value:"stop.hu.hungarian.txt"},{name:this.localize("it"),value:"stop.it.italian.txt"},{name:this.localize("no"),value:"stop.no.norwegian.txt"},{name:this.localize("se"),value:"stop.se.swedish-long.txt"},{name:this.localize("mu"),value:"stop.mu-multi.txt"}];c.sort(function(a,b){return a.name<b.name?-1:1});c.splice(0,0,{name:this.localize("auto"),value:"auto"},{name:this.localize("new"),value:"new"});Ext.apply(this,{items:[{xtype:"combo",
queryMode:"local",value:b,triggerAction:"all",editable:!0,fieldLabel:this.localize("label"),labelAlign:"right",name:"stopList",displayField:"name",valueField:"value",store:{fields:["name","value"],data:c}},{xtype:"button",text:this.localize("editList"),handler:this.editList,scope:this}]});this.callParent(arguments)},editList:function(){var a=this.up("window").panel,b=this.down("combo").getValue(),c=a.getApplication&&a.getApplication().getCorpus?a.getApplication().getCorpus().getId():void 0;"auto"!=
b||c?Ext.Ajax.request({url:a.getTromboneUrl(),params:{tool:"resource.KeywordsManager",stopList:b,corpus:c},success:function(b){b=Ext.util.JSON.decode(b.responseText).keywords.keywords.sort().join("\n");Ext.Msg.show({title:this.localize("editStopListTitle"),message:this.localize("editStopListMessage"),buttons:Ext.Msg.OKCANCEL,buttonText:{ok:this.localize("ok"),cancel:this.localize("cancel")},icon:Ext.Msg.INFO,prompt:!0,multiline:!0,value:b,original:b,fn:function(b,d,f){"ok"==b&&f.original!=d&&(b=this.down("combo"),
0==Ext.String.trim(d).length?b.setValue("empty"):Ext.Ajax.request({url:a.getTromboneUrl(),params:{tool:"resource.StoredResource",storeResource:d,corpus:c},combo:b,success:function(a,b){var c=Ext.util.JSON.decode(a.responseText),d=b.combo.getStore(),c="keywords-"+c.storedResource.id;d.add({name:"Added "+Ext.Date.format(new Date,"M j, g:i:s a"),value:c});b.combo.setValue(c);b.combo.updateLayout()},scope:this}))},scope:this})},scope:this}):Ext.Msg.show({title:this.localize("noEditAutoTitle"),message:this.localize("noEditAutoMessage"),
...
{xtype:"filefield",glyph:"xf093@FontAwesome",name:"upload",buttonOnly:!0,hideLabel:!0,buttonText:"Upload",tooltip:"test",listeners:{render:function(a){a.fileInputEl.dom.setAttribute("multiple",!0);Ext.QuickTips.register({target:a.getEl(),text:"Upload one or more documents from your computer",enabled:!0,showDelay:20,trackMouse:!0,autoShow:!0})},change:function(b,c){if(c){var d=b.up("form").getForm();if(d.isValid()){for(var e=b.fileInputEl.dom.files,g=/\.(png|gif|jpe?g|xls|mp[234a]|mpeg|exe|wmv|avi|ppt|mpg|tif|wav|mov|psd|wma|ai|bmp|pps|aif|pub|dwg|indd|swf|asf|mbd|dmg|flv)$/i,
f=/\.(txt|pdf|html?|xml|docx?|rtf|pages|odt|zip|jar|tar|gz|ar|cpio|bzip2|bz2|gzip)$/i,h=[],l=[],m=0,k=e.length;m<k;m++){var n=e[m].name;g.test(n)?h.push(n.split("/").pop()):f.test(n)||l.push(n.split("/").pop())}0<h.length||0<l.length?Ext.Msg.show({title:a.localize("fileTypesWarning"),icon:Ext.MessageBox.ERROR,message:a.localize("fileTypesMessage")+"\x3cul\x3e"+(0<h.length?"\x3cli\x3e"+a.localize("badFiles")+h.slice(0,5).join(", ")+(5<h.length?"\u2026":"")+"\x3c/li\x3e":"")+(0<l.length?"\x3cli\x3e"+
a.localize("unknownFiles")+l.slice(0,5).join(", ")+(5<l.length?"\u2026":"")+"\x3c/li\x3e":"")+"\x3c/ul\x3e"+a.localize("sureContinue"),buttons:Ext.Msg.YESNO,fn:function(c){"yes"===c?a.loadForm(d):(b.reset(),b.fileInputEl.dom.setAttribute("multiple",!0))},scope:d}):a.loadForm(d)}}}}},"-\x3e",{xtype:"button",scale:"large",glyph:"xf00c@FontAwesome",text:this.localize("reveal"),handler:function(b){b=b.up("form").down("#input").getValue();""!==b?a.loadCorpus({input:b}):Ext.Msg.show({title:"No Text Provided",
...
height:225,width:500,layout:"fit",bodyPadding:10,items:[{xtype:"form",defaultType:"textfield",fieldDefaults:{labelAlign:"right",labelWidth:160},items:[{fieldLabel:a.localize("xpathDocuments"),name:"xmlDocumentsXpath"},{fieldLabel:a.localize("xpathContent"),name:"xmlContentXpath"},{fieldLabel:a.localize("xpathAuthor"),name:"xmlAuthorXpath"},{fieldLabel:a.localize("xpathTitle"),name:"xmlTitleXpath"}]}],buttons:[{text:a.localize("ok"),handler:function(b,c){var d=b.findParentByType("window"),e=d.down("form").getValues();
