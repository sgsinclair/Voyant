...
null==h.color||""==h.color?g.colors[Math.floor(Math.random()*g.colors.length)]:h.color;var l;l=void 0==typeof h.size||null==h.size||""==h.size?Math.floor(40*Math.random()):parseFloat(h.size);var m=h.word,q=h.label,n=h.value,h=!1;null==r[m]&&(r[m]=!0,l>f.largestWordSize&&(f.largestWordSize=l,h=!0),l<f.smallestWordSize&&(f.smallestWordSize=.8*l,h=!0),m=new Word(m,l,k,q,n),p.push(m));b=h||b}c();this.setRelativeSizes();this.calculateSizeAdjustment();b?this.resizeWords():d()};this.resetWordCoordinates=
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];l=0;f=function(){var a,f,h,k,r=this.wordsToArrange-1,q=g.canvas.width,n=g.canvas.height,w=.5*q,s=.5*n;do{h=p[l];if(void 0!==h){for(var D=Math.random()*Math.PI,v=.25*Math.random()*h.size,x=.5*(Math.random()-.5),F=.5*h.width,N=.5*h.height;;){a=Math.floor((w+Math.cos(D)*v*this.ratio-F)/this.COARSENESS)*this.COARSENESS;f=Math.floor((s+Math.sin(D)*v-N)/this.COARSENESS)*this.COARSENESS;
k=a+F>=q||f+N>=n?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;D+=x;v+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<l;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}l++;if(l>=p.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(r--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;m=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
if(b.series)Ext.apply(chart,{series:b.series}),chart.series.forEach(function(a,b){Ext.applyIf(a,{type:"line",xField:void 0===this.rowKey?"row-index":this.rowKey,yField:b})}),delete b.series;else{c=[];f=0;for(d=d.length;f<d;f++)f===this.rowKey||void 0==this.rowKey&&f+1==d||c.push({type:"line",xField:void 0===this.rowKey?"row-index":this.rowKey,yField:f});Ext.apply(chart,{series:c})}b.title&&(chart.title=b.title);console.warn(chart);Ext.apply(b,{chartJson:JSON.stringify(chart)});embed(a,b)}});Ext.define("Voyant.data.table.CorpusTerms",{extend:"Voyant.data.table.Table",eachCorpusTerm:function(){this.eachRecord.apply(this,arguments)}});Ext.define("Voyant.data.model.Corpus",{alternateClassName:["Corpus"],mixins:["Voyant.notebook.util.Embed","Voyant.notebook.util.Show","Voyant.util.Transferable","Voyant.util.Localization"],transferable:["loadCorpusTerms","loadTokens","getPlainText","getText","getWords"],embeddable:"Voyant.panel.Summary Voyant.panel.Cirrus Voyant.panel.Documents Voyant.panel.CorpusTerms Voyant.panel.Reader Voyant.panel.Trends Voyant.panel.TermsRadio".split(" "),requires:["Voyant.util.ResponseError","Voyant.data.store.CorpusTerms",
"Voyant.data.store.Documents"],extend:"Ext.data.Model",config:{documentsStore:void 0},statics:{i18n:{}},fields:[{name:"documentsCount",type:"int"},{name:"lexicalTokensCount",type:"int"},{name:"lexicalTypesCount",type:"int"},{name:"createdTime",type:"int"},{name:"createdDate",type:"date",dateFormat:"c"},{name:"title",type:"string"},{name:"subTitle",type:"string"}],constructor:function(a,b){a=a||{};b=b||{};this.callParent([]);var c=Voyant.application.getDeferred(this);if(Ext.isString(a))0==/\s/.test(a)&&
-1==a.indexOf(":")?(Ext.apply(b,{corpus:a}),b={corpus:b}):Ext.apply(b,{input:a});else if(Ext.isArray(a))Ext.apply(b,{input:a});else if(Ext.isObject(a))Ext.apply(b,a);else return Voyant.application.showError(this.localize("badDataTypeCorpus")+": ("+typeof a+") "+a),Ext.defer(function(){c.reject(this.localize("badDataTypeCorpus")+": ("+typeof a+") "+a)},50,this),c.promise;if(Ext.isObject(b)){if(!b.corpus&&!b.input)return Voyant.application.showError(this.localize("noCorpusOrInput")+": "+b),Ext.defer(function(){c.reject(this.localize("noCorpusOrInput")+
": "+b)},50,this),c.promise;Ext.apply(b,{tool:"corpus.CorpusMetadata"});var d=this;Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:b}).then(function(a){d.set(Ext.JSON.decode(a.responseText).corpus.metadata);var c="titles-"+d.getId();b.title||b.subTitle?(d.set("title",b.title),d.set("subTitle",b.subTitle),Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:{tool:"resource.StoredResource",storeResource:Ext.encode({title:b.title,subTitle:b.subTitle}),resourceId:c}})):Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),
params:{tool:"resource.StoredResource",verifyResourceId:c}}).then(function(a){(a=Ext.util.JSON.decode(a.responseText))&&a.storedResource&&a.storedResource.id&&""!=a.storedResource.id&&Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:{tool:"resource.StoredResource",retrieveResourceId:c}}).then(function(a){a=Ext.util.JSON.decode(a.responseText).storedResource.resource;a=Ext.decode(a);d.set(a)})});return d}).otherwise(function(a){Voyant.application.showResponseError(d.localize("failedCreateCorpus"),
...
Ext.apply(a||{},{corpus:this}))},each:function(a){debugger;this.getDocuments().each.call(this,arguments)},getCorpusCollocates:function(a){return Ext.create("Voyant.data.store.CorpusCollocates",Ext.apply(a||{},{corpus:this}))},getDocumentQueryMatches:function(a){return Ext.create("Voyant.data.store.DocumentQueryMatches",Ext.apply(a||{},{corpus:this}))},getDocumentTerms:function(a){return Ext.create("Voyant.data.store.DocumentTerms",Ext.apply(a||{},{corpus:this}))},getContexts:function(a){return Ext.create("Voyant.data.store.Contexts",
...
getText:function(a){if(this.then)return Voyant.application.getDeferredNestedPromise(this,arguments);var b=Voyant.application.getDeferred(this);a=a||{};Ext.isNumber(a)?a={limit:a}:Ext.isString(a)&&(a={limit:parseInt(a)});Ext.applyIf(a,{limit:0,outputFormat:"text",template:"docTokens2text"});Ext.apply(a,{tool:"corpus.DocumentTokens",corpus:this.getAliasOrId()});Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:a,success:function(a,d){b.resolve(a.responseText)},failure:function(a,d){b.reject(a)},
...
height:f.getHeight()},"dickinson",a,e,"#"+f.getId());b.Start()}.bind(b))}});Ext.define("Voyant.panel.TermsBerry",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.termsberry",statics:{i18n:{},api:{stopList:"auto",context:2,numInitialTerms:75,query:void 0,docIndex:void 0,docId:void 0},glyph:"xf201@FontAwesome"},config:{options:[{xtype:"stoplistoption"}],mode:void 0,collocatesLimit:1E6,scalingFactor:3,minRawFreq:void 0,maxRawFreq:void 0,maxCollocateValue:void 0,minFillValue:void 0,maxFillValue:void 0,currentData:{},visLayout:void 0,vis:void 0,visInfo:void 0,
