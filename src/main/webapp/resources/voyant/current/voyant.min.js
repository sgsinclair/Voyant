...
this.lastClickedBubbles[b],m=0,n;for(n in k)if(-1!==this.termsFilter.indexOf(n)){var q=k[n];if(q){m++;this.SEPARATE_LINES_FOR_TERMS&&c();var s=0,p=q.color.join(",");this.ctx.strokeStyle="rgba("+p+", 1)";this.ctx.fillStyle="rgba("+p+", 0.35)";this.ctx.lineWidth=.25;for(var h=h+q.rawFreq,s=s+q.rawFreq,r=l&&this.lastClickedBubbles[b][n],t=0;t<q.pos.length;t++){var u=q.pos[t];if(0<u.radius){var x=!1;r&&this.lastClickedBubbles[b][n]==u.id&&(this.ctx.strokeStyle="rgba(0, 0, 0, 0.75)",this.ctx.fillStyle=
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];l=0;f=function(){var a,f,h,k,r=this.wordsToArrange-1,n=g.canvas.width,t=g.canvas.height,q=.5*n,s=.5*t;do{h=p[l];if(void 0!==h){for(var v=Math.random()*Math.PI,w=.25*Math.random()*h.size,A=.5*(Math.random()-.5),y=.5*h.width,z=.5*h.height;;){a=Math.floor((q+Math.cos(v)*w*this.ratio-y)/this.COARSENESS)*this.COARSENESS;f=Math.floor((s+Math.sin(v)*w-z)/this.COARSENESS)*this.COARSENESS;
k=a+y>=n||f+z>=t?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;v+=A;w+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<l;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}l++;if(l>=p.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(r--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;m=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
void 0!==this.getNetwork()&&this.getNetwork().fit()},this);this.mixins["Voyant.panel.Panel"].initComponent.apply(this,arguments);a.callParent(arguments)},initLoad:function(){this.getCorpus()&&(this.initGraph(),this.getCorpus().getCorpusTerms({leadingBufferZone:0,autoLoad:!1}).load({callback:function(a,b,c){c&&this.loadFromCorpusTermRecords(a)},scope:this,params:{limit:3,stopList:this.getApiParam("stopList")}}))},loadFromQuery:function(a){var b=this.getCorpus().getCorpusCollocates({autoLoad:!1});this.setApiParams({mode:"corpus"});
var c=this.getApiParams();c.query=a;b.load({params:c,callback:function(a,b,c){c&&this.loadFromCorpusCollocateRecords(a)},scope:this})},loadFromCorpusTermRecords:function(a){if(Ext.isArray(a)&&0<a.length){var b=[];a.forEach(function(a){b.push(a.getTerm())});this.loadFromCorpusTermStringsArray(b)}},loadFromCorpusTermStringsArray:function(a){a.forEach(function(a){this.loadFromQuery(a)},this);this.setApiParams({query:a,mode:"corpus"})},loadFromCorpusCollocateRecords:function(a,b){if(Ext.isArray(a)){var c=
{};this.getNodeDataSet().forEach(function(a){c[a.id]=!0});var d=[],e=[],f=this.getEdgeDataSet(),g=this.getApiParam("limit");a.forEach(function(a){if(a.getContextTerm()!=a.getTerm()){if(void 0===b){var k={id:a.getKeyword()+".keyword",label:a.getKeyword(),title:a.getKeyword()+" ("+a.getKeywordRawFreq()+")",type:"keyword",value:a.getKeywordRawFreq(),start:g,font:{color:"green"}};b=k.id;void 0===c[b]&&(c[b]=!0,d.push(k))}a={id:a.getContextTerm()+".context",label:a.getContextTerm(),title:a.getContextTerm()+
" ("+a.getContextTermRawFreq()+")",type:"context",value:a.getContextTermRawFreq(),start:0,font:{color:"maroon"}};var l=a.id;void 0===c[l]&&(c[l]=!0,d.push(a));var m=!1;f.forEach(function(a){if(a.from==b&&a.to==l||a.from==l&&a.to==b)m=!0});m||e.push({from:b,to:l})}});this.getNodeDataSet().add(d);f.add(e);this.forceUpdate();this.getNetwork().fit()}},initGraph:function(){var a=this.getLayout().getRenderTarget();a.setWidth(a.getWidth());a.setHeight(a.getHeight());this.setGraphHeight(a.getHeight());this.setGraphWidth(a.getWidth());
...
a.y)}),k=d3.svg.axis().scale(h).orient("bottom");if(this.getMode()===this.MODE_CORPUS){for(var n=[],q=0;q<=g;q++)n.push(q);k.tickValues(n);k.tickFormat("")}n=d3.svg.axis().scale(l).orient("left");f=this.getVis().selectAll("path").data(f,function(a){return a.name});f.attr("d",function(a){return m(a.values)}).style("fill",function(a,c){return b(c)});f.enter().append("path").attr("d",function(a){return m(a.values)}).style("fill",function(a,c){return b(c)}).append("title").text(function(a){return a.name});
