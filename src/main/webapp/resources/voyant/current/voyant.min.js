...
{docId:this.get("id")});return a.corpus?a.corpus.getDocumentTerms(a):this.get("corpus").getDocumentTerms(a)},getIndex:function(){return this.get("index")},getIndex:function(){return this.get("id")},getFullLabel:function(){return this.getTitle()},getTitle:function(){var a=this.get("title");void 0===a&&(a="");a=Ext.isArray(a)?a.join("; "):a;return a=a.trim().replace(/\s+/g," ")},getTruncated:function(a,b){if(a.length>b){var c=a.lastIndexOf("/");-1<c&&(a=a.substr(c+1));if(a.length>b){c=a.indexOf(" ",
b-5);if(0>c||c>b)c=b;a=a.substring(0,c)+"\u2026"}}return a},getShortTitle:function(){var a=this.getTitle(),a=a.replace(/\.(html?|txt|xml|docx?|pdf|rtf|\/)$/,""),a=a.replace(/^(the|a|le|l'|un|une)\s/,"");return this.getTruncated(a,25)},getTinyTitle:function(){return this.getTruncated(this.getShortTitle(),10)},getShortLabel:function(){return parseInt(this.getIndex())+1+") "+this.getShortTitle()},getTinyLabel:function(){return parseInt(this.getIndex())+1+") "+this.getTinyTitle()},getAuthor:function(){var a=
...
(params={query:c.term},c.docId?params.docId=c.docId:params.docIndex=c.docIndex?c.docIndex:0,this.setApiParams(params),this.isVisible()&&this.getStore().loadPage(1))},scope:this},documentIndexTermsClicked:{fn:function(a,c){var d={},e=[],f={},g=[];c.forEach(function(a){d[a.term]||(e.push(a.term),d[a.term]=!0);f[a.docIndex]||(g.push(a.docIndex),f[a.docIndex]=!0)});this.setApiParams({docId:void 0,docIndex:g,query:e});this.down("#corpus").show();this.isVisible()&&this.getStore().loadPage(1,{params:this.getApiParams()})},
scope:this},afterrender:function(a){a.getView().on("expandbody",function(c,d,e,f){if(""===e.innerText||f&&f.force){c=Ext.create("Voyant.data.store.Contexts",{stripTags:"all",corpus:a.getStore().getCorpus()});var g=d.getData();c.load({params:{query:g.query,docIndex:g.docIndex,position:g.position,limit:1,context:a.getApiParam("expand")},callback:function(a,b,c){c&&1==a.length&&(g=a[0].getData(),b.expandRow.firstElementChild.firstElementChild.innerHTML=g.left+" \x3cspan class\x3d'word keyword'\x3e"+
...
c=this.down("[xtype\x3dlegend]").getStore(),d=[],e=[];a.forEach(function(a,c){var f=[];a.get("distributions").forEach(function(a,b){f.push({x:b,y:a})},this);e.push(f);d.push({name:a.get("term"),mark:b(c)})},this);c.loadData(d);a=this.getVisLayout()(e);var c=this.getMode()===this.MODE_DOCUMENT?this.getApiParam("bins")-1:this.getCorpus().getDocumentsCount()-1,f=this.body.down("svg").getWidth()-this.graphMargin.left-this.graphMargin.right,g=d3.scale.linear().domain([0,c]).range([0,f]),c=d3.max(a,function(a){return d3.max(a,
function(a){return a.y0+a.y})}),f=this.body.down("svg").getHeight()-this.graphMargin.top-this.graphMargin.bottom,h=d3.scale.linear().domain([0,c]).range([f,0]),k=d3.svg.area().x(function(a){return g(a.x)}).y0(function(a){return h(a.y0)}).y1(function(a){return h(a.y0+a.y)}),c=this.getVis().selectAll("path");0===c.size()?c.data(a).enter().append("path").attr("d",function(a){return k(a)}).style("fill",function(a,c){return b(c)}):c.data(a).attr("d",function(a){return k(a)})},initGraph:function(){if(void 0===
this.getVisLayout()){var a=this.getLayout().getRenderTarget(),b=a.getWidth()-0,c=a.getHeight()-0;this.setVisLayout(d3.layout.stack().offset("silhouette"));this.setVis(d3.select(a.dom).append("svg").attr("id","streamGraph").attr("width",b).attr("height",c).attr("transform","translate(0,0)"))}}});Ext.define("Voyant.VoyantApp",{extend:"Ext.app.Application",mixins:["Voyant.util.Deferrable","Voyant.util.Localization","Voyant.util.Api"],requires:["Voyant.util.ResponseError"],name:"VoyantApp",statics:{i18n:{error:{en:"Error"},serverResponseError:{en:"The server error reponse:"}}},config:{baseUrl:void 0,tromboneUrl:void 0},constructor:function(a){this.setBaseUrl(this.config.baseUrl);this.setTromboneUrl(this.config.baseUrl+"trombone");Voyant.application=this;this.mixins["Voyant.util.Api"].constructor.apply(this,
