...
null==h.color||""==h.color?g.colors[Math.floor(Math.random()*g.colors.length)]:h.color;var l;l=void 0==typeof h.size||null==h.size||""==h.size?Math.floor(40*Math.random()):parseFloat(h.size);var m=h.word,n=h.label,r=h.value,h=!1;null==u[m]&&(u[m]=!0,l>f.largestWordSize&&(f.largestWordSize=l,h=!0),l<f.smallestWordSize&&(f.smallestWordSize=.8*l,h=!0),m=new Word(m,l,k,n,r),p.push(m));b=h||b}c();this.setRelativeSizes();this.calculateSizeAdjustment();b?this.resizeWords():d()};this.resetWordCoordinates=
...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];l=0;f=function(){var a,f,h,k,n=this.wordsToArrange-1,u=g.canvas.width,r=g.canvas.height,q=.5*u,v=.5*r;do{h=p[l];if(void 0!==h){for(var B=Math.random()*Math.PI,w=.25*Math.random()*h.size,D=.5*(Math.random()-.5),M=.5*h.width,N=.5*h.height;;){a=Math.floor((q+Math.cos(B)*w*this.ratio-M)/this.COARSENESS)*this.COARSENESS;f=Math.floor((v+Math.sin(B)*w-N)/this.COARSENESS)*this.COARSENESS;
k=a+M>=u||f+N>=r?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;B+=D;w+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<l;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}l++;if(l>=p.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(n--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;m=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
this.getIsArcLoaded()){var a=this.body.down(".map").setId(Ext.id()),b=this,c=new ol.Map({layers:[new ol.layer.Tile({preload:Infinity,source:new ol.source.Stamen({layer:"watercolor"})}),new ol.layer.Tile({preload:Infinity,source:new ol.source.Stamen({cacheize:2048,layer:"toner-hybrid"})})],target:a.getId(),loadTilesWhileInteracting:!0,view:new ol.View({center:[15E5,6E6],maxResolution:4.007501668557849E7/b.body.dom.offsetWidth,zoom:0})});c.on("singleclick",function(a){(a=c.getFeaturesAtPixel(a.pixel))&&
a.forEach(function(a){"city"===a.get("type")&&a.get("selected")&&b.dispatchEvent("termsClicked",b,[a.get("forms").join("|")])},this)});var d=new ol.layer.Vector({map:c,source:new ol.source.Vector({wrapX:!1,useSpatialIndex:!1}),zIndex:10,selected:!0,style:function(a){return"city"===a.get("type")?b.cityStyleFunction(a,c.getView().getResolution()):b.travelStyleFunction(a,c.getView().getResolution())},updateWhileAnimating:!0,updateWhileInteracting:!0});c.on("pointermove",function(a){d.getSource().clear();
var f=a.coordinate;if(a=c.getFeaturesAtPixel(a.pixel)){for(var g=0;a[g].get("selected")&&(g++,g!==a.length););if(g<a.length){var h=a[g];a={font:"12px Calibri,sans-serif",textAlign:"center",offsetY:-15,fill:new ol.style.Fill({color:[0,0,0,1]}),stroke:new ol.style.Stroke({color:[255,255,255,.5],width:4})};a.text=h.get("text");a=new ol.style.Style({text:new ol.style.Text(a),zIndex:1});g=new ol.Feature({text:h.get("text"),geometry:h.getGeometry(),forms:h.get("forms"),selected:!0,coordinates:h.get("coordinates"),
width:h.get("width"),visible:h.get("visible"),color:h.get("color"),type:h.get("type"),confidence:h.get("confidence")});d.getSource().addFeature(g);f=h.getGeometry().getClosestPoint(f);f=new ol.Feature({geometry:new ol.geom.Point(f),selected:!0});f.setStyle(a);d.getSource().addFeature(f);"city"===h.get("type")&&b.getMap().getLayers().forEach(function(a){var b=a.get("id");b&&-1!==b.indexOf("connections")&&(connections=a.getSource().getFeatures(),connections.forEach(function(a){a.get("target")!==h.get("cityId")&&
a.get("source")!==h.get("cityId")||d.getSource().addFeature(new ol.Feature({geometry:a.getGeometry(),selected:!0,coordinates:a.get("coordinates"),width:a.get("width"),visible:a.get("visible"),color:a.get("color")}))}))})}}});this.setMap(c);this.getDockedComponent("bottomToolbar").getComponent("addFilter").click()}else Ext.defer(this.tryInit,500,this)},travelStyleFunction:function(a,b){var c=new ol.style.Stroke({color:a.get("color"),width:a.get("width")}),d=[new ol.style.Style({stroke:c})],e=a.getGeometry(),
f=e.getLastCoordinate(),e=e.getCoordinateAt(.9),e=Math.atan2(f[1]-e[1],f[0]-e[0]),g=new ol.geom.LineString([f,[f[0]-10*b,f[1]+10*b]]);g.rotate(e,f);var h=new ol.geom.LineString([f,[f[0]-10*b,f[1]-10*b]]);h.rotate(e,f);d.push(new ol.style.Style({geometry:g,stroke:c}));d.push(new ol.style.Style({geometry:h,stroke:c}));return d},cityStyleFunction:function(a,b){if(a.get("visible")){var c=2*Math.PI*a.get("width"),d=a.get("confidence")?c*a.get("confidence"):c;return new ol.style.Style({image:new ol.style.Circle({radius:a.get("width"),
...
width:10})})]},showTermInCorpus:function(a,b,c){a=Ext.create("Voyant.data.model.Context",{docIndex:a,position:b,term:c});this.getApplication().dispatchEvent("termLocationClicked",this,[a])},filterUpdate:function(a){a.clearAnimation();var b=this.getMap(),c=a.getColor(),d=parseInt(this.getApiParam("citiesMaxCount")),e=0;a.getGeonames().eachCity(function(a){a.rawFreq>e&&(e=a.rawFreq)},this,d);var f=b.getLayer(a.getId()+"-cities").getSource(),g=Ext.Array.from(this.getApiParam("hide"));f.clear();var h=
parseInt(this.getApiParam("minPopulation")),k=parseInt(this.getApiParam("citiesMinFreq")),l={};a.getGeonames().eachCity(function(a){if((!h||a.population>=h)&&(!k||a.rawFreq>=k)){l[a.id]=!0;var b=[parseFloat(a.longitude),parseFloat(a.latitude)];a=new ol.Feature({geometry:(new ol.geom.Point(b)).transform(ol.proj.get("EPSG:4326"),ol.proj.get("EPSG:3857")),text:a.label+" ("+a.rawFreq+")",description:a.label,color:c,selected:!1,width:3+20*Math.sqrt(a.rawFreq/e),visible:!0,coordinates:b,forms:a.forms,cityId:a.id,
type:"city",confidence:a.confidence?100*a.confidence:void 0});f.addFeature(a)}},this,d);0<f.getFeatures().length&&b.getView().fit(f.getExtent());b.getLayer(a.getId()+"-cities").setVisible(0==Ext.Array.contains(g,"cities"));f=b.getLayer(a.getId()+"-connections").getSource();f.clear();0==Object.keys(l).length&&this.toastInfo("No cities available for current criteria.");e=0;a.getGeonames().eachConnection(function(a){a.rawFreq>e&&(e=a.rawFreq)},this);var m=0,n=parseInt(this.getApiParam("connectionsMaxCount")),
q=parseInt(this.getApiParam("connectionsMinFreq"));a.getGeonames().eachConnection(function(a){if((!q||a.rawFreq>=q)&&(!n||m<n)&&a.source.id in l&&a.target.id in l&&(!h||a.source.population>=h&&a.target.population>=h)){var b=(new arc.GreatCircle({x:a.source.longitude,y:a.source.latitude},{x:a.target.longitude,y:a.target.latitude})).Arc(100,{offset:100}),d=a.source.label+" -\x3e "+a.target.label+" ("+a.rawFreq+")";b.geometries.forEach(function(b){b=new ol.geom.LineString(b.coords);b.transform(ol.proj.get("EPSG:4326"),
ol.proj.get("EPSG:3857"));b=new ol.Feature({geometry:b,text:d,color:c,width:3+10*Math.sqrt(a.rawFreq/e),source:a.source.id,target:a.target.id});f.addFeature(b)},this);m++}},this);b.getLayer(a.getId()+"-connections").setVisible(0==Ext.Array.contains(g,"connections"));a.animate()}});
...
return f.load(a).then(function(){b.unmask();b.fireEvent("filterUpdate",b,f);return f})},clearAnimation:function(){this.getTimeout()&&clearTimeout(this.getTimeout())},animate:function(){this.clearAnimation();var a=this.getCurrentConnectionOccurrence();if(a){var b=this.getAnimationLayer();b||(b=this.up("panel").getMap().getLayer(this.getId()+"-animation"),this.setAnimationLayer(b));if(b){var c=b.getSource().getFeatures();if(0==c.length){if(c=this.up("panel")){for(var d=c.getMap().getLayer(this.getId()+
"-connections").getSource().getFeatures(),e=!1,f=0,g=d.length;f<g;f++)if(a.source.id==d[f].get("source")&&a.target.id==d[f].get("target")){e=d[f];break}if(e){this.getKeepAnimationInFrame()&&(g=c.getMap(),d=c.getMap().getView().calculateExtent(g.getSize()),ol.extent.containsExtent(d,e.getGeometry().getExtent())||g.getView().setZoom(g.getView().getZoom()-1));var e=(new arc.GreatCircle({x:a.source.longitude,y:a.source.latitude},{x:a.target.longitude,y:a.target.latitude})).Arc(100,{offset:100}),h=a.source.label+
" -\x3e "+a.target.label;e.geometries.forEach(function(a){var c=new ol.geom.LineString([]);c.transform(ol.proj.get("EPSG:4326"),ol.proj.get("EPSG:3857"));a=new ol.Feature({geometry:c,allcoords:a.coords,text:h,color:this.getColor(),width:5,start:(new Date).getTime()});b.getSource().addFeature(a)},this);c.body.down(".ticker");c.body.down(".ticker").setHtml(a.source.left+" \x3cspan class\x3d'keyword'\x3e"+a.source.middle+"\x3c/span\x3e "+a.source.right+" \u2026 "+a.target.left+" \x3cspan class\x3d'keyword'\x3e"+
a.target.middle+"\x3c/span\x3e "+a.target.right);return this.setTimeout(setTimeout(this.animate.bind(this),1))}if(this.getGeonames())return a=this.getGeonames().getConnectionOccurrence(a.index+1),this.setCurrentConnectionOccurrence(a),b.getSource().clear(),this.setTimeout(setTimeout(this.animate.bind(this),1))}}else if(0<c.length){g=c[0].getGeometry().getCoordinates();e=c[0].get("allcoords");if(g.length<e.length)return g=((new Date).getTime()-c[0].get("start"))*e.length/this.getMillisPerAnimation(),
line=new ol.geom.LineString(e.slice(0,g)),line.transform(ol.proj.get("EPSG:4326"),ol.proj.get("EPSG:3857")),c[0].set("geometry",line),this.setTimeout(setTimeout(this.animate.bind(this),25));a=this.getGeonames().getConnectionOccurrence(a.index+1);if(!this.getStepByStepMode())return this.setCurrentConnectionOccurrence(a),b.getSource().clear(),this.setTimeout(setTimeout(this.animate.bind(this),1))}}}else if(this.getGeonames()&&(a=this.getGeonames().getConnectionOccurrence(0)))return this.setCurrentConnectionOccurrence(a),
