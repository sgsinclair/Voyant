...
increment:1,minValue:this.MIN_TERMS,maxValue:this.MAX_TERMS,listeners:{afterrender:function(a){a.setValue(this.getNumInitialTerms())},changecomplete:function(a,b){this.setNumInitialTerms(b);this.doLoad()},scope:this}},{fieldLabel:this.localize("scaling"),labelWidth:50,width:120,xtype:"slider",increment:1,minValue:this.MIN_SCALING,maxValue:this.MAX_SCALING,listeners:{afterrender:function(a){a.setValue(this.getScalingFactor())},changecomplete:function(a,b){var c=Math.abs(b-(this.MAX_SCALING+1));this.setScalingFactor(c);
this.getVisLayout().value(function(a){return Math.pow(a.rawFreq,1/c)});var d=this.processRecordsForVis([]);this.resetVis();this.buildVisFromData(d)},scope:this}}]}]});this.callParent(arguments)},listeners:{boxready:function(){this.initVisLayout()},resize:function(a,b,c){this.getVisLayout()&&this.getCorpus()&&(a=this.getLayout().getRenderTarget(),b=a.getWidth(),c=a.getHeight(),a.down("svg").set({width:b,height:c}),this.getVisLayout().size([b,c]),b=this.processRecordsForVis([]),this.resetVis(),this.buildVisFromData(b))},
loadedCorpus:function(a,b){this.doLoad()}},doLoad:function(){this.resetVis();this.resetMinMax();this.setCurrentData({});this.getMode()===this.MODE_DOCUMENT?this.loadFromDocuments():this.loadFromCorpus()},resetMinMax:function(){this.setMinRawFreq(void 0);this.setMaxRawFreq(void 0);this.setMaxCollocateValue(void 0);this.setMinFillValue(void 0);this.setMaxFillValue(void 0)},resetVis:function(){this.getVis().selectAll(".node").remove()},loadFromCorpus:function(){this.setApiParams({docId:void 0,docIndex:void 0});
this.getCorpus().getCorpusTerms().load({params:{limit:this.getNumInitialTerms(),stopList:this.getApiParam("stopList")},callback:function(a,b,c){c&&this.loadFromRecords(a)},scope:this})},loadFromDocuments:function(){var a=Math.round(this.getNumInitialTerms()/this.getCorpus().getDocumentsCount()),b={};this.getCorpus().getDocuments().each(function(c,d){this.getCorpus().getDocumentTerms().load({addRecords:!0,params:{docIndex:d,limit:a,stopList:this.getApiParam("stopList"),sort:"TFIDF",dir:"DESC"},callback:function(a,
c,g){if(g){b[d]=a;a=!0;for(c=0;c<this.getCorpus().getDocumentsCount();c++)if(void 0===b[c]){a=!1;break}if(a){a=[];for(var h in b)a=a.concat(b[h]);this.loadFromRecords(a)}}},scope:this})},this)},loadFromRecords:function(a){if(Ext.isArray(a)&&0<a.length){var b=this.getMaxRawFreq(),c=this.getMinRawFreq(),d=this.getMinFillValue(),e=this.getMaxFillValue(),f=[];a.forEach(function(a){var h=a.getTerm(),k=a.getRawFreq(),l=this.getMode()===this.MODE_DOCUMENT?a.get("tfidf"):a.getInDocumentsCount();if(void 0===
b||k>b)b=k;if(void 0===c||k<c)c=k;if(void 0===e||l>e)e=l;if(void 0===d||l<d)d=l;this.getCurrentData()[h]={term:h,rawFreq:k,relativeFreq:a.get("relativeFreq"),fillValue:l,collocates:[]};f.push(h)},this);this.setMaxRawFreq(b);this.setMinRawFreq(c);this.setMinFillValue(d);this.setMaxFillValue(e);this.loadFromQuery(f)}},loadFromQuery:function(a){this.setApiParams({mode:"corpus"});var b=this.getApiParams();this.getCorpus().getCorpusCollocates().load({params:Ext.apply(Ext.clone(b),{query:a,collocatesWhitelist:a,
limit:this.getCollocatesLimit()}),callback:function(a,b,e){e&&this.buildVisFromData(this.processRecordsForVis(a))},scope:this})},processRecordsForVis:function(a){for(var b=this.getCurrentData(),c=this.getMaxCollocateValue(),d=0;d<a.length;d++){var e=a[d],f=e.getTerm(),g=e.getContextTerm(),e=e.getContextTermRawFreq();if(void 0===c||e>c)c=e;void 0!==b[f]&&f!=g&&b[f].collocates.push({term:g,value:e})}this.setMaxCollocateValue(c);a=[];for(f in b)0<b[f].collocates.length&&a.push(b[f]);return a},buildVisFromData:function(a){a=
this.getVisLayout().nodes({children:a,collocates:[],term:"",rawFreq:1});a=this.getVis().selectAll(".node").data(a,function(a){return a.term});a.attr("transform",function(a){return"translate("+a.x+","+a.y+")"});a.selectAll("circle").attr("r",function(a){return a.r});var b=this,c=d3.scale.pow().exponent(1/3).domain([0,this.getMaxCollocateValue()]).range(["#ffffff","#bd3163"]),d=d3.scale.pow().exponent(1/3).domain([this.getMinFillValue(),this.getMaxFillValue()]).range(["#fff","#ddd"]),e=a[0].length-
1,f=d3.scale.pow().exponent(.5).domain([this.MIN_TERMS,this.MAX_TERMS]).range([16,8])(e),e=d3.scale.pow().exponent(.5).domain([this.MIN_TERMS,this.MAX_TERMS]).range([20,12])(e),g=d3.scale.pow().exponent(.5).domain([this.getMinRawFreq(),this.getMaxRawFreq()]).range([f,e]),f=a.enter().append("g").attr("class","node").style("visibility",function(a){return 0<a.depth?"visible":"hidden"}).attr("transform",function(a){return"translate("+a.x+","+a.y+")"}).on("mouseover",function(a,e){b.getVis().selectAll("circle").style("stroke-width",
1).style("stroke","#111").style("fill",function(a){return d(a.inDocumentsCount)});d3.select(this).select("circle").style("fill","#89e1c2");var f="\x3cb\x3e"+a.term+"\x3c/b\x3e ("+a.rawFreq+")",g=b.getTip();g.update(f);g.show();for(e=0;e<a.collocates.length;e++){var n=a.collocates[e],f=b.getVis().selectAll(".node").filter(function(a){return a.term===n.term});f.select("circle").style("fill",function(a){return c(n.value)}).style("stroke","#314b8b").style("stroke-width",3);f.select("tspan.value").text(function(a){return n.value})}}).on("mousemove",
function(){var a=Ext.get(b.getVisId()).dom,a=d3.mouse(a);a[1]+=20;b.getTip().setPosition(a)}).on("mouseout",function(){b.getVis().selectAll("circle").style("stroke-width",1).style("stroke","#111").style("fill",function(a){return d(a.fillValue)});b.getVis().selectAll("tspan.value").text("");b.getTip().hide()});f.append("circle").attr("id",function(a){return a.term}).attr("r",function(a){return a.r}).style("fill",function(a){return d(a.fillValue)}).style("stroke","#111").style("stroke-opacity",.3).style("stroke-width",
1);f.append("clipPath").attr("id",function(a){return"clip-"+a.term}).append("use").attr("xlink:href",function(a){return"#"+a.term});f=f.append("text").attr("clip-path",function(a){return"url(#clip-"+a.term+")"}).style("text-anchor","middle").style("cursor","default");f.append("tspan").attr("class","term").style("font-size",function(a){return g(a.rawFreq)}).attr("x",0).attr("y",0).text(function(a){return a.term});f.append("tspan").attr("class","value").style("font-size",function(a){return g(a.rawFreq)-
2}).attr("x",0).attr("y",function(a){return g(a.rawFreq)+1});a.exit().remove()},initVisLayout:function(){var a=this.getLayout().getRenderTarget();a.update("");var b=a.getWidth(),c=a.getHeight(),d=this;this.setVisLayout(d3.layout.pack().sort(function(a,b){return a.rawFreq<b.rawFreq?1:a.rawFreq>b.rawFreq?-1:0}).value(function(a){return Math.pow(a.rawFreq,1/d.getScalingFactor())}).size([b,c]).padding(1.5));a=d3.select(a.dom).append("svg").attr("id",this.getVisId()).attr("width",b).attr("height",c);this.setVis(a.append("g"));
this.setVisInfo(a.append("text").attr("x",10).attr("y",10));void 0===this.getTip()&&this.setTip(Ext.create("Ext.tip.Tip",{}))}});Ext.define("Voyant.VoyantApp",{extend:"Ext.app.Application",mixins:["Voyant.util.Deferrable","Voyant.util.Localization","Voyant.util.Api"],requires:["Voyant.util.ResponseError"],name:"VoyantApp",statics:{i18n:{},api:{palette:"default",lang:void 0,debug:void 0}},config:{baseUrl:void 0,tromboneUrl:void 0},constructor:function(a){this.setBaseUrl(this.config.baseUrl);this.setTromboneUrl(this.config.baseUrl+"trombone");Voyant.application=this;this.mixins["Voyant.util.Api"].constructor.apply(this,arguments);
...
{msg:Ext.isString(a)?a:a.msg,response:b}))},showError:function(a,b){debugger;if(a instanceof Voyant.util.ResponseError)a={message:a.getMsg()+"\x3cp class\x3d'error'\x3e\n"+a.getError()+" \u2026 \x3ca href\x3d'#' onclick\x3d\"window.open('').document.write(unescape('\x3cpre\x3e"+escape(a.getDetails())+"\x3c/pre\x3e')); return false;\"\x3emore\x3c/a\x3e\x3c/p\x3e"};else if(Ext.isString(a))a={message:a};else if(Ext.isObject(a)){debugger;if(a.responseText)return this.showResponseError(a.statusText,a)}Ext.applyIf(a,
