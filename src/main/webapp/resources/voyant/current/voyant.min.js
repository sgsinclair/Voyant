...
this.getProxy().setExtraParam("corpus",Ext.isString(a)?a:a.getId());this.callParent(arguments)}});Ext.define("Voyant.data.store.DocSimAnalysis",{extend:"Ext.data.Store",model:"Voyant.data.model.StatisticalAnalysis",config:{corpus:void 0},constructor:function(a){a=a||{};Ext.apply(a,{proxy:{type:"ajax",url:Voyant.application.getTromboneUrl(),extraParams:{tool:"corpus.DocumentSimilarity",corpus:a&&a.corpus?Ext.isString(a.corpus)?a.corpus:a.corpus.getId():void 0,withDistributions:!0},reader:{type:"json",rootProperty:"documentSimilarity",totalProperty:"documentSimilarity.totalTerms"},simpleSortMode:!0}});
...
this,c)},scope:this}}}),plugins:[{ptype:"rowexpander",rowBodyTpl:new Ext.XTemplate("")}],dockedItems:[{dock:"bottom",xtype:"toolbar",enableOverflow:!0,items:[{xtype:"querysearchfield"},{xtype:"totalpropertystatus"},this.localize("context"),{xtype:"slider",minValue:5,value:5,maxValue:50,increment:5,width:50,listeners:{render:function(b){b.setValue(a.getApiParam("context"))},changecomplete:function(b,c){a.setApiParam("context",b.getValue());a.getStore().loadPage(1,{params:a.getApiParams()})}}},this.localize("expand"),
...
toolTip:this.localize("documentTip"),width:"autoSize",dataIndex:"docIndex",sortable:!0,renderer:function(a,c,d,e,f,g){return g.getCorpus().getDocument(a).getTinyLabel()}},{text:this.localize("left"),tooltip:this.localize("leftTip"),align:"right",dataIndex:"left",sortable:!0,flex:1},{text:this.localize("term"),tooltip:this.localize("termTip"),dataIndex:"term",sortable:!0,width:"autoSize"},{text:this.localize("right"),tooltip:this.localize("rightTip"),dataIndex:"right",sortable:!0,flex:1},{text:this.localize("position"),
tooltip:this.localize("positionTip"),dataIndex:"position",sortable:!0,hidden:!0,flex:1}],listeners:{scope:this,corpusSelected:function(){this.getStore().getCorpus()&&(this.setApiParams({docId:void 0,docIndex:void 0}),this.getStore().loadPage(1))},documentsSelected:function(a,c){var d=[],e=this.getStore().getCorpus();c.forEach(function(a){d.push(e.getDocument(a).getId())},this);this.setApiParams({docId:d,docIndex:void 0});this.getStore().loadPage(1)},documentSegmentTermClicked:{fn:function(a,c){c.term&&
(params={query:c.term},c.docId?params.docId=c.docId:params.docIndex=c.docIndex?c.docIndex:0,this.setApiParams(params),this.isVisible()&&this.getStore().loadPage(1))},scope:this},documentIndexTermsClicked:{fn:function(a,c){var d={},e=[],f={},g=[];c.forEach(function(a){d[a.term]||(e.push(a.term),d[a.term]=!0);f[a.docIndex]||(g.push(a.docIndex),f[a.docIndex]=!0)});this.setApiParams({docId:void 0,docIndex:g,query:e});this.isVisible()&&this.getStore().loadPage(1,{params:this.getApiParams()})},scope:this},
afterrender:function(a){a.getView().on("expandbody",function(c,d,e,f){if(""===e.innerText||f&&f.force){c=Ext.create("Voyant.data.store.Contexts",{stripTags:"all",corpus:a.getStore().getCorpus()});var g=d.getData();c.load({params:{query:g.query,docIndex:g.docIndex,position:g.position,limit:1,context:a.getApiParam("expand")},callback:function(a,b,c){c&&1==a.length&&(g=a[0].getData(),b.expandRow.firstElementChild.firstElementChild.innerHTML=g.left+" \x3cspan class\x3d'word keyword'\x3e"+g.middle+"\x3c/span\x3e "+
g.right)},expandRow:e})}})}}});a.on("loadedCorpus",function(b,c){this.getStore().setCorpus(c);Ext.create("Voyant.data.store.CorpusTerms",{corpus:c}).load({callback:function(a,b,c){c&&0<a.length&&(this.setApiParam("query",a[0].getTerm()),this.getStore().load({params:this.getApiParams()}))},scope:a,params:{limit:1,stopList:this.getApiParam("stopList")}})});a.on("query",function(a,c){this.setApiParam("query",c);this.getStore().loadPage(1,{params:this.getApiParams()})},a);a.on("documentTermsClicked",
function(a,c){var d=[];c.forEach(function(a){d.push({term:a.getTerm(),docIndex:a.getDocIndex()})});this.fireEvent("documentIndexTermsClicked",this,d)});a.on("termsClicked",function(a,c){var d=[];Ext.isString(c)&&(c=[c]);c.forEach(function(a){void 0!==a.docIndex&&d.push({term:a.term,docIndex:a.docIndex})});0<d.length&&this.fireEvent("documentIndexTermsClicked",this,d)});a.callParent(arguments)}});Ext.define("Voyant.panel.CorpusCollocates",{extend:"Ext.grid.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.corpuscollocates",config:{corpus:void 0},statics:{i18n:{title:{en:"Collocates"},emptyText:{en:"No matching results."},helpTip:{en:"\x3cp\x3eCorpus Collocates is a table view of which terms appear more frequently in proximity to keywords across the entire corpus. Features include:\x3c/p\x3e\x3cul\x3e\x3cli\x3ereordering by keyword, collocate word, collocate word count\x3c/li\x3e\x3cli\x3ea search box for queries (hover over the magnifying icon for help with the syntax)\x3c/li\x3e\x3c/ul\x3e"},
...
f=/\.(png|gif|jpe?g|xls|mp[234a]|mpeg|exe|wmv|avi|ppt|mpg|tif|wav|mov|psd|wma|ai|bmp|pps|aif|pub|dwg|indd|swf|asf|mbd|dmg|flv)$/i,g=/\.(txt|pdf|html?|xml|docx?|rtf|pages|odt|zip|jar|tar|gz|ar|cpio|bzip2|bz2|gzip)$/i,h=[],k=[],l=0,m=e.length;l<m;l++){var n=e[l].name;f.test(n)?h.push(n.split("/").pop()):g.test(n)||k.push(n.split("/").pop())}0<h.length||0<k.length?Ext.Msg.show({title:a.localize("fileTypesWarning"),icon:Ext.MessageBox.ERROR,message:a.localize("fileTypesMessage")+"\x3cul\x3e"+(0<h.length?
"\x3cli\x3e"+a.localize("badFiles")+h.slice(0,5).join(", ")+(5<h.length?"\u2026":"")+"\x3c/li\x3e":"")+(0<k.length?"\x3cli\x3e"+a.localize("unknownFiles")+k.slice(0,5).join(", ")+(5<k.length?"\u2026":"")+"\x3c/li\x3e":"")+"\x3c/ul\x3e"+a.localize("sureContinue"),buttons:Ext.Msg.YESNO,fn:function(c){"yes"===c?a.loadForm(d):(b.reset(),b.fileInputEl.dom.setAttribute("multiple",!0))},scope:d}):a.loadForm(d)}}}}},"-\x3e",{xtype:"button",scale:"large",glyph:"xf00d@FontAwesome",text:this.localize("cancel"),
hidden:void 0==this.getCorpus(),handler:function(a){(a=this.up("window"))&&a.isFloating()&&a.close()}},{xtype:"button",scale:"large",glyph:"xf00c@FontAwesome",text:this.localize("reveal"),handler:function(b){var c=b.up("form").down("#input").getValue();if(""!==c){var d=a.getApiParams();delete d.view;delete d.stopList;d.inputFormat&&0!==c.trim().indexOf("\x3c")?Ext.Msg.confirm(a.localize("error"),a.localize("errorNotXmlContinue"),function(b){"yes"==b&&a.loadCorpus(Ext.apply(d,{input:c}))},a):a.loadCorpus(Ext.apply(d,
{input:c}))}else Ext.Msg.show({title:"No Text Provided",message:"Please provide text in the text box (or choose open or upload).",buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR})}}]}],items:{xtype:"textareafield",width:800,height:100,itemId:"input",emptyText:this.localize("emptyInput")}});a.callParent(arguments)},loadForm:function(a){var b={tool:this.getCorpus()?"corpus.CorpusMetadata":"corpus.CorpusCreator"};this.getCorpus()&&Ext.apply(b,{corpus:this.getCorpus().getId(),addDocuments:!0});var c=this.getApiParams();
delete c.view;delete c.stopList;Ext.apply(b,c);var d=this.getApplication().getViewport();d.mask(this.localize("uploadingCorpus"));a.submit({url:this.getTromboneUrl(),params:b,failure:function(a,b){d.unmask();b.result&&(this.setCorpus(void 0),this.loadCorpus({corpus:b.result.corpus?b.result.corpus.metadata.id:b.result.stepEnabledCorpusCreator.storedId}))},scope:this})},loadCorpus:function(a){this.getCorpus()&&Ext.apply(a,{corpus:this.getCorpus().getId(),addDocuments:!0});var b=this.up("window");b&&
...
e=[{term:f,docIndex:e.docIndex}];this.loadQueryTerms([f]);this.getApplication().dispatchEvent("termsClicked",this,e)}},this);this.getCorpus()&&this.load()},this);Ext.apply(this,{dockedItems:[{dock:"bottom",xtype:"toolbar",enableOverflow:!0,items:[{glyph:"xf060@FontAwesome",handler:function(){this.fetchPrevious(!0)},scope:this},{glyph:"xf061@FontAwesome",handler:function(){this.fetchNext(!0)},scope:this},{xtype:"tbseparator"},{xtype:"querysearchfield"}]}],listeners:{loadedCorpus:function(a,c){this.setCorpus(c);
this.getTokensStore().setCorpus(c);this.getDocumentTermsStore().getProxy().setExtraParam("corpus",c.getId());var d=c.getDocuments();this.setDocumentsStore(d);d=this.down('panel[region\x3d"south"]');this.setLocationMarker(Ext.DomHelper.append(d.el,{tag:"div",style:"background-color: #157fcc; height: 100%; width: 2px; position: absolute; top: 0; left: 0;"}));this.generateChart(c,d);this.rendered&&this.load()},termsClicked:function(a,c){var d=[];c.forEach(function(a){Ext.isString(a)?d.push(a):a.term?
d.push(a.term):a.getTerm&&d.push(a.getTerm())});0<d.length&&this.loadQueryTerms(d)},corpusTermsClicked:function(a,c){var d=[];c.forEach(function(a){a.getTerm()&&d.push(a.getTerm())});this.loadQueryTerms(d)},documentTermsClicked:function(a,c){var d=[];c.forEach(function(a){a.getTerm()&&d.push(a.getTerm())});this.loadQueryTerms(d)},documentsClicked:function(a,c,d){c&&(this.setApiParams({skipToDocId:c[0].getId(),start:0}),this.load(!0))},termLocationClicked:function(a,c){if(void 0!==c[0]){var d=c[0],
...
c.show();break;case 1:b[0].getLabel().show();b[1].getLabel().show();c.hide();break;case 2:b[0].getLabel().hide(),b[1].getLabel().hide(),c.hide()}a.redraw()},selectTerm:function(a){var b=this.down("#chart").getSeries()[0];a=b.getStore().findExact("term",a);if(-1!==a){var c=b.getStore().getAt(a),d=b.getSprites()[0];a={series:b,category:b.getItemInstancing()?"items":"markers",index:a,record:c,field:b.getYField(),sprite:d};b.setHighlightItem(a)}},getCurrentTerms:function(){var a=[];this.termStore.each(function(b){b.get("isDoc")||
a.push(b.get("term"))});return a},getNearbyForTerm:function(a){var b=Math.max(2E3,Math.round(this.getCorpus().getWordTokensCount()/100));this.setApiParams({limit:b,target:a});this.loadFromApis();this.setApiParam("target",void 0)},removeTerm:function(a){var b=this.down("#chart").getSeries()[0],c=b.getStore().findExact("term",a);b.getStore().removeAt(c);c=this.termStore.findExact("term",a);this.termStore.removeAt(c);a=this.termStore.getCount();this.queryById("limit").setRawValue(a)},loadFromApis:function(a){var b=
this.down("#chart");null!==b&&b.mask(this.localize("loading"));var b={},c=this.getCurrentTerms();null!==this.newTerm&&(c.push(this.newTerm),this.setApiParam("limit",c.length));if(0<c.length){if(null!==this.newTerm||a)b.query=c.join(",");b.term=c}Ext.apply(b,this.getApiParams());"pca"===b.analysis?this.pcaStore.load({params:b}):this.caStore.load({params:b})},interpolate:function(a,b,c,d,e){return d+(e-d)*Math.max(0,Math.min(1,(a-b)/(c-b)))}});
...
noTermSelected:{en:"No term selected."}},api:{limit:50,dimensions:3,clusters:3,stopList:"auto"},glyph:"xf06e@FontAwesome"},tokenFreqTipTemplate:null,docFreqTipTemplate:null,chartMenu:null,labelsMode:0,constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(){Ext.apply(this,{title:this.localize("title"),docSimStore:Ext.create("Voyant.data.store.DocSimAnalysis"),autoDestroy:!0,layout:"fit",items:{layout:"fit",itemId:"chartParent"}});
this.on("loadedCorpus",function(a,b){this.setCorpus(b);this.docSimStore.setCorpus(b);this.loadFromApis()},this);this.docSimStore.on("load",function(a,b){this.buildChart(a)},this);this.tokenFreqTipTemplate=new Ext.Template(this.localize("tokenFreqTip"));this.docFreqTipTemplate=new Ext.Template(this.localize("docFreqTip"));this.callParent(arguments)},buildChart:function(a){var b=this,c=this.queryById("chart");null!==c&&this.queryById("chartParent").remove(c);a=a.getAt(0);var d=0,e=Number.MAX_VALUE,
f=0,g=Number.MAX_VALUE,h=[],k=[];a.getTokens().forEach(function(a){var b=a.get("rawFreq"),c="part"===a.get("category");c||(b>d&&(d=b),b<e&&(e=b));var q=a.get("vector")[2];q<g&&(g=q);q>f&&(f=q);b={term:a.get("term"),rawFreq:b,relativeFreq:a.get("relativeFreq"),cluster:a.get("cluster"),x:a.get("vector")[0],y:a.get("vector")[1],z:a.get("vector")[2],isDoc:c};c?(b.docIndex=a.get("docIndex"),a=this.getCorpus().getDocument(b.docIndex),b.term=a.getTinyTitle(),b.title=a.getTitle(),k.push(b)):h.push(b)},this);
a=Ext.create("Ext.data.JsonStore",{fields:"term x y z rawFreq relativeFreq cluster isDoc docIndex".split(" "),data:h});c=Ext.create("Ext.data.JsonStore",{fields:"term x y z rawFreq relativeFreq cluster isDoc docIndex".split(" "),data:k});a={itemId:"chart",xtype:"cartesian",interactions:["crosszoom","panzoom","itemhighlight"],plugins:{ptype:"chartitemevents"},axes:[{type:"numeric",position:"bottom",fields:["x"]},{type:"numeric",position:"left",fields:["y"]}],innerPadding:{top:25,right:25,bottom:25,
left:25},series:[{type:"customScatter",xField:"x",yField:"y",store:a,tooltip:{trackMouse:!0,style:"background: #fff",renderer:function(a,c){this.setHtml(b.tokenFreqTipTemplate.apply([a.get("term"),a.get("rawFreq"),a.get("relativeFreq")]))}},marker:{type:"circle"},highlight:{fillStyle:"yellow",strokeStyle:"black"},renderer:function(a,b,c,h){c=c.store.getAt(h);if(null!==c){var k=c.get("cluster");a=this.getParent().up("documentsimilarity");-1===k&&(k=0);h=.65;h=a.interpolate(c.get("z"),g,f,0,1);k=a.getApplication().getColor(k);
b.fillStyle="rgba("+k.join(",")+","+h+")";b.strokeStyle="rgba("+k.join(",")+",1)";c=c.get("rawFreq");a=a.interpolate(c,e,d,2,20);b.radius=a}}},{type:"customScatter",xField:"x",yField:"y",store:c,tooltip:{trackMouse:!0,style:"background: #fff",renderer:function(a,c){this.setHtml(b.docFreqTipTemplate.apply([a.get("title"),a.get("rawFreq")]))}},marker:{type:"diamond"},highlight:{fillStyle:"yellow",strokeStyle:"black"},renderer:function(a,b,c,d){c=c.store.getAt(d);null!==c&&(c.get("cluster"),d=this.getParent().up("documentsimilarity"),
a=.65,a=d.interpolate(c.get("z"),g,f,0,1),c=d.getApplication().getColor(6),b.fillStyle="rgba("+c.join(",")+","+a+")",b.strokeStyle="rgba("+c.join(",")+",1)",b.radius=5)}}],listeners:{itemclick:function(a,b,c){a=b.record.data;a.isDoc?(a=this.getCorpus().getDocument(a.docIndex),this.getApplication().dispatchEvent("documentsClicked",this,[a])):(a=Ext.create("Voyant.data.model.CorpusTerm",a),this.getApplication().dispatchEvent("corpusTermsClicked",this,[a]))}}};a.series[0].label={field:"term",display:"over"};
a.series[1].label={field:"term",display:"over"};a=Ext.create("Ext.chart.CartesianChart",a);this.queryById("chartParent").insert(0,a)},loadFromApis:function(a){a=this.down("#chart");null!==a&&a.mask(this.localize("loading"));a={};Ext.apply(a,this.getApiParams());this.docSimStore.load({params:a})},interpolate:function(a,b,c,d,e){return d+(e-d)*Math.max(0,Math.min(1,(a-b)/(c-b)))}});
