...
d.distributions[l];n>k&&(k=n);f.push({id:Ext.id(null,"bub"),x:m,freq:n,radius:0,bin:l,tokenIds:d.positions.slice(h,h+=n)});m+=g}a.terms[c].maxDistribution=k;a.terms[c].pos=f;k>this.maxFreq.value&&(b=!0,this.setMaxFreq({term:c,value:k}))}return b},calculateBubbleRadii:function(a,b){var c=Math.log(this.maxFreq.value),d=Math.log(2)/2,e;for(e in a.terms){var g=a.terms[e];if(g&&(null==b||e==b))for(var f=0;f<g.pos.length;f++){var h=g.pos[f];if(0<h.freq){var k=Math.max(Math.log(h.freq),d)/c*this.maxRadius;
