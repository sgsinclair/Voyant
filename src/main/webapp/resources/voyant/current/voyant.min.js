...
!1}else{!this.embeddable||a&&!Ext.isObject(a)||(Ext.isObject(a)&&(b=a),a=this.embeddable[0]);Ext.isString(a)&&(a=Ext.ClassManager.getByAlias("widget."+a.toLowerCase())||Ext.ClassManager.get(a));var e=!1;if(Ext.isFunction(a)){var f=a.getName();this.embeddable&&!0===Ext.Array.each(this.embeddable,function(c){if(c==f){b=b||{};c={};for(key in Ext.ClassManager.get(Ext.getClassName(a)).api)key in b&&(c[key]=b[key]);c.corpus||("Voyant.data.model.Corpus"==Ext.getClassName(this)?c.corpus=this.getId():this.getCorpus&&
(c.corpus=this.getCorpus().getId()));Ext.applyIf(b,{style:"width: "+(b.width||"90%")+(Ext.isNumber(b.width)?"px":"")+"; height: "+(b.height||"400px")+(Ext.isNumber(b.height)?"px":"")});delete b.width;delete b.height;-1<document.location.search.indexOf("debug\x3dtrue")&&(c.debug=!0);c=Ext.Object.toQueryString(c);var d=new Ext.XTemplate('\x3ciframe style\x3d"'+b.style+'" src\x3d"'+Voyant.application.getBaseUrl()+"tool/"+f.substring(f.lastIndexOf(".")+1)+'/?{0}"\x3e\x3c/iframe\x3e');1950>c.length?show(d.apply(["minimal\x3dtrue\x26"+
c])):Ext.Ajax.request({url:Voyant.application.getTromboneUrl(),params:{tool:"resource.StoredResource",storeResource:c}}).then(function(a){a=Ext.util.JSON.decode(a.responseText);show(d.apply(["minimal\x3dtrue\x26embeddedConfig\x3d"+a.storedResource.id]))}).otherwise(function(a){showError(a)});e=!0;return!1}},this)&&Voyant.notebook.util.Embed.showWidgetNotRecognized.call(this)}e||Voyant.notebook.util.Embed.showWidgetNotRecognized.call(this)}},showWidgetNotRecognized:function(){var a=Voyant.notebook.util.Embed.i18n.widgetNotRecognized;
...
Ext.isString(a)?a:a.getId());this.callParent(arguments)}});Ext.define("Voyant.data.store.CAAnalysis",{extend:"Ext.data.Store",model:"Voyant.data.model.StatisticalAnalysis",config:{corpus:void 0},constructor:function(a){a=a||{};Ext.apply(a,{proxy:{type:"ajax",url:Voyant.application.getTromboneUrl(),extraParams:{tool:"corpus.CA",corpus:a&&a.corpus?Ext.isString(a.corpus)?a.corpus:a.corpus.getId():void 0},reader:{type:"json",rootProperty:"correspondenceAnalysis",totalProperty:"correspondenceAnalysis.totalTerms"},simpleSortMode:!0}});this.callParent([a])},setCorpus:function(a){a&&
...
Ext.define("Voyant.data.store.CorpusTerms",{extend:"Ext.data.Store",mixins:["Voyant.data.store.CorpusTermsMixin"],model:Voyant.data.model.CorpusTerm,constructor:function(a){a=a||{};this.mixins["Voyant.data.store.CorpusTermsMixin"].constructor.apply(this,[a]);this.callParent([a])},getString:function(a){return(new Ext.XTemplate(this.localize("getString"))).apply([this.getCount(),this.sum("rawFreq")])}});
...
a.count.forEach(function(a){c[a]=c[a]?c[a]+1:1});var d=[],e;for(e in c)d.push([e,c[e]]);d.sort(function(a,b){return b[1]-a[1]});a.limit&&d.length>a.limit&&d.splice(a.limit);a.orientation&&"horizontal"==a.orientation?(this.setHeaders(d.map(function(a){return a[0]})),this.setRows([d.map(function(a){return a[1]})])):(this.setHeaders(a.headers?a.headers:["Item","Count"]),this.setRowKey(a.rowKey?a.rowKey:this.getHeaders()[0]),this.setRows(d))}if(a.isStore||a.store){d=a.store?a.store:a;b&&b.headers?this.setHeaders(b.headers):
(e=d.getAt(0))&&this.setHeaders(e.getFields().map(function(a){return a.getName()}));d.each(function(a){this.addRow(a.getData())},this);debugger}this.callParent()},addRow:function(a){Ext.isObject(a)?this.getRows().push(this.getHeaders().map(function(b){return a[b]})):Ext.isArray(a)&&this.getRows().push(a)},eachRecord:function(a,b){for(var c,d=0,e=this.getRows().length;d<e&&(c=this.getRecord(d),!1!==a.call(b||c,c,d,e));d++);},eachRow:function(a,b,c){for(var d,e=0,f=this.getRows().length;e<f&&(d=this.getRow(e,
...
"\x3c/div\x3e"+(-1==this.localize("translatedBy").indexOf("English")?"\x3cdiv\x3e"+this.localize("translatedBy")+"\x3c/div\x3e":"")}]},{layout:"fit",itemId:"toolsContainer-main",items:{xtype:c}}]}]});this.callParent(arguments)}});