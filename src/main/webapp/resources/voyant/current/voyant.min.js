...
0;g<=c;g+=this.COARSENESS)if(e[f]&&e[f][g]&&null!=this.grid[f+a]&&null!=this.grid[f+a][g+b])return!0;return!1},f;this.grid=[];l=0;f=function(){var a,f,h,k,q=this.wordsToArrange-1,y=g.canvas.width,n=g.canvas.height,r=.5*y,M=.5*n;do{h=p[l];if(void 0!==h){for(var s=Math.random()*Math.PI,C=.25*Math.random()*h.size,w=.5*(Math.random()-.5),t=.5*h.width,u=.5*h.height;;){a=Math.floor((r+Math.cos(s)*C*this.ratio-t)/this.COARSENESS)*this.COARSENESS;f=Math.floor((M+Math.sin(s)*C-u)/this.COARSENESS)*this.COARSENESS;
k=a+t>=y||f+u>=n?!0:d(a,f,h.height,h.width,h.mask);if(!k)break;s+=w;C+=.05}c(a,f,h);if(g.useFadeEffect)for(a=h.alpha=0;a<l;a++)f=p[a],1>f.alpha&&b(f);else h.alpha=1,h.draw(g.context)}l++;if(l>=p.length){clearTimeout(m);this.doingArrange=!1;this.toggleLoadingText(!1);e();break}}while(q--)}.createDelegate(this);d=d.createDelegate(this);c=c.createDelegate(this);b=b.createDelegate(this);a=a.createDelegate(this);this.doingArrange=!0;m=setInterval(f,50)}else alert("Error: There are no words to arrange.")};
...
background:"rgba(255,255,255,1)"},scaling:{label:{min:10,max:30}}},edgeOptions:{color:{color:"rgba(0,0,0,0.1)",highlight:"black",hover:"red"},labelHighlightBold:!1},highlightOptions:{font:{color:"white"},color:{background:"black",hover:{border:"#CB157F",background:"#EB42A5"}}},constructor:function(a){this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments)},initComponent:function(){var a=this;Ext.apply(a,{title:this.localize("title"),dockedItems:[{dock:"bottom",
...
this.getCorpus().getCorpusCollocates({autoLoad:!1});this.setApiParams({mode:"corpus"});var c=this.getApiParams();c.query=a;b.load({params:c,callback:function(a,b,c){c&&this.loadFromCorpusCollocateRecords(a)},scope:this})},loadFromCorpusTermRecords:function(a){if(Ext.isArray(a)&&0<a.length){var b=[];a.forEach(function(a){b.push(a.getTerm())});this.loadFromCorpusTermStringsArray(b)}},loadFromCorpusTermStringsArray:function(a){a.forEach(function(a){this.loadFromQuery(a)},this);this.setApiParams({query:a,
mode:"corpus"})},loadFromCorpusCollocateRecords:function(a,b){if(Ext.isArray(a)){var c={};this.getNodeDataSet().forEach(function(a){c[a.id]=!0});var d=[],e=[],f=this.getEdgeDataSet(),g=this.getApiParam("limit");a.forEach(function(a){if(a.getContextTerm()!=a.getTerm()){if(void 0===b){var k={id:a.getKeyword()+".keyword",label:a.getKeyword(),title:a.getKeyword()+" ("+a.getKeywordRawFreq()+")",type:"keyword",value:a.getKeywordRawFreq(),start:g,font:{color:"green"}};b=k.id;void 0===c[b]&&(c[b]=!0,d.push(k))}var k=
{id:a.getContextTerm()+".context",label:a.getContextTerm(),title:a.getContextTerm()+" ("+a.getContextTermRawFreq()+")",type:"context",value:a.getContextTermRawFreq(),start:0,font:{color:"maroon"}},l=k.id;void 0===c[l]&&(c[l]=!0,d.push(k));var m=!1;f.forEach(function(a){if(a.from==b&&a.to==l||a.from==l&&a.to==b)m=!0});m||e.push({from:b,to:l,value:a.getContextTermRawFreq()})}});this.getNodeDataSet().add(d);this.getEdgeDataSet().add(e);this.forceUpdate();this.getNetwork().fit()}},initGraph:function(){var a=
this.getLayout().getRenderTarget();a.setWidth(a.getWidth());a.setHeight(a.getHeight());this.setGraphHeight(a.getHeight());this.setGraphWidth(a.getWidth());if(void 0===this.getNetwork()){var b={autoResize:!0,interaction:{hover:!0,hoverConnectedEdges:!0,multiselect:!1},physics:{barnesHut:{gravitationalConstant:-1500,centralGravity:6,damping:.5,avoidOverlap:.5}},nodes:this.nodeOptions,edges:this.edgeOptions},a=new vis.Network(a.dom,{nodes:this.getNodeDataSet(),edges:this.getEdgeDataSet()},b);this.setNetwork(a);
this.getNodeDataSet().on("remove",function(a,b,e){var f=b.items[0];a=this.getEdgeDataSet().get({filter:function(a){return a.from==f||a.to==f}});this.getEdgeDataSet().remove(a);var g=[];this.getNodeDataSet().forEach(function(a){0==this.getEdgeDataSet().get({filter:function(b){return b.from==a.id||b.to==a.id}}).length&&g.push(a.id)}.bind(this));this.getNodeDataSet().remove(g)}.bind(this));a.on("dragStart",function(a){a=a.nodes[0];null!=a&&this.getNodeDataSet().update({id:a,fixed:!1})}.bind(this));a.on("dragging",
function(a){null!=a.nodes[0]&&(this.isMasked()?this.isOffCanvas(a.pointer.DOM)||this.unmask():this.isOffCanvas(a.pointer.DOM)&&this.mask(this.localize("releaseToRemove")))}.bind(this));a.on("dragEnd",function(a){var b=a.nodes[0];null!=b&&(this.isOffCanvas(a.pointer.DOM)?(this.unmask(),this.mask("cleaning"),this.getNodeDataSet().remove(b),this.forceUpdate(),this.unmask()):this.getNodeDataSet().update({id:b,fixed:!0}))}.bind(this));a.on("click",function(a){this.getContextMenu().hide()}.bind(this));
a.on("doubleClick",function(a){a=a.nodes[0];null!=a&&(a=this.getNodeDataSet().get(a),this.itemdblclick(a))}.bind(this));a.on("oncontext",function(a){a.event.preventDefault();var b=this.getNetwork().getNodeAt(a.pointer.DOM);if(null!=b){this.getNetwork().selectNodes([b]);var b=this.getNodeDataSet().get(b),e=this.getContextMenu();e.queryById("label").setText(b.label);e.queryById("fixed").setChecked(b.fixed);e.showAt(a.event.pageX,a.event.pageY)}}.bind(this))}},forceUpdate:function(){var a=this.getNodeDataSet().map(function(a){return{id:a.id}});
...
this.graphMargin.right,h=d3.scale.linear().domain([0,g]).range([0,c]),k=d3.max(f,function(a){return d3.max(a.values,function(a){return a.y0+a.y})});a=this.body.down("svg").getHeight()-this.graphMargin.top-this.graphMargin.bottom;var l=d3.scale.linear().domain([0,k]).range([a,0]),m=d3.svg.area().x(function(a){return h(a.x)}).y0(function(a){return l(a.y0)}).y1(function(a){return l(a.y0+a.y)}),k=d3.svg.axis().scale(h).orient("bottom");if(this.getMode()===this.MODE_CORPUS){for(var n=[],r=0;r<=g;r++)n.push(r);
