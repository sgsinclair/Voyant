...
"facet.authorTitle":{en:"Authors"},"facet.languageTitle":{en:"Languages"},"facet.titleTitle":{en:"Titles"},"facet.keywordTitle":{en:"Keywords"},"facet.pubDateTitle":{en:"Publication Dates"},"facet.publisherTitle":{en:"Publishers"},"facet.pubPlaceTitle":{en:"Publication Locations"},lexicalTitle:{en:"Terms"},noMatches:{en:new Ext.Template("No matches (out of {0} documents).",{compiled:!0})},queryMatches:{en:new Ext.Template("{0} matching documents (out of {1}).",{compiled:!0})},clickToOpenCorpus:{en:new Ext.Template('Please \x3ca href\x3d"{0}" target\x3d"_blank" class\x3d"link"\x3eclick here\x3c/a\x3e to access your new corpus (since popup windows are blocked).',
...
[e])}).getTargetEl().dom.querySelector("a"),e.addEventListener("click",function(){f.close()}),Ext.get(e).frame().frame())},failure:function(c,d){a.unmask();me.showError(c)}})},scope:this},{xtype:"querysearchfield",width:200,flex:1},{itemId:"status",xtype:"tbtext"}]}]});this.callParent(arguments);this.mixins["Voyant.panel.Panel"].constructor.apply(this,arguments);this.on("loadedCorpus",function(a,c){this.setCorpus(c);this.queryById("status").update(this.localize("noMatches",[c.getDocumentsCount()]));
this.query("facet").forEach(function(a){a.setCorpus(c)})});this.on("afterrender",function(a){var c=this.getApiParam("facet");Ext.isString(c)&&(c=[c]);var d=this.queryById("facets");c.forEach(function(c){var f=d.add({title:a.localize(c+"Title"),facet:c,bbar:[{xtype:"querysearchfield",tokenType:c.replace("facet.",""),inDocumentsCountOnly:!0}]});f.getSelectionModel().on("selectionchange",function(d,f){var k=[];f.forEach(function(a){k.push({facet:a.getFacet(),label:a.getLabel()})});a.getFacets()[c]=k;
a.updateResults()});f.on("query",function(c,d){a.getFacets()[f.facet]=[];a.updateResults()})});d.add({title:a.localize("lexicalTitle"),store:Ext.create("Voyant.data.store.CorpusTerms",{parentPanel:this}),facet:"lexical",columns:[{renderer:function(a,b,c){return"("+c.getRawFreq()+") "+c.getTerm()},flex:1}],bbar:[{xtype:"querysearchfield"}]}).getSelectionModel().on("selectionchange",function(c,d){var g=[];d.forEach(function(a){g.push({facet:"lexical",label:a.getTerm()})});a.getFacets().lexical=g;a.updateResults()})})},
updateResults:function(a){var b=this.getFacets();if(!a){a=[];for(facet in b)b[facet].forEach(function(b){a.push(b.facet+":"+b.label)});if(a)return this.updateResults(a)}var c=this.queryById("results").getTargetEl();c.update("");this.queryById("export").setDisabled(!0);if(0<a.length){this.mask(this.localize("loading"));var d=this.getCorpus().getDocumentQueryMatches();d.load({params:{query:a,includeDocIds:!0},callback:function(a,f,g){this.unmask();if(a&&0<a.length){this.queryById("status").setHtml(a.length);
var h="\x3cul\x3e",k=[];a.forEach(function(a){a.getDocIds().forEach(function(a){k.push(a);a=d.getCorpus().getDocument(a);var c;c="\x3cli\x3e"+("\x3ci\x3e"+a.getTitle()+"\x3c/i\x3e");for(facet in b)if(0!=b[facet].length){var e="";if("facet.title"!=facet){var f=facet.replace(/^.+?\./,""),g=a.get(f);if(g){var l=Ext.isArray(g);l?e+="\x3cli\x3e"+f+"\x3cul\x3e":g=[g];g.forEach(function(a){var c=!1;b[facet].forEach(function(b){b.label==a?c=!0:-1==b.facet.indexOf("facet")&&b.label.split(/\W+/).forEach(function(b){0<
b.trim().length&&-1<a.toLowerCase().indexOf(b.toLowerCase())&&(c=!0)})});e+="\x3cli\x3e"+(l?"":f+": ")+(c?'\x3cspan class\x3d"keyword"\x3e'+a+"\x3c/span\x3e":a)+"\x3c/li\x3e"});l&&(e+="\x3c/ul\x3e\x3c/li\x3e")}}e&&(c+="\x3cul\x3e"+e+"\x3c/ul\x3e")}h+=c+"\x3c/li\x3e"})});h+="\x3c/ul\x3e";c.update(h);this.queryById("status").update(this.localize("queryMatches",[k.length,this.getCorpus().getDocumentsCount()]));this.setMatchingDocIds(k);0<k.length&&this.queryById("export").setDisabled(!1)}},scope:this})}}});Ext.define("Voyant.panel.Cirrus",{extend:"Ext.panel.Panel",mixins:["Voyant.panel.Panel"],alias:"widget.cirrus",statics:{i18n:{title:{en:"Cirrus"},helpTip:{en:"\x3cp\x3eCirrus provides a wordcloud view of the most frequently occurring words in the corpus or document \u2013\u00a0this provides a convenient (though reductive) overview of the content. Features include\x3c/p\x3e\x3cul\x3e\x3cli\x3eterm frequency appears when hovering over words\x3c/li\x3e\x3cli\x3eclicking on terms may produce results in other tools if any are displayed\x3c/li\x3e\x3c/ul\x3e"},
