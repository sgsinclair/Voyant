...
gearTip:{en:"Options"},gearWinTitle:{en:"Options"},inputFormat:{en:"Input Format"},inputFormatAuto:{en:"Auto-Detect (recommended)"},advancedOptionsText:{en:"The advanced options below only apply to certain input formats. See the documentation on \x3ca href\x3d'{0}' target\x3d'voyantdocs'\x3ecreating a corpus\x3c/a\x3e."},xmlOptions:{en:"XML"},xpathDocuments:{en:"XPath to Documents"},xpathContent:{en:"XPath to Content"},xpathTitle:{en:"XPath to Title"},xpathAuthor:{en:"XPath to Author"},tableOptions:{en:"Tabular Data"},
tableDocuments:{en:"Documents Columns"},tableContent:{en:"Content Columns"},tableTitle:{en:"Title Columns"},tableAuthor:{en:"Author Columns"},tokenizationOptions:{en:"Tokenization"},tokenization:{en:"tokenization"},tokenizationAuto:{en:"Automatic (highly recommended)"},tokenizationWordBoundaries:{en:"Simple Word Boundaries"},emptyInput:{en:"Type in one or more URLs on separate lines or paste in a full text."},uploadingCorpus:{en:"Uploading corpus\u2026"},fileTypesWarning:{en:"File Types Warning"},
...
xmlDocumentsXpath:void 0,xmlContentXpath:void 0,xmlTitleXpath:void 0,xmlAuthorXpath:void 0,tokenization:void 0}},config:{corpus:void 0},constructor:function(a){this.callParent(arguments);a=a||{};this.mixins["Voyant.panel.Panel"].constructor.call(this,Ext.apply(a,{includeTools:{gear:!0,help:!0}}))},initComponent:function(){var a=this;Ext.apply(a,{title:this.localize("title"),width:800,frame:!0,padding:10,style:{borderColor:"#aaa",borderStyle:"solid"},frameHeader:!0,layout:{type:"vbox",align:"middle"},
dockedItems:[{xtype:"toolbar",dock:"bottom",buttonAlign:"right",defaultButtonUI:"default",items:[{text:"Open",glyph:"xf115@FontAwesome",tooltip:"Select an exsting corpus",hidden:void 0!=this.getCorpus(),handler:function(){Ext.create("Ext.window.Window",{title:"Open an Existing Corpus",layout:"fit",modal:!0,items:{xtype:"form",margin:"5,5,5,5",items:{xtype:"combo",labelWidth:150,fieldLabel:"Choose a corpus:",name:"corpus",queryMode:"local",store:[["shakespeare","Shakespeare's Plays"],["austen","Austen's Novels"]],
forceSelection:!0},buttons:[{text:"Open",glyph:"xf00c@FontAwesome",handler:function(b){b.up("form").getForm();var c=b.up("form").getForm().getValues().corpus;""!=c?(a.loadCorpus({corpus:c}),b.up("window").close()):Ext.Msg.show({title:"Select a Corpus",message:"Please be sure to select a corpus.",buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR})},flex:1},{text:"Cancel",glyph:"xf00d@FontAwesome",flex:1,handler:function(a){a.up("window").close()}}]}}).show()}},{xtype:"filefield",glyph:"xf093@FontAwesome",name:"upload",
buttonOnly:!0,hideLabel:!0,buttonText:"Upload",tooltip:"test",listeners:{render:function(a){a.fileInputEl.dom.setAttribute("multiple",!0);Ext.QuickTips.register({target:a.getEl(),text:"Upload one or more documents from your computer",enabled:!0,showDelay:20,trackMouse:!0,autoShow:!0})},change:function(b,c){if(c){var d=b.up("form").getForm();if(d.isValid()){for(var e=b.fileInputEl.dom.files,f=/\.(png|gif|jpe?g|xls|mp[234a]|mpeg|exe|wmv|avi|ppt|mpg|tif|wav|mov|psd|wma|ai|bmp|pps|aif|pub|dwg|indd|swf|asf|mbd|dmg|flv)$/i,
g=/\.(txt|pdf|html?|xml|docx?|rtf|pages|odt|zip|jar|tar|gz|ar|cpio|bzip2|bz2|gzip)$/i,h=[],k=[],m=0,l=e.length;m<l;m++){var n=e[m].name;f.test(n)?h.push(n.split("/").pop()):g.test(n)||k.push(n.split("/").pop())}0<h.length||0<k.length?Ext.Msg.show({title:a.localize("fileTypesWarning"),icon:Ext.MessageBox.ERROR,message:a.localize("fileTypesMessage")+"\x3cul\x3e"+(0<h.length?"\x3cli\x3e"+a.localize("badFiles")+h.slice(0,5).join(", ")+(5<h.length?"\u2026":"")+"\x3c/li\x3e":"")+(0<k.length?"\x3cli\x3e"+
...
c,a)}).fail(function(a,b){d.unmask();c.showErrorResponse({message:a},b)})},showOptionsClick:function(a){void 0===a.optionsWin&&(a.optionsWin=Ext.create("Ext.window.Window",{title:a.localize("gearWinTitle"),closeAction:"hide",layout:"fit",bodyPadding:10,items:[{xtype:"form",defaultType:"textfield",fieldDefaults:{labelAlign:"right",labelWidth:130,width:400},items:[{xtype:"combo",fieldLabel:a.localize("inputFormat"),labelWidth:140,name:"inputFormat",queryMode:"local",store:[["",a.localize("inputFormatAuto")],
["TEI","TEI"],["RSS","RSS"]],forceSelection:!0,value:""},{xtype:"container",html:"\x3cp\x3e\x3ci\x3e"+(new Ext.Template(a.localize("advancedOptionsText"))).applyTemplate([a.getBaseUrl()+"docs/#!/guide/creatingcorpus"])+"\x3c/i\x3e\x3c/p\x3e",width:375},{xtype:"fieldset",title:"\x3ca href\x3d'"+a.getBaseUrl()+"docs/#!/guide/creatingcorpus-section-xml' target\x3d'voyantdocs'\x3e"+a.localize("xmlOptions")+"\x3c/a\x3e",collapsible:!0,collapsed:!0,defaultType:"textfield",items:[{fieldLabel:a.localize("xpathDocuments"),
name:"xmlDocumentsXpath"},{fieldLabel:a.localize("xpathContent"),name:"xmlContentXpath"},{fieldLabel:a.localize("xpathAuthor"),name:"xmlAuthorXpath"},{fieldLabel:a.localize("xpathTitle"),name:"xmlTitleXpath"}]},{xtype:"fieldset",title:"\x3ca href\x3d'"+a.getBaseUrl()+"docs/#!/guide/creatingcorpus-section-table' target\x3d'voyantdocs'\x3e"+a.localize("tableOptions")+"\x3c/a\x3e",collapsible:!0,collapsed:!0,defaultType:"textfield",items:[{fieldLabel:a.localize("tableDocuments"),name:"tableDocuments"},
{fieldLabel:a.localize("tableContent"),name:"tableContent"},{fieldLabel:a.localize("tableAuthor"),name:"tableAuthor"},{fieldLabel:a.localize("tableTitle"),name:"tableTitle"}]},{xtype:"fieldset",title:"\x3ca href\x3d'"+a.getBaseUrl()+"docs/#!/guide/creatingcorpus-section-tokenization' target\x3d'voyantdocs'\x3e"+a.localize("tokenizationOptions")+"\x3c/a\x3e",collapsible:!0,items:[{xtype:"combo",fieldLabel:a.localize("tokenization"),name:"tokenization",queryMode:"local",store:[["",a.localize("tokenizationAuto")],
