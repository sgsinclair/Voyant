...
init:function(){window.Corpus=Spyral.Corpus;window.Table=Spyral.Table;window.loadCorpus=function(){return Spyral.Corpus.load.apply(Spyral.Corpus.load,arguments)};window.createTable=function(){return Spyral.Table.create(arguments)};Ext.Ajax.request({url:this.getApplication().getBaseUrlFull()+"resources/spyral/docs/spyral.json",callback:function(a,b,c){b&&(this.spyralTernDocs=Ext.JSON.decode(c.responseText),this.spyralTernDocs.Corpus=this.spyralTernDocs.Spyral.Corpus,this.spyralTernDocs.Table=this.spyralTernDocs.Spyral.Table,
this.spyralTernDocs.loadCorpus=this.spyralTernDocs.Spyral.Corpus.load,this.spyralTernDocs.createTable=this.spyralTernDocs.Spyral.Table.create);a=Ext.Object.fromQueryString(document.location.search,!0);b=Ext.isDefined(a.run);c=/\/spyral\/([\w-]+)\/?$/.exec(location.pathname);var d=Ext.isDefined(a.githubId);"inputJsonArrayOfEncodedBase64"in a?Ext.decode(a.inputJsonArrayOfEncodedBase64).forEach(function(a){a=decodeURIComponent(atob(a));0==a.trim().indexOf("\x3c")?this.addText(a):this.addCode(a)},this):
a.input?0===a.input.indexOf("http")&&this.loadFromUrl(a.input,b):c?(this.loadFromId(c[1]),this.setStorageSolution("voyant")):d?(this.githubDialogs.loadFileFromId(a.githubId),this.setStorageSolution("github")):this.addNew();if(b){var e=this;Ext.defer(function(){e.runAll()},100)}},scope:this})},getBlock:function(a){a=a||0;var b=this.query("notebookcodeeditorwrapper"),c=this.getCurrentBlock().id,d=b.findIndex(function(a){return a.id==c});if(0>d+a||d+a>b.length-1)Ext.Msg.show({title:this.localize("error"),
