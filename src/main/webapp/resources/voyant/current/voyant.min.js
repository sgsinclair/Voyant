...
var a=this.getCorpus();a&&Ext.Ajax.request({url:this.getTromboneUrl(),params:{tool:"corpus.DocumentTokens",corpus:a.getAliasOrId(),outputFormat:"text",template:"docTokens2text",limit:0,noOthers:!0,perDocLimit:1==a.getDocumentsCount()?void 0:parseInt(this.getApiParam("perDocLimit"))},success:function(b,c){this.unmask();this.mask(this.localize("parsingDocuments"));var d=b.responseText.trim().split(/(\r\n|\r|\n)/);if(1==d.length){if(d=d[0].split(": "),1<d.length)for(var e=a.getDocument(d[0]),f=d[1].split(" "),
e=e.getLexicalTokensCount(),e=10>e?e:1E3>e?10:100,e=10,g=Math.floor(f.length/e),h=0;h<e;h++){var k=f.slice(h*g,h*g+g).join(" ");this.parseLine(d[0]+"-"+h+"\t"+d[0]+"-"+h+"\t"+k)}}else d.forEach(function(b){b=b.split(": ");if(1<b.length){var c=a.getDocument(b[0]);this.parseLine(b[0]+"\t"+c.getTinyLabel()+"\t"+b[1])}},this);this.unmask();this.runIterations()},scope:this})},resetData:function(){var a=parseInt(this.getApiParam("numTopics"));this.setVocabularyCounts({});this.setSumDocSortSmoothing(this.getDocSortSmoothing()*
