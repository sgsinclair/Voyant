        ...
        if (isNaN(parseInt(activeArg))) {
            if (data.doc) {
                ...
                var createLinks = function (str) {
                    try {
                        // CHANGED: names of proto placeholders. can't have http because the regex below will match it
                        var httpProto = 'HTP_PROTO_PLACEHOLDER';
                        var httpsProto = 'HTPS_PROTO_PLACEHOLDER';
                        var re = /\bhttps?:\/\/[^\s<>"`{}|\^\[\]\\]+/gi;
                        var m;
                        while ((m = re.exec(str)) !== null) {
                            if (m.index === re.lastIndex) {
