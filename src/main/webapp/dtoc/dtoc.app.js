Ext.define('VoyantDTOCApp', {
	...
	listeners: {
    	loadedCorpus: function(src, corpus) {
    		...
    		if (this.getCuratorId() !== undefined) {
    			if (this.getCuratorId() === 'inkeTags') {
    				var data = {
    				...
    				this.getStoredResource('curation-'+this.getCuratorId()).then(function(curation) {
	...
	launch: function() {
    	...
    	if (qParams.curatorId) {
    		this.setCuratorId(qParams.curatorId);
    	} else if (qParams.inkeTags) {
    		this.setCuratorId('inkeTags');
    	}
